...
import research_v260_fixed10_reentry_cooldown_20260822 as round9
...
OUTPUT_ROOT = (
    REPO
    / "quant/data_file/reports/strategy_agent_v260_fixed10_renewal_policy_20260822"
)
RENEWAL_POLICIES = (
    "none",
    "top1_rebalance",
...
def candidate_policy(renewal_policy: str) -> dict:
    current = round9.candidate_policy(0)
...
def main() -> None:
    ...
    if access["logical_max_date"] != round1.DEVELOPMENT_END:
        raise PermissionError("2026 data entered renewal-policy development")
    definition = harness.production_definition(protocol)
    score, order = harness.v95.score_pair(arrays, 0.0, 7, 0.1)

    results = {}
    run_cache = {}
    for renewal_policy in RENEWAL_POLICIES:
        candidate_id = f"renewal_{renewal_policy}"
        current = candidate_policy(renewal_policy)
    ...
    if not all(deterministic.values()):
        raise RuntimeError("renewal-policy deterministic replay failed")
    ...
    result = {
        ...
        "candidate_budget": list(RENEWAL_POLICIES),
        "selection_rule": (
