...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_3d_rankic_blend40_lowstd_gate_20260626"
...
TARGET_TABLE = "stock_predict_data_model_agent_3d_rankic_blend40_lowstd_gate_20260626_executable_3d_open_return_research"
ALT_PATH = (
    DATA_DIR
    / "reports"
...
def evaluate(scored: pd.DataFrame, labels: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, dict[str, object]]:
    ...
    candidate = daily_metrics(eval_frame, "pred_prob", "blend40_lowstd_gate")
...
def write_outputs(
    ...
    metrics_path = REPORT_DIR / "lowstd_gate_daily_metrics.csv"
    delta_formal_path = REPORT_DIR / "lowstd_gate_delta_vs_formal.csv"
    delta_base_path = REPORT_DIR / "lowstd_gate_delta_vs_base.csv"
    manifest_path = REPORT_DIR / "lowstd_gate_research_manifest.json"
    report_path = REPORT_DIR / "lowstd_gate_research_report.md"
    ...
    manifest = {
        ...
        "candidate_id": "model_agent_3d_rankic_blend40_lowstd_gate_20260626",
        "score_formula": stats["formula"],
        "research_hypothesis": "use_blend40_60_on_low_base_std_days_with_current_3d_mainline",
    ...
    lines = [
        "# 3D rankic 主线与 blend40_60 的 low-std 门控研究报告（20260626）",
        "",
        "## 当前结论",
        "",
        "这是一个 research-only 的 3D 全覆盖候选，不是 formal L4 或 L5 生产资产。",
        f"它只在与 blend40_60 候选重叠的 {stats['overlap_days']} 个交易日里，当 `base_std <= {THRESHOLD:.15f}` 时切到 blend40_60，其余日期保持当前 3D 主线不变。",
        "",
        "## 评分公式",
        "",
        "```text",
        ...
        "## 当前判断",
        "",
        "- 这条分支说明：3D 当前主线虽然整体仍优于四年训练候选，但在低波动交易日里，blend40_60 的局部替换可以带来额外增益。",
        "- 这是当前 3D 主线的增量修补候选，不是新的独立训练主线。",
        "",
        "## 边界",
        "",
        "- 未训练模型。",
