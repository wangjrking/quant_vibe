...
OUT_DIR = DATA_DIR / "reports" / "model_agent_1d_proxy5d_20260623"
...
RESEARCH_TABLE = "stock_predict_data_model_agent_1d_proxy5d_20260623_executable_1d_open_return_research"
...
def main() -> int:
    ...
    with sqlite3.connect(MODEL_DB) as conn:
        base_scores = pd.read_sql_query(
            f"select trade_date, stock_code, [{LABEL_COL}] as {LABEL_COL}, pred_prob as score from '{LABEL_TABLE}' where [{LABEL_COL}] is not null",
            conn,
        )
    base_scores["trade_date"] = base_scores["trade_date"].astype(str)
    base_eval, _ = evaluate(base_scores, "score")
    selected_eval, selected_daily = evaluate(frame.assign(score=frame["rank5"]), "score")

    candidate = frame[["trade_date", "stock_code", LABEL_COL]].copy()
    candidate["pred_prob"] = frame["rank5"].astype(float)
    ...
    payload = {
        ...
        "rule": {
            "type": "pure_proxy_rank",
            "formula": "score = rank5_best",
        },
        ...
        "governance_notes": [
            "research only",
            "cross_horizon_proxy_5d_for_1d_label",
            "no training",
    ...
    manifest = {
        ...
        "notes": "1D 研究候选采用跨周期代理：直接使用当前最优 5D 研究表的 rank 作为 1D 标签排序，仅用于 research 验证，不进入 formal L4/L5。",
    }
    ...
    report_lines = [
        ...
        f"- `{(OUT_DIR / 'selected_daily_eval.csv').as_posix()}`",
    ]
