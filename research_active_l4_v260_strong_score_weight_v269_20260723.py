...
OUT = (
    ROOT
    / "quant/data_file/reports/strategy_agent_active_l4_v260_strong_score_weight_v269_20260723"
)
...
def stable_id(definition: dict) -> str:
    ...
    return "v269_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def definition_for(protocol: dict, strong_current_weight: float) -> dict:
    return {
        **v260.definition_for(protocol, 50),
        "current_weight": 0.1,
        "strong_current_weight": float(strong_current_weight),
    }


def state_score_pair(arrays, definition):
    base_score, base_order = v95.score_pair(arrays, 0.0, 7, 0.1)
    strong_weight = float(definition["strong_current_weight"])
    if abs(strong_weight - 0.1) < 1e-12:
        return base_score, base_order
    strong_score, strong_order = v95.score_pair(
        arrays, 0.0, 7, strong_weight
    )
    momentum = v134.market_momentum(
        arrays, int(definition["market_lookback"])
    )
    strong_days = np.isfinite(momentum) & (
        momentum >= float(definition["strong_return_min"])
    )
    base_score[strong_days] = strong_score[strong_days]
    base_order[strong_days] = strong_order[strong_days]
    return base_score, base_order


def main() -> None:
    parser = argparse.ArgumentParser(description="V260 强势市场分数响应研究")
    ...
    definitions = [
        ...
        for value in protocol["grid"]["strong_current_weight"]
    ]
    rows = []
    full_rows = []
    frozen = []
        ...
        if args.open_known_2026:
            for start in protocol["known_stress_starts"]:
                ...
                rows.append(
                    {
                        "case_id": case_id,
                        "strong_current_weight": definition[
                            "strong_current_weight"
                        ],
                        "buy_start": start,
                        **core.metrics(
    ...
    (
        frame.groupby("strong_current_weight")
        .agg(
