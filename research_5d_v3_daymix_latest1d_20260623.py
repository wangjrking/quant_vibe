...
OUT_DIR = DATA_DIR / "reports" / "model_agent_5d_v3_daymix_latest1d_20260623"
...
ONE_TABLE = "stock_predict_data_model_agent_1d_endgate_threshold_refine_20260623_executable_1d_open_return_research"
TEN_SIGNAL_TABLE = "stock_predict_data_model_agent_10d_daygate_joint_refine_20260623_executable_10d_open_return_research"
PREVIOUS_BEST_TABLE = "stock_predict_data_model_agent_5d_v3_daymix_local1d_20260623_executable_5d_open_return_research"
RESEARCH_TABLE = "stock_predict_data_model_agent_5d_v3_daymix_latest1d_20260623_executable_5d_open_return_research"
...
def main() -> int:
    ...
    newly_used_days = best_choice_df[best_choice_df["chosen_model"] == "one"]["trade_date"].astype(str).tolist()

    payload = {
        "generated_at": datetime.now().isoformat(timespec="seconds"),
        "candidate_id": "model_agent_5d_v3_daymix_latest1d_20260623",
        ...
        "selected_one_days": newly_used_days,
        "db_summary": db_summary,
        "governance_notes": [
            "research only",
            "no training",
            "daily switch mixes current 5d v3 best with local reblend and latest retained 1d research candidate",
    ...
    manifest = {
        ...
        "notes": "5D 研究候选在当前 v3 最优基础上，替换为最新 retained 1D 评分源做按天切换，仅用于 research 验证，不进入 formal L4/L5。",
    }
    ...
    report = f"""# 5D day-mix 接入最新 1D 研究候选

## 当前结论

- 当前 5D 基线：`{BASE_TABLE}`
- 新研究候选：`{RESEARCH_TABLE}`
- 本次只替换 1D 分数源，其余按天切换逻辑保持不变。
- Focus：`{base_focus:.12f}` -> `{best_focus:.12f}`
- Focus 增量：`{best_focus - base_focus:.12f}`
- recent80 守卫通过：`{best_passed}`

## 最优规则

- `start_date={best_rule['start_date']}`
- `thr10={best_rule['thr10']}`
- `thr1={best_rule['thr1']}`
- `priority={best_rule['priority']}`
- `switch_local_days={best_rule['switch_local_days']}`
- `switch_one_days={best_rule['switch_one_days']}`
