...
def build_signal_rows(trades: list[dict], target_total_pct: float) -> list[dict]:
    grouped: dict[str, list[dict]] = {}
    for row in trades:
        buy_date = str(row.get("entry_signal_date") or "").strip()
...
def main(argv=None):
    ...
    signals = build_signal_rows(trades, args.target_total_pct)
    if not signals:
