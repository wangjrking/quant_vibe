...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_5d_condition_gate_v6_shrink_scan_20260625"
TARGET_TABLE = (
    "stock_predict_data_model_agent_5d_condition_gate_v6_shrink_20260625_"
    "executable_5d_open_return_research"
)
LABEL = "executable_5d_open_return"

SOURCES = {
    "base5": "stock_predict_data_model_agent_5d_condition_gate_v6_20260625_executable_5d_open_return_research",
    "formal5": "stock_predict_data_model_agent_best_full_20260623_executable_5d_open_return_formal",
    "stable5": "stock_predict_data_model_agent_5d_stability_guard_v3_20260625_executable_5d_open_return_research",
    "recent5": "stock_predict_data_model_agent_5d_recent_enhance_v4_20260625_executable_5d_open_return_research",
    "risk5": "stock_predict_data_model_agent_5d_risk_balanced_v7_20260625_executable_5d_open_return_research",
...
def build_features(scores: pd.DataFrame) -> pd.DataFrame:
    ...
    for trade_date, group in scores.groupby("trade_date", sort=True):
        base_top10 = top_set(group, "base5_rank", 10)
        base_top20 = top_set(group, "base5_rank", 20)
        base_top50 = top_set(group, "base5_rank", 50)
        row: dict[str, float | str] = {"trade_date": trade_date}
        row["base5_top1_top5_gap"] = float(
            group.nlargest(1, "base5_rank")["base5_rank"].mean()
            - group.nlargest(5, "base5_rank")["base5_rank"].min()
        )
        row["base5_top5_top10_gap"] = float(
            group.nlargest(5, "base5_rank")["base5_rank"].mean()
            - group.nlargest(10, "base5_rank")["base5_rank"].mean()
        )
        for alias in SOURCES:
            score_col = f"{alias}_score"
            ...
            if alias != "base5":
                ...
                row[f"overlap10_base5_{alias}"] = len(base_top10 & top_set(group, rank_col, 10))
                row[f"overlap20_base5_{alias}"] = len(base_top20 & top_set(group, rank_col, 20))
                row[f"overlap50_base5_{alias}"] = len(base_top50 & top_set(group, rank_col, 50))
        rows.append(row)
    ...
    *,
    ...
    for prefix, current, base in [
        ...
        for metric in ["rank_ic", "top1", "top3", "top5", "top10", "top20", "top50"]:
            row[f"{prefix}_{metric}_delta_vs_base5"] = current[metric] - base[metric]
...
def scan(scores: pd.DataFrame, labels: pd.DataFrame, features: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    base_daily = source_metrics[source_metrics["source"] == "base5"].set_index("trade_date")
    feature_idx = features.set_index("trade_date")
    rows = []
    daily_rows = []
    alt_sources = ["formal5", "stable5", "recent5", "risk5", "rank3", "best1", "best10"]
    ...
    for alt in alt_sources:
        ...
        candidate_features = [
            ...
            if col.startswith("base5_")
            or col.startswith(f"{alt}_")
            or col.endswith(f"_{alt}")
            or col.startswith("stable5_")
            or col.startswith("recent5_")
            or col.startswith("risk5_")
        ...
        for feature in sorted(set(candidate_features)):
            ...
            for threshold in thresholds:
                for op in ["<=", ">="]:
                    ...
                    row["pass_basic"] = bool(
                        row["full_rank_ic_delta_vs_base5"] >= -0.00025
                        and row["min_period_rank_ic_delta"] >= -0.00075
                        and row["recent63_top1_delta_vs_base5"] >= -0.008
                        and row["recent63_top5_delta_vs_base5"] >= 0.0
                        and row["recent20_top5_delta_vs_base5"] >= 0.0
                        and row["full_top5_delta_vs_base5"] >= 0.0
                        and row["min_period_top5_delta"] >= -0.0002
                        and row["positive_top5_periods"] >= 3
                    )
                    row["objective"] = (
                        1.4 * row["full_top5_delta_vs_base5"]
                        + 1.0 * row["recent63_top5_delta_vs_base5"]
                        + 0.8 * row["recent20_top5_delta_vs_base5"]
                        + 0.8 * row["full_top1_delta_vs_base5"]
                        + 0.6 * row["recent63_top1_delta_vs_base5"]
                        + 0.4 * row["recent20_top1_delta_vs_base5"]
                        + 0.8 * row["full_rank_ic_delta_vs_base5"]
...
def write_candidate(scores: pd.DataFrame, features: pd.DataFrame, best: pd.Series) -> dict[str, object]:
    ...
    active_dates = set(active[active].index.astype(str))
    out = scores[
        ["trade_date", "stock_code", "base5_score", "base5_rank", f"{alt}_score", f"{alt}_rank"]
    ].copy()
...
def write_outputs(scan_result: pd.DataFrame, daily: pd.DataFrame, source_metrics: pd.DataFrame, stats: dict[str, object] | None) -> None:
    ...
    scan_path = REPORT_DIR / "condition_gate_v6_shrink_scan_results.csv"
    daily_path = REPORT_DIR / "condition_gate_v6_shrink_daily_deltas.csv"
    source_path = REPORT_DIR / "condition_gate_v6_shrink_source_daily_metrics.csv"
    ...
    manifest = {
        ...
        "baseline_for_scan": "research_5d_condition_gate_v6",
        "research_hypothesis": "shrink_high_risk_days_of_current_best_5d_candidate_without_losing_topn_strength",
        "best_scan": best,
        "asset_stats": stats,
    ...
    manifest_path = REPORT_DIR / "condition_gate_v6_shrink_manifest.json"
    manifest_path.write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")

    lines = [
        "# 5D condition_gate_v6 收缩扫描报告（20260625）",
        "",
        "## 当前结论",
        "",
    ]
    if stats:
        lines.extend(
            [
                "本轮在 research-only 范围内找到一个相对 `condition_gate_v6` 更稳的收缩候选。",
                "",
                "候选规则：",
                "",
                "```text",
                ...
                "## 相对 5D 当前最强研究基线的扫描摘要",
                "",
                f"- 全样本 RankIC 增量：`{best['full_rank_ic_delta_vs_base5']:.6f}`",
                f"- 全样本 Top1 增量：`{best['full_top1_delta_vs_base5']:.6f}`",
        ...
        lines.append("本轮没有找到相对 `condition_gate_v6` 更优、且满足收缩约束的候选，因此不写入新的研究预测表。")
    lines.extend(
        [
            "",
            "## 证据路径",
    ...
    (REPORT_DIR / "condition_gate_v6_shrink_report.md").write_text("\n".join(lines), encoding="utf-8")
