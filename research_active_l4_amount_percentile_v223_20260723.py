...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_amount_percentile_v223_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v223_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def run_case(arrays, score, order, amount_rank, definition, protocol, end, start=None):
    local_protocol = copy.deepcopy(protocol)
    local_protocol["fixed_universe"]["amount_min"] = int(definition["amount_min"])
    mask = v174.selection_mask(arrays, definition["max_rank_deterioration"])
    mask &= np.isfinite(amount_rank) & (amount_rank >= float(definition["amount_percentile_min"]))
    return v162.run_case(
        ...
        start,
        selection_mask_override=mask,
        max_positions_override=int(definition["max_positions"]),
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 成交额分位数过滤研究")
    ...
    amount_rank_obs = amount_rank[: len(observation["dates"])]
    rows, definitions = [], {}
    for case in protocol["grid"]["cases"]:
        definition = {
            **protocol["fixed_definition"],
            "amount_min": int(case["amount_min"]),
            "amount_percentile_min": float(case["amount_percentile_min"]),
        }
