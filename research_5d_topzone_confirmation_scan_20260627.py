...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_5d_topzone_confirmation_scan_20260627"

LABEL = "executable_5d_open_return"
BASE_TABLE = "stock_predict_data_model_agent_5d_recent_top_condblend_latest_20260626_executable_5d_open_return_research"
NEW_TABLE = "stock_predict_data_model_agent_5d_topzone_confirmation_best_20260627_executable_5d_open_return_research"

SOURCE_TABLES = {
    "base": BASE_TABLE,
    "balanced5d": "stock_predict_data_model_agent_5d_balanced_condblend_latest_20260627_executable_5d_open_return_research",
...
def load_scores() -> pd.DataFrame:
    with sqlite3.connect(MODEL_DB, timeout=300) as conn:
        ...
        for name, table in SOURCE_TABLES.items():
            ...
            merged = part if merged is None else merged.merge(part, on=["trade_date", "stock_code"], how="inner", validate="one_to_one")
    if merged is None:
...
def scan(eval_frame: pd.DataFrame) -> tuple[pd.DataFrame, dict, dict]:
    ...
    best_score = None
    best_payload = None
    for aux in ["balanced5d"]:
        for base_q in [0.995, 0.999]:
            for aux_hi in [0.90, 0.95]:
                for aux_lo in [0.10]:
                    for boost in [0.03, 0.08]:
                        for penalty in [0.03, 0.08]:
                            ...
                            rows.append(
                                {
                                    **{k: payload[k] for k in ["aux", "base_q", "aux_hi", "aux_lo", "boost", "penalty", "objective", "delta_vs_base"]},
    ...
    best_payload["daily_eval"] = daily_eval(best_daily, "_best_score")
    return result, {"objective": base_objective, "summary": base_summary}, best_payload


def write_best_table(scores: pd.DataFrame, best: dict) -> dict:
    out = scores[["trade_date", "stock_code"]].copy()
    out["pred_prob"] = apply_candidate(scores, best["aux"], best["base_q"], best["aux_hi"], best["aux_lo"], best["boost"], best["penalty"])
    with sqlite3.connect(MODEL_DB, timeout=300) as conn:
...
def main() -> int:
    ...
    results.to_csv(REPORT_DIR / "topzone_confirmation_scan_results.csv", index=False, encoding="utf-8-sig")
    best["daily_eval"].to_csv(REPORT_DIR / "best_daily_eval.csv", index=False, encoding="utf-8-sig")
    best.pop("daily_eval")
    db_summary = write_best_table(scores, best) if best["objective"] > base["objective"] else None
    summary = {
        "generated_at": now_iso(),
        "scope": "research_only_5d_topzone_confirmation_scan",
        "label": LABEL,
    ...
    print(json.dumps({"summary": str(REPORT_DIR / "topzone_confirmation_summary.json"), "improves": bool(db_summary)}, ensure_ascii=False, indent=2))
    return 0
