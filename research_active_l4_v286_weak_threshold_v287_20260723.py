...
OUT = (
    ROOT
    / "quant/data_file/reports/strategy_agent_active_l4_v286_weak_threshold_v287_20260723"
)
...
def stable_id(definition):
    ...
    return "v287_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def definitions(protocol):
    base = v260.definition_for(protocol, protocol["position_warmup_days"])
    return [
        {**base, "weak_top2_momentum_threshold": float(value)}
        for value in protocol["grid"]["weak_top2_momentum_threshold"]
    ]


def entry_slots(arrays, definition):
    ...
    result[
        ...
        | (
            momentum
            < float(definition["weak_top2_momentum_threshold"])
        )
    ] = 2
    return result
...
def run_case(
    ...
    return v252.v162.run_case(
        ...
        entry_slots_schedule_override=entry_slots(arrays, definition),
...
def summarize_starts(frame):
    return (
        frame.groupby(["case_id", "weak_top2_momentum_threshold"])
        .agg(
...
def select_candidate(protocol, cases, full, starts, folds):
    ...
    baseline_id = next(
        ...
        if np.isclose(definition["weak_top2_momentum_threshold"], 0.0)
    )
    rows = []
    for case_id, definition in by_id.items():
        ...
        eligible = bool(
            ...
            and float(v["fold_min_linear_annual_proxy"]) >= 0.10
            and float(f["linear_annual_proxy"]) >= 0.70
            and float(f["sharpe"]) >= 1.2
            and float(f["max_drawdown"]) <= 0.30
            and float(s["min_linear_annual_proxy"]) >= 0.45
        )
        rows.append(
            {
                "case_id": case_id,
                "weak_top2_momentum_threshold": definition[
                    "weak_top2_momentum_threshold"
                ],
                "eligible": eligible,
...
def run_observation(protocol, arrays, score, order, cases):
    ...
    for definition in cases:
        ...
        threshold = definition["weak_top2_momentum_threshold"]
        daily = run_case(
            arrays,
            score,
            order,
        ...
        full_rows.append(
            {
                "case_id": case_id,
                "weak_top2_momentum_threshold": threshold,
                **core.metrics(
                    daily,
                    str(arrays["dates"][0]),
                    protocol["observation_end"],
    ...
    summarize_starts(starts).to_csv(
...
def main():
    parser = argparse.ArgumentParser(description="V286弱势Top2触发阈值研究")
