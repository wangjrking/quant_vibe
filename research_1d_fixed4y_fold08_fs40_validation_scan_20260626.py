...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_research_1d_fixed4y_fold08_fs40_validation_scan_20260626"
...
CANDIDATES = [
    Candidate("base_fs40", 3, 84, 0.005, 10.0, False),
    Candidate("k5_t84_w05_m10", 5, 84, 0.005, 10.0, True),
    Candidate("k5_t63_w05_m10", 5, 63, 0.005, 10.0, True),
    Candidate("k5_t63_w01_m6", 5, 63, 0.01, 6.0, True),
    Candidate("k10_t63_w01_m6", 10, 63, 0.01, 6.0, True),
]
...
def run_candidate(candidate: Candidate) -> dict:
    ...
    command = [
        ...
        "--output-table",
        f"stock_predict_data_model_agent_{candidate.name}_1d_20260626",
...
def main() -> int:
    ...
    for candidate in CANDIDATES:
        ...
        for baseline_name in ["research1d_current", "formal1d_current", "base_fs40"]:
            if baseline_name == "base_fs40":
                base = next(row for row in rows if row["asset"] == "base_fs40")
            else:
    ...
    packet = {
        ...
        "scope": "research_only_1d_fixed4y_fold08_fs40_validation_scan",
        "label": LABEL,
        ...
        "baselines": {**BASELINE_TABLES, "base_fs40": str(BASE_PREDICTION)},
        "boundaries": {
            "research_only": True,
    ...
    lines = [
        ...
        "## 实验目的",
        "",
        "围绕当前 1D 近四年训练分支最优候选 `fs40_l16_lr005_n3600`，验证改变验证指标、验证尾窗和样本权重机制，是否能形成新的真实增益。",
        "",
        "## 结果摘要",
        "",
        f"- 当前排序第一：`{best['asset']}`",
