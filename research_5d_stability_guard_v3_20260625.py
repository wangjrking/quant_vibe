...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_5d_stability_guard_v3_research_20260625"
TARGET_TABLE = "stock_predict_data_model_agent_5d_stability_guard_v3_20260625_executable_5d_open_return_research"
...
SOURCES = {
    ...
    "base5": "stock_predict_data_model_agent_5d_cross_horizon_guard_v2_20260625_executable_5d_open_return_research",
...
def scan(scores: pd.DataFrame, labels: pd.DataFrame, features: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    rows = []
    daily_rows = []
    alt_sources = ["blend5", "cross5", "dyn5", "strict5", "formal5", "cross10", "cross3v2", "cross3", "formal10", "formal3"]
    for alt in alt_sources:
        alt_daily = metric_by_source[metric_by_source["source"] == alt].set_index("trade_date")
        ...
        for feature in candidate_features:
            ...
            for threshold in thresholds:
                for op in ["<=", ">="]:
                    ...
                    row["pass_basic"] = bool(
                        row["recent63_active_days"] >= 5
                        and row["recent63_top1_delta_vs_base5"] >= -0.002
                        and row["recent63_top5_delta_vs_base5"] >= -0.001
                        and row["recent20_top5_delta_vs_base5"] >= -0.001
                        and row["full_top5_delta_vs_base5"] >= -0.0005
...
def write_outputs(scan_result: pd.DataFrame, daily: pd.DataFrame, source_metrics: pd.DataFrame, stats: dict | None) -> None:
    ...
    manifest = {
        ...
        "generated_at": now_iso(),
        "decision": "candidate" if stats else "rejected",
        "baseline_for_scan": "research_5d_cross_horizon_guard_v2",
        "best_scan": best,
        "asset_stats": stats,
        "promotion_requires_user_confirmation": True,
        "audit_required_before_formal": True,
    ...
    if stats:
        lines.extend(
            [
                "本轮找到一个相对 5D v2 基准通过稳定化门槛的 research-only 候选。",
                "",
                "候选规则：",
                "",
                "```text",
        ...
        lines.append("本轮未找到相对 5D v2 基准通过稳定化门槛的候选。")
    lines.extend(
        [
            "",
            "## 证据路径",
