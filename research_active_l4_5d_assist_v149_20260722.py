...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_5d_assist_v149_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v149_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4策略5D辅助权重研究")
    ...
    if args.open_known_2026:
        ...
        for item in frozen["candidates"]:
            ...
            score, order = v95.score_pair(full, float(definition["weight_5d"]), 7, 0.1)
            for start in protocol["known_stress_buy_starts"]:
                daily = v139.run_case(full, score, order, definition, protocol, protocol["known_stress_end"], start)
    ...
    for weight_5d in protocol["grid"]["weight_5d"]:
        ...
        score, order = v95.score_pair(observation, float(weight_5d), 7, 0.1)
        daily = v139.run_case(observation, score, order, definition, protocol, protocol["observation_end"])
