...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_min_hold_threshold_v210_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v210_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def min_hold_schedule(arrays: dict, definition: dict) -> np.ndarray:
    momentum = v134.market_momentum(arrays, 10)
    threshold = float(definition["momentum_threshold"])
    result = np.full(len(momentum), 4, dtype=np.int16)
    finite = np.isfinite(momentum)
    if definition["three_day_when"] == "momentum_ge":
        result[finite & (momentum >= threshold)] = 3
    elif definition["three_day_when"] == "momentum_lt":
        result[finite & (momentum < threshold)] = 3
    else:
        raise ValueError("未知的三天持有触发方向")
    return result
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 最短持有期动量阈值研究")
    ...
    rows, definitions = [], {}
    for direction, threshold in product(
        protocol["grid"]["three_day_when"],
        protocol["grid"]["momentum_threshold"],
    ):
        definition = {
            **protocol["fixed_definition"],
            "three_day_when": direction,
            "momentum_threshold": float(threshold),
        }
