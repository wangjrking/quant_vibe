...
REPORT_DIR = DATA / "reports" / "strategy_agent_prod_amt9p5w_signal_field_micro_20260625"
BASE_SIGNAL = DATA / "reports" / "strategy_agent_prod_filter_amount_interpolate_20260625" / "signals" / "prod_filter_amt9p5w_mv20w.csv"

CASES = [
    {"case_name": "baseline", "continue_ratio": 0.975, "stop_loss": 0.06, "take_profit": 0.07},
    {"case_name": "continue097", "continue_ratio": 0.97, "stop_loss": 0.06, "take_profit": 0.07},
    {"case_name": "continue098", "continue_ratio": 0.98, "stop_loss": 0.06, "take_profit": 0.07},
    {"case_name": "sl055_tp07", "continue_ratio": 0.975, "stop_loss": 0.055, "take_profit": 0.07},
    {"case_name": "sl06_tp075", "continue_ratio": 0.975, "stop_loss": 0.06, "take_profit": 0.075},
    {"case_name": "sl055_tp075", "continue_ratio": 0.975, "stop_loss": 0.055, "take_profit": 0.075},
]
...
def _build_signal(case: dict) -> Path:
    rows = _read_rows(BASE_SIGNAL)
    out = []
    for row in rows:
        item = dict(row)
        item["score_continue_entry_ratio"] = f"{float(case['continue_ratio']):.5f}"
        item["signal_stop_loss_pct"] = f"{float(case['stop_loss']):.5f}"
        item["signal_take_profit_pct"] = f"{float(case['take_profit']):.5f}"
        item["strategy_variant"] = f"prod_filter_amt9p5w_mv20w_{case['case_name']}"
        out.append(item)
    signal_path = REPORT_DIR / "signals" / f"{case['case_name']}.csv"
    _write_rows(signal_path, out)
    return signal_path
...
def main() -> None:
    ...
    summary = []
    for case in CASES:
        signal_file = _build_signal(case)
        case_rows = []
