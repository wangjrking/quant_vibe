...
OUTPUT_ROOT = (
    REPO
    / "quant/data_file/reports/strategy_agent_v260_fixed10_entry_gap_down_guard_20260822"
)
CASES = {
    "production_no_gap_down_guard": {"min_gap": None, "selectable": False},
    "control_min_entry_gap_m003": {"min_gap": -0.03, "selectable": False},
    "candidate_min_entry_gap_m005": {"min_gap": -0.05, "selectable": True},
    "control_min_entry_gap_m007": {"min_gap": -0.07, "selectable": False},
}
...
def main() -> None:
    ...
    if access["logical_max_date"] != round1.DEVELOPMENT_END:
        raise PermissionError("2026 data entered gap-down development")
    ...
    for case_id, case in CASES.items():
        common = dict(
            ...
            min_entry_open_gap_override=case["min_gap"],
        )
        ...
        results[case_id] = {
            ...
            "minimum_entry_open_gap": case["min_gap"],
    ...
    baseline_id = "production_no_gap_down_guard"
    candidate_id = "candidate_min_entry_gap_m005"
    ...
    if not all(checkpoint_equivalence.values()):
        raise RuntimeError("gap-down baseline drifted")
    utility_deltas = {
        case_id: float(item["utility"] - baseline["utility"])
        for case_id, item in results.items() if case_id != baseline_id
    }
    local_robust = bool(
        utility_deltas["control_min_entry_gap_m003"] > 0
        and utility_deltas["control_min_entry_gap_m007"] > 0
    )
    candidate_passed = bool(
        local_robust
        and candidate["metrics_0_30pct"]["cagr"] > baseline["metrics_0_30pct"]["cagr"]
        and candidate["metrics_0_30pct"]["sharpe"] > baseline["metrics_0_30pct"]["sharpe"]
        and candidate["metrics_0_30pct"]["max_drawdown"] <= baseline["metrics_0_30pct"]["max_drawdown"]
        and candidate["metrics_0_65pct"]["cumulative_return"] > baseline["metrics_0_65pct"]["cumulative_return"]
    ...
    repeat_daily, repeat_actions = round1.run_fixed10(
        ...
        entry_block_mask_override=np.zeros(score.shape, dtype=np.bool_),
        maintenance_buy_block_mask_override=maintenance_block,
        min_entry_open_gap_override=CASES[selected]["min_gap"],
    )
    deterministic = {
    ...
    if not all(deterministic.values()):
        raise RuntimeError("gap-down replay failed")
    selected_policy = copy.deepcopy(policy)
    selected_policy["minimum_entry_open_gap"] = CASES[selected]["min_gap"]
    result = {
        ...
        "only_rule_changed": (
            ...
            "is below the minimum gap; refill from unchanged ranking"
        ),
        "selectable_candidate": candidate_id,
        "nonselectable_controls": ["control_min_entry_gap_m003", "control_min_entry_gap_m007"],
        "results": results,
