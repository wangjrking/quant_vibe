...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_research_5d_fixed4y_fold08_fs120_validation_scan_20260626"
...
CANDIDATES = [
    Candidate("base_fs120", 5, 126, 1.0, 1.0, False),
...
def run_candidate(candidate: Candidate) -> dict:
    if not candidate.retrain:
        return {"stdout": "reused existing base fs120 artifact", "stderr": "", "returncode": 0, "reused": True}
...
def main() -> int:
    ...
    for candidate in CANDIDATES:
        ...
        for baseline_name in ["research5d_current", "formal5d_current", "base_fs120"]:
            if baseline_name == "base_fs120":
                base = next(row for row in rows if row["asset"] == "base_fs120")
            else:
    ...
    packet = {
        ...
        "scope": "research_only_5d_fixed4y_fold08_fs120_validation_scan",
        "label": LABEL,
        ...
        "baselines": {**BASELINE_TABLES, "base_fs120": str(BASE_PREDICTION)},
        "boundaries": {
            "research_only": True,
    ...
    lines = [
        ...
        "## 实验目的",
        "",
        "围绕当前近四年训练分支里最好的 `fs120` 候选，验证改变验证指标与验证尾窗长度，是否能形成新的真实增益。",
        "",
        "## 结果摘要",
        "",
        f"- 当前排序第一：`{best['asset']}`",
