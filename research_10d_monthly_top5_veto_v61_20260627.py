...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_10d_monthly_top5_veto_v61_20260627"
...
TARGET_TABLE = "stock_predict_data_model_agent_10d_monthly_top5_veto_v61_20260627_executable_10d_open_return_research"
...
def scan(base_daily: pd.DataFrame, cand_daily: pd.DataFrame, features: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, object] | None, pd.DataFrame | None]:
    ...
    for feature in FEATURES:
        values = features.loc[features["trade_date"].isin(base_gate_dates), feature].dropna()
        if values.empty:
            continue
        thresholds = sorted(set(float(values.quantile(q)) for q in QUANTILES))
        for threshold in thresholds:
            for op in ["<=", ">="]:
                allowed = set(features.loc[features[feature] <= threshold, "trade_date"] if op == "<=" else features.loc[features[feature] >= threshold, "trade_date"])
                active_dates = base_gate_dates & allowed
    ...
    result = pd.DataFrame(rows).sort_values(
        ["pass_hard", "objective", "full_top1_delta", "nonnegative_top5_months"],
...
def write_table(scores: pd.DataFrame, active_dates: set[str]) -> dict[str, object]:
    ...
    out.loc[out["trade_date"].isin(active_dates), "score_formula"] = "cand_rank after monthly_top5_veto_v61"
    with sqlite3.connect(MODEL_DB, timeout=300) as conn:
        ...
        conn.execute(f"create index if not exists idx_10d_monthly_top5_veto_v61_date_code on {quote(TARGET_TABLE)}(trade_date, stock_code)")
        conn.commit()
        row = conn.execute(
...
def main() -> int:
    ...
    result.to_csv(REPORT_DIR / "monthly_top5_veto_scan.csv", index=False, encoding="utf-8-sig")
    asset_stats = None
    if best is not None:
    ...
    summary = {
        ...
        "scope": "research_only_10d_monthly_top5_veto_v61",
        "label": LABEL,
        ...
        "decision": "promote_v61_research_candidate" if asset_stats else "no_monthly_top5_safe_candidate_found",
        "boundaries": {
            "research_only": True,
            "no_training": True,
    ...
    (REPORT_DIR / "monthly_top5_veto_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    lines = [
        "# 10D 月度 Top5 veto 搜索 v61",
        "",
        ...
        "",
        "本轮只在 research-only 范围内扫描 10D 日期级 veto，用于修复 v60 暴露的最差月份 Top5 负增量。",
        "",
        "## 边界",
        "",
        "- 未训练模型",
    ...
    (REPORT_DIR / "monthly_top5_veto_report.md").write_text("\n".join(lines), encoding="utf-8")
