...
base.REPORT_DIR = (
    ...
    / "reports"
    / "strategy_agent_goal_high_annual_20260623"
    / "current_formal_flat_noscoreexit"
)
base.FUSION_DB = base.REPORT_DIR / "fusion_flat_noscoreexit.db"
base.SCORE_DB = base.REPORT_DIR / "scores_flat_noscoreexit.db"


def _cfg(name: str, *, holding_days: int, continue_ratio: str | None = None) -> dict:
    env = {
        "GM_OPEN_DAILY_SCORE_EXIT": "0",
        "GM_MAX_HOLDING_DAYS": str(max(holding_days, 8)),
    }
...
base.VARIANTS = [
    _cfg("noscore_h3", holding_days=3),
    _cfg("noscore_h4", holding_days=4),
    _cfg("noscore_h5", holding_days=5),
    _cfg("noscore_h6", holding_days=6),
    _cfg("noscore_h4_cont100", holding_days=4, continue_ratio="1.00"),
    _cfg("noscore_h4_cont098", holding_days=4, continue_ratio="0.98"),
    _cfg("noscore_h5_cont100", holding_days=5, continue_ratio="1.00"),
    _cfg("noscore_h5_cont098", holding_days=5, continue_ratio="0.98"),
]
        ...
        signals.append(
            {
                ...
                "max_holding_days": max(int(cfg["holding_days"]), 8),
            }
        )
    return signals
