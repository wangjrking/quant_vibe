...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_5d_refined_recent_scan_20260626"
...
TARGET_TABLE = "stock_predict_data_model_agent_5d_refined_recent_scan_20260626_executable_5d_open_return_research"

SOURCES = {
    "base": BASE_TABLE,
    "lowstd_mainline": "stock_predict_data_model_agent_5d_v6shrink_lowstd_guard_20260626_executable_5d_open_return_research",
    "recent_v4": "stock_predict_data_model_agent_5d_recent_enhance_v4_20260625_executable_5d_open_return_research",
    "risk_v7": "stock_predict_data_model_agent_5d_risk_balanced_v7_20260625_executable_5d_open_return_research",
...
def objective(row: dict[str, float | int]) -> float:
    return (
        2.3 * row["recent63_top5_delta"]
        + 2.0 * row["recent20_top5_delta"]
        + 0.8 * row["full_top5_delta"]
        + 1.0 * row["recent63_top1_delta"]
        + 0.6 * row["recent20_top1_delta"]
        + 0.3 * row["full_top1_delta"]
        + 0.5 * row["recent63_rank_ic_delta"]
        + 0.2 * row["full_rank_ic_delta"]
...
def scan(scores: pd.DataFrame, labels: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, pd.DataFrame], pd.DataFrame]:
    ...
    for alt in ["lowstd_mainline", "recent_v4", "risk_v7", "formal5"]:
        ...
        for feature in FEATURES:
            ...
            for threshold in thresholds:
                for op in ["<=", ">="]:
                    ...
                    if len(active_dates) < 4 or len(active_dates) > int(0.20 * len(feature_table)):
                        continue
                    ...
                    row["pass_basic"] = bool(
                        row["full_rank_ic_delta"] >= -0.00010
                        and row["recent63_rank_ic_delta"] >= -0.00020
                        and row["full_top5_delta"] >= 0.0
                        and row["recent63_top5_delta"] >= 0.0
                        and row["recent20_top5_delta"] >= 0.0
                        and row["recent63_top1_delta"] >= -0.001
                        and row["min_period_top5_delta"] >= -0.0001
...
def main() -> int:
    ...
    result.to_csv(REPORT_DIR / "refined_recent_scan_results.csv", index=False, encoding="utf-8-sig")
    source_daily.to_csv(REPORT_DIR / "refined_recent_source_daily.csv", index=False, encoding="utf-8-sig")
    ...
    summary = {
        ...
        "scope": "research_only_5d_refined_recent_scan",
    ...
    (REPORT_DIR / "refined_recent_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    return 0
