...
OUT_DIR = DATA_DIR / "reports" / "model_agent_5d_topgate_3dformal_20260623"
...
RESEARCH_TABLE = "stock_predict_data_model_agent_5d_topgate_3dformal_20260623_executable_5d_open_return_research"
...
def materialize(merged: pd.DataFrame, selected_summary: dict, window_summaries: dict) -> dict:
    ...
    manifest = {
        ...
        "score_formula": (
            "base_5d_rank + 0.02 * 3d_formal_rank when base_5d_rank >= 0.995; "
            "otherwise keep base_5d_rank"
        ),
        ...
        "notes": "5D 使用 3D formal 做顶部局部重排的研究资产，仅供研究验证，不进入 formal 或 L5。",
    }
...
def main() -> int:
    ...
    merged["topgate_applied"] = merged["rank_base_5d"] >= BASE_RANK_THRESHOLD
    merged.loc[merged["topgate_applied"], "topgate_score"] = (
        merged.loc[merged["topgate_applied"], "rank_base_5d"]
        + AUX_ADD_ALPHA * merged.loc[merged["topgate_applied"], "rank_aux_3d_formal"]
    )

    baseline, baseline_daily = evaluate("base_5d_formal", merged, "base_5d_pred_prob")
    selected, selected_daily = evaluate("topgate_3d_formal", merged, "topgate_score")
    ...
    payload = {
        ...
        "manifest": manifest,
        "selection_rule": (
            "固定使用 5D 顶部 0.5% + 3D formal rank 局部重排，"
            "作为 research-only 候选保留，不切换 formal。"
        ),
    }
    (OUT_DIR / "topgate_3dformal_summary.json").write_text(
        json.dumps(payload, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )

    lines = [
        "# 5D 顶部 3D formal 重排研究",
        "",
        "## 当前结论",
        "",
        "本轮将快速筛选中最优的 5D + 3D formal 顶部局部重排方案单独落为 research-only 资产。",
        "该候选的特点是保住 5D 全窗口 Top1，同时把 recent80 Top1 从负值拉到正值。",
        "",
        "## 参数",
        "",
        f"- 基线表：`{BASE_TABLE}`",
