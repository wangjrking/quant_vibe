...
OUTPUT_ROOT = (
    ...
    "strategy_agent_v260_fixed10_price_stress_min_hold_exception_20260822"
)
CHECKPOINT = (
    REPO
    / "quant/data_file/reports/"
...
def run(context, policy: dict, min_hold_schedule, priority_matrix, cost: float):
    ...
    return age_guard.run_policy_at_cost(
        ...
        cost,
        min_hold_days_override=min_hold_schedule,
...
def main() -> None:
    ...
    if checkpoint["validation_2026_opened"]:
        raise PermissionError("2026 data entered price-stress min-hold development")
    ...
    schedules = {
        "constant_min_hold4": np.full(len(context.arrays["dates"]), 4, dtype=np.int64),
        "price_stress_min_hold1": np.where(price_stress, 1, 4).astype(np.int64),
    }
    results, cache = {}, {}
    for candidate_id, schedule in schedules.items():
        ...
        for cost in COST_LEVELS:
            daily, actions = run(
                context, policy, schedule, priority_matrix, cost
            )
    ...
    baseline_id = "constant_min_hold4"
    candidate_id = "price_stress_min_hold1"
    ...
    if not all(equivalent.values()):
        raise RuntimeError("price-stress min-hold baseline drifted")
    ...
    gates = {
        ...
        "training_drawdown_improved": candidate["train_2022_2024"]["max_drawdown"]
    ...
    repeat_daily, repeat_actions = run(
        context, policy, schedules[selected], priority_matrix, 0.0030
    )
    deterministic = {
    ...
    if not all(deterministic.values()):
        raise RuntimeError("price-stress min-hold replay failed")
    result = {
        "status": "price_stress_min_hold_test_complete_2026_not_opened",
        "only_change": (
            "minimum hold falls from four sessions to one only when trailing "
            "20-session cross-sectional median return is below -5%"
        ),
