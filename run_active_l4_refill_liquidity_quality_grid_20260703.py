...
OUT_DIR = REPORT_DIR / "variants_refill_liquidity_quality_grid"
RUN_DIR = REPORT_DIR / "juejin_runs_refill_liquidity_quality_grid"
SUMMARY_CSV = REPORT_DIR / "juejin_refill_liquidity_quality_grid_summary.csv"
RAW_CSV = REPORT_DIR / "juejin_refill_liquidity_quality_grid_raw.csv"


# name, extra SQL filter
CASES = [
    ("base_scale124", ""),
    ("amt8w_scale124", "AND buy_amount >= 80000 AND signal_amount >= 80000"),
    ("amt12w_scale124", "AND buy_amount >= 120000 AND signal_amount >= 120000"),
    ("no_mv30_50_scale124", "AND NOT (buy_total_mv >= 300000 AND buy_total_mv < 500000)"),
    ("amt8w_no_mv30_50_scale124", "AND buy_amount >= 80000 AND signal_amount >= 80000 AND NOT (buy_total_mv >= 300000 AND buy_total_mv < 500000)"),
    ("mv_ge50_or_lt30_scale124", "AND (buy_total_mv < 300000 OR buy_total_mv >= 500000)"),
    ("amount_pref_sort_scale124", ""),
]
...
def target_for(row: pd.Series) -> float:
    ...
    if 2.0 <= atr < 4.0:
        target -= 0.22
    return float(max(0.05, min(0.82, target * 1.24)))


def export_case(con: duckdb.DuckDBPyConnection, case: tuple) -> dict:
    name, extra_filter = case
    order_expr = "buy_open_gap_raw_pct ASC, pred_prob DESC, stock_code"
    if name == "amount_pref_sort_scale124":
        order_expr = "CASE WHEN buy_amount >= 80000 THEN 0 ELSE 1 END ASC, buy_open_gap_raw_pct ASC, pred_prob DESC, stock_code"
    df = con.execute(
        f"""
        WITH picked AS (
            SELECT
    ...
    return {
        ...
        "avg_buy_amount": float(df["buy_amount"].mean()),
    }
