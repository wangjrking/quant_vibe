...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_3d_cross_horizon_guard_research_20260625"
TARGET_TABLE = "stock_predict_data_model_agent_3d_cross_horizon_guard_20260625_executable_3d_open_return_research"
...
def write_outputs(scan_result: pd.DataFrame, daily: pd.DataFrame, source_metrics: pd.DataFrame, stats: dict | None) -> None:
    ...
    lines = ["# 3D 跨周期动态守卫研究报告（20260625）", "", "## 当前结论", ""]
    if stats:
        lines.extend(
            [
                "本轮找到一个通过基础模型评价门槛的 3D cross-horizon research-only 候选。",
                "",
                "候选规则：",
                "",
                "```text",
