...
SIGNAL_DIR = OUT_DIR / "top2_feature_risk_refine_signals"
RUN_DIR = OUT_DIR / "top2_feature_risk_refine_grid"
SUMMARY = OUT_DIR / "top2_feature_risk_refine_summary.csv"
...
# name, extra_where, neutral_mult, mv_mid_mult, upper_score, row_scale, cap
CASES = [
    ("base", "", 1.0, 1.0, 1.0, 1.50, 0.82),
    ("neutral_down50", "", 0.50, 1.0, 1.0, 1.50, 0.82),
    ("neutral_filter", "AND NOT (signal_pct_chg > -2.0 AND signal_pct_chg <= 0.0)", 1.0, 1.0, 1.0, 1.50, 0.82),
    ("mv300_1000_down50", "", 1.0, 0.50, 1.0, 1.50, 0.82),
    ("mv300_1000_filter", "AND NOT (signal_total_mv > 300000 AND signal_total_mv <= 1000000)", 1.0, 1.0, 1.0, 1.50, 0.82),
    ("neutral_mv_down", "", 0.50, 0.50, 1.0, 1.50, 0.82),
    ("score_mid_only", "AND score_pct_rank < 0.985", 1.0, 1.0, 0.985, 1.50, 0.82),
    ("score_mid_neutral_down", "AND score_pct_rank < 0.985", 0.50, 1.0, 0.985, 1.50, 0.82),
    ("conservative_combo", "AND score_pct_rank < 0.985", 0.50, 0.50, 0.985, 1.20, 0.65),
]
...
def export_signal(case: tuple[str, str, float, float, float, float, float]) -> tuple[Path, dict[str, object]]:
    name, extra_where, neutral_mult, mv_mid_mult, upper_score, row_scale, cap = case
        ...
        df = con.execute(
            ...
            WITH picked AS (
                ...
                WHERE score_pct_rank >= 0.95
                  AND score_pct_rank < {float(upper_score) + 1e-12}
    ...
    mult.loc[(df["signal_pct_chg"] > -2.0) & (df["signal_pct_chg"] <= 0.0)] *= neutral_mult
    mult.loc[(df["signal_total_mv"] > 300000) & (df["signal_total_mv"] <= 1000000)] *= mv_mid_mult
...
def run_case(case: tuple[str, str, float, float, float, float, float]) -> dict[str, object]:
    name, extra_where, neutral_mult, mv_mid_mult, upper_score, row_scale, cap = case
