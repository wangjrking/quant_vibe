...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_filtered_strong_allocation_v181_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v181_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 filtered strong-allocation study")
    ...
    rows, definitions = [], {}
    for target in protocol["grid"]["strong_target_pct"]:
        definition = {**protocol["fixed_definition"], "strong_target_pct": float(target)}
