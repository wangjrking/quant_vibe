...
REPORT_DIR = SOURCE_DIR / "strict_position_sync_validation_20260624"
...
SELECTED_NAMES = [
    ...
    "div_top1_w90_5d10_h5_e099_pos90",
    "div_top1_w90_5d10_h5_e098",
    "div_top1_w91_5d09_h5_e099",
    "div_top1_w89_5d11_h5_e099",
    "div_top1_w89_5d11_h5_e098",
    "div_top1_w88_5d12_h5_e098",
    "div_top1_w87_5d13_h5_e098",
    "div_top1_w90_5d10_h5_e099_mh2",
    "div_top1_w90_5d10_h5_e098_mh2",
    "div_top2_w90_5d10_55_43_h5_e098",
    "div_top3_w90_5d10_34_33_32_h5_e098",
]
...
def _load_cases() -> list[dict]:
    ...
    cases: dict[str, dict] = {}
    with (SOURCE_DIR / "cases.csv").open("r", encoding="utf-8-sig", newline="") as file:
        for row in csv.DictReader(file):
            name = str(row.get("name") or "")
            if name not in selected or name in cases:
                continue
            config = json.loads(row["config_json"])
            targets = [float(value) for value in config["targets"]]
            cases[name] = {
                "name": name,
                "target_pct": max(targets),
                "max_positions": len(targets),
                "holding_days": int(config["holding_days"]),
                "exit_ratio": float(config["exit_ratio"]),
                "min_hold": int(config["min_hold"]),
            }
    missing = sorted(selected - set(cases))
    if missing:
        raise RuntimeError(f"selected cases missing from source cases.csv: {missing}")
    return [cases[name] for name in SELECTED_NAMES]
...
def _run_case(case: dict, start_name: str, start: str) -> dict:
    ...
    if indicator is None:
        ...
        command = [
            ...
            str(STRATEGY_DIR),
            "--signal-file",
            str(SOURCE_DIR / "signals" / f"{case['name']}.csv"),
            "--log-file",
            str(log_file),
            "--max-positions",
...
def _summarize(rows: list[dict]) -> list[dict]:
    ...
    for name in sorted({row["name"] for row in rows}):
        ...
        late = [float(row["annual"]) for row in items if str(row["start_name"]).startswith("late_") and row.get("annual") is not None]
        if not full:
            continue
        out.append(
            {
    ...
    out.sort(key=lambda row: float(row["full_annual"] or -999), reverse=True)
    return out
...
def main() -> int:
    ...
    for case in cases:
        for start_name, start in STARTS:
            ...
            rows.append(row)
            _write_rows(REPORT_DIR / "strict_sync_cases.csv", rows)
            print(
    ...
    _write_rows(REPORT_DIR / "strict_sync_summary.csv", summary)
    (REPORT_DIR / "strict_sync_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    return 0
