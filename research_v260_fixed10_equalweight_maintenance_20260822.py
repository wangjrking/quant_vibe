...
OUTPUT_ROOT = (
    REPO
    / "quant/data_file/reports/strategy_agent_v260_fixed10_equalweight_maintenance_20260822"
)

CANDIDATES = {
    "max20_hold_only": (20, "no_score_exit"),
    "max20_periodic_equalweight": (20, "no_score_exit_rebalance"),
    "max25_hold_only": (25, "no_score_exit"),
    "max25_periodic_equalweight": (25, "no_score_exit_rebalance"),
}


def candidate_policy(max_hold: int, renewal_policy: str) -> dict:
    current = round2.policy(max_hold, 1)
    current["renewal_policy"] = str(renewal_policy)
    return current


def maintenance_action_metrics(actions) -> dict:
    if actions.empty:
        return {
            "position_exit_count": 0,
            "maintenance_sell_count": 0,
            "all_sell_action_count": 0,
        }
    sells = actions[actions["action"] == "SELL"].copy()
    targets = sells["target_pct"].astype(float)
    return {
        "position_exit_count": int((targets == 0.0).sum()),
        "maintenance_sell_count": int((targets > 0.0).sum()),
        "all_sell_action_count": int(len(sells)),
    }
...
def main() -> None:
    ...
    if access["logical_max_date"] != round1.DEVELOPMENT_END:
        raise PermissionError("2026 data entered equalweight maintenance development")
    definition = harness.production_definition(protocol)
    score, order = harness.v95.score_pair(arrays, 0.0, 7, 0.1)

    results = {}
    run_cache = {}
    for candidate_id, (max_hold, renewal_policy) in CANDIDATES.items():
        current = candidate_policy(max_hold, renewal_policy)
        ...
        results[candidate_id] = {
            ...
            "action_diagnostics": maintenance_action_metrics(actions),
    ...
    if not all(deterministic.values()):
        raise RuntimeError("equalweight maintenance deterministic replay failed")
    original_daily, original_actions = round1.run_fixed10(
        harness,
        arrays,
        protocol,
        definition,
    ...
    production_source_equivalence = {
        "daily": round1.frame_hash(selected_daily) == round1.frame_hash(original_daily),
        "actions": round1.frame_hash(selected_actions)
        == round1.frame_hash(original_actions),
    }
    if not all(production_source_equivalence.values()):
        raise RuntimeError("safe runtime drifted from production source on valid path")
    ...
    result = {
        ...
        "selection_rule": (
            ...
            "drawdown, invested ratio and turnover; 2025 is confirmation only"
        ),
        "results": results,
        ...
        "production_source_equivalence_on_valid_path": production_source_equivalence,
        "data_access": access,
