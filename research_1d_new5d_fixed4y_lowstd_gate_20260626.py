...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_1d_new5d_fixed4y_lowstd_gate_20260626"
...
TARGET_TABLE = "stock_predict_data_model_agent_1d_new5d_fixed4y_lowstd_gate_20260626_executable_1d_open_return_research"
ALT_PATH = (
    DATA_DIR
    / "reports"
...
def evaluate(scored: pd.DataFrame, labels: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, dict[str, object]]:
    ...
    candidate = daily_metrics(eval_frame, "pred_prob", "fixed4y_lowstd_gate")
...
def write_outputs(
    ...
    manifest = {
        ...
        "candidate_id": "model_agent_1d_new5d_fixed4y_lowstd_gate_20260626",
        "score_formula": stats["formula"],
        "research_hypothesis": "use_fixed4y_fs40_candidate_on_low_base_std_days_with_current_1d_mainline",
    ...
    lines = [
        "# 1D new5d 主线与近四年候选的 low-std 门控研究报告（20260626）",
        "",
        "## 当前结论",
        "",
        ...
        "## 当前判断",
        "",
        "- 这条分支说明：1D 当前主线整体仍强于近四年训练候选，但在低波动交易日里，近四年候选的局部替换可以带来额外增益。",
        "- 这是当前 1D 主线的增量修补候选，不是新的独立训练主线。",
        "",
        "## 边界",
        "",
        "- 未训练模型。",
