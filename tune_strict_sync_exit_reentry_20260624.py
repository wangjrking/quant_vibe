...
REPORT_DIR = SOURCE_DIR / "strict_sync_exit_reentry_20260624"
...
SOURCES = [
    ...
    "div_top1_w90_5d10_h3_e099",
]
TARGET_PCTS = [0.75, 0.90, 0.99]
HOLDING_DAYS = [3, 5]
EXIT_RATIOS = [0.97, 0.99, 1.01]
MIN_HOLDS = [0, 1]
...
def _case_name(source: str, target_pct: float, holding_days: int, exit_ratio: float, min_hold: int) -> str:
    return (
        f"exit_{source}"
            ...
            for holding_days in HOLDING_DAYS:
                for exit_ratio in EXIT_RATIOS:
                    for min_hold in MIN_HOLDS:
                        cases.append(
                            {
                                "name": _case_name(source, target_pct, holding_days, exit_ratio, min_hold),
                                "source": source,
...
def _summarize(rows: list[dict]) -> list[dict]:
        ...
        out.append(
            {
                ...
                "holding_days": full.get("holding_days"),
                "exit_ratio": full.get("exit_ratio"),
                "min_hold": full.get("min_hold"),
    ...
    out.sort(key=lambda row: _f(row["score"]), reverse=True)
    return out
