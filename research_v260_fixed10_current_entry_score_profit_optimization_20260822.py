...
import research_v260_fixed10_entry_score_1d70_10d30_current_rules_20260822 as entry_tools
...
ENTRY_1D_WEIGHTS = (0.0, 0.1, 0.3, 0.7)


def run_case(context, policy: dict, weight_1d: float, cost: float):
    _, entry_order = entry_tools.entry_score(context.arrays, weight_1d)
    case_context = replace(context, order=entry_order)
...
def main() -> None:
    ...
    if checkpoint["validation_2026_opened"]:
        raise PermissionError("2026 data entered entry-score optimization")
    policy = copy.deepcopy(checkpoint["selected_policy"])
    context = harness.load_context(policy)

    results = {}
    frames = {}
    for weight_1d in ENTRY_1D_WEIGHTS:
        daily, actions = run_case(
            context, policy, weight_1d, width.BASELINE_COST
        )
        stress_daily, stress_actions = run_case(
            context, policy, weight_1d, width.STRESS_COST
        )
        baseline = width.metrics(daily, actions, 10)
        stress = width.metrics(stress_daily, stress_actions, 10)
        key = f"{weight_1d:.1f}"
        results[key] = {
            "entry_weight_1d": weight_1d,
            "entry_weight_10d": 1.0 - weight_1d,
    ...
    if not all(deterministic.values()):
        raise RuntimeError("selected entry-score replay is not deterministic")

    result = {
        "status": "current_entry_score_profit_optimization_complete_2026_not_opened",
        "only_change": "one-day versus ten-day weight used for new-entry ordering",
        "selection_rule": "maximize_pre2026_cumulative_return_subject_to_basic_risk_floors",
        "candidates": results,
        "selected_entry_weight_1d": float(selected),
