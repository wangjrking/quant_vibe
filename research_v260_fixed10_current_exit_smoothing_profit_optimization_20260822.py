...
EXIT_SMOOTHING_WINDOWS = (3, 5, 7, 10)


def exit_score(context, window: int):
    score, _order = context.harness.v95.score_pair(
        context.arrays, 0.0, int(window), 0.1
    )
    return score


def run_case(context, policy: dict, window: int, cost: float):
    ...
    return age_guard.run_policy_at_cost(
        ...
        exit_score_override=exit_score(context, window),
    )
...
def main() -> None:
    ...
    if checkpoint["validation_2026_opened"]:
        raise PermissionError("2026 data entered exit-smoothing optimization")
    policy = copy.deepcopy(checkpoint["selected_policy"])
    context = harness.load_context(policy)

    results = {}
    frames = {}
    for window in EXIT_SMOOTHING_WINDOWS:
        daily, actions = run_case(
            context, policy, window, width.BASELINE_COST
        )
        stress_daily, stress_actions = run_case(
            context, policy, window, width.STRESS_COST
        )
        baseline = width.metrics(daily, actions, 10)
        stress = width.metrics(stress_daily, stress_actions, 10)
        key = str(window)
        results[key] = {
            "entry_smoothing_window": 7,
            "exit_smoothing_window": window,
    ...
    if not all(deterministic.values()):
        raise RuntimeError("selected exit-smoothing replay is not deterministic")

    result = {
        "status": "current_exit_smoothing_profit_optimization_complete_2026_not_opened",
        "only_change": "trailing session count used by the exit score",
        ...
        "candidates": results,
        "selected_exit_smoothing_window": int(selected),
        "selected_minus_current_7d_exit": round1.numeric_delta(
            results[selected]["metrics_0_30pct"],
            results["7"]["metrics_0_30pct"],
        ),
