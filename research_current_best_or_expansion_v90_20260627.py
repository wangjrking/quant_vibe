...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_current_best_or_expansion_v90_20260627"
SNAPSHOT = DATA_DIR / "reports" / "model_agent_current_best_research_snapshot_20260627_v88" / "current_best_research_snapshot_v88.json"
...
TARGET_TABLES = {
    "executable_1d_open_return": "stock_predict_data_model_agent_1d_or_expansion_v90_20260627_executable_1d_open_return_research",
    "executable_3d_open_return": "stock_predict_data_model_agent_3d_or_expansion_v90_20260627_executable_3d_open_return_research",
    "executable_5d_open_return": "stock_predict_data_model_agent_5d_or_expansion_v90_20260627_executable_5d_open_return_research",
    "executable_10d_open_return": "stock_predict_data_model_agent_10d_or_expansion_v90_20260627_executable_10d_open_return_research",
}
RANK_IC_FLOORS = {
...
def active_dates_from_table(table: str) -> set[str]:
    with sqlite3.connect(DB_PATH, timeout=300) as conn:
        ...
        if "score_formula" in cols:
            ...
            rows = conn.execute(f"select distinct trade_date from {quote(table)}").fetchall()
...
def evaluate_daily_pair(pair: pd.DataFrame, formal_daily: pd.DataFrame, active_dates: set[str]) -> dict[str, object]:
...
def scan_or_expansion(label: str, base_active: set[str], features: pd.DataFrame, pair: pd.DataFrame, formal_daily: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, object]]:
    rows = []
    evaluated = []

    base_result = evaluate_daily_pair(pair, formal_daily, base_active)
    base_row = make_row(label, "v88_base_condition", len(base_active), base_active, base_result)
    ...
    for feature in guarded.FEATURES:
        ...
        for threshold in thresholds:
            for op in ["<=", ">="]:
                ...
                if len(active_dates) <= len(base_active) or len(active_dates) > 280:
                    continue
                condition = f"v88_base OR {feature} {op} {threshold:.12g}"
                result = evaluate_daily_pair(pair, formal_daily, active_dates)
...
def run_label(label: str, item: dict[str, object], config: dict[str, object], expected_latest: str, expected_rows: int) -> dict[str, object]:
    ...
    spec = {
        "asset": f"research_{label.replace('executable_', '').replace('_open_return', '')}_or_expansion_v90_20260627",
    ...
    scan, best = scan_or_expansion(label, base_active, features, pair, formal_daily)
    scan.to_csv(output_dir / "or_expansion_scan.csv", index=False, encoding="utf-8-sig")
    ...
    candidate = {
        ...
        "prefer_simpler_formula": "v88 base condition plus one OR expansion filter",
        "prefer_fewer_active_days": row["active_days"],
        "prefer_fewer_upstream_sources": "formal+candidate score tables",
        "prefer_clearer_roll_back_path": "drop research-only v90 table and keep current formal unchanged",
    }
    ...
    manifest = {
        ...
        "feature_input": "quant/data_file/production_factor_parts/",
        "label_input": "quant/data_file/prediction_label_parts/",
        "formula": "v88 base condition plus one OR expansion filter",
    ...
    (output_dir / f"{label}_v90_summary.json").write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    report = f"""# {label} OR 扩展门控研究 v90

## 结论

本轮只在 v88 实际触发日期基础上额外 OR 一个状态条件。未训练模型，未修改 formal/production manifest，未生成信号或回测结论。
...
"""
    (output_dir / f"{label}_v90_report.md").write_text(report, encoding="utf-8")
    return payload
...
def main() -> int:
    ...
    payload = {
        ...
        "scope": "research_only_current_best_or_expansion_v90",
    ...
    matrix.to_csv(REPORT_DIR / "v90_summary_matrix.csv", index=False, encoding="utf-8-sig")
    payload["summary_matrix"] = str(REPORT_DIR / "v90_summary_matrix.csv")
    (REPORT_DIR / "v90_run_summary.json").write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    lines = [
        "# 当前最优 OR 扩展门控研究 v90",
        "",
        "## 结论",
        "",
        "本轮只扫描 v88 实际触发日期上的 OR 扩展条件，不训练模型，不修改 formal/production manifest，不生成信号，不运行回测。",
        "",
    ]
    for row in rows:
        lines.extend(
    ...
    (REPORT_DIR / "v90_report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
