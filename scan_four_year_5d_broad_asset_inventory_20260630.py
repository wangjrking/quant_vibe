...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_four_year_5d_broad_asset_inventory_20260630"
...
def compare_against_base(
    ...
    result: dict[str, object] = {
        ...
        "relative_daily_path": relative_daily_path,
    ...
    for metric in METRICS:
        ...
        result[f"recent20_{metric}_delta"] = float((recent20[f"{metric}_cand"] - joined[f"{metric}_base"].tail(len(recent20))).mean())
...
def is_candidate_daily(path: Path) -> bool:
    text = str(path).lower()
    if "four_year" not in text:
        return False
    if "executable_5d_open_return" not in path.name.lower():
        return False
    if "daily_eval.csv" not in path.name.lower():
        return False
    if BASE_REPORT_DIR_NAME.lower() in text:
        return False
    if "fold1_daily_eval" in path.name.lower():
        return False
    return True
...
def write_markdown(path: Path, payload: dict[str, object]) -> None:
    ...
    lines = [
        ...
        "## 结论",
        "",
        f"- 扫描到的四年 5D 资产数：{payload['candidate_count']}",
        f"- 满足当前替换规则的资产数：{payload['replaceable_count']}",
        f"- 按 Full Top5 相对当前 bestset 排名第一：`{best['candidate_dir']}` / `{best['daily_file']}`",
        f"- 该资产的 `full_top5_delta={best['full_top5_delta']:.6f}`，`full_rank_ic_delta={best['full_rank_ic_delta']:.6f}`，`recent63_top5_delta={best['recent63_top5_delta']:.6f}`，`recent20_top5_delta={best['recent20_top5_delta']:.6f}`，`month_min_top5_delta={best['month_min_top5_delta']:.6f}`",
        "",
        "## 判断",
        "",
        "- 即使把扫描范围从同源 5D 目录扩到全部可见四年 5D 资产，当前仍没有现成资产可以直接替代 `current_bestset_direct_guard_v1`。",
        "- 最接近的候选仍然主要是近期 TopN 看起来更强，但 Full RankIC 或月度尾部不达标。",
        "- 下一步重点不应继续做库存比对，而应转入新的 5D 训练族或新的跨源构造族。",
        "",
    ]
...
def main() -> int:
    ...
    for daily_file in sorted(REPORTS_DIR.rglob("*daily_eval.csv")):
        if not is_candidate_daily(daily_file):
            continue
        relative_daily_path = str(daily_file.relative_to(REPORTS_DIR))
        relative_parts = daily_file.relative_to(REPORTS_DIR).parts
        candidate_dir = relative_parts[0]
        daily = load_daily(daily_file)
        row = compare_against_base(
            base,
            daily,
            candidate_dir=candidate_dir,
        ...
        if int(row["joined_days"]) < MIN_JOINED_DAYS:
            continue
        rows.append(row)
    ...
    payload = {
        ...
        "scope": "scan_broad_four_year_5d_assets_against_current_bestset",
    ...
    (REPORT_DIR / "broad_asset_inventory_summary.json").write_text(
        json.dumps(payload, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    write_markdown(REPORT_DIR / "broad_asset_inventory_summary.md", payload)
