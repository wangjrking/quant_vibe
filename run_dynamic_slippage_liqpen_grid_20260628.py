...
SCORE_DB = DATA / "reports" / "strategy_agent_tune_four_year_formal_current_rules_20260628" / "scores" / "grid_scores.duckdb"
...
def _run(signal_file: Path, slice_name: str, start: str, end: str) -> dict[str, Any]:
    case_key = signal_file.stem
    log_file = REPORT_DIR / "logs" / f"{case_key}__adaptive70w__{slice_name}.log"
    ...
    if indicator is None:
        ...
        cmd = [
            ...
            str(SCORE_DB),
            "--score-table",
            "score_w75_20_05_amt90_mv20__base_h2m3_c097_e097_ddtight",
            "--market-db",
            str(MARKET_DB),
            "--backtest-start",
            start,
        ...
        if proc.returncode != 0 or indicator is None:
            return {"case_key": case_key, "slice": slice_name, "returncode": proc.returncode, "log_file": str(log_file)}
    return {
        "case_key": case_key,
        "slice": slice_name,
...
def main() -> None:
    signals = sorted((REPORT_DIR / "signals").glob("*liqpen*.csv"))
    detail: list[dict[str, Any]] = []
    summary: list[dict[str, Any]] = []
    for signal in signals:
        rows = [_run(signal, *slice_args) for slice_args in SLICES]
        detail.extend(rows)
        out = {"case_key": signal.stem, "signal_file": str(signal)}
        for row in rows:
            prefix = row["slice"]
            for key in ["annual", "pnl_ratio", "sharpe", "max_drawdown", "win_ratio", "open_count"]:
                out[f"{prefix}_{key}"] = row.get(key)
        summary.append(out)
        _write_rows(REPORT_DIR / "liqpen_dynamic_detail.csv", detail)
        _write_rows(REPORT_DIR / "liqpen_dynamic_summary.csv", summary)
        print(json.dumps(out, ensure_ascii=False), flush=True)
    (REPORT_DIR / "liqpen_dynamic_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
