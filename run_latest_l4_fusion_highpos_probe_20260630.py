...
OUT_SIGNAL_DIR = REPORT_DIR / "fusion_highpos_signals"
OUT_LOG_DIR = REPORT_DIR / "fusion_highpos_logs"
OUT_CSV = REPORT_DIR / "fusion_highpos_probe_20260630.csv"
OUT_JSON = REPORT_DIR / "fusion_highpos_probe_20260630.json"
...
BASES: dict[str, dict[str, Any]] = {
    ...
    "timing4540": {
        "signal_file": REPORT_DIR / "timing1d_signals" / "timing_45_40_10_05_g1p4_top5_dyn.csv",
        "score_table": "score_timing_45_40_10_05_g1p4_top5_dyn",
    },
    "timing3545": {
...
CASES: list[dict[str, Any]] = [
    {"name": "fhp_timing55_s170_cap27_h2m3", "base": "timing55", "target_scale": 1.70, "cap": 0.27, "topn": 5, "new_scale": 0.70, "turnover_high": 80.0, "turn_scale": 0.85, "gap_mode": "none"},
    {"name": "fhp_timing55_s190_cap30_h2m3", "base": "timing55", "target_scale": 1.90, "cap": 0.30, "topn": 5, "new_scale": 0.80, "turnover_high": 80.0, "turn_scale": 0.90, "gap_mode": "none"},
    {"name": "fhp_timing55_s210_cap33_h2m3", "base": "timing55", "target_scale": 2.10, "cap": 0.33, "topn": 5, "new_scale": 0.80, "turnover_high": 80.0, "turn_scale": 0.90, "gap_mode": "none"},
    {"name": "fhp_timing55_gap_s210_cap33_h2m3", "base": "timing55", "target_scale": 2.10, "cap": 0.33, "topn": 5, "new_scale": 0.80, "turnover_high": 80.0, "turn_scale": 0.90, "gap_mode": "scale"},
    {"name": "fhp_timing4540_s190_cap30_h2m3", "base": "timing4540", "target_scale": 1.90, "cap": 0.30, "topn": 5, "new_scale": 0.80, "turnover_high": 80.0, "turn_scale": 0.90, "gap_mode": "none"},
    {"name": "fhp_timing3545_s190_cap30_h2m3", "base": "timing3545", "target_scale": 1.90, "cap": 0.30, "topn": 5, "new_scale": 0.80, "turnover_high": 80.0, "turn_scale": 0.90, "gap_mode": "none"},
    {"name": "fhp_skip50_s190_cap30_h2m3", "base": "skip50", "target_scale": 1.90, "cap": 0.30, "topn": 5, "new_scale": 0.80, "turnover_high": 80.0, "turn_scale": 0.90, "gap_mode": "none"},
    {"name": "fhp_skip50_gap_s210_cap33_h2m3", "base": "skip50", "target_scale": 2.10, "cap": 0.33, "topn": 5, "new_scale": 0.80, "turnover_high": 80.0, "turn_scale": 0.90, "gap_mode": "scale"},
]
...
def _market_cache(rows: list[dict[str, Any]]) -> dict[tuple[str, str], dict[str, float]]:
    ...
    keys.update(
        (str(row["stock_code"]), str(row["buy_date"]))
        for row in rows
        if str(row.get("buy_date") or "").strip()
    )
    ...
    try:
        ...
        for stock_code, trade_date in sorted(keys):
            row = con.execute(
                """
                SELECT open, pre_close, list_date
            ...
            if not row:
                continue
            open_price, pre_close, list_date = row
            item: dict[str, float] = {}
...
def _target_pct(row: dict[str, Any], case: dict[str, Any], market: dict[tuple[str, str], dict[str, float]]) -> float:
    ...
    if atr is None or list_age_days is None or list_age_days < 60:
        target *= float(case["new_scale"])
    elif turnover >= float(case["turnover_high"]):
        target *= float(case["turn_scale"])

    if case.get("gap_mode") == "scale":
        buy_date = str(row.get("buy_date") or "").strip()
        gap = None
        if buy_date:
            buy = market.get((stock_code, buy_date), {})
            if buy.get("open") is not None and buy.get("pre_close"):
                gap = buy["open"] / buy["pre_close"] - 1.0
        if gap is not None:
            if gap >= 0.02:
                target *= 0.55
            elif gap <= -0.08:
...
def _run(case: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        **signal_meta,
        ...
        "note": "research-only high-position fusion probe on active formal L4 signals; no production parameter change",
    }
...
def main() -> None:
    for name, base in BASES.items():
        if not base["signal_file"].exists():
            raise FileNotFoundError(f"{name}: {base['signal_file']}")
