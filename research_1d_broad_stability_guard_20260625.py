...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_1d_broad_stability_guard_research_20260625"
TARGET_TABLE = "stock_predict_data_model_agent_1d_broad_stability_guard_20260625_executable_1d_open_return_research"
...
SOURCES = {
    ...
    "broad1": "stock_predict_data_model_agent_1d_dynamic_guard_20260625_executable_1d_open_return_research",
...
def load_scores() -> pd.DataFrame:
    with sqlite3.connect(MODEL_DB) as conn:
        out = read_source(conn, "formal1", SOURCES["formal1"])
...
def build_features(scores: pd.DataFrame) -> pd.DataFrame:
    ...
    for trade_date, group in scores.groupby("trade_date", sort=True):
        ...
        base20 = top_set(group, "formal1_rank", 20)
        ...
        for alias in SOURCES:
            ...
            if alias != "formal1":
                ...
                row[f"overlap20_formal1_{alias}"] = len(base20 & top_set(group, rank_col, 20))
...
def scan(scores: pd.DataFrame, labels: pd.DataFrame, features: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    base = metric_by_source[metric_by_source["source"] == "formal1"].set_index("trade_date")
    feature_idx = features.set_index("trade_date")
    rows = []
    daily_rows = []
    alt_sources = ["broad1", "strict1", "cross3", "cross5", "cross10", "formal3", "formal5", "formal10"]
...
def write_outputs(scan_result: pd.DataFrame, daily: pd.DataFrame, source_metrics: pd.DataFrame, stats: dict | None) -> None:
    ...
    if stats:
        lines.extend(
            [
                "本轮找到一个通过基础模型评价门槛的 1D cross-horizon research-only 候选。",
                "",
                "候选规则：",
                "",
                "```text",
        ...
        lines.append("本轮未找到通过基础模型评价门槛的 1D cross-horizon 候选。")
    lines.extend(
        [
            "",
            "## 证据路径",
...
def scan(scores: pd.DataFrame, labels: pd.DataFrame, features: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    metric_by_source = pd.concat([source_daily_metrics(eval_frame, alias) for alias in SOURCES], ignore_index=True)
    dates = sorted(metric_by_source["trade_date"].unique().tolist())
    base = metric_by_source[metric_by_source["source"] == "broad1"].set_index("trade_date")
    feature_idx = features.set_index("trade_date")
    rows = []
    daily_rows = []
    alt_sources = ["strict1", "formal1", "cross3", "cross5", "cross10", "formal3", "formal5", "formal10"]
    for alt in alt_sources:
        alt_daily = metric_by_source[metric_by_source["source"] == alt].set_index("trade_date")
        candidate_features = [c for c in feature_idx.columns if c.startswith(f"{alt}_") or c.endswith(f"_{alt}") or c.startswith("broad1_")]
        for feature in candidate_features:
            vals = feature_idx[feature].replace([np.inf, -np.inf], np.nan).dropna()
            if vals.nunique() < 3:
                continue
            ...
            for threshold in thresholds:
                for op in ["<=", ">="]:
                    ...
                    for prefix, cur, base_sum in [("full", full, base_full), ("recent63", r63, b63), ("recent20", r20, b20)]:
                        for metric in ["rank_ic", "top1", "top3", "top5", "top10", "top20", "top50"]:
                            row[f"{prefix}_{metric}_delta_vs_broad"] = cur[metric] - base_sum[metric]
                    row.update(period_stats(merged))
                    row["pass_basic"] = bool(
                        row["recent63_active_days"] >= 3
                        and row["recent63_top1_delta_vs_broad"] >= -0.002
                        and row["recent63_top5_delta_vs_broad"] >= -0.001
                        and row["recent20_top5_delta_vs_broad"] >= -0.001
                        and row["full_top5_delta_vs_broad"] >= -0.0005
                        and row["positive_top5_periods"] >= 3
                        and row["min_period_top5_delta"] >= -0.0002
                    )
                    row["objective"] = (
                        1.5 * row["recent63_top1_delta_vs_broad"]
                        + 2.0 * row["recent63_top5_delta_vs_broad"]
                        + row["recent20_top5_delta_vs_broad"]
                        + 0.8 * row["full_top5_delta_vs_broad"]
...
def write_candidate(scores: pd.DataFrame, features: pd.DataFrame, best: pd.Series) -> dict:
    ...
    active = active <= threshold if op == "<=" else active >= threshold
    active_dates = set(active[active].index.astype(str))
    out = scores[["trade_date", "stock_code", "broad1_score", "broad1_rank", f"{alt}_score", f"{alt}_rank"]].copy()
    out["guard_active"] = out["trade_date"].isin(active_dates)
    out["pred_prob"] = np.where(out["guard_active"], out[f"{alt}_rank"], out["broad1_rank"])
    out["score_formula"] = f"if {feature} {op} {threshold:.12g} then {alt}_rank else broad1_rank"
    out = out.rename(columns={"broad1_score": "broad1", f"{alt}_score": alt})
    keep = ["trade_date", "stock_code", "pred_prob", "broad1", alt, "broad1_rank", f"{alt}_rank", "guard_active", "score_formula"]
    with sqlite3.connect(MODEL_DB, timeout=300) as conn:
        conn.execute("PRAGMA busy_timeout=300000")
        conn.execute(f"drop table if exists {quote(TARGET_TABLE)}")
        out[keep].to_sql(TARGET_TABLE, conn, if_exists="replace", index=False)
...
def write_outputs(scan_result: pd.DataFrame, daily: pd.DataFrame, source_metrics: pd.DataFrame, stats: dict | None) -> None:
    REPORT_DIR.mkdir(parents=True, exist_ok=True)
    scan_path = REPORT_DIR / "broad_stability_guard_scan_results.csv"
    daily_path = REPORT_DIR / "broad_stability_guard_daily_deltas.csv"
    source_path = REPORT_DIR / "broad_stability_source_daily_metrics.csv"
    scan_result.to_csv(scan_path, index=False, encoding="utf-8-sig")
    daily.to_csv(daily_path, index=False, encoding="utf-8-sig")
    source_metrics.to_csv(source_path, index=False, encoding="utf-8-sig")
    best = scan_result.iloc[0].to_dict() if not scan_result.empty else None
    manifest = {
        ...
        "generated_at": now_iso(),
        "decision": "candidate" if stats else "rejected",
        "baseline_for_scan": "research_1d_dynamic_guard_broad",
        "best_scan": best,
        "asset_stats": stats,
        "promotion_requires_user_confirmation": True,
        "audit_required_before_formal": True,
    ...
    manifest_path = REPORT_DIR / "broad_stability_guard_research_manifest.json"
    manifest_path.write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")
    lines = [
        "# 1D broad 稳定化守卫研究报告（20260625）",
        "",
        "## 当前结论",
        "",
    ]
    if stats:
        lines.extend(
            [
                "本轮找到一个相对 1D broad 基准通过稳定化门槛的 research-only 候选。",
                "",
                "候选规则：",
                "",
                "```text",
                ...
                "## 相对 broad 基准的扫描摘要",
                "",
                f"- 近 63 日 Top1 增量：`{best['recent63_top1_delta_vs_broad']:.6f}`",
                f"- 近 63 日 Top5 增量：`{best['recent63_top5_delta_vs_broad']:.6f}`",
                f"- 近 20 日 Top5 增量：`{best['recent20_top5_delta_vs_broad']:.6f}`",
                f"- 全样本 Top5 增量：`{best['full_top5_delta_vs_broad']:.6f}`",
                f"- 正 Top5 自然阶段数：`{int(best['positive_top5_periods'])}/4`",
                f"- 最差自然阶段 Top5 增量：`{best['min_period_top5_delta']:.6f}`",
            ]
        )
    else:
        lines.append("本轮未找到相对 1D broad 基准通过稳定化门槛的候选。")
    lines.extend(
        [
            "",
            "## 证据路径",
    ...
    (REPORT_DIR / "broad_stability_guard_research_report.md").write_text("\n".join(lines), encoding="utf-8")
