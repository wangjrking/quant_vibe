...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_max_hold_renewal_v225_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v225_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def run_case(arrays, score, order, definition, protocol, end, start=None):
    return v162.run_case(
        ...
        max_hold_renewal_policy_override=str(definition["max_hold_renewal_policy"]),
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 最大持有期到期复核研究")
    ...
    rows, definitions = [], {}
    for policy in protocol["grid"]["max_hold_renewal_policy"]:
        definition = {**protocol["fixed_definition"], "max_hold_renewal_policy": str(policy)}
