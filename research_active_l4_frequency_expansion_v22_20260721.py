...
REPORT_DIR = ROOT / "quant/data_file/reports/strategy_agent_active_l4_frequency_expansion_v22_20260721"
...
REPORT_PATH = REPORT_DIR / "频率扩展研究结论.md"
...
def case_id(profile):
    ...
    return "fx_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def main():
    ...
    qualities = {
        key: candidate_health.candidate_quality(
            masked, score, orders[key], int(protocol["health_to_exposure"]["shadow_top_n"])
        )
        for key, score in scores.items()
    }
    exposures = {
        (key, lookback, floor, denom): continuous.exposure_series(
            qualities[key], lookback, floor, denom
        )
        for key in scores
                        ...
                        for top_n in protocol["stage1"]["top_n"]:
                            for entry_rank_min in protocol["stage1"]["entry_rank_min"]:
                                profile = continuous.ExposureProfile(
                                    blend_id, amount_min, 200000, top_n, entry_rank_min,
                                    lookback, floor, denom, **fixed
                                )
                                daily = continuous.simulate(
                                    masked, scores[blend_id], orders[blend_id],
                                    exposures[(blend_id, lookback, floor, denom)],
                                    profile, protocol["observation_end"]
                                )
                                rows.append({
                                    "case_id": case_id(profile), **asdict(profile),
                                    **evaluate(daily, protocol["year_folds"])
                                })
    ...
    seeds = dual_track(
        stage1, int(protocol["stage1"]["promote_return"]),
        int(protocol["stage1"]["promote_sharpe"])
    )
                ...
                for sell_rank_below in grid["sell_rank_below"]:
                    for replacement_advantage in grid["replacement_advantage"]:
                        for invested_ratio in grid["invested_ratio"]:
                            profile = continuous.ExposureProfile(
                                str(seed.blend_id), int(seed.amount_min), int(seed.mv_min),
                                int(seed.top_n), float(seed.entry_rank_min),
                                int(seed.health_lookback), float(seed.floor_fraction),
                                float(seed.scale_denominator), min_hold, max_hold,
                                sell_rank_below, replacement_advantage, invested_ratio
                            )
                            daily = continuous.simulate(
                            ...
                            rows.append({
                                "case_id": case_id(profile), "seed_case_id": str(seed.case_id),
    ...
    for item in payload["profiles"]:
        ...
        action_rows.append({
            ...
            "sha256": digest(path), "content_sha256": action_hash,
    ...
    REPORT_PATH.write_text("\n".join([
        ...
        "本轮为 research-only，未修改生产策略或正式信号。"
    ]), encoding="utf-8")
    print(json.dumps({
