...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_breadth_low_exposure_v114_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v114_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def case_protocol(protocol: dict, definition: dict) -> dict:
    result = copy.deepcopy(protocol)
    result["breadth_state"] = {**protocol["fixed_breadth"], "low_gross": float(definition["low_gross"])}
    return result


def main() -> None:
    parser = argparse.ArgumentParser(description="7日平滑延续型策略的弱状态仓位研究")
    ...
    rows, definitions = [], {}
    for low_gross, sell_threshold in product(protocol["grid"]["low_gross"], protocol["grid"]["sell_score_below"]):
        definition = {**protocol["fixed_exit"], "low_gross": low_gross, "sell_score_below": sell_threshold}
