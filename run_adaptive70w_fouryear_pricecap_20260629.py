...
BASE_SCORE_TABLE = "score_w72_23_05_amt150_mv30__hold3m5_c097_e096_ddtight_pos65"
BASE_CASE = "w72_23_05_amt150_mv30__hold3m5_c097_e096_ddtight_pos65"
PRICE_CAPS = [100.0, 120.0, 140.0, 150.0, 160.0, 180.0, 200.0, 300.0]
...
def _build_signal(price_cap: float) -> dict[str, Any]:
    cap_name = str(int(price_cap))
    case_key = f"{BASE_CASE}_pricecap{cap_name}"
    ...
    try:
        ...
        con.execute(f'CREATE TABLE scoredb."{score_table}" AS SELECT * FROM scoredb."{BASE_SCORE_TABLE}"')
        sql = f"""
        ...
        scored AS (
            ...
            FROM scoredb."{BASE_SCORE_TABLE}" s
            ...
            WHERE NOT (s.stock_code LIKE '%.BJ' OR substr(s.stock_code, 1, 1) IN ('4', '8'))
              ...
              AND try_cast(md.close AS DOUBLE) <= {price_cap}
        ),
        with_buy_checks AS (
            SELECT
                s.*,
                ...
                CASE
                    ...
                    WHEN try_cast(bm.open AS DOUBLE) > {price_cap} THEN FALSE
                    ELSE TRUE
                END AS buy_day_ok
            FROM scored s
        ...
        SELECT
            ...
            'amt150000_mv300000_pricecap{cap_name}' AS filter_name,
            'w72_23_05_amt150_mv30' AS entry_weight_name,
            'hold3m5_c097_e096_ddtight_pos65_pricecap{cap_name}' AS dynamic_hold_name,
...
def _run(case: dict[str, Any], tag: str, start: str, end: str) -> dict[str, Any]:
    log_file = REPORT_DIR / "logs" / f"{case['case_key']}__adaptive70w_buyonly__{tag}.log"
...
def main() -> None:
    ...
    for cap in PRICE_CAPS:
        case = _build_signal(cap)
        rows = [_run(case, tag, start, end) for tag, start, end in SLICES]
        detail.extend(rows)
        by_slice = {row["slice"]: row for row in rows}
        out = {"case_key": case["case_key"], "price_cap": cap, "signal_rows": case["signal_rows"]}
        ...
        _write_rows(REPORT_DIR / "adaptive70w_fouryear_pricecap_detail.csv", detail)
        _write_rows(REPORT_DIR / "adaptive70w_fouryear_pricecap_summary.csv", summary)
