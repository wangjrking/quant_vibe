...
import research_active_l4_strong_exit_refine_v163_20260722 as v163
import research_preregistered_active_l4_rank_rotation_20260721 as core


ROOT = Path(__file__).resolve().parents[2]
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_strong_exit_threshold_v164_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v164_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4强势退出阈值窄邻域研究")
    ...
    if args.open_known_2026:
        ...
        for item in frozen["candidates"]:
            for start in protocol["known_stress_buy_starts"]:
                daily = v163.run_case(full, score, order, item["definition"], protocol, protocol["known_stress_end"], start)
    ...
    rows, definitions = [], {}
    for threshold in protocol["grid"]["exit_strong_return_min"]:
        definition = {**protocol["fixed_definition"], "exit_strong_return_min": threshold}
        case_id = stable_id(definition)
        definitions[case_id] = definition
        daily = v163.run_case(observation, score_obs, order_obs, definition, protocol, protocol["observation_end"])
