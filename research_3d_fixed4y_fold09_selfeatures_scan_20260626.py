...
FEATURES_PATH = (
    DATA_DIR
    / "reports"
    / "model_agent_research_3d_fixed4y_v2_adjacent_folds_20260625"
    / "fold09"
    / "feature_scores"
    / "selected_features_executable_3d_open_return_rolling_fold9.json"
)
REPORT_DIR = DATA_DIR / "reports" / "model_agent_research_3d_fixed4y_fold09_selfeatures_scan_20260626"
...
CANDIDATES = [
    Candidate("self_v2_ref", 5, 126, 0.005, 10.0, 3000, 0.006, 2, 12.0, 0.2),
    Candidate("self_v2_reg16", 5, 126, 0.005, 10.0, 3200, 0.006, 2, 16.0, 0.3),
    Candidate("self_v2_lr003_n6000", 5, 126, 0.005, 10.0, 6000, 0.003, 2, 12.0, 0.2),
]
...
def prediction_path(candidate: Candidate) -> Path:
    return out_dir(candidate) / "fold09" / "fold_predictions" / "fold09.parquet"
...
def run_candidate(candidate: Candidate) -> dict:
    ...
    command = [
        ...
        "--summary-output",
        os.path.relpath(out_dir(candidate) / "fold09" / "fold_results.csv", MAIN_DIR),
        "--start-fold",
        "9",
        "--end-fold",
        "9",
...
def main() -> int:
    ...
    for candidate in CANDIDATES:
        ...
        summary.update(
            {
                ...
                "model_path": str(out_dir(candidate) / "fold09" / "models" / "model_fold09.json"),
        ...
        for baseline_name, table in BASELINE_TABLES.items():
            if baseline_name not in cached_baselines:
                ...
                base_summary.update({"asset": baseline_name, "date_min": start, "date_max": end, "objective": objective(base_summary)})
    ...
    summary_path = REPORT_DIR / "selfeatures_scan_summary.csv"
    delta_path = REPORT_DIR / "selfeatures_scan_deltas.csv"
    logs_path = REPORT_DIR / "selfeatures_run_logs.json"
    packet_path = REPORT_DIR / "selfeatures_scan_packet.json"
    ...
    packet = {
        ...
        "scope": "research_only_3d_fixed4y_fold09_selfeatures_scan",
        "summary_csv": str(summary_path),
        "delta_csv": str(delta_path),
        "run_logs": str(logs_path),
        "feature_path": str(FEATURES_PATH),
        "best_asset": summary_df.iloc[0].to_dict(),
        "boundaries": {
            "research_only": True,
