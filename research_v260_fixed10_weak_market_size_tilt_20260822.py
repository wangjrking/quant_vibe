...
OUTPUT_ROOT = (
    REPO
    / "quant/data_file/reports/strategy_agent_v260_fixed10_weak_market_size_tilt_20260822"
)
CASES = {
    "production_entry_order": {"penalty": 0.0, "selectable": False},
    "control_size_penalty_0025": {"penalty": 0.025, "selectable": False},
    "candidate_size_penalty_0050": {"penalty": 0.050, "selectable": True},
    "control_size_penalty_0075": {"penalty": 0.075, "selectable": False},
}


def size_percentile_matrix(
    total_mv: np.ndarray,
    universe_mask: np.ndarray,
) -> np.ndarray:
    values = np.asarray(total_mv, dtype=np.float64)
    mask = np.asarray(universe_mask, dtype=np.bool_)
    if values.shape != mask.shape:
        raise ValueError("size percentile matrices do not align")
    result = np.full(values.shape, np.nan, dtype=np.float64)
    for day in range(len(values)):
        indices = np.flatnonzero(mask[day] & np.isfinite(values[day]))
        if not len(indices):
            continue
        order = indices[np.argsort(values[day, indices], kind="stable")]
        denominator = max(len(order) - 1, 1)
        result[day, order] = np.arange(len(order), dtype=np.float64) / denominator
    return result


def weak_market_size_tilt_order(
    ...
    size = np.asarray(size_percentile, dtype=np.float64)
    weak = np.asarray(weak_market, dtype=np.bool_)
    if score.shape != original.shape or size.shape != score.shape:
        raise ValueError("size tilt matrices do not align")
    if weak.shape != (score.shape[0],):
        raise ValueError("weak-market vector does not align")
    amount = float(penalty)
    if amount < 0:
        raise ValueError("size penalty cannot be negative")
    result = original.copy()
    if amount == 0:
        return result
    original_position = np.arange(score.shape[1], dtype=np.int64)
    for day in np.flatnonzero(weak):
        row = original[day]
        row_score = score[day, row]
        row_size = size[day, row]
        adjusted = row_score - amount * (1.0 - np.nan_to_num(row_size, nan=0.0))
        safe = np.where(np.isfinite(row_score), adjusted, -np.inf)
        rearranged = np.lexsort((original_position, -safe))
...
def main() -> None:
    ...
    if access["logical_max_date"] != round1.DEVELOPMENT_END:
        raise PermissionError("2026 data entered weak-market size-tilt development")
    ...
    size_percentile = size_percentile_matrix(
        arrays["total_mv"], production_mask & arrays["signal_clean"]
    )
    ...
    for case_id, case in CASES.items():
        current_order = weak_market_size_tilt_order(
            score,
            production_order,
            size_percentile,
            weak_market,
        ...
        results[case_id] = {
            ...
            "weak_market_size_penalty": float(case["penalty"]),
    ...
    baseline_id = "production_entry_order"
    candidate_id = "candidate_size_penalty_0050"
    ...
    if not all(checkpoint_equivalence.values()):
        raise RuntimeError("size-tilt baseline drifted")
    utility_deltas = {
    ...
    local_robust = bool(
        utility_deltas["control_size_penalty_0025"] > 0
        and utility_deltas["control_size_penalty_0075"] > 0
    )
    ...
    if not all(deterministic.values()):
        raise RuntimeError("size-tilt replay failed")
    selected_policy = copy.deepcopy(policy)
    selected_policy["weak_market_new_entry_size_penalty"] = CASES[selected][
        "penalty"
    ]
    result = {
        ...
        "formula": "entry_score = production_score - penalty * (1 - same_day_size_percentile), weak_market_only",
        "selectable_candidate": candidate_id,
        "nonselectable_controls": [
            "control_size_penalty_0025",
            "control_size_penalty_0075",
        ],
        "results": results,
