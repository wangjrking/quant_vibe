"""Strict 2022-2024 only, daily standardized-target 5D research candidate."""
...
OUT=DATA_DIR/'reports'/'model_agent_5d_daily_zscore_target_dev2022_2024_20260830'/'build_r1'
...
def main():
    ...
    if contract['candidate_id']!='expanding_pit_5d_daily_zscore_raw_target_v1' or contract['confirmation_window']!='2025 sealed_for_5d_candidate_only' or not contract['validation_2026_closed_for_5d_candidate_only']: raise RuntimeError('contract drift')
    ...
    try:
        for fold in folds:
            ...
            model.fit(xtr,daily_zscore(tr).astype('float32'),sample_weight=base.daily_weights(tr,info['metadata'].get('sample_weight_config')),verbose=False)
