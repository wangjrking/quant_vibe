...
OUT = ROOT / "quant" / "data_file" / "reports" / "strategy_agent_current_formal_l4_next_open_shortlist_selection_20260720"
...
CASES = (
    [{"name": "baseline_postgate", "topk": 1, "selector": "rank10", "gap_min": None, "gap_max": None}]
    + [
        {"name": f"top{topk}_cap{str(cap).replace('.', 'p')}", "topk": topk, "selector": "rank10", "gap_min": None, "gap_max": cap}
        for topk in (3, 5, 10)
        for cap in (1.5, 3.0, 5.0)
    ]
    + [
        {"name": f"top{topk}_closest0", "topk": topk, "selector": "closest0", "gap_min": None, "gap_max": None}
        for topk in (3, 5, 10)
    ]
    + [
        {"name": f"top5_band_m{abs(int(low))}_{str(high).replace('.', 'p')}", "topk": 5, "selector": "rank10", "gap_min": low, "gap_max": high}
        for low, high in ((-3.0, 1.5), (-2.0, 2.0), (-1.0, 3.0))
    ]
)
...
def prepare_signal(frame: pd.DataFrame, name: str) -> Path:
    ...
    frame["filter_name"] = "next_open_shortlist_selection"
    frame["entry_weight_name"] = "10d_shortlist_open_selector"
...
def build_signal(con: duckdb.DuckDBPyConnection, case: dict) -> Path:
    gap_terms = ["rank_1d >= 0.88"]
    if case["gap_min"] is not None:
        gap_terms.append(f"buy_open_gap_raw_pct >= {float(case['gap_min'])}")
    if case["gap_max"] is not None:
        gap_terms.append(f"buy_open_gap_raw_pct <= {float(case['gap_max'])}")
    execution_filter = " AND ".join(gap_terms)
    selector = (
        "abs(buy_open_gap_raw_pct) ASC, rank_10d DESC, stock_code"
        if case["selector"] == "closest0"
        else "rank_10d DESC, stock_code"
    )
    frame = con.execute(
        f"""
        WITH eligible AS (
            ...
            WHERE rank10_short <= {int(case['topk'])}
              AND {execution_filter}
        )
        SELECT * FROM executable
        QUALIFY row_number() OVER (PARTITION BY trade_date ORDER BY {selector}) = 1
        ORDER BY trade_date
        """
    ).fetchdf()
