...
OUT = ROOT / "quant" / "data_file" / "reports" / "strategy_agent_current_formal_l4_profit_state_exit_20260720"
LOGS = OUT / "logs"
STRATEGY = OUT / "research_code_snapshot"
SIGNAL = ROOT / "quant" / "data_file" / "reports" / "strategy_agent_current_formal_l4_cap5_rank_hold_refine_20260720" / "signals" / "r1_88_min12_max13.csv"
CASES = (
    [{"name": "disabled", "profit": None, "extend": 13, "loss": None}]
    + [
        {"name": f"profit_{int(profit * 100):02d}_max{extend}", "profit": profit, "extend": extend, "loss": None}
        for profit in (0.02, 0.04, 0.06, 0.08)
        for extend in (14, 15)
    ]
    + [
        {"name": f"loss_m{abs(int(loss * 100)):02d}", "profit": None, "extend": 13, "loss": loss}
        for loss in (-0.02, -0.04, -0.06)
    ]
    + [
        {
            "name": f"profit_{int(profit * 100):02d}_max15_loss_m{abs(int(loss * 100)):02d}",
            "profit": profit, "extend": 15, "loss": loss,
        }
        for profit in (0.04, 0.06)
        for loss in (-0.04, -0.06)
    ]
)
...
def run_case(case: dict) -> dict:
    ...
    env.update({
        ...
        "GM_INDEPENDENT_PROFIT_EXTEND_MIN_RETURN": "" if case["profit"] is None else str(case["profit"]),
        "GM_INDEPENDENT_PROFIT_EXTEND_MAX_HOLD": str(case["extend"]),
        "GM_INDEPENDENT_MIN_HOLD_LOSS_EXIT_RETURN": "" if case["loss"] is None else str(case["loss"]),
    })
    command = [
