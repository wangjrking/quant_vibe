...
CASES = [
    {
        "case_key": "w72_23_05_amt150_mv30__hold3m5_c097_e096_ddtight_pos65",
        "holding_days": 3,
        "max_holding_days": 5,
        "target_pct": 0.65,
        "stop_loss": 0.05,
    ...
    {
        "case_key": "w72_23_05_amt90_mv20__hold4m6_c096_e094_ddtight_pos70",
        "holding_days": 4,
        "max_holding_days": 6,
        "target_pct": 0.70,
        "stop_loss": 0.05,
...
def _run(case: dict[str, Any], light_stop: float, tag: str, start: str, end: str) -> dict[str, Any]:
    case_key = case["case_key"]
    stop_name = str(light_stop).replace(".", "p")
    variant = f"{case_key}__lightstop{stop_name}"
    log_file = REPORT_DIR / "logs" / f"{variant}__adaptive70w_buyonly__{tag}.log"
    indicator = _extract_indicator(log_file)
    returncode = 0
    if indicator is None:
        env = os.environ.copy()
        env.update(BASE_ENV)
        env["GM_LIGHT_STOP_LOSS_PCT"] = str(light_stop)
        env["GM_SCORE_CONTINUE_ENTRY_RATIO"] = str(case["score_continue"])
        env["GM_SCORE_EXIT_ENTRY_RATIO"] = str(case["score_exit"])
        cmd = [
            str(JUEJIN_PYTHON),
            ...
            str(STRATEGY_DIR),
            "--signal-file",
            str(REPORT_DIR / "signals" / f"{case_key}.csv"),
            "--log-file",
            str(log_file),
            "--max-positions",
            "1",
...
def main() -> None:
    ...
    for case in CASES:
        for light_stop in LIGHT_STOPS:
            rows = [_run(case, light_stop, tag, start, end) for tag, start, end in SLICES]
            detail.extend(rows)
            by_slice = {row["slice"]: row for row in rows}
            out = {"variant": rows[0]["variant"], "case_key": case["case_key"], "light_stop": light_stop}
            ...
            _write_rows(REPORT_DIR / "adaptive70w_fouryear_lightstop_detail.csv", detail)
            _write_rows(REPORT_DIR / "adaptive70w_fouryear_lightstop_summary.csv", summary)
