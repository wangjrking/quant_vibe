"""Strict 2022-2024-only date-grouped NDCG@10 3D ranker candidate."""
...
OUT = DATA_DIR / "reports" / "model_agent_3d_daily_ndcg10_ranker_dev2022_2024_20260830" / "build_r1"
...
def relevance(frame: pd.DataFrame) -> np.ndarray:
    ordered = frame.sort_values(["trade_date", "target", "stock_code"], ascending=[True, False, True], kind="mergesort").copy()
    ordered["rank"] = ordered.groupby("trade_date", sort=False).cumcount() + 1
    ordered["count"] = ordered.groupby("trade_date", sort=False)["target"].transform("size")
    ordered["relevance"] = (ordered["rank"] <= np.maximum(1, np.floor(ordered["count"] * 0.10))).astype("int8")
    result = ordered.sort_index()["relevance"].to_numpy(dtype="int8")
    if result.min() != 0 or result.max() != 1 or not (0.09 <= float(result.mean()) <= 0.11):
        raise RuntimeError("invalid fixed top-decile relevance")
    return result


def ranker_params(metadata: dict) -> dict:
    params = base.model_params(metadata)
    params.update({"objective": "rank:ndcg", "eval_metric": "ndcg@10", "lambdarank_pair_method": "topk", "lambdarank_num_pair_per_sample": 10})
    return params
...
def main() -> int:
    ...
    if (contract["candidate_id"] != "expanding_pit_3d_daily_ndcg10_ranker_v1" or contract["single_change"]["kind"] != "date_grouped_ndcg10_ranking_objective" or contract["selection_and_sealing"]["confirmation_window"] != "2025 sealed_for_3d_candidate_only" or contract["selection_and_sealing"]["validation_window"] != "2026 closed_for_3d_candidate_only"): raise RuntimeError("contract drift")
    dates = base.read_open_dates(); _, prepared = base.preflight(dates); info = prepared["3d"]
    ...
    base.dump_json(OUT / "preflight.json", {"contract_sha256": base.sha256_file(CONTRACT), "features": info["resolved"], "folds": folds, "production_weight_config": info["metadata"].get("sample_weight_config"), "candidate_training_weight": "none; date-group relevance is the only ranking supervision", "candidate_target": "per-date top 10% binary relevance", "confirmation_2025_read": False, "validation_2026_closed": True, "production_unchanged": True, "runtime_agent_route": contract["runtime_agent_route"]})
    fd = duckdb.connect(str(base.FEATURE_DB), read_only=True); ld = duckdb.connect(str(base.LABEL_DB), read_only=True); rows = []
    try:
        for fold in folds:
            train = base.query_features(fd, info["resolved"], fold["train_start"], fold["train_end"]).merge(base.query_labels(ld, "executable_3d_open_return", fold["train_start"], fold["train_end"]), on=["trade_date", "stock_code"], how="inner", validate="one_to_one").dropna(subset=["target"]).reset_index(drop=True)
            test = base.query_features(fd, info["resolved"], fold["test_start"], fold["test_end"]).merge(base.query_labels(ld, "executable_3d_open_return", fold["test_start"], fold["mature_label_cutoff"]), on=["trade_date", "stock_code"], how="left", validate="one_to_one")
            # qid requires the input rows be ordered by the date group.
            train = train.sort_values(["trade_date", "stock_code"], kind="mergesort").reset_index(drop=True)
            model = xgb.XGBRanker(**ranker_params(info["metadata"]))
            model.fit(train[info["resolved"]].astype("float32"), relevance(train), qid=pd.factorize(train["trade_date"], sort=True)[0], verbose=False)
