...
def _run(sell_mult: float, tag: str, start: str, end: str) -> dict[str, Any]:
    mult_name = str(sell_mult).replace(".", "p")
    log_file = OUT_DIR / "logs" / f"{CASE_KEY}__sellmult{mult_name}__{tag}.log"
    ...
    if indicator is None:
        ...
        env.update({
            ...
            "GM_ADAPTIVE_SELL_SLIPPAGE_MULT": str(sell_mult),
        })
        cmd = [
            str(JUEJIN_PYTHON),
...
def main() -> None:
    ...
    for sell_mult in SELL_MULTS:
        for tag, start, end in SLICES:
            rows.append(_run(sell_mult, tag, start, end))
            _write_rows(OUT_DIR / "pos60_partial_sell_slippage_detail.csv", rows)
    print(rows)
