...
OUT = ROOT / "quant" / "data_file" / "reports" / "strategy_agent_current_formal_l4_cap5_score_state_exit_grid_20260720"
...
STRATEGY = ROOT / "quant" / "data_file" / "reports" / "strategy_agent_current_formal_l4_score_state_exit_20260720" / "research_code_snapshot"
CASES = [
    {"min_hold": min_hold, "max_hold": 16, "current_max": current_max}
    for min_hold in (1, 2, 4, 6)
    for current_max in (0.30, 0.50, 0.70, 0.85)
]


def case_name(case: dict) -> str:
    return f"min{case['min_hold']}_max{case['max_hold']}_curmax{int(case['current_max'] * 100)}"
...
def main() -> None:
    ...
    for case in CASES:
        if case_name(case) in completed:
            continue
