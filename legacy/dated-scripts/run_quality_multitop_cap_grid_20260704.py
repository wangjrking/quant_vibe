...
SIGNAL_DIR = OUT_DIR / "quality_multitop_cap_signals"
RUN_DIR = OUT_DIR / "quality_multitop_cap_grid"
SUMMARY = OUT_DIR / "quality_multitop_cap_summary.csv"
...
CASES = [
    # name, topn, scale, cap, weak_mult, strong_mult
    ("top2_q_s080_cap50", 2, 0.80, 0.50, 0.62, 1.08),
    ("top2_q_s090_cap55", 2, 0.90, 0.55, 0.62, 1.08),
    ("top2_q_s100_cap60", 2, 1.00, 0.60, 0.62, 1.08),
    ("top2_q_s110_cap65", 2, 1.10, 0.65, 0.62, 1.08),
    ("top2_q_s120_cap70", 2, 1.20, 0.70, 0.62, 1.08),
    ("top3_q_s070_cap32", 3, 0.70, 0.32, 0.62, 1.08),
    ("top3_q_s090_cap38", 3, 0.90, 0.38, 0.62, 1.08),
    ("top3_q_s110_cap45", 3, 1.10, 0.45, 0.62, 1.08),
    ("top3_q_s130_cap52", 3, 1.30, 0.52, 0.62, 1.08),
    ("top4_q_s060_cap25", 4, 0.60, 0.25, 0.62, 1.08),
    ("top4_q_s080_cap30", 4, 0.80, 0.30, 0.62, 1.08),
    ("top4_q_s100_cap36", 4, 1.00, 0.36, 0.62, 1.08),
    ("top4_q_s120_cap42", 4, 1.20, 0.42, 0.62, 1.08),
]
...
def export_signal(case: tuple[str, int, float, float, float, float]) -> tuple[Path, dict[str, object]]:
    name, topn, scale, cap, weak_mult, strong_mult = case
        ...
        df = con.execute(
            ...
            WITH picked AS (
                ...
                WHERE score_pct_rank >= 0.95
                  ...
                  AND NOT (signal_total_mv > 300000 AND signal_total_mv <= 500000)
                  AND signal_total_mv IS NOT NULL
            )
            SELECT
                signal_date,
                buy_date,
    ...
    for col in ["signal_pct_chg", "target_pct", "up_ratio"]:
    ...
    mult.loc[df["up_ratio"] > 0.80] *= 0.60
    ...
    return signal_file, {
        ...
        "days_below_topn": int((counts < topn).sum()) if len(counts) else 0,
