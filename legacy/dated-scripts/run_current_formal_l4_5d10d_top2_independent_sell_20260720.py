...
OUT = ROOT / "quant" / "data_file" / "reports" / "strategy_agent_current_formal_l4_5d10d_top2_independent_sell_20260720"
SIGNALS = OUT / "signals"
LOGS = OUT / "logs"
CASES = [
...
def run_case(rank1: float, min_hold: int, max_hold: int) -> dict:
    name = f"r1ge{int(rank1 * 100)}_top2_min{min_hold}_max{max_hold}"
