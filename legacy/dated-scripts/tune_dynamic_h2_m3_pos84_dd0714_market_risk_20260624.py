...
REPORT_DIR = BASE_DIR / "dynamic_h2_m3_pos84_dd0714_market_risk_20260624"
...
RISK_CASES = [
    {
        "case_name": "base_no_market_risk",
        "index": False,
        "breadth": False,
    },
    {
        "case_name": "index_cc4_intra4_scale50",
        "index": True,
        "index_cc": -0.04,
        "index_intraday": -0.04,
        "index_scale": 0.50,
    ...
    {
        "case_name": "index_cc3_intra3_scale60",
        "index": True,
        "index_cc": -0.03,
        "index_intraday": -0.03,
        "index_scale": 0.60,
    ...
    {
        "case_name": "breadth_up08_avgm3_scale50",
        "index": False,
        "breadth": True,
        "breadth_up": 0.08,
        "breadth_avg": -3.0,
    ...
    {
        "case_name": "breadth_up12_avgm25_scale60",
        "index": False,
        "breadth": True,
        "breadth_up": 0.12,
        "breadth_avg": -2.5,
    ...
    {
        "case_name": "combo_index4_breadth08_scale50",
        "index": True,
        "index_cc": -0.04,
        "index_intraday": -0.04,
        "index_scale": 0.50,
...
def _signal_for_case(case: dict) -> Path:
    ...
    for row in rows:
        ...
        row["strategy_variant"] = f"dynamic_h2_m3_pos84_dd0714_market_risk_{case['case_name']}"
...
def _apply_market_risk_env(env: dict[str, str], case: dict) -> None:
    env["GM_INDEX_RISK_EXIT_MODE"] = "1" if case.get("index") else "0"
    env["GM_BREADTH_RISK_EXIT_MODE"] = "1" if case.get("breadth") else "0"
    if case.get("index"):
        env["GM_INDEX_RISK_CC_THRESHOLD"] = str(case["index_cc"])
        env["GM_INDEX_RISK_INTRADAY_THRESHOLD"] = str(case["index_intraday"])
        env["GM_INDEX_RISK_BUY_SCALE"] = str(case["index_scale"])
    if case.get("breadth"):
        env["GM_BREADTH_RISK_UP_RATIO_THRESHOLD"] = str(case["breadth_up"])
        env["GM_BREADTH_RISK_AVG_PCT_THRESHOLD"] = str(case["breadth_avg"])
        env["GM_BREADTH_RISK_BUY_SCALE"] = str(case["breadth_scale"])
...
def _run(case: dict, signal_file: Path, start_name: str, start: str, end: str) -> dict:
    ...
    if indicator is None:
        ...
        _apply_market_risk_env(env, case)
        command = [
    ...
    return {
        "case_name": case_name,
        "index_risk": bool(case.get("index")),
        "breadth_risk": bool(case.get("breadth")),
...
def _summarize(detail: list[dict], signal_stats: dict[str, dict]) -> list[dict]:
    ...
    for case_name in sorted({row["case_name"] for row in detail}):
        ...
        out.append(
            {
                "case_name": case_name,
                "index_risk": rows[0]["index_risk"],
                "breadth_risk": rows[0]["breadth_risk"],
...
def _write_report(summary: list[dict], audit: dict) -> None:
    ...
    for row in ranked:
        lines.append(
            f"| {row['case_name']} | {row['index_risk']} | {row['breadth_risk']} | {_f(row['full_annual']):.2%} | "
            f"{_f(row['full_sharpe']):.2f} | {_f(row['full_max_drawdown']):.2%} | {_f(row['recent60_annual']):.2%} | "
            f"{_f(row['recent60_sharpe']):.2f} | {_f(row['late_min_annual']):.2%} |"
        )
    lines.extend(
        [
            "",
    ...
    (REPORT_DIR / "market_risk_report.md").write_text("\n".join(lines), encoding="utf-8")
