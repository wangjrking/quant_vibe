...
ARTIFACT_DIR = OUT_DIR / "sig025_mid025_missing050_s145_artifacts"
SIGNAL_FILE = OUT_DIR / "scale124_signal_mv_loss_bucket_refine_signals/sig025_mid025_missing050_s145.csv"
TRADES_OUT = OUT_DIR / "sig025_mid025_missing050_s145_paired_trades_with_features.csv"
SEGMENT_OUT = OUT_DIR / "sig025_mid025_missing050_s145_open_gap_segment_summary.csv"
REPORT_OUT = OUT_DIR / "sig025_mid025_missing050_s145_open_gap_segment_report_20260704.md"
...
def summarize(df: pd.DataFrame, feature: str) -> pd.DataFrame:
    g = (
        ...
        .agg(
            ...
            avg_signal_gap=("signal_open_gap_raw_pct", "mean"),
    ...
    g["feature"] = feature
    return g[["feature", "bucket", "trades", "avg_return", "median_return", "win_rate", "total_pnl", "avg_target", "avg_buy_gap", "avg_signal_gap", "avg_sigchg", "avg_score"]]
...
def main() -> int:
    ...
    merged["buy_gap_bin"] = pd.cut(merged["buy_open_gap_raw_pct"], [-10, -4, -3, -2, -1.5, -1, -0.5, 0, 2], include_lowest=True).astype(str)
    merged["signal_gap_bin"] = pd.cut(merged["signal_open_gap_raw_pct"], [-10, -4, -3, -2, -1.5, -1, -0.5, 0, 2], include_lowest=True).astype(str)
    ...
    merged["target_bin"] = pd.cut(merged["target_pct"], [0, 0.3, 0.5, 0.65, 0.75, 0.82, 1.0], include_lowest=True).astype(str)
    merged.to_csv(TRADES_OUT, index=False, encoding="utf-8-sig")
    features = ["buy_year", "market", "buy_gap_bin", "signal_gap_bin", "sigchg_bin", "amount_bin", "mv_bin_signal", "target_bin"]
    summary = pd.concat([summarize(merged, feature) for feature in features], ignore_index=True)
    summary = summary.sort_values(["feature", "bucket"])
    summary.to_csv(SEGMENT_OUT, index=False, encoding="utf-8-sig")

    gap_summary = summary[summary["feature"].isin(["buy_gap_bin", "signal_gap_bin", "sigchg_bin"])]
    best = summary.sort_values(["total_pnl"], ascending=False).head(20)
    worst = summary.sort_values(["total_pnl"], ascending=True).head(20)
    report = [
        "# sig025_mid025_missing050_s145 开盘缺口分桶分析",
        "",
        "## 汇总",
        "",
        f"- 配对交易数：{len(merged)}",
        ...
        "",
        "```csv",
        gap_summary.to_csv(index=False, lineterminator="\n").strip(),
        "```",
        "",
        "## 最强分桶",
        "",
