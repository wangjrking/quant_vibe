...
CASE_NAME = "dynamic_h2_m3_c098_w78_5d12_3d10_pos56_e097_mh1"
SOURCE_DIR = liq.SOURCE_DIR / "strict_sync_liquidity_neighborhood_20260624"
REPORT_DIR = SOURCE_DIR / "formal_horizon_entry_confirmation_20260624" / "dynamic_h2_m3_c098_validation_20260624"
SIGNAL_FILE = SOURCE_DIR / "formal_horizon_entry_confirmation_20260624" / "dynamic_hold_20260624" / "signals" / f"{CASE_NAME}.csv"
...
def _write_report(time_rows: list[dict], nearby_summary: list[dict], stress_summary: list[dict], proxy_stats: dict, audit: dict) -> None:
    lines = [
        "# dynamic h2_m3_c098 完整验证",
        "",
        "## 当前结论",
        "",
        f"验证对象：`{CASE_NAME}`。该版本第 2 个交易日检查分数，若当前 10D 核心分数不低于入场分数的 `0.98` 则允许延持，最长持有到第 3 个交易日。",
        "",
        "该规则把原本表现很弱的固定 h2 改造成接近固定 h3 的动态规则，是当前降低持仓天数敏感性的主要候选。",
        "",
        "## 时间切片",
        "",
