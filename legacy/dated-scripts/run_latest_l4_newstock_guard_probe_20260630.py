...
SCORE_DB = REPORT_DIR / "scores" / "newstock_guard_scores.duckdb"
OUT_SIGNAL_DIR = REPORT_DIR / "newstock_guard_signals"
OUT_LOG_DIR = REPORT_DIR / "newstock_guard_logs"
OUT_CSV = REPORT_DIR / "newstock_guard_probe_20260630.csv"
OUT_JSON = REPORT_DIR / "newstock_guard_probe_20260630.json"
...
CASES: list[dict[str, Any]] = [
    {"name": "guard_atr_age20_top5_s120_h2m3", "min_age_days": 20, "turnover_cap": None, "atr_cap": None, "target_scale": 1.20, "cap": 0.18, "holding_days": 2, "max_holding_days": 3, "score_exit": 0.98, "score_continue": 0.99, "max_daily_sells": 1},
    {"name": "guard_atr_age40_top5_s120_h2m3", "min_age_days": 40, "turnover_cap": None, "atr_cap": None, "target_scale": 1.20, "cap": 0.18, "holding_days": 2, "max_holding_days": 3, "score_exit": 0.98, "score_continue": 0.99, "max_daily_sells": 1},
    {"name": "guard_atr_age60_top5_s120_h2m3", "min_age_days": 60, "turnover_cap": None, "atr_cap": None, "target_scale": 1.20, "cap": 0.18, "holding_days": 2, "max_holding_days": 3, "score_exit": 0.98, "score_continue": 0.99, "max_daily_sells": 1},
    {"name": "guard_atr_age120_top5_s120_h2m3", "min_age_days": 120, "turnover_cap": None, "atr_cap": None, "target_scale": 1.20, "cap": 0.18, "holding_days": 2, "max_holding_days": 3, "score_exit": 0.98, "score_continue": 0.99, "max_daily_sells": 1},
    {"name": "guard_atr_age60_turn30_top5_s120_h2m3", "min_age_days": 60, "turnover_cap": 30.0, "atr_cap": None, "target_scale": 1.20, "cap": 0.18, "holding_days": 2, "max_holding_days": 3, "score_exit": 0.98, "score_continue": 0.99, "max_daily_sells": 1},
    {"name": "guard_atr_age60_turn20_top5_s120_h2m3", "min_age_days": 60, "turnover_cap": 20.0, "atr_cap": None, "target_scale": 1.20, "cap": 0.18, "holding_days": 2, "max_holding_days": 3, "score_exit": 0.98, "score_continue": 0.99, "max_daily_sells": 1},
    {"name": "guard_atr_age60_atr12_top5_s120_h2m3", "min_age_days": 60, "turnover_cap": None, "atr_cap": 0.12, "target_scale": 1.20, "cap": 0.18, "holding_days": 2, "max_holding_days": 3, "score_exit": 0.98, "score_continue": 0.99, "max_daily_sells": 1},
    {"name": "guard_atr_age60_s130_top5_h2m3", "min_age_days": 60, "turnover_cap": None, "atr_cap": None, "target_scale": 1.30, "cap": 0.195, "holding_days": 2, "max_holding_days": 3, "score_exit": 0.98, "score_continue": 0.99, "max_daily_sells": 1},
    {"name": "guard_atr_age60_s140_top5_h2m3", "min_age_days": 60, "turnover_cap": None, "atr_cap": None, "target_scale": 1.40, "cap": 0.21, "holding_days": 2, "max_holding_days": 3, "score_exit": 0.98, "score_continue": 0.99, "max_daily_sells": 1},
    {"name": "guard_atr_age60_top5_h1m2", "min_age_days": 60, "turnover_cap": None, "atr_cap": None, "target_scale": 1.20, "cap": 0.18, "holding_days": 1, "max_holding_days": 2, "score_exit": 0.99, "score_continue": 0.995, "max_daily_sells": 1},
]
...
def _extra_filter(case: dict[str, Any]) -> str:
    clauses = [
        "try_cast(md.atr_qfq AS DOUBLE) IS NOT NULL",
        "try_strptime(md.list_date, '%Y%m%d') IS NOT NULL",
        "date_diff('day', try_strptime(md.list_date, '%Y%m%d'), try_strptime(js.trade_date, '%Y%m%d')) >= " + str(int(case["min_age_days"])),
    ]
    if case.get("turnover_cap") is not None:
        clauses.append(f"try_cast(md.turnover_rate AS DOUBLE) <= {float(case['turnover_cap'])}")
    if case.get("atr_cap") is not None:
        clauses.append(f"try_cast(md.atr_qfq AS DOUBLE) <= {float(case['atr_cap'])}")
    return " AND ".join(clauses)


def _ensure_signal(case: dict[str, Any]) -> tuple[Path, str, dict[str, Any]]:
    sources = {label: _load_manifest(label) for label in ("1d", "3d", "5d", "10d")}
    table = _score_table(case)
    signal_file = OUT_SIGNAL_DIR / f"{case['name']}.csv"
    topn = int(case.get("topn", 5))
    if signal_file.exists():
        rows = list(csv.DictReader(signal_file.open("r", encoding="utf-8-sig", newline="")))
        counts = Counter(row["signal_date"] for row in rows)
        return signal_file, table, {"signal_rows": len(rows), "signal_days": len(counts), "days_below_target": sum(1 for value in counts.values() if value < topn)}

    OUT_SIGNAL_DIR.mkdir(parents=True, exist_ok=True)
    SCORE_DB.parent.mkdir(parents=True, exist_ok=True)
    score_expr = (
        "0.45 * pow(greatest(rank_10d, 0.0), 1.4)"
        " + 0.45 * pow(greatest(rank_5d, 0.0), 1.4)"
        " + 0.10 * pow(greatest(rank_3d, 0.0), 1.4)"
    )
    con = duckdb.connect()
    try:
        ...
        con.execute(f"ATTACH '{Path(sources['1d']['db_path']).as_posix()}' AS l4_1d (READ_ONLY)")
        ...
        con.execute(
            ...
            WITH base AS (
                ...
                INNER JOIN l4_1d."{sources['1d']['table']}" p1
                    ON p10.trade_date = p1.trade_date AND p10.stock_code = p1.stock_code
            ),
            ranked AS (
                SELECT
                    trade_date,
        ...
        con.execute(f'CREATE TABLE scoredb."{table}" AS SELECT trade_date, stock_code, entry_score AS pred_prob FROM joined_scores')
        selected_sql = f"""
        WITH market_with_prev AS (
            SELECT *, lag(pct_chg, 1) OVER (PARTITION BY stock_code ORDER BY trade_date) AS prev_pct_chg
        ...
        next_dates AS (
            SELECT trade_date AS signal_date, lead(trade_date) OVER (ORDER BY trade_date) AS buy_date, max(trade_date) OVER () AS latest_market_date
            FROM market_dates
        ),
        scored AS (
            SELECT
                ...
                md.pct_chg,
                md.prev_pct_chg,
                ((1 + try_cast(md.pct_chg AS DOUBLE) / 100.0) * (1 + try_cast(md.prev_pct_chg AS DOUBLE) / 100.0) - 1.0) AS two_day_ret,
                date_diff('day', try_strptime(md.list_date, '%Y%m%d'), try_strptime(js.trade_date, '%Y%m%d')) AS list_age_days,
                js.pred_1d,
                js.pred_3d,
                js.pred_5d,
                js.pred_10d,
            ...
            LEFT JOIN market_with_prev md ON js.trade_date = md.trade_date AND js.stock_code = md.stock_code
            ...
            WHERE NOT (js.stock_code LIKE '%.BJ' OR substr(js.stock_code, 1, 1) IN ('4', '8'))
              ...
              AND coalesce(((1 + try_cast(md.pct_chg AS DOUBLE) / 100.0) * (1 + try_cast(md.prev_pct_chg AS DOUBLE) / 100.0) - 1.0), -999.0) < 0.10
              AND {_extra_filter(case)}
        ),
        with_buy_checks AS (
            SELECT
                s.*,
                ...
                END AS buy_day_ok
            FROM scored s
            LEFT JOIN marketdb.STOCK_DAILY_DATA bm ON s.buy_date = bm.trade_date AND s.stock_code = bm.stock_code
        )
        SELECT
            signal_date,
            ...
            {int(case['holding_days'])} AS holding_days,
            {int(case['max_holding_days'])} AS max_holding_days,
            printf('%.5f', {float(case['score_exit'])}) AS score_exit_entry_ratio,
            1 AS min_holding_days_before_score_exit,
            printf('%.5f', {float(case['score_continue'])}) AS score_continue_entry_ratio,
            '0.05000' AS signal_stop_loss_pct,
            '0.08000' AS signal_take_profit_pct,
            '{case['name']}' AS strategy_variant,
            '{case['name']}' AS filter_name,
            'newstock_guard' AS entry_weight_name,
            'newstock_guard' AS dynamic_hold_name,
        ...
        WHERE buy_day_ok
        QUALIFY row_number() OVER (PARTITION BY signal_date ORDER BY entry_score DESC, stock_code ASC) <= {topn}
...
def _run(case: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        **signal_meta,
        ...
        "intraday_risk": 0,
        "note": "research-only new-stock/ATR guard probe; production parameters unchanged",
    }
