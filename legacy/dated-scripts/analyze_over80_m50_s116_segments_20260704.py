...
ARTIFACT_DIR = OUT_DIR / "over80_m50_s116_artifacts"
SIGNAL_FILE = OUT_DIR / "top2_overheat_mv_quality_signals/over80_m50_s116.csv"
TRADES_OUT = OUT_DIR / "over80_m50_s116_paired_trades_with_features.csv"
SEGMENT_OUT = OUT_DIR / "over80_m50_s116_segment_summary.csv"
REPORT_OUT = OUT_DIR / "over80_m50_s116_segment_report_20260704.md"
...
def main() -> int:
    ...
    worst_rtv = summary.sort_values(["return_to_vol", "total_pnl"], ascending=[True, True]).head(30)
    best = summary.sort_values(["return_to_vol", "total_pnl"], ascending=[False, False]).head(30)
    report = [
        "# over80_m50_s116 波动贡献拆解",
        "",
        "## 汇总",
        "",
        f"- 配对交易数：{len(merged)}",
        ...
        "",
        "```csv",
        worst_rtv.to_csv(index=False, lineterminator="\n").strip(),
        "```",
        "",
        "## return_to_vol 最强分桶",
        "",
