...
SIGNAL_ROOT = DATA / "reports" / "strategy_agent_adaptive70w_fouryear_momentum_cooldown_20260629"
OUT_DIR = DATA / "reports" / "strategy_agent_adaptive70w_fouryear_sell_rule_focus_20260629"
...
CASE_KEY = "w72_23_05_amt150_mv30__hold3m5_c097_e096_ddtight_pos65__amt150000_mv300000_pc150_pos50_cool2d20"
...
SELL_CASES = [
    {
        ...
        "day_drop_ratio": None,
        "light_stop_loss_pct": None,
        "min_light_stop_days": 1,
    },
    {
        "name": "tight_e097_c098",
    ...
    {
        "name": "tight_e0965_c098",
        "score_continue_ratio": 0.98,
        "score_exit_ratio": 0.965,
        "min_score_exit_days": 1,
        "score_exit_rank": None,
        "day_drop_ratio": None,
        "light_stop_loss_pct": None,
...
def _env_for_case(case: dict[str, Any]) -> dict[str, str]:
    ...
    env["GM_SCORE_EXIT_RANK"] = "none" if case["score_exit_rank"] is None else str(case["score_exit_rank"])
    env["GM_SCORE_STOP_LOSS_DAY_DROP_RATIO"] = "none" if case["day_drop_ratio"] is None else str(case["day_drop_ratio"])
    env["GM_LIGHT_STOP_LOSS_PCT"] = "none" if case["light_stop_loss_pct"] is None else str(case["light_stop_loss_pct"])
    env["GM_MIN_HOLDING_DAYS_BEFORE_LIGHT_STOP"] = str(case["min_light_stop_days"])
    return env


def _run_case(case: dict[str, Any], signal_file: Path, tag: str, start: str, end: str, signal_rows: int) -> dict[str, Any]:
    log_file = OUT_DIR / "logs" / f"{CASE_KEY}__{case['name']}__rowcfg__{tag}.log"
    ...
    return {
        ...
        "score_exit_rank": case["score_exit_rank"],
        "day_drop_ratio": case["day_drop_ratio"],
        "light_stop_loss_pct": case["light_stop_loss_pct"],
        "min_light_stop_days": case["min_light_stop_days"],
...
def main() -> None:
    ...
    for case in SELL_CASES:
        ...
        _write_rows(OUT_DIR / "sell_rule_focus_detail.csv", detail)
        _write_rows(OUT_DIR / "sell_rule_focus_summary.csv", summary)
