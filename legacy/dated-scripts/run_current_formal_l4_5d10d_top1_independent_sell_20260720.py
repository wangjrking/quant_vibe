...
OUT = ROOT / "quant" / "data_file" / "reports" / "strategy_agent_current_formal_l4_5d10d_top1_independent_sell_20260720"
SIGNALS = OUT / "signals"
LOGS = OUT / "logs"
CASES = [
    (rank1, min_hold, max_hold)
    for rank1 in (0.86, 0.88, 0.90, 0.92, 0.94)
    for min_hold, max_hold in ((10, 13), (12, 13), (12, 14))
]
...
def run_case(rank1: float, min_hold: int, max_hold: int) -> dict:
    name = f"r1ge{int(rank1 * 100)}_top1_min{min_hold}_max{max_hold}"
