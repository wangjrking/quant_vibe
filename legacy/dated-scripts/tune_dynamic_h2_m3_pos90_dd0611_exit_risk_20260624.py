...
REPORT_DIR = BASE_DIR / "dynamic_h2_m3_pos90_dd0611_exit_risk_20260624"
...
RISK_CASES = [
    {"case_name": f"pos90_sl{sl:02d}_tp{tp:02d}", "stop_loss": sl / 100, "take_profit": tp / 100}
    for sl in [5, 6, 7]
    for tp in [5, 6, 7, 8]
]
...
def _signal_for_case(case: dict) -> Path:
    ...
    for row in rows:
        ...
        row["strategy_variant"] = (
            f"dynamic_h2_m3_dd0611_pos90_sl{case['stop_loss']:.2f}_tp{case['take_profit']:.2f}"
        )
...
def _run(case: dict, signal_file: Path, start_name: str, start: str, end: str) -> dict:
    ...
    old_soft = risk.DD_SOFT
    old_hard = risk.DD_HARD
    old_recover = risk.DD_RECOVER
    old_soft_scale = risk.DD_SOFT_SCALE
    old_hard_scale = risk.DD_HARD_SCALE
    try:
        risk.REPORT_DIR = REPORT_DIR
        risk.TARGET_PCT = TARGET_PCT
        risk.DD_SOFT = DD_SOFT
        risk.DD_HARD = DD_HARD
        risk.DD_RECOVER = DD_RECOVER
        risk.DD_SOFT_SCALE = DD_SOFT_SCALE
        risk.DD_HARD_SCALE = DD_HARD_SCALE
        return risk._run(case, signal_file, start_name, start, end)
    finally:
        risk.REPORT_DIR = old_report
        risk.TARGET_PCT = old_target
        risk.DD_SOFT = old_soft
        risk.DD_HARD = old_hard
        risk.DD_RECOVER = old_recover
        risk.DD_SOFT_SCALE = old_soft_scale
        risk.DD_HARD_SCALE = old_hard_scale
...
def _write_report(summary: list[dict], audit: dict) -> None:
    ...
    lines = [
        "# pos90 dd0611 止损止盈邻域验证",
        "",
        "## 当前结论",
        "",
        "本轮固定 formal L4 融合信号、Top1、目标仓位 90%、账户回撤缩放 dd0611_s7050 和动态持有 h2_m3，只测试通用止损/止盈阈值。收益指标只作为弱准入排序；最大回撤、recent60、近期开仓和硬过滤作为强准入判断。",
        "",
