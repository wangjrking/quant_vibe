"""Strict 2022-2024 only, daily-percentile-target 5D research candidate."""
...
OUT=DATA_DIR/'reports'/'model_agent_5d_daily_percentile_target_dev2022_2024_20260830'/'build_r1'
...
def daily_percentile(frame):
    value=frame.groupby('trade_date',sort=False).target.rank(method='average',pct=True)
    if value.isna().any() or not np.isfinite(value).all() or (value<=0).any() or (value>1).any(): raise RuntimeError('invalid daily percentile target')
    return value

def main():
    if OUT.exists(): raise RuntimeError(f'refusing overwrite: {OUT}')
    c=json.loads(CONTRACT.read_text(encoding='utf-8'))
    if c['candidate_id']!='expanding_pit_5d_daily_percentile_target_v1' or c['confirmation_window']!='2025 sealed_for_5d_candidate_only' or not c['validation_2026_closed_for_5d_candidate_only']: raise RuntimeError('contract drift')
    dates=base.read_open_dates(); _,prepared=base.preflight(dates); info=prepared['5d']; folds=[f for f in info['folds'] if f['test_end']<='20241231']
    ...
    OUT.mkdir(parents=True); base.dump_json(OUT/'preflight.json',{'contract_sha256':base.sha256_file(CONTRACT),'features':info['resolved'],'folds':folds,'production_weight_config':info['metadata'].get('sample_weight_config'),'candidate_target_transform':'daily target rank(method=average,pct=true) on mature training rows','confirmation_2025_read':False,'validation_2026_closed':True,'production_unchanged':True,'runtime_agent_route':{'model':'gpt-5.6-terra','thinking':'medium'}})
    fd=duckdb.connect(str(base.FEATURE_DB),read_only=True); ld=duckdb.connect(str(base.LABEL_DB),read_only=True); rows=[]
    try:
        for f in folds:
            tr=base.query_features(fd,info['resolved'],f['train_start'],f['train_end']).merge(base.query_labels(ld,'executable_5d_open_return',f['train_start'],f['train_end']),on=['trade_date','stock_code'],how='inner',validate='one_to_one').dropna(subset=['target']).reset_index(drop=True)
            te=base.query_features(fd,info['resolved'],f['test_start'],f['test_end']).merge(base.query_labels(ld,'executable_5d_open_return',f['test_start'],f['mature_label_cutoff']),on=['trade_date','stock_code'],how='left',validate='one_to_one')
            ...
            model.fit(xtr,daily_percentile(tr).astype('float32'),sample_weight=base.daily_weights(tr,info['metadata'].get('sample_weight_config')),verbose=False)
