...
REPORT_DIR = BASE_DIR / "dynamic_h2_m3_pos76_stoploss_20260624"
...
STOP_LOSSES = [0.05, 0.055, 0.06, 0.065, 0.07]
...
def _case_name(stop_loss: float) -> str:
    return f"pos76_sl{int(round(stop_loss * 1000)):03d}_dd0612"


def _signal_for_case(stop_loss: float) -> Path:
    if not BASE_SIGNAL_FILE.exists():
        raise FileNotFoundError(BASE_SIGNAL_FILE)
    path = REPORT_DIR / "signals" / f"{_case_name(stop_loss)}.csv"
    if path.exists():
        return path
    ...
    for row in rows:
        ...
        row["strategy_variant"] = f"dynamic_h2_m3_pos76_sl{stop_loss:.3f}_dd0612"
...
def _run(stop_loss: float, signal_file: Path, start_name: str, start: str, end: str) -> dict:
    case_name = _case_name(stop_loss)
...
def _write_report(summary: list[dict], audit: dict) -> None:
    ...
    for row in ranked:
        lines.append(
            f"| {row['case_name']} | {_f(row['stop_loss']):.1%} | {_f(row['full_annual']):.2%} | {_f(row['full_sharpe']):.2f} | "
    ...
    (REPORT_DIR / "stoploss_report.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> int:
    ...
    for stop_loss in STOP_LOSSES:
        signal_file = _signal_for_case(stop_loss)
        ...
        for start_name, start, end in STARTS:
            row = _run(stop_loss, signal_file, start_name, start, end)
            detail.append(row)
