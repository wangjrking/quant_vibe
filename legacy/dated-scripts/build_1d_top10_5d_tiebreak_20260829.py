"""Evaluate one fixed strict-OOF 1D Top10 / 5D ordering candidate."""
...
OUTPUT_DIR = DATA_DIR / "reports" / "model_agent_1d_top10_5d_tiebreak_20260829" / "build_r1"
...
def top10_tiebreak(frame: pd.DataFrame) -> np.ndarray:
    ...
    for _, group in frame.groupby("trade_date", sort=False):
        ...
        top = primary.head(10).sort_values(["score_5d", "score_1d", "stock_code"], ascending=[False, False, True], kind="mergesort")
        ordered = pd.concat([top, primary.iloc[10:]], ignore_index=False)
        if set(ordered.head(10)["stock_code"]) != set(primary.head(10)["stock_code"]):
            raise RuntimeError("Top10 membership changed")
        output[ordered.index.to_numpy()] = np.arange(len(ordered), 0, -1, dtype=np.float64)
    return output
...
def main() -> int:
    ...
    if contract["candidate_id"] != "expanding_pit_1d_top10_5d_tiebreak_v1" or change["top_n"] != 10 or not contract["development_window"]["validation_2026_closed"]:
    ...
    if len(frame) != len(one):
        raise RuntimeError("same-key input closure failed")
    frame = frame.rename(columns={"pred_prob": "score_1d"})
    first = top10_tiebreak(frame)
    second = top10_tiebreak(frame)
    third = top10_tiebreak(frame)
    if not (np.array_equal(first, second) and np.array_equal(first, third)):
        raise RuntimeError("deterministic replay 3/3 failed")
    frame["pred_prob"] = first
