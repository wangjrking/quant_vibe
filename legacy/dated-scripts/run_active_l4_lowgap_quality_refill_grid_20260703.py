...
OUT_DIR = REPORT_DIR / "variants_lowgap_quality_refill_grid"
RUN_DIR = REPORT_DIR / "juejin_runs_lowgap_quality_refill_grid"
RAW_CSV = REPORT_DIR / "juejin_lowgap_quality_refill_grid_raw.csv"
SUMMARY_CSV = REPORT_DIR / "juejin_lowgap_quality_refill_grid_summary.csv"
MANIFEST_CSV = REPORT_DIR / "variants_lowgap_quality_refill_grid_manifest.csv"


CASES = [
    ("base_lowgap", "buy_open_gap_raw_pct ASC, pred_prob DESC, stock_code", ""),
    ("no_mid_gap", "buy_open_gap_raw_pct ASC, pred_prob DESC, stock_code", "AND NOT (buy_open_gap_raw_pct > -2.5 AND buy_open_gap_raw_pct <= -1.5)"),
    ("no_atr24", "buy_open_gap_raw_pct ASC, pred_prob DESC, stock_code", "AND NOT (buy_atr_qfq >= 2.0 AND buy_atr_qfq < 4.0)"),
    ("no_siggt2", "buy_open_gap_raw_pct ASC, pred_prob DESC, stock_code", "AND signal_pct_chg <= 2.0"),
    ("no_mid_atr24", "buy_open_gap_raw_pct ASC, pred_prob DESC, stock_code", "AND NOT (buy_open_gap_raw_pct > -2.5 AND buy_open_gap_raw_pct <= -1.5) AND NOT (buy_atr_qfq >= 2.0 AND buy_atr_qfq < 4.0)"),
    ("no_mid_siggt2", "buy_open_gap_raw_pct ASC, pred_prob DESC, stock_code", "AND NOT (buy_open_gap_raw_pct > -2.5 AND buy_open_gap_raw_pct <= -1.5) AND signal_pct_chg <= 2.0"),
    ("quality_sort_a", "CASE WHEN signal_pct_chg > 0 AND signal_pct_chg <= 2 THEN 0 WHEN buy_open_gap_raw_pct <= -2.5 THEN 1 WHEN buy_open_gap_raw_pct <= 0 THEN 2 ELSE 3 END ASC, CASE WHEN buy_atr_qfq >= 2.0 AND buy_atr_qfq < 4.0 THEN 1 ELSE 0 END ASC, buy_open_gap_raw_pct ASC, pred_prob DESC, stock_code", ""),
    ("quality_sort_b", "CASE WHEN buy_atr_qfq >= 2.0 AND buy_atr_qfq < 4.0 THEN 1 ELSE 0 END ASC, CASE WHEN buy_open_gap_raw_pct > -2.5 AND buy_open_gap_raw_pct <= -1.5 THEN 1 ELSE 0 END ASC, CASE WHEN signal_pct_chg > 2 THEN 1 ELSE 0 END ASC, buy_open_gap_raw_pct ASC, pred_prob DESC, stock_code", ""),
    ("quality_sort_c", "CASE WHEN signal_pct_chg > 0 AND signal_pct_chg <= 2 THEN 0 WHEN signal_pct_chg <= 0 THEN 1 ELSE 2 END ASC, score_pct_rank DESC, buy_open_gap_raw_pct ASC, stock_code", "AND NOT (buy_atr_qfq >= 2.0 AND buy_atr_qfq < 4.0)"),
]
...
def export_case(con: duckdb.DuckDBPyConnection, case: tuple) -> dict:
    name, order_expr, extra_filter = case
    df = con.execute(
        f"""
        WITH picked AS (
            SELECT
    ...
    return {
        ...
        "avg_gap": float(df["buy_open_gap_raw_pct"].mean()) if len(df) else None,
        "avg_sigchg": float(df["signal_pct_chg"].mean()) if len(df) else None,
    }
