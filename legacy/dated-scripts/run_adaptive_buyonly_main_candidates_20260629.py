...
CASES = [
    {
        "name": "w75_ddtight",
        "signal": BASE_REPORT / "signals" / "w75_20_05_amt90_mv20__base_h2m3_c097_e097_ddtight.csv",
        "score_table": "score_w75_20_05_amt90_mv20__base_h2m3_c097_e097_ddtight",
    },
    {
        "name": "w80_ddtight",
        "signal": BASE_REPORT / "signals" / "w80_15_05_amt90_mv20__base_h2m3_c097_e097_ddtight.csv",
        "score_table": "score_w80_15_05_amt90_mv20__base_h2m3_c097_e097_ddtight",
    },
]
...
def _run(case: dict[str, Any], tag: str, start: str, end: str) -> dict[str, Any]:
    log_file = REPORT_DIR / "logs" / f"{case['name']}__adaptive70w_buyonly__{tag}.log"
    ...
    if indicator is None:
        ...
        if proc.returncode != 0 or indicator is None:
            return {"case": case["name"], "slice": tag, "returncode": proc.returncode, "log_file": str(log_file)}
    return {
        "case": case["name"],
        "slice": tag,
...
def main() -> None:
    ...
    for case in CASES:
        ...
        detail.extend(rows)
        out = {"case": case["name"], "signal_file": str(case["signal"]), "score_table": case["score_table"]}
        for row in rows:
            prefix = row["slice"]
            for key in ["annual", "pnl_ratio", "sharpe", "max_drawdown", "win_ratio", "open_count"]:
                out[f"{prefix}_{key}"] = row.get(key)
        summary.append(out)
        _write_rows(REPORT_DIR / "adaptive70w_buyonly_detail.csv", detail)
        _write_rows(REPORT_DIR / "adaptive70w_buyonly_summary.csv", summary)
        print(json.dumps(out, ensure_ascii=False), flush=True)
    (REPORT_DIR / "adaptive70w_buyonly_summary.json").write_text(
