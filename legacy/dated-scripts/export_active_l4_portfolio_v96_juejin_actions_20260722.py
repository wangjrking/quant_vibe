...
import research_active_l4_portfolio_refine_v96_20260722 as v96
...
def main() -> None:
    parser = argparse.ArgumentParser(description="导出 V96 冻结候选的掘金逐日动作")
    ...
    for case_id in args.case_id:
        ...
        daily, actions = v86.simulate(
            ...
            score,
            order,
            v86.engine_params(definition),
            args.end_date,
            record_actions=True,
        )
        action_path = action_dir / "{}_through_{}.csv".format(case_id, args.end_date)
        ...
        runs.append(
            {
                ...
                "action_sha256": digest(action_path),
                ...
                "target_pct_min": float(actions.loc[actions["action"] == "BUY", "target_pct"].min()),
                "target_pct_max": float(actions.loc[actions["action"] == "BUY", "target_pct"].max()),
