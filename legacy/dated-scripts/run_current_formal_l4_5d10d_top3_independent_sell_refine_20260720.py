...
# First test the local neighborhood. The final four cases add a modest score
# advantage requirement so replacement is not triggered by a negligible edge.
CASES = [
    (rank1, min_hold, max_hold, 1.0, 0.0)
    for rank1 in (0.84, 0.86, 0.88, 0.90)
    for min_hold in (11, 12)
    for max_hold in (13, 14)
] + [
    (rank1, 12, 13, ratio, edge)
    for rank1 in (0.86, 0.88)
    for ratio, edge in ((1.01, 0.0), (1.0, 0.01))
]
...
def case_name(rank1: float, min_hold: int, max_hold: int, ratio: float, edge: float) -> str:
    return (
        f"r1ge{int(rank1 * 100)}_top3_min{min_hold}_max{max_hold}"
        f"_ratio{int(round(ratio * 100))}_edge{int(round(edge * 100))}"
    )


def run_case(rank1: float, min_hold: int, max_hold: int, ratio: float, edge: float) -> dict:
    name = case_name(rank1, min_hold, max_hold, ratio, edge)
    frame = pd.read_csv(BASE_SIGNAL)
    frame = frame[frame["rank_1d"] >= rank1].copy()
    frame["target_pct"] = 1 / 3
    frame["holding_days"] = min_hold
    frame["max_holding_days"] = max_hold
    ...
    env.update(
        {
            ...
            "GM_INDEPENDENT_REPLACE_RATIO": str(ratio),
            "GM_INDEPENDENT_REPLACE_EDGE": str(edge),
...
def main() -> None:
    ...
    (OUT / "result.json").write_text(
        json.dumps({"status": "research_only", "results": rows}, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
