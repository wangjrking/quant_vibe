...
OUT_DIR = REPORT_DIR / "variants_lowgap_refill_gapmax_fine"
RUN_DIR = REPORT_DIR / "juejin_runs_lowgap_refill_gapmax_fine"
RAW_CSV = REPORT_DIR / "juejin_lowgap_refill_gapmax_fine_raw.csv"
SUMMARY_CSV = REPORT_DIR / "juejin_lowgap_refill_gapmax_fine_summary.csv"


GAP_MAX_CASES = [0.6, 0.8, 0.9, 1.0, 1.1, 1.2, 1.3]
...
def export_case(con: duckdb.DuckDBPyConnection, gap_max: float) -> dict:
    name = f"gapmax_{str(gap_max).replace('.', 'p')}"
    df = con.execute(
        f"""
        WITH picked AS (
            SELECT
    ...
    return {
        ...
        "avg_gap": float(df["buy_open_gap_raw_pct"].mean()),
    }
...
def main() -> int:
    ...
    try:
        rows = [export_case(con, gap_max) for gap_max in GAP_MAX_CASES]
    finally:
        con.close()
