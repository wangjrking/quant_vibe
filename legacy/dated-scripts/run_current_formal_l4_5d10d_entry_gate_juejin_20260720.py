...
SRC=ROOT/"quant"/"data_file"/"reports"/"strategy_agent_current_formal_l4_5d10d_gate_juejin_20260720";BASE=SRC/"signals"/"w10_100_plain_f50_pct3_m98_top1_h12_full100.csv";SCORE=SRC/"score_assets"/"w10_100.duckdb";MARKET=ROOT/"quant"/"data_file"/"production_assets"/"duckdb"/"l2_stock_daily_data.duckdb";OUT=ROOT/"quant"/"data_file"/"reports"/"strategy_agent_current_formal_l4_5d10d_entry_gate_juejin_20260720";SIG=OUT/"signals";LOG=OUT/"logs"
CASES={"r1ge50":lambda x:x.rank_1d>=.5,"r1ge70":lambda x:x.rank_1d>=.7,"r1ge80":lambda x:x.rank_1d>=.8,"r1ge85":lambda x:x.rank_1d>=.85,"r1ge90":lambda x:x.rank_1d>=.9,"r1ge95":lambda x:x.rank_1d>=.95,"r1ge98":lambda x:x.rank_1d>=.98,"gap5m_1p":lambda x:(x.buy_open_gap_raw_pct>=-5)&(x.buy_open_gap_raw_pct<=1),"gap3m_1p":lambda x:(x.buy_open_gap_raw_pct>=-3)&(x.buy_open_gap_raw_pct<=1),"r1ge70_gap5m_1p":lambda x:(x.rank_1d>=.7)&(x.buy_open_gap_raw_pct>=-5)&(x.buy_open_gap_raw_pct<=1)}
def parse(p):
 ...
 return {}
def run(n,fn):
 f=pd.read_csv(BASE);f=f[fn(f)].copy();s=SIG/f"{n}.csv";f.to_csv(s,index=False,encoding="utf-8-sig");log=LOG/f"{n}.log";e=os.environ.copy();e.update({"GM_INTRADAY_RISK_MODE":"0","GM_ADAPTIVE_LIQUIDITY_SLIPPAGE":"0","GM_OPEN_DAILY_SCORE_EXIT":"0","GM_INDEPENDENT_REPLACE_MODE":"0","GM_RESIZE_HELD_ON_SIGNAL":"0"});cmd=[sys.executable,str(RUNNER),"--strategy-dir",str(STRATEGY),"--signal-file",str(s),"--log-file",str(log),"--max-positions","12","--holding-days","12","--max-holding-days","12","--target-position-pct",str(1/12),"--score-db",str(SCORE),"--score-table","blended_rank_score","--market-db",str(MARKET),"--backtest-adjust","none","--backtest-slippage-ratio","0.003","--backtest-start","2022-06-07 09:00:00","--backtest-end","2026-07-17 15:30:00"];p=subprocess.run(cmd,cwd=str(MAIN),env=e,capture_output=True,text=True);i=parse(log);return {"name":n,"signal_days":f.signal_date.nunique(),"returncode":p.returncode,"annual_return":i.get("pnl_ratio_annual"),"sharpe":i.get("sharp_ratio"),"max_drawdown":i.get("max_drawdown"),"win_ratio":i.get("win_ratio"),"open_count":i.get("open_count"),"close_count":i.get("close_count"),"signal_file":str(s),"log_file":str(log)}
def main():
