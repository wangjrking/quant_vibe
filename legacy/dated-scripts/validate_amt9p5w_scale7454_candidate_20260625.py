...
CASE_NAME = "amt9p5w_mv20w_scale7454_candidate_v20260625"
REPORT_DIR = core.SOURCE_DIR / "formal_horizon_entry_confirmation_20260624" / "amt9p5w_mv20w_scale7454_candidate_validation_20260625"
SIGNAL_FILE = (
...
def _rewrite_report() -> None:
    ...
    lines = [
        "# amt9p5w_mv20w + scale7454 候选验证报告",
        "",
        "## 当前结论",
        "",
        f"- 验证对象：`{CASE_NAME}`",
        "- 该版本在 `amt9p5w_mv20w` 候选池基础上，执行口径改为 `dd_soft=7% / dd_hard=11.5% / soft_scale=74% / hard_scale=54%`，其余规则与当前生产动态链路一致。",
        "- 不增加行业、月份、日期、最新状态回填等规则；不买北交所、不买 ST/风险警示、不买退市、不买开盘涨停。",
        f"- 全周期年化：`{_f(full.get('annual')):.2%}`，Sharpe：`{_f(full.get('sharpe')):.3f}`，最大回撤：`{_f(full.get('max_drawdown')):.2%}`。",
        f"- 近端冷启动最差年化：`{late_min:.2%}`。",
        "",
        "## 时间切片",
        "",
    ...
    for row in time_rows:
        lines.append(
            f"| {row['slice']} | {_f(row['annual']):.2%} | {_f(row['sharpe']):.3f} | "
    ...
    lines.extend(
        [
            ...
            f"- 最高代理日期：`{proxy_stats['top_day']}`，占正代理贡献 `{_f(proxy_stats['top_day_share']):.2%}`。",
            f"- 最高代理月份：`{proxy_stats['top_month']}`，占正代理贡献 `{_f(proxy_stats['top_month_share']):.2%}`。",
            "",
            "| 压力变体 | 说明 | 年化 | Sharpe | 最大回撤 | 近端最差年化 |",
            "| --- | --- | ---: | ---: | ---: | ---: |",
        ]
    )
    for row in stress_summary:
        lines.append(
            ...
            f"{_f(row['full_sharpe']):.3f} | {_f(row['full_max_drawdown']):.2%} | {_f(row['late_min_annual']):.2%} |"
        )
    lines.extend(
        [
            "",
            ...
            f"- 冷启动：`{REPORT_DIR / 'nearby_summary.csv'}`",
            f"- 代理压力：`{REPORT_DIR / 'stress_summary.csv'}`",
            f"- 代理明细：`{REPORT_DIR / 'proxy_signal_contribution.csv'}`",
            f"- 硬约束：`{REPORT_DIR / 'hard_gate_audit.json'}`",
        ]
    )
