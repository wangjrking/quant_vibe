...
OUT = ROOT / "quant" / "data_file" / "reports" / "strategy_agent_current_formal_l4_cap5_position_count_20260720"
...
def run_case(positions: int) -> dict:
    frame = pd.read_csv(BASE)
    frame["target_pct"] = 1.0 / positions
    signal = SIGNALS / f"pos{positions}.csv"
    frame.to_csv(signal, index=False, encoding="utf-8-sig")
    log = LOGS / f"pos{positions}.log"
    env = os.environ.copy()
    env.update({
    ...
    command = [
        ...
        "--log-file", str(log), "--max-positions", str(positions), "--holding-days", "12",
        "--max-holding-days", "13", "--target-position-pct", str(1.0 / positions),
    ...
    return {"positions": positions, "returncode": process.returncode, "annual_return": indicator.get("pnl_ratio_annual"), "sharpe": indicator.get("sharp_ratio"), "max_drawdown": indicator.get("max_drawdown"), "win_ratio": indicator.get("win_ratio"), "open_count": indicator.get("open_count"), "close_count": indicator.get("close_count"), "signal_file": str(signal), "log_file": str(log)}
