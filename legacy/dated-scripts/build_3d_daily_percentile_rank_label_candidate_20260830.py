"""Build one frozen daily-rank 3D label candidate under strict pre-2025 PIT.
...
training label: same-day 3D cross-sectional percentile rank.  No 2025 or
2026 data, calendar, label, or metric is read.
"""
...
CANDIDATE_ID = "v261_3d_daily_percentile_rank_label_v1"
...
DEFAULT_OUTPUT = Path("quant/data_file/reports/model_agent_3d_daily_percentile_rank_label_20260830")
...
def combined_target(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    # This isolates daily cross-sectional target scaling from horizon blending.
    valid["candidate_training_target"] = valid["rank_3d"]
...
def run(output: Path) -> int:
    ...
    contract = {
        ...
        "training_target": "daily_percentile_rank(executable_3d_open_return)",
