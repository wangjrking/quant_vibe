...
CASE_NAME = "dynamic_h2_m3_c098_w78_5d12_3d10_pos84_e097_mh1_sl06_dd0714_tp060"
SOURCE_DIR = liq.SOURCE_DIR / "strict_sync_liquidity_neighborhood_20260624"
REPORT_DIR = SOURCE_DIR / "formal_horizon_entry_confirmation_20260624" / "dynamic_h2_m3_sl06_dd0714_pos84_tp060_validation_20260624"
SIGNAL_FILE = (
    SOURCE_DIR
    / "formal_horizon_entry_confirmation_20260624"
    / "dynamic_h2_m3_tp060_pos84_dd_neighborhood_20260624"
    / "signals"
    / "tp060_pos84_dd0714_s8060.csv"
)
...
def _write_report(time_rows: list[dict], nearby_summary: list[dict], stress_summary: list[dict], proxy_stats: dict, audit: dict) -> None:
    lines = [
        ...
        "## 当前结论",
        "",
        f"验证对象：`{CASE_NAME}`。该版本固定 formal L4 多周期模型入场、Top1 单票、目标仓位 `84%`、动态持有 `h2_m3_c098`、分数退出 `0.97`、日内 `6%` 止损、账户回撤缩放 `soft=7% / hard=14% / recover=4% / scale=0.80/0.60`，新增通用 `6%` 止盈。",
        "",
