...
OUT_LOG_DIR = REPORT_DIR / "next_open_light_stop_exit_logs"
OUT_CSV = REPORT_DIR / "next_open_light_stop_exit_probe_20260630.csv"
OUT_JSON = REPORT_DIR / "next_open_light_stop_exit_probe_20260630.json"
OUT_MD = REPORT_DIR / "next_open_light_stop_exit_probe_summary_20260630.md"
...
CASES: list[dict[str, Any]] = [
    ...
    _case(
        "nors_top3_w20_ls03_ms3_score98",
        "nors_top3_w20",
        light_stop=0.03,
        min_hold_light=1,
        max_daily_sells=3,
    ...
    _case(
        "nors_top3_w20_ls05_ms3_score98",
        "nors_top3_w20",
        light_stop=0.05,
        min_hold_light=1,
        max_daily_sells=3,
    ...
    _case(
        "nors_top3_w20_ls03_ms5_score995",
        "nors_top3_w20",
        light_stop=0.03,
        min_hold_light=1,
        max_daily_sells=5,
...
def _run(case: dict[str, Any]) -> dict[str, Any]:
    ...
    if indicator is None:
        env = os.environ.copy()
        dd_enabled = bool(case["dd"])
        env.update(
            {
                "GM_OPEN_DAILY_SCORE_EXIT": "1",
                "GM_MAX_DAILY_SELLS": str(int(case["max_daily_sells"])),
                "GM_LIGHT_STOP_LOSS_PCT": str(float(case["light_stop"])),
                "GM_MIN_HOLDING_DAYS_BEFORE_LIGHT_STOP": str(int(case["min_hold_light"])),
                "GM_LOG_EXPOSURE": "1",
                ...
                "GM_EQUITY_DD_RISK_MODE": "1" if dd_enabled else "0",
                "GM_EQUITY_DD_RESIZE_EXISTING": "1" if dd_enabled else "0",
                "GM_EQUITY_DD_SOFT_TRIGGER": "0.06",
                "GM_EQUITY_DD_HARD_TRIGGER": "0.10",
                "GM_EQUITY_DD_RECOVER_TRIGGER": "0.03",
                "GM_EQUITY_DD_SOFT_SCALE": "0.55",
                "GM_EQUITY_DD_HARD_SCALE": "0.25",
    ...
    return {
        **case,
        ...
        "note": "research-only next-open state plus open light-stop exit frequency; production unchanged",
    }
...
def _write_report(results: list[dict[str, Any]]) -> None:
    ...
    lines = [
        "# 娆℃棩寮€鐩樼姸鎬佹満涓庡紑鐩樿交姝㈡崯鍗栧嚭棰戠巼澶嶆祴 20260630",
        "",
        "## 杈圭晫",
        "",
        "鏈姤鍛婁负 L5/L6 research-only 楠岃瘉锛屼笉淇敼鐢熶骇绛栫暐鍙傛暟锛屼笉鐢熸垚姝ｅ紡鐢熶骇淇″彿锛屼笉瑙﹀彂浜ゆ槗銆?",
        "鏈疆娴嬭瘯寮€鐩樿交姝㈡崯銆佹渶澶ф棩鍗栧嚭鏁般€佸垎鏁拌“閫€閫€鍑轰笌鍙€夋棩绾у洖鎾ょ缉鏀撅紝鍧囦负寮€鐩樻棩绾ц鍒欍€?",
        "",
        "## 缁撴灉鎽樿",
        "",
    ...
    lines.extend(
        [
            ...
            "## 鍒ゆ柇",
            "",
            "鑻ュ紑鐩樿交姝㈡崯鎻愰珮鍗栧嚭棰戠巼鍚庝粛鏃犳硶鍚屾椂婊¤冻楂樺勾鍖栧拰浣庡洖鎾わ紝璇存槑涓昏鐭涚浘涓嶆槸鍗栧嚭瑙﹀彂婊炲悗锛岃€屾槸楂樻敹鐩婁俊鍙锋湰韬甫鏉ョ殑楂樻尝鍔ㄦ毚闇层€?",
            "",
            "## 璇佹嵁璺緞",
            "",
            f"- `{OUT_CSV}`",
