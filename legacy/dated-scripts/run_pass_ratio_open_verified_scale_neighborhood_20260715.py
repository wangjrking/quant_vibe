...
OUT_CSV = base.REPORT_DIR / "pass_ratio_open_verified_scale_neighborhood_juejin_20260715.csv"
OUT_JSON = base.REPORT_DIR / "pass_ratio_open_verified_scale_neighborhood_juejin_20260715.json"
OUT_MD = base.REPORT_DIR / "pass_ratio_open_verified_scale_neighborhood_review_20260715.md"
SIGNAL_DIR = base.REPORT_DIR / "signals" / "open_verified_scale_neighborhood"
LOG_DIR = base.REPORT_DIR / "logs" / "open_verified_scale_neighborhood_20260715"

CASES = [
    {"case": "ov_top1_s138_cap065", "scale": 1.38, "cap": 0.65},
    {"case": "ov_top1_s142_cap065", "scale": 1.42, "cap": 0.65},
    {"case": "ov_top1_s145_cap065", "scale": 1.45, "cap": 0.65},
    {"case": "ov_top1_s148_cap065", "scale": 1.48, "cap": 0.65},
    {"case": "ov_top1_s142_cap067", "scale": 1.42, "cap": 0.67},
    {"case": "ov_top1_s145_cap067", "scale": 1.45, "cap": 0.67},
    {"case": "ov_top1_s148_cap067", "scale": 1.48, "cap": 0.67},
]
...
def main() -> None:
    ...
    lines = [
        "# 开盘验证 Top1 仓位邻域掘金复跑 20260715",
        "",
        "## 结论",
        "",
        "- 本轮验证 `scale=1.45, cap=0.65` 附近是否为单点尖峰。",
        "- 仍然不补位、不扩候选，只使用已通过开盘硬门槛的每日 Top1。",
        "",
    ...
    for row in gframe.to_dict("records"):
        lines.append(
            ...
            f"{base.pct(row['recent60_annual'])} | {base.pct(row['min_annual'])} | {float(row['min_sharpe']):.4f} |"
        )
    lines.extend(
        [
            "",
