"""Build one frozen 1D liquidity-aware feature candidate under strict PIT.

The candidate keeps the approved 1D parameters, target, expanding history and
evaluation target.  Its only changed semantic is appending current-day
``amount``, which the 78-feature production model does not consume.  No 2025
...
CANDIDATE_ID = "v261_1d_amount_feature_expansion_v1"
...
DEFAULT_OUTPUT = Path("quant/data_file/reports/model_agent_1d_amount_feature_expansion_20260830")
...
def folds(dates: list[str], train_start: str) -> list[dict[str, str]]:
    ...
    for year in ("2022", "2023", "2024"):
        ...
        result.append({
            ...
            "test_end": mature_end,
            "embargo_rule": "2 official open sessions (1D label settlement)",
        })
    return result


def preflight(dates: list[str], additional_feature: str) -> tuple[dict[str, object], dict[str, object]]:
    ...
    try:
        ...
        if aliases["missing"]:
            raise RuntimeError(f"blocked_fail_closed_1d_feature_aliases:{aliases['missing']}")
        alias_map = dict(aliases["alias_pairs"])
        resolved = [alias_map.get(value, value) for value in requested]
        if len(resolved) != 78 or len(set(resolved)) != len(resolved):
            raise RuntimeError("blocked_fail_closed_1d_feature_contract")
        if additional_feature not in available or additional_feature in resolved:
            raise RuntimeError("blocked_fail_closed_additional_feature_contract")
        count = label_connection.execute(
            f"SELECT count({quote('executable_1d_open_return')}) FROM {quote(LABEL_TABLE)} WHERE trade_date BETWEEN ? AND ?",
            [train_start, DEVELOPMENT_END],
        ).fetchone()[0]
        if not count:
            raise RuntimeError("blocked_fail_closed_missing_1d_label")
    finally:
    ...
    }, {"metadata": metadata, "features": [*resolved, additional_feature], "folds": folds(dates, train_start)}
...
def load_baseline(fold: dict[str, str]) -> pd.DataFrame:
    ...
    try:
        ...
        baseline = connection.execute(
            ...
            FROM read_parquet('{root}/1d_oof.parquet')
...
def run(output: Path, candidate_id: str, additional_feature: str, selection_evidence: Path | None) -> int:
    output.mkdir(parents=True, exist_ok=False)
    dates = strict_open_dates()
    evidence, config = preflight(dates, additional_feature)
    selection = None
    if selection_evidence is not None:
        selection = json.loads(selection_evidence.read_text(encoding="utf-8"))
        if selection.get("selected_feature") != additional_feature:
            raise RuntimeError("blocked_fail_closed_selection_evidence_feature_mismatch")
        evidence["selection_evidence_path"] = str(selection_evidence)
        evidence["selection_evidence_sha256"] = sha256_file(selection_evidence)
    contract = {
        ...
        "training_target": "executable_1d_open_return (unchanged)",
        "model_semantics": f"approved 1D parameters and original sample weighting; append exactly one current-day feature: {additional_feature}",
        "additional_feature": additional_feature,
        "selection_evidence": selection,
        "evaluation_target": "executable_1d_open_return",
        "free_parameters": 0,
    ...
    try:
        for fold in config["folds"]:
            ...
            train_label = query_labels(label_connection, "executable_1d_open_return", fold["train_start"], fold["train_end"])
            ...
            if weights is not None and len(weights) != len(candidate_train):
            ...
            test_label = query_labels(label_connection, "executable_1d_open_return", fold["test_start"], fold["test_end"])
...
def main() -> int:
    ...
    parser.add_argument("--additional-feature", default="amount")
    parser.add_argument("--selection-evidence")
    args = parser.parse_args()
    evidence = Path(args.selection_evidence) if args.selection_evidence else None
    return run(Path(args.output_dir), args.candidate_id, args.additional_feature, evidence)
