"""Build one frozen 1D top-one-percent label candidate under strict PIT.

The candidate keeps the approved 1D feature set, model parameters, expanding
history and raw-return evaluation target.  Its only changed semantic is a
same-day binary label for the realized Top1 percent.  No 2025 or 2026 data,
calendar, label, or metric is read.
"""
...
CANDIDATE_ID = "v261_1d_daily_top1pct_binary_target_v1"
...
DEFAULT_OUTPUT = Path("quant/data_file/reports/model_agent_1d_daily_top1pct_binary_target_20260830")
...
def combined_target(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    ranks = valid.groupby("trade_date", sort=False)["target"].rank(method="first", pct=True)
    valid["candidate_training_target"] = (ranks > 0.99).astype("float32")
    if valid["candidate_training_target"].sum() <= 0:
        raise RuntimeError("blocked_fail_closed_no_top1pct_train_rows")
...
def run(output: Path) -> int:
    ...
    contract = {
        ...
        "training_target": "1 if executable_1d_open_return is within the same-day top 1 percent by deterministic first rank, else 0",
        "model_semantics": "approved 1D feature set and fixed production hyperparameters; no sample reweighting",
