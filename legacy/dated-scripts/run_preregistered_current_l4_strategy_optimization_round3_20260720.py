...
CONFIG = MAIN / "config" / "strategy_research" / "current_l4_preregistered_optimization_round3_20260720.json"
OUT = ROOT / "quant" / "data_file" / "reports" / "strategy_agent_preregistered_current_l4_optimization_round3_20260720"
...
def entry_cases(config: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for (blend, weights), topn, rank_min, support, amount, mv, pct, open_gate in values:
        name = (
            ...
            f"a{int(amount)}_mv{int(mv)}_{pct['name']}_{open_gate['name']}"
        )
        rows.append({
...
def filter_case_frame(base: pd.DataFrame, case: dict[str, Any]) -> pd.DataFrame:
    ...
    mask = (
        ...
        & (base["rank_3d"] >= float(support["rank_3d_min"]))
