...
OUT_SIGNAL_DIR = REPORT_DIR / "timing1d_signals"
OUT_LOG_DIR = REPORT_DIR / "timing1d_logs"
OUT_CSV = REPORT_DIR / "timing1d_probe_20260630.csv"
OUT_JSON = REPORT_DIR / "timing1d_probe_20260630.json"
...
CASES: list[dict[str, Any]] = [
    {"name": "timing_45_40_10_05_g1p4_top5_dyn", "w10": 0.45, "w5": 0.40, "w3": 0.10, "w1": 0.05, "gamma": 1.4, "topn": 5, "cap": 0.15, "target_mode": "mild"},
    {"name": "timing_40_45_10_05_g1p4_top5_dyn", "w10": 0.40, "w5": 0.45, "w3": 0.10, "w1": 0.05, "gamma": 1.4, "topn": 5, "cap": 0.15, "target_mode": "mild"},
    {"name": "timing_35_45_10_10_g1p4_top5_dyn", "w10": 0.35, "w5": 0.45, "w3": 0.10, "w1": 0.10, "gamma": 1.4, "topn": 5, "cap": 0.15, "target_mode": "mild"},
    {"name": "timing_45_35_10_10_g1p4_top5_dyn", "w10": 0.45, "w5": 0.35, "w3": 0.10, "w1": 0.10, "gamma": 1.4, "topn": 5, "cap": 0.15, "target_mode": "mild"},
    {"name": "timing_55_30_10_05_g1p4_top5_dyn", "w10": 0.55, "w5": 0.30, "w3": 0.10, "w1": 0.05, "gamma": 1.4, "topn": 5, "cap": 0.15, "target_mode": "mild"},
    {"name": "timing_base_1d_scale50_top5_dyn", "w10": 0.45, "w5": 0.45, "w3": 0.10, "w1": 0.00, "gamma": 1.4, "topn": 5, "cap": 0.15, "target_mode": "mild_1d_scale50"},
    {"name": "timing_base_1d_skip50_top5_dyn", "w10": 0.45, "w5": 0.45, "w3": 0.10, "w1": 0.00, "gamma": 1.4, "topn": 5, "cap": 0.15, "target_mode": "mild", "row_filter": "rank_1d >= 0.50"},
    {"name": "timing_base_1d_skip70_top5_dyn", "w10": 0.45, "w5": 0.45, "w3": 0.10, "w1": 0.00, "gamma": 1.4, "topn": 5, "cap": 0.15, "target_mode": "mild", "row_filter": "rank_1d >= 0.70"},
]
...
def _target_expr(case: dict[str, Any]) -> str:
    ...
    if case["target_mode"] == "mild":
        return base
    if case["target_mode"] == "mild_1d_scale50":
        return f"CASE WHEN rank_1d < 0.50 THEN ({base}) * 0.4 ELSE ({base}) END"
...
def _ensure_signal(case: dict[str, Any]) -> tuple[Path, str, dict[str, Any]]:
    ...
    score_expr = (
        f"{float(case['w10'])} * pow(greatest(rank_10d, 0.0), {gamma})"
        f" + {float(case['w5'])} * pow(greatest(rank_5d, 0.0), {gamma})"
        f" + {float(case['w3'])} * pow(greatest(rank_3d, 0.0), {gamma})"
        f" + {float(case['w1'])} * pow(greatest(rank_1d, 0.0), {gamma})"
    )
    row_filter = case.get("row_filter", "TRUE")
    ...
    try:
        ...
        SELECT
            ...
            'timing1d_shape' AS entry_weight_name,
    ...
    with duckdb.connect(str(SCORE_DB)) as score_conn:
        ...
        score_conn.execute(f'CREATE INDEX IF NOT EXISTS "{index_name}" ON "{table}" (trade_date, stock_code)')
...
def _run(case: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        **signal_meta,
        ...
        "intraday_risk": 0,
        "note": "research-only 1D timing probe on active formal L4; production parameters unchanged",
    }
