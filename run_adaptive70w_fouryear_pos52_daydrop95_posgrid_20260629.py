...
SOURCE_DIR = DATA / "reports" / "strategy_agent_adaptive70w_fouryear_momentum_cooldown_focused_20260629"
OUT_DIR = DATA / "reports" / "strategy_agent_adaptive70w_fouryear_pos52_daydrop95_posgrid_20260629"
...
SIGNAL_FILE = SOURCE_DIR / "signals" / f"{CASE_KEY}.csv"
...
def _signal_for_target(target: float, base_rows: list[dict[str, Any]]) -> Path:
    signal_path = OUT_DIR / "signals" / f"{CASE_KEY}__pos{int(target * 100)}.csv"
...
def _run(target: float, signal_file: Path, tag: str, start: str, end: str, signal_rows: int) -> dict[str, Any]:
    log_file = OUT_DIR / "logs" / f"{CASE_KEY}__pos{int(target * 100)}__{tag}.log"
...
def main() -> None:
    ...
    base_rows = _load_rows(SIGNAL_FILE)
    detail: list[dict[str, Any]] = []
    summary: list[dict[str, Any]] = []
    for target in TARGETS:
        signal_file = _signal_for_target(target, base_rows)
        rows = [_run(target, signal_file, tag, start, end, signal_rows) for tag, start, end in SLICES]
        detail.extend(rows)
        ...
        _write_rows(OUT_DIR / "posgrid_detail.csv", detail)
        _write_rows(OUT_DIR / "posgrid_summary.csv", summary)
