...
REPORT_DIR = BASE_DIR / "dynamic_h2_m3_pos94_sl07dd0510_score_exit_20260624"
...
EXIT_CASES = [
    {"case_name": "pos94_dd0510_exit095_cont098", "exit_ratio": 0.95, "continue_ratio": 0.98},
    {"case_name": "pos94_dd0510_exit096_cont098", "exit_ratio": 0.96, "continue_ratio": 0.98},
    {"case_name": "pos94_dd0510_exit097_cont098", "exit_ratio": 0.97, "continue_ratio": 0.98},
    {"case_name": "pos94_dd0510_exit098_cont098", "exit_ratio": 0.98, "continue_ratio": 0.98},
    {"case_name": "pos94_dd0510_exit095_cont099", "exit_ratio": 0.95, "continue_ratio": 0.99},
    {"case_name": "pos94_dd0510_exit096_cont099", "exit_ratio": 0.96, "continue_ratio": 0.99},
    {"case_name": "pos94_dd0510_exit097_cont099", "exit_ratio": 0.97, "continue_ratio": 0.99},
    {"case_name": "pos94_dd0510_exit098_cont099", "exit_ratio": 0.98, "continue_ratio": 0.99},
]
        ...
        out.append(
            {
                ...
                "continue_ratio": first["continue_ratio"],
...
def _write_report(summary: list[dict], audit: dict) -> None:
    ranked = sorted(
        summary,
        key=lambda row: (_f(row["full_max_drawdown"]) <= 0.40, _f(row["full_sharpe"]), _f(row["full_annual"])),
        reverse=True,
    )
    lines = [
        "# pos94 sl07/dd0510 分数退出邻域验证",
        "",
        "## 当前结论",
        "",
        "本轮固定 formal L4 融合信号、Top1、pos94、7%止损、7%止盈和 dd0510 账户回撤缩放，只调整分数退出参数。收益指标只作弱准入排序，最大回撤、近期开仓和硬过滤为强准入判断。",
        "",
        "| 候选 | exit ratio | continue ratio | 年化 | Sharpe | 最大回撤 | recent60 年化 | 近期开仓最差年化 |",
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for row in ranked:
        lines.append(
            f"| {row['case_name']} | {_f(row['exit_ratio']):.2f} | {_f(row['continue_ratio']):.2f} | "
    ...
    (REPORT_DIR / "score_exit_report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    _write_rows(REPORT_DIR / "summary_by_sharpe.csv", sorted(summary, key=lambda row: _f(row["full_sharpe"]), reverse=True))
