...
SOURCE_DIR = REPORT_DIR / "signals" / "core_structural_decay_refine"
SIGNAL_DIR = REPORT_DIR / "signals" / "structural_decay_hit_fragility"
LOG_DIR = REPORT_DIR / "logs" / "structural_decay_hit_fragility"
OUT_CSV = REPORT_DIR / "structural_decay_hit_fragility_juejin_results_20260714.csv"
OUT_JSON = REPORT_DIR / "structural_decay_hit_fragility_juejin_results_20260714.json"
PROXY_CSV = REPORT_DIR / "structural_decay_hit_proxy_contribution_20260714.csv"
REPORT_MD = REPORT_DIR / "结构衰减修正候选准入脆弱性复核_20260714.md"
...
SOURCES = {
    "x125_drop_bigmv_lowturn": SOURCE_DIR / "x125_drop_bigmv_lowturn.csv",
    "x115_drop_bigmv_lowturn": SOURCE_DIR / "x115_drop_bigmv_lowturn.csv",
}
...
def cases_for_source(proxy: pd.DataFrame, source_case: str) -> list[dict]:
    ...
    return [
        ...
        {"case": f"{source_case}_remove_2026", "source_case": source_case, "remove_months": [], "remove_years": ["2026"], "remove_stocks": []},
    ]
...
def run_or_parse(row: dict) -> dict:
    ...
    if log.exists() and log.stat().st_size > 0:
        ...
        if isinstance(ind, dict) and ind.get("pnl_ratio_annual") is not None:
            ...
            return out
    run_row = dict(row)
    run_row["max_positions"] = 1
    run_row["holding_days"] = 1
    run_row["max_holding_days"] = 3
    return base_mod.run_juejin(run_row)
...
def write_report(frame: pd.DataFrame) -> None:
    lines = [
        ...
        "## 结论",
        "",
        "- 本报告复核 `drop_bigmv_lowturn` 两个表面最强候选。",
        "- 若去 2024、去最强月份、去强贡献股票后仍保持目标，才可能进入下一步准入。",
        "",
        "## 掘金复核结果",
        "",
...
def main() -> None:
    ...
    for row in manifests:
        ...
        frame.sort_values(["source_case", "case"]).to_csv(OUT_CSV, index=False, encoding="utf-8-sig")
    ...
    (REPORT_DIR / "structural_decay_hit_fragility_summary_20260714.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2, default=str), encoding="utf-8")
