...
SIGNAL_DIR = REPORT_DIR / "signals" / "deepdrop_midbucket_boost"
LOG_DIR = REPORT_DIR / "logs" / "deepdrop_midbucket_boost"
OUT_CSV = REPORT_DIR / "deepdrop_midbucket_boost_juejin_results_20260714.csv"
OUT_JSON = REPORT_DIR / "deepdrop_midbucket_boost_juejin_results_20260714.json"
...
CASES = []
for deep_scale, global_scale in [(0.86, 1.075), (0.86, 1.070), (0.87, 1.070)]:
    for q1_boost in [1.01, 1.015, 1.02, 1.025, 1.03]:
        CASES.append(
            {
                "case": f"dmb_x{int(deep_scale*100):02d}_g{int(global_scale*1000):04d}_q1b{int(q1_boost*1000):04d}",
