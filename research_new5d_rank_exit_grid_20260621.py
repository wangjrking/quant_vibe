...
for signal_name, signal_config in SIGNALS.items():
    thresholds = (0.10, 0.20, 0.30, 0.40) if signal_config["max_positions"] == 6 else (0.10, 0.20, 0.30)
    for score_exit_rank in thresholds:
        for min_hold in (1, 2):
            GRID.append(
                {
                    "name": f"{signal_name}_rankexit{str(score_exit_rank).replace('.', 'p')}_min{min_hold}",
