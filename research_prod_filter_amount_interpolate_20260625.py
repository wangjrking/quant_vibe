...
REPORT_DIR = DATA / "reports" / "strategy_agent_prod_filter_amount_interpolate_20260625"
...
FILTERS = [
    {"name": "amt8p5w_mv20w", "amount_min": 85000, "total_mv_min": 200000, "total_mv_max": None, "turnover_min": None, "require_atr": False},
    {"name": "amt9w_mv20w", "amount_min": 90000, "total_mv_min": 200000, "total_mv_max": None, "turnover_min": None, "require_atr": False},
    {"name": "amt9p5w_mv20w", "amount_min": 95000, "total_mv_min": 200000, "total_mv_max": None, "turnover_min": None, "require_atr": False},
    {"name": "amt10w_mv20w", "amount_min": 100000, "total_mv_min": 200000, "total_mv_max": None, "turnover_min": None, "require_atr": False},
]
...
def _load_base() -> pd.DataFrame:
    ...
    for column in ["rank_1d", "rank_3d", "rank_5d", "rank_10d", "amount", "turnover_rate", "total_mv"]:
...
def _build_signal(base: pd.DataFrame, market: dict[str, dict[str, dict]], next_date: dict[str, str], cfg: dict) -> tuple[Path, dict]:
        ...
        rows.append(
            {
                ...
                "buy_date": buy_date,
                "symbol": liq.top1.to_gm_symbol(str(chosen["stock_code"])),
...
def _run(cfg: dict, signal_file: Path, slice_name: str, start: str, end: str) -> dict:
...
def main() -> None:
    ...
    summary = []
    for cfg in FILTERS:
        signal_file, stats = _build_signal(base, market, next_date, cfg)
        case_rows = []
        for slice_name, start, end in SLICES:
            row = _run(cfg, signal_file, slice_name, start, end)
            detail.append(row)
