...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_strong_trend_micro_v154_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v154_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4强趋势仓位微邻域研究")
    ...
    for strong_min, strong_pct in product(protocol["grid"]["strong_return_min"], protocol["grid"]["strong_target_pct"]):
        ...
        daily = v153.run_case(observation, score_obs, order_obs, definition, protocol, protocol["observation_end"])
