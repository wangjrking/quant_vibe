...
OUT_DIR = DATA_DIR / "reports" / "model_agent_3d_proxy10d5d_blend_20260623"
...
RESEARCH_TABLE = "stock_predict_data_model_agent_3d_proxy10d5d_blend_20260623_executable_3d_open_return_research"
...
def main() -> int:
    ...
    payload = {
        ...
        "rule": {
            ...
            "w10": W10,
            "w5": W5,
            "formula": "score = 0.60 * rank10_proxy + 0.40 * rank5_best",
        },
    ...
    manifest = {
        ...
        "notes": "3D 研究候选采用跨周期 rank blend：使用 10D 跨周期代理表与当前最优 5D 研究表做 0.60/0.40 线性 rank 组合。仅用于 research 验证，不进入 formal L4/L5。",
    }
    ...
    report_lines = [
        "# 3D 跨周期 10D/5D rank blend 研究候选",
        "",
        "## 当前结论",
        "",
        f"- 基线 10D 代理表：`{BASE10_TABLE}`",
        f"- 辅助 5D 表：`{BASE5_TABLE}`",
