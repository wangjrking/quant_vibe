...
base.REPORT_DIR = (
    ...
    / "reports"
    / "strategy_agent_goal_high_annual_20260623"
    / "current_formal_flat_liq_mv_refine"
)
base.FUSION_DB = base.REPORT_DIR / "fusion_flat_liq_mv_refine.db"
base.SCORE_DB = base.REPORT_DIR / "scores_flat_liq_mv_refine.db"


def _cfg(
    name: str,
    *,
    mv_cap: float | None = None,
...
base.VARIANTS = [
    _cfg("base_no_filter"),
    _cfg("mv_cap_200k", mv_cap=200000.0),
    _cfg("mv_cap_150k", mv_cap=150000.0),
    _cfg("mv_cap_120k", mv_cap=120000.0),
    _cfg("amount_floor_5000", amount_floor=5000.0),
    _cfg("amount_floor_8000", amount_floor=8000.0),
    _cfg("turnover_floor_1p0", turnover_floor=1.0),
    _cfg("mv150k_amt5000", mv_cap=150000.0, amount_floor=5000.0),
    _cfg("mv120k_amt5000", mv_cap=120000.0, amount_floor=5000.0),
    _cfg("mv150k_turn1", mv_cap=150000.0, turnover_floor=1.0),
]
...
def _build_signals(cfg: dict) -> list[dict]:
    ...
    for row in rows:
        item = dict(row)
        mv = base._to_float(item.get("total_mv"))
        amount = base._to_float(item.get("amount"))
        turnover = base._to_float(item.get("turnover_rate"))
        if cfg.get("mv_cap") is not None and (mv is None or mv > float(cfg["mv_cap"])):
            continue
        if cfg.get("amount_floor") is not None and (amount is None or amount < float(cfg["amount_floor"])):
            continue
        if cfg.get("turnover_floor") is not None and (turnover is None or turnover < float(cfg["turnover_floor"])):
            continue
