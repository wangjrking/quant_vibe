...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_10d_recent_enhance_v4_research_20260625"
TARGET_TABLE = "stock_predict_data_model_agent_10d_recent_enhance_v4_20260625_executable_10d_open_return_research"
...
SOURCES = {
    ...
    "base10": "stock_predict_data_model_agent_10d_cross_horizon_guard_20260625_executable_10d_open_return_research",
    ...
    "stable5v3": "stock_predict_data_model_agent_5d_stability_guard_v3_20260625_executable_5d_open_return_research",
}
...
def load_scores() -> pd.DataFrame:
    with sqlite3.connect(MODEL_DB) as conn:
        out = read_source(conn, "formal10", SOURCES["formal10"])
    ...
    for alias in SOURCES:
        ...
        out[score_col] = out[score_col].fillna(out["formal10_score"])
...
def build_features(scores: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for trade_date, group in scores.groupby("trade_date", sort=True):
        base10 = top_set(group, "base10_rank", 10)
        base20 = top_set(group, "base10_rank", 20)
        base50 = top_set(group, "base10_rank", 50)
        row: dict[str, float | str] = {"trade_date": trade_date}
        for alias in SOURCES:
            score_col = f"{alias}_score"
            rank_col = f"{alias}_rank"
            ...
            row[f"{alias}_top50_score_mean"] = float(group.nlargest(min(50, len(group)), rank_col)[score_col].mean())
            if alias != "base10":
                row[f"corr_base10_{alias}"] = float(group["base10_rank"].corr(group[rank_col]))
                row[f"overlap10_base10_{alias}"] = len(base10 & top_set(group, rank_col, 10))
                row[f"overlap20_base10_{alias}"] = len(base20 & top_set(group, rank_col, 20))
                row[f"overlap50_base10_{alias}"] = len(base50 & top_set(group, rank_col, 50))
        rows.append(row)
    return pd.DataFrame(rows)
...
def scan(scores: pd.DataFrame, labels: pd.DataFrame, features: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    metric_by_source = pd.concat([source_daily_metrics(eval_frame, alias) for alias in SOURCES], ignore_index=True)
    dates = sorted(metric_by_source["trade_date"].unique().tolist())
    base = metric_by_source[metric_by_source["source"] == "base10"].set_index("trade_date")
    feature_idx = features.set_index("trade_date")
    rows = []
    daily_rows = []
    alt_sources = ["stable5v3", "cross5v2", "cross3v3", "stable1", "dyn10", "formal10", "formal5", "formal3"]
    for alt in alt_sources:
        alt_daily = metric_by_source[metric_by_source["source"] == alt].set_index("trade_date")
        candidate_features = [c for c in feature_idx.columns if c.startswith(f"{alt}_") or c.endswith(f"_{alt}") or c.startswith("base10_")]
        for feature in candidate_features:
            vals = feature_idx[feature].replace([np.inf, -np.inf], np.nan).dropna()
            if vals.nunique() < 3:
                continue
            ...
            for threshold in thresholds:
                for op in ["<=", ">="]:
                    ...
                    for prefix, cur, base_sum in [("full", full, base_full), ("recent63", r63, b63), ("recent20", r20, b20)]:
                        for metric in ["rank_ic", "top1", "top3", "top5", "top10", "top20", "top50"]:
                            row[f"{prefix}_{metric}_delta_vs_base10"] = cur[metric] - base_sum[metric]
                    row.update(period_stats(merged))
                    row["pass_basic"] = bool(
                        row["recent63_active_days"] >= 5
                        and row["recent63_top1_delta_vs_base10"] > 0
                        and row["recent63_top5_delta_vs_base10"] > 0
                        and row["recent63_top10_delta_vs_base10"] > 0
                        and row["recent20_top5_delta_vs_base10"] >= 0
                        and row["full_top5_delta_vs_base10"] >= 0
                        and row["min_period_top5_delta"] >= -0.00025
                        and row["positive_top5_periods"] >= 2
                    )
                    row["objective"] = (
                        1.2 * row["recent63_top1_delta_vs_base10"]
                        + 2.0 * row["recent63_top5_delta_vs_base10"]
                        + row["recent20_top5_delta_vs_base10"]
                        + 0.8 * row["full_top5_delta_vs_base10"]
...
def write_candidate(scores: pd.DataFrame, features: pd.DataFrame, best: pd.Series) -> dict:
    ...
    active = active <= threshold if op == "<=" else active >= threshold
    active_dates = set(active[active].index.astype(str))
    out = scores[["trade_date", "stock_code", "base10_score", "base10_rank", f"{alt}_score", f"{alt}_rank"]].copy()
    out["guard_active"] = out["trade_date"].isin(active_dates)
    out["pred_prob"] = np.where(out["guard_active"], out[f"{alt}_rank"], out["base10_rank"])
    out["score_formula"] = f"if {feature} {op} {threshold:.12g} then {alt}_rank else base10_rank"
    out = out.rename(columns={"base10_score": "base10", f"{alt}_score": alt})
    keep = ["trade_date", "stock_code", "pred_prob", "base10", alt, "base10_rank", f"{alt}_rank", "guard_active", "score_formula"]
    with sqlite3.connect(MODEL_DB, timeout=300) as conn:
        conn.execute("PRAGMA busy_timeout=300000")
        conn.execute(f"drop table if exists {quote(TARGET_TABLE)}")
        out[keep].to_sql(TARGET_TABLE, conn, if_exists="replace", index=False)
...
def write_outputs(scan_result: pd.DataFrame, daily: pd.DataFrame, source_metrics: pd.DataFrame, stats: dict | None) -> None:
    ...
    manifest = {
        ...
        "generated_at": now_iso(),
        "decision": "candidate" if stats else "rejected",
        "baseline_for_scan": "research_10d_cross_horizon_guard",
    ...
    if stats:
        lines.extend(
            [
                "本轮找到一个相对 10D 当前研究基准改善近期 Top 指标的 research-only 候选；它不是稳定化发布候选，仍需后续评价和审批。",
                "",
                "候选规则：",
                "",
                "```text",
                ...
                f"- 近 63 日 Top10 增量：`{best['recent63_top10_delta_vs_base10']:.6f}`",
        ...
        lines.append("本轮未找到相对 10D 当前研究基准通过近期增强门槛的候选。")
    lines.extend(
        [
            "",
            "## 证据路径",
