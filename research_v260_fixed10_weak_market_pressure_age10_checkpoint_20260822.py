...
import research_v260_fixed10_current_weak_pressure_age_boundary_20260822 as boundary
...
def main() -> None:
    ...
    if current["validation_2026_opened"] or original["validation_2026_opened"]:
        raise PermissionError("2026 data entered weak-market age10 checkpoint")
    policy = copy.deepcopy(current["selected_policy"])
    policy["score_sell_pressure_extra_min_age_rule"] = "4_strong_market_10_weak_market"
    context = harness.load_context(policy)
    strong = regime.strong_market_mask(context.score, context.protocol)
    schedule = boundary.pressure_age_schedule(strong, 10)
    ...
    if not all(deterministic.values()):
        raise RuntimeError("weak-market age10 replay failed")
    ...
    gates = {
        ...
        "all_cost_levels_not_worse": all(
            cost_results[key]["cumulative_return"] >= prior_costs[key]["cumulative_return"]
    ...
    if not all(gates.values()):
        raise RuntimeError("weak-market age10 candidate did not pass robustness gates")

    result = {
        "status": "weak_market_pressure_age10_accepted_pre2026_validation_not_opened",
        "source_strategy": context.rules["strategy_id"],
        "development_boundary": [research_base.FIRST_BUY, round1.DEVELOPMENT_END],
        "selected_policy": policy,
        "strong_market_days": int(strong.sum()),
    ...
    checkpoint = {
        ...
        "source_strategy": context.rules["strategy_id"],
        "selected_candidate": "weak_market_pressure_extra_sell_min_age10",
        "selected_policy": policy,
