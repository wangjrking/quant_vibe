...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_four_year_3d5d10d_xgb_residual_20260714"
MODEL_DIR = DATA_DIR / "experimental_assets" / "model-agent" / "models" / "xgb_residual_3d5d10d_20260714"
...
def _baseline_daily(base: pd.DataFrame, label_key: str) -> pd.DataFrame:
    ...
    return rb._daily_eval(frame, pred_col, label_col)


def _candidate_daily(base: pd.DataFrame, label_key: str, model_pred: np.ndarray, beta: float) -> pd.DataFrame:
    label_col = f"label_{label_key}"
    pred_col = f"pred_{label_key}_rank"
    frame = base[base[label_col].notna()].copy()
    frame["_model_pred"] = model_pred
    frame["_model_rank"] = frame.groupby("trade_date")["_model_pred"].rank(method="average", pct=True)
    frame["candidate_score"] = frame[pred_col] + beta * (frame["_model_rank"] - 0.5)
...
def _train_sample(frame: pd.DataFrame, label_key: str) -> pd.DataFrame:
    label_col = f"label_{label_key}"
    pred_col = f"pred_{label_key}_rank"
    train = frame[(frame["trade_date"] <= TRAIN_END) & frame[label_col].notna()].copy()
    top_mask = train[pred_col] >= 0.85
    sample_mask = top_mask | (train.groupby("trade_date").cumcount() % 20 == 0)
    sample = train[sample_mask].copy()
    return sample
...
def _train_model(frame: pd.DataFrame, label_key: str) -> tuple[xgb.XGBRegressor, dict[str, Any]]:
    ...
    info = {
        ...
        "train_min_trade_date": str(sample["trade_date"].min()) if len(sample) else None,
        "train_max_trade_date": str(sample["trade_date"].max()) if len(sample) else None,
...
def _train_objective(candidate: dict[str, Any], baseline: dict[str, Any]) -> float:
    ...
    return float(
        40.0 * (train["top5"] or 0.0)
        + 8.0 * (train["top1"] or 0.0)
        + 6.0 * (train["rank_ic"] or 0.0)
...
def _evaluate_label(base: pd.DataFrame, label_key: str) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    model_path = MODEL_DIR / f"xgb_residual_{label_key}_research_20260714.json"
    ...
    for beta in BETAS:
        candidate = _metrics(_candidate_daily(base, label_key, model_pred, beta))
        full = _delta_block(candidate, baseline, "full")
        train = _delta_block(candidate, baseline, "train")
        holdout = _delta_block(candidate, baseline, "holdout")
        recent63 = _delta_block(candidate, baseline, "recent63")
        ...
        objective = _train_objective(candidate, baseline)
        row = {
            "label_key": label_key,
            "beta": beta,
        ...
        if selected_key is None or key > selected_key:
            ...
            selected = {
                ...
                "selection_rule": "XGB residual model is trained on 20220606-20251231; beta is selected by train-window objective only.",
                "selected_beta": beta,
                "train_objective": objective,
...
def _write_markdown(payload: dict[str, Any], path: Path) -> None:
    lines = [
        "# 3D/5D/10D XGBoost Residual 研究（20260714）",
        "",
        "## 结论",
        "",
        "本轮训练 research-only XGBoost residual learner。模型只在 `20220606-20251231` 训练，beta 也只用训练窗口选择；2026 holdout 和 Recent20/63 只做验证。",
        "",
        "| 标签 | beta | Full Top5 Δ | Holdout Top5 Δ | Recent63 Top5 Δ | Recent20 Top5 Δ | 是否过门 |",
        "|---|---:|---:|---:|---:|---:|---:|",
    ]
    for key in LABEL_KEYS:
        item = payload["summaries"][key]
        lines.append(
            "| {key} | {beta:.2f} | {full:.6f} | {holdout:.6f} | {r63:.6f} | {r20:.6f} | {passed} |".format(
                key=key,
...
def _update_status_and_frontier(report_json: Path, report_md: Path, payload: dict[str, Any]) -> None:
    ...
    decision = "xgb_residual_passed_for_some_labels_candidate_discussion_required" if passed else "no_3d_5d_10d_candidate_passed_xgb_residual_scan"
    if status_path.exists():
        status = json.loads(status_path.read_text(encoding="utf-8"))
        status.setdefault("not_promoted_research_lines", {})["xgb_residual_3d5d10d"] = {
            "decision": decision,
            "summary": str(report_json),
            "review_md": str(report_md),
            "passed_labels": sorted(passed.keys()),
            "reason": "XGBoost residual learner trained on train window and validated on holdout/recent.",
        }
        if passed:
    ...
    if frontier_path.exists():
        ...
        frontier["latest_xgb_residual_scan"] = {
            "summary": str(report_json),
            "review_md": str(report_md),
            "decision": decision,
            "passed_labels": sorted(passed.keys()),
...
def main() -> int:
    ...
    scan_csv = REPORT_DIR / "xgb_residual_scan.csv"
    pd.DataFrame(scan_rows).to_csv(scan_csv, index=False, encoding="utf-8-sig")
    payload = {
        "generated_at": pd.Timestamp.now(tz="Asia/Shanghai").isoformat(),
        "actor": "model-agent",
        "scope": "research_only_3d5d10d_xgb_residual",
    ...
    report_json = REPORT_DIR / "xgb_residual_report.json"
    report_md = REPORT_DIR / "xgb_residual_report.md"
