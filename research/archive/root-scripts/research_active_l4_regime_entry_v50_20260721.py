...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_regime_entry_v50_20260721"
...
def market_on_state(market_return, lookback, threshold):
    values = market.market_exposure(market_return, int(lookback), float(threshold), 0.0)
    return values >= 0.999
...
def main():
    ...
    try:
        rows, grid = [], p["market_state_grid"]
        contexts = {}
        for lookback in grid["lookback"]:
            for threshold in grid["mean_return_threshold"]:
                state = market_on_state(market_return, lookback, threshold)
                for floor in grid["risk_off_floor"]:
                    exposure = np.where(state, 1.0, float(floor)).astype(np.float32)
                    context_id = f"l{lookback}_t{int(round(threshold*10000))}_f{int(round(floor*100))}"
                    contexts[context_id] = (state, exposure)
                    for off_entry in grid["risk_off_entry_rank_min"]:
                        for on_entry in grid["risk_on_entry_rank_min"]:
                            ...
                            bp = v39.BuyProfile(2, -1.0, fixed["amount_min"], fixed["mv_min"], fixed["normal_total_pct"], False, 0.99, 0.005, 1.0, fixed["weight_scheme"])
                            ...
                            cid = f"{context_id}_oe{int(off_entry*1000)}_ne{int(on_entry*1000)}"
                            rows.append({"case_id": cid, "context_id": context_id, "lookback": lookback, "market_threshold": threshold, "risk_off_floor": floor, "risk_off_entry_rank_min": off_entry, "risk_on_entry_rank_min": on_entry, **metrics})
        ...
        by_return = pool.sort_values(["full_cumulative_return", "full_sharpe", "case_id"], ascending=[False, False, True]).head(int(p["promotion"]["return"]))
        by_robust = robust_sort(pool).head(int(p["promotion"]["robust"]))
        ...
        for _, item in frozen.iterrows():
            state, exposure = contexts[str(item.context_id)]
    ...
    (OUT / "市场状态入场阈值结论.md").write_text("\n".join(["# 市场状态入场阈值结论", "", "本轮使用全历史统一市场状态规则动态调整入场分数，不使用年份或日期过滤。", "", f"固定网格 {len(results)} 组，逐年为正 {int(results.all_year_positive.sum())} 组，强过滤后 {len(pool)} 组。", f"冻结 {len(action_rows)} 条独立掘金路径。", "", "2026验证期未打开，生产资产未修改。"]), encoding="utf-8")
