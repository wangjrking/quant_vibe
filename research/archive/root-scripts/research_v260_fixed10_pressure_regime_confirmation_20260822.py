...
OUTPUT_ROOT = (
    ...
    "strategy_agent_v260_fixed10_pressure_regime_confirmation_20260822"
)
CHECKPOINT_PATH = (
    REPO
    / "quant/data_file/reports/"
...
CANDIDATES = (
    "confirmation1_all_markets",
    "confirmation1_strong_confirmation2_weak",
)


def pressure_confirmation_schedule(strong_market_mask) -> np.ndarray:
    strong = np.asarray(strong_market_mask, dtype=np.bool_)
    return np.where(strong, 1, 2).astype(np.int16)


def run_policy(context, policy: dict, confirmation_override):
    ...
    common = dict(
        ...
        score_sell_pressure_confirmation_days_override=confirmation_override,
    )
...
def simplified_gates(candidate: dict, baseline: dict) -> dict[str, bool]:
    return {
        "cumulative_return_improved": candidate["metrics_0_30pct"][
            "cumulative_return"
        ]
        > baseline["metrics_0_30pct"]["cumulative_return"],
        "cagr_improved": candidate["metrics_0_30pct"]["cagr"]
        > baseline["metrics_0_30pct"]["cagr"],
        "sharpe_improved": candidate["metrics_0_30pct"]["sharpe"]
...
def main() -> None:
    ...
    if checkpoint["validation_2026_opened"]:
        raise PermissionError("2026 validation is not available for confirmation research")
    policy = copy.deepcopy(checkpoint["selected_policy"])
    context = harness.load_context(policy)
    strong = regime.strong_market_mask(context.score, context.protocol)
    schedules = {
        "confirmation1_all_markets": 1,
        "confirmation1_strong_confirmation2_weak": (
            pressure_confirmation_schedule(strong)
        ),
    }
    results, cache = {}, {}
    ...
    if not all(checkpoint_equivalence.values()):
        raise RuntimeError("regime-confirmation baseline drifted")
    ...
    if not all(deterministic.values()):
        raise RuntimeError("regime-confirmation replay failed")

    selected_policy = copy.deepcopy(policy)
    selected_policy["score_sell_pressure_confirmation_rule"] = (
        "1_all_markets"
        if selected == baseline_id
        else "1_strong_market_2_weak_market"
    )
    result = {
        "status": "pressure_regime_confirmation_complete_2026_not_opened",
        ...
        "only_change": (
            "weak-market pressure requires two consecutive qualifying days; "
            "strong-market pressure remains immediate"
        ),
        "results": results,
