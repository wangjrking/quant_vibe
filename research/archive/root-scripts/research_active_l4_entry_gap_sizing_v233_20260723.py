...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_entry_gap_sizing_v233_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v233_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def target_multiplier(
    ...
    threshold = float(definition["entry_gap_threshold"])
    multiplier = float(definition["low_gap_target_multiplier"])
    result = np.ones(score.shape, dtype=np.float32)
    if threshold <= 0 or multiplier >= 1:
        return result

    selection = v174.selection_mask(arrays, definition["max_rank_deterioration"])
    for t in range(len(arrays["dates"])):
        clean = arrays["signal_clean"][t] & np.isfinite(score[t]) & selection[t]
        ...
        gap = float(score[t, ranked[0]] - score[t, ranked[1]]) if len(ranked) >= 2 else -np.inf
        if gap < threshold:
            result[t, :] = multiplier
    return result
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 入选分差温和仓位缩放研究")
