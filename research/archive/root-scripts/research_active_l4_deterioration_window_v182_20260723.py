...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_deterioration_window_v182_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v182_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def selection_mask(arrays: dict, definition: dict) -> np.ndarray:
    raw = arrays["rank_10d"].astype(np.float32)
    smooth = v94.rolling_nanmean(raw, int(definition["deterioration_window"]))
    return np.isfinite(raw) & np.isfinite(smooth) & ((raw - smooth) >= -float(definition["max_rank_deterioration"]))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 deterioration-window study")
    ...
    rows, definitions = [], {}
    for window, threshold in product(protocol["grid"]["deterioration_window"], protocol["grid"]["max_rank_deterioration"]):
        definition = {**protocol["fixed_definition"], "deterioration_window": int(window), "max_rank_deterioration": float(threshold)}
