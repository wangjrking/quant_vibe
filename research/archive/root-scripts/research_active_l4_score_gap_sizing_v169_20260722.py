...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_score_gap_sizing_v169_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v169_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def gap_multiplier(score: np.ndarray, order: np.ndarray, definition: dict) -> np.ndarray:
    threshold = float(definition["score_gap_threshold"])
    high_multiplier = float(definition["high_confidence_multiplier"])
    result = np.ones(score.shape, dtype=np.float32)
    if high_multiplier == 1.0:
        return result
    for t in range(score.shape[0]):
        ranked = order[t]
        values = score[t, ranked]
        ...
        result[t, ranked[:-1][gaps >= threshold]] = high_multiplier
    return result
...
def run_case(arrays, score, order, definition, protocol, end, start=None):
    return v162.run_case(
        ...
        start,
        candidate_target_multiplier_override=gap_multiplier(score, order, definition),
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4分数领先幅度动态仓位研究")
