...
import research_v260_fixed10_daily_exit_after_renewal_gate_20260822 as current_round
...
OUTPUT_ROOT = (
    REPO
    / "quant/data_file/reports/strategy_agent_v260_fixed10_weak_market_exit_capacity_20260822"
)
WEAK_MARKET_EXIT_LIMITS = (1, 2, 3)
STRONG_MARKET_EXIT_LIMIT = 1
TARGET_CRASH_WINDOW = ("20231205", "20240208")


def candidate_policy(weak_exit_limit: int) -> dict:
    current = current_round.candidate_policy(1)
    current["weak_market_max_daily_score_sells"] = int(weak_exit_limit)
    current["strong_market_max_daily_score_sells"] = STRONG_MARKET_EXIT_LIMIT
    current["market_state_source"] = "production_breadth_state"
    return current


def exit_limit_schedule(score: np.ndarray, protocol: dict, weak_exit_limit: int) -> np.ndarray:
...
def main() -> None:
    ...
    if access["logical_max_date"] != round1.DEVELOPMENT_END:
        raise PermissionError("2026 data entered weak-market exit-capacity development")
    definition = harness.production_definition(protocol)
    score, order = harness.v95.score_pair(arrays, 0.0, 7, 0.1)

    results = {}
    run_cache = {}
    for daily_limit in WEAK_MARKET_EXIT_LIMITS:
        candidate_id = f"weak_market_daily_score_exit_limit_{daily_limit}"
        current = candidate_policy(daily_limit)
        limit_schedule = exit_limit_schedule(score, protocol, daily_limit)
        daily, actions = round1.run_fixed10(
            ...
            max_daily_score_sells_override=limit_schedule,
        )
    ...
    selected_limit = int(
        results[selected_id]["policy"]["weak_market_max_daily_score_sells"]
    )
    repeat_daily, repeat_actions = round1.run_fixed10(
        ...
        record_actions=True,
        simulator=renewal_quality_v109.simulate,
        max_daily_score_sells_override=exit_limit_schedule(
            score, protocol, selected_limit
        ),
    )
    deterministic = {
    ...
    if not all(deterministic.values()):
        raise RuntimeError("weak-market exit-capacity replay failed")
    ...
    result = {
        ...
        "candidate_budget": [int(value) for value in WEAK_MARKET_EXIT_LIMITS],
        "selection_rule": (
            "all 2022-2024 calendar returns positive, then training Sharpe, CAGR, "
            "drawdown and turnover; 2025 is confirmation only"
        ),
        "results": results,
        "walk_forward_selection_evidence": round2.walk_forward_selections(results),
