...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_10d_staggered_v83_20260722"
PROTOCOL = OUT / "preregistered_protocol.json"
FROZEN = OUT / "frozen_candidates_before_2026.json"


def parse_args():
    parser = argparse.ArgumentParser(description="10D低频错峰持有研究")
...
def stable_id(payload):
    ...
    return "v83_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def staggered_arrays(arrays, every):
    result = {key: value.copy() if key == "signal_clean" else value for key, value in arrays.items()}
    mask = np.zeros(len(arrays["dates"]), dtype=np.bool_)
    mask[:: int(every)] = True
    result["signal_clean"] &= mask[:, None]
    return result


def engine_params(definition, fixed):
    cohorts = int(math.ceil(float(definition["hold_days"]) / float(definition["rebalance_every"])))
    internal_gross = float(definition["target_gross_exposure"]) * float(definition["hold_days"]) / float(cohorts)
    return {
        "daily_top_n": int(definition["top_n_per_rebalance"]),
...
def main():
    ...
    if not args.open_2026:
        ...
        for blend, top_n, hold_days, every, gross in product(protocol["blends"], protocol["grid"]["top_n_per_rebalance"], protocol["grid"]["hold_days"], protocol["grid"]["rebalance_every"], protocol["grid"]["target_gross_exposure"]):
            ...
            arrays = staggered_arrays(base, every)
            score, order = ranks[blend["id"]]
            params = engine_params(definition, protocol["fixed"])
        ...
        for _, item in selected.iterrows():
            ...
            arrays = staggered_arrays(base, definition["rebalance_every"])
    ...
    for candidate in frozen["candidates"]:
        ...
        arrays = staggered_arrays(full_arrays, definition["rebalance_every"])
        score, order = ranks[definition["blend"]["id"]]
        params = engine_params(definition, protocol["fixed"])
