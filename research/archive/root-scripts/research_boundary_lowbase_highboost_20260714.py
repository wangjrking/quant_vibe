...
SIGNAL_DIR = REPORT_DIR / "signals" / "boundary_lowbase_highboost"
LOG_DIR = REPORT_DIR / "logs" / "boundary_lowbase_highboost"
OUT_CSV = REPORT_DIR / "boundary_lowbase_highboost_juejin_results_20260714.csv"
OUT_JSON = REPORT_DIR / "boundary_lowbase_highboost_juejin_results_20260714.json"
...
CASES = []
for overall in [0.925, 0.930, 0.935]:
    for high in [1.02, 1.04, 1.06, 1.08]:
        CASES.append({"overall": overall, "low": 1.0, "mid": 1.0, "high": high, "cap": 1.0})
...
def write_variant(params: dict) -> dict:
    ...
    target = pd.to_numeric(df["target_pct"], errors="coerce").fillna(0.0) * params["overall"] * mult
    ...
    case = f"blhb_o{int(params['overall']*1000):04d}_h{int(params['high']*1000):04d}"
