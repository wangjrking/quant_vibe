...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_event_rebalance_v129_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v129_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def case_protocol(protocol: dict, definition: dict) -> dict:
    result = copy.deepcopy(protocol)
    result["fixed_strategy"] = {
        "top_n_per_rebalance": int(definition["top_n_per_rebalance"]),
        "max_hold_days": int(definition["max_hold_days"]),
        "rebalance_every": int(definition["rebalance_interval"]),
    }
    return result


def event_active(score: np.ndarray, definition: dict) -> np.ndarray:
    counts = np.sum(np.isfinite(score) & (score >= float(definition["breadth_score_threshold"])), axis=1)
    high = counts >= int(definition["breadth_count_threshold"])
    interval = int(definition["rebalance_interval"])
    active = np.zeros(len(high), dtype=np.bool_)
    last = -10_000
    for t, is_high in enumerate(high):
        transition = bool(is_high and (t == 0 or not high[t - 1]))
        if is_high and (transition or t - last >= interval):
            active[t] = True
            last = t
        if not is_high:
            last = -10_000
...
def run_case(arrays, score, order, definition, protocol, end, start=None):
    return v109.simulate(
        ...
        start,
        rebalance_active_override=event_active(score, definition),
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4慢分数宽度转强事件调仓研究")
    ...
    rows, definitions = [], {}
    grid = protocol["grid"]
    for top_n, max_hold, interval in product(grid["top_n_per_rebalance"], grid["max_hold_days"], grid["rebalance_interval"]):
        if max_hold < interval:
            continue
        definition = {**protocol["fixed_definition"], "top_n_per_rebalance": top_n, "max_hold_days": max_hold, "rebalance_interval": interval}
