...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_score_trend_surface_v244_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v244_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 分数趋势仓位二维邻域研究")
    ...
    if protocol.get("include_baseline"):
        parameter_pairs.append((1, 0.0))

    rows, definitions = [], {}
    for lookback, sensitivity in parameter_pairs:
        definition = {
