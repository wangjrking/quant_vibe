...
import research_v260_fixed10_renewal_quality_20260822 as renewal_quality
...
OUTPUT_ROOT = (
    REPO
    / "quant/data_file/reports/strategy_agent_v260_fixed10_max_hold_after_renewal_gate_20260822"
)
MAX_HOLD_DAYS = (20, 25, 30)


def candidate_policy(max_hold_days: int) -> dict:
    current = renewal_quality.candidate_policy(0.80)
    current["max_hold_days"] = int(max_hold_days)
    return current
...
def main() -> None:
    ...
    if access["logical_max_date"] != round1.DEVELOPMENT_END:
        raise PermissionError("2026 data entered max-hold-after-renewal recheck")
    ...
    for max_hold_days in MAX_HOLD_DAYS:
        candidate_id = f"max_hold_{max_hold_days}"
        current = candidate_policy(max_hold_days)
    ...
    if not all(deterministic.values()):
        raise RuntimeError("max-hold-after-renewal deterministic replay failed")
    ...
    result = {
        ...
        "candidate_budget": [int(value) for value in MAX_HOLD_DAYS],
        "selection_rule": (
