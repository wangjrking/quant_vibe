...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_1d_timing_v46_20260721"
...
def timed_mask(base, rank1, spec):
    result = {key: value.copy() for key, value in base.items()}
    timing = np.isfinite(rank1) & (rank1 >= float(spec["min"])) & (rank1 <= float(spec["max"]))
    result["signal_clean"] &= timing
    return result


def profile(entry, total, scheme, fixed):
    return v39.BuyProfile(2, float(entry), int(fixed["amount_min"]), int(fixed["mv_min"]), float(total), False, 0.99, 0.005, 1.0, str(scheme))
...
def main():
    ...
    try:
        rows = []
        g = p["grid"]
        for timing in p["one_day_timing_specs"]:
            masked = timed_mask(base_masked, arrays["rank_1d"], timing)
            ...
            for entry in g["entry_rank_min"]:
                for total in g["normal_total_pct"]:
                    for scheme in g["weight_scheme"]:
                        bp = profile(entry, total, scheme, g["fixed"])
                        ...
                        rows.append({"case_id": f"{timing['id']}_{bp.case_id}", "timing_id": timing["id"], "rank1_min": timing["min"], "rank1_max": timing["max"], **bp.__dict__, **metrics})
        ...
        timing_specs = {item["id"]: item for item in p["one_day_timing_specs"]}
        for _, item in frozen.iterrows():
            masked = timed_mask(base_masked, arrays["rank_1d"], timing_specs[str(item.timing_id)])
            quality = health.candidate_quality(masked, buy_score, order, 3)
            model_exp = continuous.exposure_series(quality, int(e["health_lookback"]), float(e["health_floor_fraction"]), float(e["health_scale_denominator"]))
            exposure = np.minimum(model_exp, market_exp).astype(np.float32)
    ...
    (OUT / "一日评分时机研究结论.md").write_text("\n".join([
        "# 一日评分时机研究结论", "", "本轮1D评分只作入场门控，不参与3D/5D/10D主排序。", "",
        f"固定网格 {len(results)} 组，逐年为正 {int(results.all_year_positive.sum())} 组，强过滤后 {len(pool)} 组。",
        f"冻结 {len(action_rows)} 条独立掘金路径。", "", "2026验证期未打开，生产资产未修改。"
    ]), encoding="utf-8")
