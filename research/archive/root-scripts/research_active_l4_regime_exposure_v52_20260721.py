...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_regime_exposure_v52_20260721"
PROTOCOL = OUT / "preregistered_protocol.json"
CURRENT_TOP_WEIGHT = 0.80
...
def main():
    ...
    try:
                ...
                for floor in g["risk_off_floor"]:
                    ...
                    for off_entry in g["risk_off_entry_rank_min"]:
                        for on_entry in g["risk_on_entry_rank_min"]:
                            ...
                            bp = v39.BuyProfile(2, -1.0, fixed["amount_min"], fixed["mv_min"], float(total), False, 0.99, 0.005, 1.0, "dynamic")
                            ...
                            cid = f"w{int(top_weight*100)}_p{int(total*100)}_f{int(floor*100)}_oe{int(off_entry*1000)}_ne{int(on_entry*1000)}"
                            rows.append({"case_id": cid, "top1_weight": top_weight, "normal_total_pct": total, "risk_off_floor": floor, "risk_off_entry_rank_min": off_entry, "risk_on_entry_rank_min": on_entry, **metrics})
        ...
        for _, item in frozen.iterrows():
            ...
            masked = {key: value.copy() for key, value in base_masked.items()}
            masked["signal_clean"] &= buy_score >= dynamic_entry[:, None]
            bp = v39.BuyProfile(2, -1.0, fixed["amount_min"], fixed["mv_min"], float(item.normal_total_pct), False, 0.99, 0.005, 1.0, "dynamic")
    ...
    (OUT / "市场状态仓位邻域结论.md").write_text("\n".join(["# 市场状态仓位邻域结论", "", "本轮围绕V51新高连续检验Top2总仓位、弱市仓位、内部权重和动态入场阈值。", "", f"固定网格 {len(results)} 组，逐年为正 {int(results.all_year_positive.sum())} 组，强过滤后 {len(pool)} 组。", f"冻结 {len(action_rows)} 条独立掘金路径。", "", "2026验证期未打开，生产资产未修改。"]), encoding="utf-8")
