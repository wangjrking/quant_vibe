...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_market_state_v202_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v202_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 市场状态窗口与阈值研究")
    ...
    if args.open_known_2026:
        ...
        for item in frozen["candidates"]:
            for start in protocol["known_stress_buy_starts"]:
                daily = v196.run_case(
                    full, score, order, item["definition"], protocol, protocol["known_stress_end"], start
                )
                rows.append(
    ...
    rows, definitions = [], {}
    for market_lookback, market_return_min in product(
        protocol["grid"]["market_lookback"], protocol["grid"]["market_return_min"]
    ):
        definition = {
            **protocol["fixed_definition"],
            "market_lookback": int(market_lookback),
            "market_return_min": float(market_return_min),
        }
        case_id = stable_id(definition)
        definitions[case_id] = definition
        daily = v196.run_case(observation, score_obs, order_obs, definition, protocol, protocol["observation_end"])
