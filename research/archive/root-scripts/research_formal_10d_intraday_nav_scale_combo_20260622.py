...
import research_formal_10d_intraday_replace_refine_20260622 as intraday_base
...
REPORT_DIR = (
    ...
    / "formal_10d_intraday_nav_scale_combo_20260622"
)
SOURCE_SIGNAL = (
    DATA_DIR
    / "reports"
...
BASELINE_LOG = (
    ...
    / "logs"
    / "sl05_full_replace1_soft_dd.log"
)
SCORE_DB = intraday_base.SCORE_DB
SCORE_TABLE = intraday_base.SCORE_TABLE
MARKET_DB = intraday_base.MARKET_DB
STRATEGY_DIR = intraday_base.STRATEGY_DIR
JUEJIN_PYTHON = intraday_base.JUEJIN_PYTHON

INTRADAY_ENV = dict(intraday_base.BASE_ENV)
INTRADAY_ENV.update(
    {
...
def _variant(
    ...
    *,
    ...
    cap: float,
    momentum: bool = False,
    recent_loss: float = -0.035,
    recent_loss_scale: float = 0.88,
) -> dict:
    return {
        "name": name,
        "window": window,
        ...
        "recent_loss_scale": recent_loss_scale,
        "env": dict(INTRADAY_ENV),
    }


VARIANTS = [
    _variant("repro_intraday_soft", window=20, target_vol=1.0, floor=1.0, cap=1.0),
    _variant("vol10_t24_f85_c110", window=10, target_vol=0.024, floor=0.85, cap=1.10),
    _variant("vol15_t24_f85_c110", window=15, target_vol=0.024, floor=0.85, cap=1.10),
    _variant("vol20_t24_f85_c110", window=20, target_vol=0.024, floor=0.85, cap=1.10),
    _variant("vol15_t28_f90_c105", window=15, target_vol=0.028, floor=0.90, cap=1.05),
    _variant("vol20_t28_f90_c105", window=20, target_vol=0.028, floor=0.90, cap=1.05),
    _variant("vol15_momloss_f88_c108", window=15, target_vol=0.026, floor=0.88, cap=1.08, momentum=True, recent_loss=-0.04, recent_loss_scale=0.86),
]
...
def _rolling_scales(cfg: dict) -> dict[str, float]:
    if cfg["floor"] == 1.0 and cfg["cap"] == 1.0:
        return {}
    rows = _nav_series()
    ...
    for row in rows:
        ...
        if cfg["momentum"] and len(returns) >= 5 and sum(returns[-5:]) < float(cfg["recent_loss"]):
            scale *= float(cfg["recent_loss_scale"])
        scale = min(max(scale, float(cfg["floor"])), float(cfg["cap"]))
        scales[row["date"]] = scale
...
def _write_signal(cfg: dict, signal_file: Path) -> dict:
    ...
    for field in ["intraday_nav_scale", "intraday_nav_scale_rule"]:
    ...
    for row in rows:
        ...
        row["target_pct"] = f"{min(target * scale, 0.42):.5f}"
        row["intraday_nav_scale"] = f"{scale:.6f}"
        row["intraday_nav_scale_rule"] = str(cfg["name"])
    ...
    return {
        ...
        "avg_nav_scale": sum(scale_values) / len(scale_values),
        "min_nav_scale": min(scale_values),
        "max_nav_scale": max(scale_values),
    }
...
def main() -> int:
    if not BASELINE_LOG.exists():
        raise SystemExit(f"baseline log not found: {BASELINE_LOG}")
    results: list[dict] = []
