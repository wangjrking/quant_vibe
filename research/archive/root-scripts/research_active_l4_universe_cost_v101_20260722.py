...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_universe_cost_v101_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v101_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="固定成本低换手结构下的股票池研究")
    ...
    if args.open_2026:
        ...
        for item in frozen["candidates"]:
            for start in protocol["validation_buy_starts"]:
                daily = v98.evaluate(full, score, order, item["definition"], protocol, start)
    ...
    grid, fixed = protocol["universe_grid"], protocol["fixed_strategy"]
    rows, definitions = [], {}
    for listed, amount, mv, turnover in product(
        grid["listed_days_min"], grid["amount_min"], grid["mv_min"], grid["turnover_max"]
    ):
        definition = {
            **fixed,
            "blend": "fixed_10d_smooth5_c0p5",
            "listed_days_min": listed,
            "amount_min": amount,
            "mv_min": mv,
        ...
        daily = v98.evaluate(observation, score_obs, order_obs, definition, protocol)
