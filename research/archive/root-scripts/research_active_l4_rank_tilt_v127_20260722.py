...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_rank_tilt_v127_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v127_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def run_case(arrays, score, order, definition, protocol, end, start=None, record_actions=False):
    return v109.simulate(
        ...
        rank_weight_power=float(definition["rank_weight_power"]),
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4慢分数Top12排名倾斜权重研究")
    ...
    rows, definitions = [], {}
    for power in protocol["grid"]["rank_weight_power"]:
        definition = {**protocol["fixed_definition"], "rank_weight_power": power}
