...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_score_velocity_v35_20260721"
PROTOCOL = OUT / "preregistered_protocol.json"


def velocity_score(base, spec):
    lag1 = np.vstack([base[0:1], base[:-1]])
    lag2 = np.vstack([base[0:1], base[0:1], base[:-2]])
    if spec["kind"] == "current":
        value = base
    elif spec["kind"] == "weighted":
        w = spec["weights"]
        value = float(w[0]) * base + float(w[1]) * lag1 + float(w[2]) * lag2
    elif spec["kind"] == "trend":
        value = base + float(spec["beta"]) * (base - lag1)
    else:
...
def main():
    ...
    base = core.blend_scores(arrays, p["base_score"])
    ...
    contexts, rows = {}, []
    g = p["stage1"]
    for spec in p["velocity_formulas"]:
        score = velocity_score(base, spec)
        ...
        for entry in g["entry_rank_min"]:
            for top in g["strong_top_score_min"]:
                for gap in g["strong_gap_min"]:
                    ...
                    rows.append({"case_id": cid, "velocity_id": spec["id"], **asdict(profile), **metrics})
    ...
    for _, seed in seeds.iterrows():
        score, order, exposure = contexts[str(seed.velocity_id)]
        for exit_rule in p["stage2_exit_profiles"]:
            ...
            daily = v30.simulate(arrays, masked, score, order, exposure, profile, p["observation_end"])
            metrics = v30.evaluate(daily, p["year_folds"])
            cid = str(seed.velocity_id) + "_" + v30.case_id(profile)
            rows.append({"case_id": cid, "velocity_id": seed.velocity_id, "exit_profile_id": exit_rule["id"], **asdict(profile), **metrics})
    ...
    for _, item in frozen.iterrows():
        score, order, exposure = contexts[str(item.velocity_id)]
        ...
        action_rows.append({"case_id": item.case_id, "velocity_id": item.velocity_id, "path": str(path.relative_to(ROOT)).replace("\\", "/"), "sha256": v30.digest(path), "rows": len(actions), "buy_rows": int((actions.action == "BUY").sum()), "max_positions": 2})
    ...
    (OUT / "评分变化速度研究结论.md").write_text("\n".join(["# 评分变化速度研究结论", "", "本轮仅使用T日及更早正式评分，所有公式和参数运行前冻结。", "", f"第一阶段 {len(stage1)} 组，逐年为正 {int(stage1.all_year_positive.sum())} 组。", f"第二阶段 {len(stage2)} 组，逐年为正 {int(stage2.all_year_positive.sum())} 组。", f"冻结 {len(action_rows)} 条独立掘金路径。", "", "2026验证期未打开，生产资产未修改。"]), encoding="utf-8")
