...
OUTPUT_ROOT = (
    ...
    "strategy_agent_v260_fixed10_current_weak_pressure_age_boundary_20260822"
)
CHECKPOINT_PATH = (
    REPO
    / "quant/data_file/reports/"
...
def pressure_age_schedule(strong_market: np.ndarray, weak_age: int) -> np.ndarray:
    strong = np.asarray(strong_market, dtype=np.bool_)
    return np.where(strong, 4, int(weak_age)).astype(np.int64)
...
def main() -> None:
    ...
    if checkpoint["validation_2026_opened"]:
        raise PermissionError("2026 validation is unavailable for age boundaries")
    policy = copy.deepcopy(checkpoint["selected_policy"])
    context = harness.load_context(policy)
    strong = regime.strong_market_mask(context.score, context.protocol)
    results, cache = {}, {}
    for weak_age in WEAK_AGES:
        candidate_id = f"strong4_weak{weak_age}"
        schedule = pressure_age_schedule(strong, weak_age)
        results[candidate_id], daily, actions = age_guard.run_policy(
            context, policy, schedule
        )
        cache[candidate_id] = (daily, actions)

    baseline_id = "strong4_weak8"
    ...
    if not all(checkpoint_equivalence.values()):
        raise RuntimeError("weak pressure age checkpoint drifted")
    ...
    eligible = [
        candidate_id
        for candidate_id, item in gates.items()
        if all(item.values())
    ]
    selected = max(
    ...
    selected_age = int(selected.removeprefix("strong4_weak"))
    repeat, repeat_daily, repeat_actions = age_guard.run_policy(
        context, policy, pressure_age_schedule(strong, selected_age)
    )
    deterministic = {
    ...
    if not all(deterministic.values()):
        raise RuntimeError("weak pressure age boundary replay failed")
    result = {
        "status": "current_weak_pressure_age_boundary_complete_2026_not_opened",
        "candidate_budget": [f"strong4_weak{age}" for age in WEAK_AGES],
        "only_change": "weak-market minimum age for the second same-day pressure exit",
        "results": results,
        "selection_gates_against_weak8": gates,
