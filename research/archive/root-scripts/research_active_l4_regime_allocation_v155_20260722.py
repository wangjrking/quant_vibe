...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_regime_allocation_v155_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v155_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def run_case(arrays, score, order, definition, protocol, end, start=None):
    return v153.run_case(arrays, score, order, definition, case_protocol(protocol, definition), end, start)


def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4分市场状态仓位研究")
    ...
    rows, definitions = [], {}
    for normal_pct, weak_pct in product(protocol["grid"]["normal_target_pct"], protocol["grid"]["weak_target_pct"]):
        definition = {
            **protocol["fixed_definition"],
            "normal_target_pct": normal_pct,
            "weak_target_pct": weak_pct,
        }
