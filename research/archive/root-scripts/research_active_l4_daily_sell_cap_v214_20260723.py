...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_daily_sell_cap_v214_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v214_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def run_case(arrays, score, order, definition, protocol, end, start=None):
    cap = int(definition["max_daily_score_sells"])
    return v162.run_case(
        ...
        max_daily_score_sells_override=None if cap >= 999 else cap,
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 每日分数卖出数量上限研究")
    ...
    rows, definitions = [], {}
    for cap in protocol["grid"]["max_daily_score_sells"]:
        definition = {**protocol["fixed_definition"], "max_daily_score_sells": int(cap)}
