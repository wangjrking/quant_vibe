...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_entry_gap_v232_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v232_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def run_case(arrays, score, order, definition, protocol, end, start=None):
    return v162.run_case(
        ...
        min_entry_top_gap_override=float(definition["min_entry_top_gap"]),
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 Top1-Top2 分数差入场研究")
    ...
    rows, definitions = [], {}
    for gap in protocol["grid"]["min_entry_top_gap"]:
        definition = {**protocol["fixed_definition"], "min_entry_top_gap": float(gap)}
