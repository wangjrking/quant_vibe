...
import research_v260_fixed10_sell_threshold_after_renewal_gate_20260822 as final_threshold
...
OUTPUT_ROOT = (
    REPO
    / "quant/data_file/reports/strategy_agent_v260_fixed10_daily_exit_after_renewal_gate_20260822"
)
DAILY_EXIT_LIMITS = (1, 2, 3)


def candidate_policy(daily_exit_limit: int) -> dict:
    current = final_threshold.candidate_policy(0.85)
    current["max_daily_score_sells"] = int(daily_exit_limit)
    return current
...
def main() -> None:
    ...
    if access["logical_max_date"] != round1.DEVELOPMENT_END:
        raise PermissionError("2026 data entered daily-exit-after-renewal development")
    definition = harness.production_definition(protocol)
    score, order = harness.v95.score_pair(arrays, 0.0, 7, 0.1)

    results = {}
    run_cache = {}
    for daily_limit in DAILY_EXIT_LIMITS:
        candidate_id = f"daily_score_exit_limit_{daily_limit}"
    ...
    if not all(deterministic.values()):
        raise RuntimeError("daily-exit-after-renewal replay failed")
    ...
    result = {
        ...
        "candidate_budget": [int(value) for value in DAILY_EXIT_LIMITS],
        "selection_rule": (
