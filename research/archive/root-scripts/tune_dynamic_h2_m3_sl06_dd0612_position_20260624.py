...
REPORT_DIR = BASE_DIR / "dynamic_h2_m3_sl06_dd0612_position_20260624"
BASE_SIGNAL_FILE = (
    BASE_DIR
    / "dynamic_hold_20260624"
    / "signals"
    / "dynamic_h2_m3_c098_w78_5d12_3d10_pos56_e097_mh1_sl06_dd0612.csv"
)
...
TARGET_PCTS = [0.50, 0.56, 0.62, 0.68, 0.70, 0.72, 0.74, 0.75, 0.76, 0.77, 0.78, 0.80]
...
def _case_name(target_pct: float) -> str:
    return f"sl06_dd0612_pos{int(round(target_pct * 100)):02d}"


def _signal_for_target(target_pct: float) -> Path:
    ...
    for row in rows:
        ...
        row["strategy_variant"] = f"dynamic_h2_m3_sl06_dd0612_{_case_name(target_pct)}"
...
def _write_report(summary: list[dict], audit: dict) -> None:
    ...
    for row in ranked:
        lines.append(
            ...
            f"{_f(row['full_max_drawdown']):.2%} | {_f(row['full_avg_invested_pct']):.2%} | {_f(row['recent60_annual']):.2%} | "
            f"{_f(row['recent60_sharpe']):.2f} | {_f(row['recent60_max_drawdown']):.2%} | {_f(row['late_min_annual']):.2%} |"
        )
    lines.extend(
        [
            "",
...
def main() -> int:
    ...
    for target_pct in TARGET_PCTS:
        signal_file = _signal_for_target(target_pct)
