...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_market_gate_refine_v137_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v137_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4市场状态门控窄邻域研究")
    ...
    score, order = v95.score_pair(
        full,
        0.0,
        int(protocol["score"]["smooth_window"]),
        float(protocol["score"]["current_weight"]),
    )
    ...
    rows, definitions = [], {}
    grid = protocol["grid"]
    momentum_cache = {lookback: v134.market_momentum(observation, int(lookback)) for lookback in grid["market_lookback"]}
    for lookback, return_min, low_gross in product(grid["market_lookback"], grid["market_return_min"], grid["low_gross"]):
        definition = {
            **protocol["fixed_definition"],
            "market_lookback": lookback,
            "market_return_min": return_min,
            "low_gross": low_gross,
        }
