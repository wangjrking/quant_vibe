...
import research_v260_fixed10_sell_threshold_20260822 as round5
...
OUTPUT_ROOT = (
    REPO
    / "quant/data_file/reports/strategy_agent_v260_fixed10_max_hold_recheck_20260822"
)
MAX_HOLD_DAYS = (15, 20, 25)


def candidate_policy(max_hold_days: int) -> dict:
    current = round5.candidate_policy(0.85)
...
def main() -> None:
    ...
    if access["logical_max_date"] != round1.DEVELOPMENT_END:
        raise PermissionError("2026 data entered max-hold recheck")
    ...
    if not all(deterministic.values()):
        raise RuntimeError("max-hold deterministic replay failed")
