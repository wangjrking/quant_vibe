...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_continuous_regime_sizing_v170_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v170_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def schedules(arrays: dict, definition: dict):
    ...
    high = np.isfinite(momentum) & (momentum >= 0.0)
    gross = np.where(high, 1.0, 0.2).astype(float)
    target = np.full(len(momentum), 0.05, dtype=float)
    ramp_end = float(definition["ramp_end_return"])
    progress = np.clip(np.nan_to_num(momentum, nan=-1.0) / ramp_end, 0.0, 1.0)
    target[high] = 0.25 + (0.375 - 0.25) * progress[high]
    return gross, target
...
def run_case(arrays, score, order, definition, protocol, end, start=None):
    ...
    return v109.simulate(
        ...
        min_hold_days_override=v162.min_hold_schedule(arrays, "strong_3_else_4"),
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4连续市场状态仓位研究")
    ...
    rows, definitions = [], {}
    for ramp_end in protocol["grid"]["ramp_end_return"]:
        definition = {**protocol["fixed_definition"], "ramp_end_return": ramp_end}
