...
def main() -> None:
    ...
    if current["validation_2026_opened"] or original["validation_2026_opened"]:
        raise PermissionError("2026 data entered volatility-priority checkpoint")
    ...
    if not all(deterministic.values()):
        raise RuntimeError("confirmed weak priority checkpoint replay failed")
    ...
    if not all(gates.values()):
        raise RuntimeError("confirmed weak priority did not pass checkpoint gates")
    result = {
        "status": "confirmed_weak_vol_priority_accepted_pre2026_validation_not_opened",
    ...
    checkpoint = {
        ...
        "source_strategy": context.rules["strategy_id"],
        "selected_candidate": "fixed10_age10_confirmed_weak_vol_priority005",
        "selected_policy": policy,
