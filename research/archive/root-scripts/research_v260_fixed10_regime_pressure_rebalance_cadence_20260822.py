...
OUTPUT_ROOT = (
    ...
    "strategy_agent_v260_fixed10_regime_pressure_rebalance_cadence_20260822"
)
CHECKPOINT_PATH = (
    REPO
    / "quant/data_file/reports/"
...
CANDIDATES = {"rebalance_20d": 20, "rebalance_10d": 10}


def run_policy(context, policy: dict, interval_days: int):
    ...
    common = dict(
        ...
        portfolio_rebalance_active_override=cadence.rebalance_schedule(
            len(context.arrays["dates"]), int(interval_days)
        ),
    ...
    return {
        "rebalance_interval_days": int(interval_days),
...
def main() -> None:
    ...
    if checkpoint["validation_2026_opened"]:
        raise PermissionError("2026 validation is not available for cadence research")
    policy = copy.deepcopy(checkpoint["selected_policy"])
    context = harness.load_context(policy)
    results, cache = {}, {}
    for candidate_id, interval_days in CANDIDATES.items():
        results[candidate_id], daily, actions = run_policy(
            context, policy, interval_days
        )
    ...
    if not all(checkpoint_equivalence.values()):
        raise RuntimeError("regime-pressure cadence baseline drifted")
    ...
    if not all(deterministic.values()):
        raise RuntimeError("regime-pressure cadence replay failed")

    selected_policy = copy.deepcopy(policy)
    selected_policy["portfolio_rebalance_interval_days"] = CANDIDATES[selected]
    result = {
        "status": "regime_pressure_rebalance_cadence_complete_2026_not_opened",
        ...
        "only_change": "restore each held position toward 10 percent every 10 rather than 20 sessions",
        "results": results,
