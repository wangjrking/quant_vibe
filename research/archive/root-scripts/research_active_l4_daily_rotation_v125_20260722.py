...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_daily_rotation_v125_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v125_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def run_case(arrays, score, order, definition, protocol, end, start=None):
    return v109.simulate(
        ...
        order,
        definition,
        v122.case_protocol(protocol, definition),
        end,
        start,
        daily_exit_candidates=True,
        daily_replacement=bool(definition["daily_replacement"]),
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4慢分数每日独立卖出与替换研究")
    ...
    rows, definitions = [], {}
    grid = protocol["grid"]
    for min_hold, sell_below, advantage, replacement in product(grid["min_hold_days"], grid["sell_score_below"], grid["replacement_advantage"], grid["daily_replacement"]):
        definition = {**protocol["fixed_definition"], "min_hold_days": min_hold, "sell_score_below": sell_below, "replacement_advantage": advantage, "daily_replacement": replacement}
