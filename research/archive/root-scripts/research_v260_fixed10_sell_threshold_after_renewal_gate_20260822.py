...
import research_v260_fixed10_replacement_margin_after_renewal_gate_20260822 as margin_recheck
...
OUTPUT_ROOT = (
    REPO
    / "quant/data_file/reports/strategy_agent_v260_fixed10_sell_threshold_after_renewal_gate_20260822"
)
SELL_THRESHOLDS = (0.80, 0.85, 0.90)


def candidate_policy(sell_threshold: float) -> dict:
    current = margin_recheck.candidate_policy(0.05)
    current["sell_score_below"] = float(sell_threshold)
    return current
...
def main() -> None:
    ...
    if access["logical_max_date"] != round1.DEVELOPMENT_END:
        raise PermissionError("2026 data entered sell-threshold-after-renewal recheck")
    ...
    for threshold in SELL_THRESHOLDS:
        candidate_id = f"sell_below_{int(round(threshold * 100)):02d}"
    ...
    if not all(deterministic.values()):
        raise RuntimeError("sell-threshold-after-renewal replay failed")
    ...
    result = {
        ...
        "candidate_budget": [float(value) for value in SELL_THRESHOLDS],
        "selection_rule": (
