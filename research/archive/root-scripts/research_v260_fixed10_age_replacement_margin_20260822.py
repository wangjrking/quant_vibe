...
import research_v260_fixed10_exit_confirmation_20260822 as round11
...
OUTPUT_ROOT = (
    REPO
    / "quant/data_file/reports/strategy_agent_v260_fixed10_age_replacement_margin_20260822"
)
AGE_MARGIN_POLICIES = {
    "constant_05": None,
    "moderate_08_05_03": [(0, 0.08), (10, 0.05), (20, 0.03)],
    "strict_10_05_00": [(0, 0.10), (10, 0.05), (20, 0.00)],
}


def candidate_policy(age_bands: list[tuple[int, float]] | None) -> dict:
    current = round11.candidate_policy(1)
    current["age_bands"] = copy.deepcopy(age_bands)
    return current
...
def main() -> None:
    ...
    if access["logical_max_date"] != round1.DEVELOPMENT_END:
        raise PermissionError("2026 data entered age-margin development")
    definition = harness.production_definition(protocol)
    score, order = harness.v95.score_pair(arrays, 0.0, 7, 0.1)

    results = {}
    run_cache = {}
    for candidate_id, age_bands in AGE_MARGIN_POLICIES.items():
        current = candidate_policy(age_bands)
    ...
    if not all(deterministic.values()):
        raise RuntimeError("age-margin deterministic replay failed")
    ...
    result = {
        ...
        "candidate_budget": list(AGE_MARGIN_POLICIES),
        "selection_rule": (
