...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_slow_portfolio_horizon_v126_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v126_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def score_pair(arrays: dict, definition: dict):
    return v95.score_pair(arrays, float(definition["weight_5d"]), int(definition["smooth_window"]), float(definition["current_weight"]))


def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4单批组合的5D与10D期限融合研究")
    ...
    if args.open_known_2026:
        ...
        for item in frozen["candidates"]:
            ...
            score, order = score_pair(full, definition)
            for start in protocol["known_stress_buy_starts"]:
                daily = v109.simulate(full, score, order, definition, v122.case_protocol(protocol, definition), protocol["known_stress_end"], start)
    ...
    rows, definitions = [], {}
    for weight_5d in protocol["grid"]["weight_5d"]:
        definition = {**protocol["fixed_definition"], "weight_5d": weight_5d}
        case_id = stable_id(definition)
        definitions[case_id] = definition
        score_obs, order_obs = score_pair(observation, definition)
        daily = v109.simulate(observation, score_obs, order_obs, definition, v122.case_protocol(protocol, definition), protocol["observation_end"])
