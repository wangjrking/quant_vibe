...
REPORT_DIR = ROOT / "quant/data_file/reports/strategy_agent_active_l4_limited_rotation_v29_20260721"
...
REPORT_PATH = REPORT_DIR / "每日限换仓研究结论.md"
...
def case_id(profile):
    ...
    return "lr_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def simulate(arrays, masked, score, order, exposure, profile, end_signal_date, record_actions=False):
    ...
    for t, signal_date in enumerate(dates[:-1]):
        ...
        for idx in shares:
            ...
            if max_exit or score_exit:
                candidates.append((0 if max_exit else 1, current, stocks[idx], idx))
        already = {item[3] for item in candidates}
        remaining = [idx for idx in shares if idx not in already]
        excess = max(len(remaining) + len(candidates) - target_positions, 0)
        scale = sorted((idx for idx in remaining if t - entry_index[idx] >= profile.min_hold), key=lambda idx: (float(score[t, idx]) if np.isfinite(score[t, idx]) else -np.inf, stocks[idx]))[:excess]
        for idx in scale:
            current = float(score[t, idx]) if np.isfinite(score[t, idx]) else -np.inf
            candidates.append((2, current, stocks[idx], idx))
        sell_list = [item[3] for item in sorted(candidates)[:profile.max_sells_per_day]]
        ...
        for idx in sell_list:
            ...
            trades += 1
            if record_actions:
                actions.append({"signal_date": signal_date, "buy_date": buy_date, "action": "SELL", "stock_code": stocks[idx], "target_pct": 0.0, "execution_open_raw": float(opens[idx]), "max_sells_per_day": profile.max_sells_per_day})
            del shares[idx]
            del entry_index[idx]
            last_price.pop(idx, None)
        slots = 0 if sell_failed else min(max(target_positions - len(shares), 0), profile.max_buys_per_day)
        ...
        for idx in ranked:
            ...
            slots -= 1
            if record_actions:
                actions.append({"signal_date": signal_date, "buy_date": buy_date, "action": "BUY", "stock_code": stocks[idx], "target_pct": profile.invested_ratio / profile.top_n, "execution_open_raw": float(opens[idx]), "max_buys_per_day": profile.max_buys_per_day})
...
def main():
        ...
        for mv_min in grid["total_mv_min"]:
            for top_n in grid["top_n"]:
                for entry_rank_min in grid["entry_rank_min"]:
                    for max_buys in grid["max_buys_per_day"]:
                        for max_sells in grid["max_sells_per_day"]:
                            profile = RotationProfile(amount_min, mv_min, top_n, entry_rank_min, max_buys, max_sells, int(fixed["min_hold"]), int(fixed["max_hold"]), float(fixed["sell_rank_below"]), float(fixed["replacement_advantage"]), 1.0)
    ...
    for _, seed in seeds.iterrows():
        for exit_rule in protocol["stage2_exit_profiles"]:
            profile = RotationProfile(int(seed.amount_min), int(seed.mv_min), int(seed.top_n), float(seed.entry_rank_min), int(seed.max_buys_per_day), int(seed.max_sells_per_day), int(exit_rule["min_hold"]), int(exit_rule["max_hold"]), float(exit_rule["sell_rank_below"]), float(exit_rule["replacement_advantage"]), 1.0)
            daily = simulate(arrays, masked, score, order, exposure, profile, protocol["observation_end"])
            rows.append({"case_id": case_id(profile), "seed_case_id": str(seed.case_id), "exit_profile_id": exit_rule["id"], **asdict(profile), **evaluate(daily, protocol["year_folds"])})
    ...
    for item in payload["profiles"]:
        profile = RotationProfile(**{key: item[key] for key in RotationProfile.__dataclass_fields__})
        ...
        action_rows.append({"case_id": item["case_id"], "promotion_track": item["promotion_track"], "path": str(path.relative_to(ROOT)).replace("\\", "/"), "sha256": digest(path), "content_sha256": content_hash, "rows": len(actions), "buy_rows": int((actions.action == "BUY").sum()), "sell_rows": int((actions.action == "SELL").sum()), "max_positions": int(item["top_n"])})
