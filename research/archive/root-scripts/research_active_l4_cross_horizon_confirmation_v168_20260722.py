...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_cross_horizon_confirmation_v168_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v168_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def confirmation_mask(arrays: dict, definition: dict) -> np.ndarray:
    mode = definition["confirmation_mode"]
    threshold = float(definition["confirmation_threshold"])
    if mode == "none":
        return np.ones_like(arrays["signal_clean"], dtype=np.bool_)
    if mode == "rank_3d":
        return np.isfinite(arrays["rank_3d"]) & (arrays["rank_3d"] >= threshold)
    if mode == "rank_5d":
        return np.isfinite(arrays["rank_5d"]) & (arrays["rank_5d"] >= threshold)
    if mode == "both_3d_5d":
        return (
            np.isfinite(arrays["rank_3d"])
            & np.isfinite(arrays["rank_5d"])
            & (arrays["rank_3d"] >= threshold)
            & (arrays["rank_5d"] >= threshold)
        )
    raise ValueError("未知确认模式：{}".format(mode))
...
def run_case(arrays, score, order, definition, protocol, end, start=None):
    return v162.run_case(
        ...
        start,
        selection_mask_override=confirmation_mask(arrays, definition),
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4跨周期确认研究")
