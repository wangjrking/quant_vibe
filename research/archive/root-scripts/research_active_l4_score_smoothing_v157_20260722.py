...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_score_smoothing_v157_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v157_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4 10D评分平滑邻域研究")
    ...
    if args.open_known_2026:
        ...
        for item in frozen["candidates"]:
            ...
            for start in protocol["known_stress_buy_starts"]:
                daily = v153.run_case(full, score, order, item["definition"], protocol, protocol["known_stress_end"], start)
    ...
    for window, current_weight in product(protocol["grid"]["smooth_window"], protocol["grid"]["current_weight"]):
        definition = {**protocol["fixed_definition"], "smooth_window": window, "current_weight": current_weight}
        case_id = stable_id(definition)
        definitions[case_id] = definition
        score, order = score_pair(observation, definition)
        daily = v153.run_case(observation, score, order, definition, protocol, protocol["observation_end"])
