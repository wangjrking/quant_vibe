...
PENALTIES = (0.0, 0.025, 0.05, 0.075, 0.10)
...
def main() -> None:
    ...
    for penalty in PENALTIES:
        candidate_id = f"weak2_vol_penalty_{penalty:.3f}"
    ...
    baseline_id = "weak2_vol_penalty_0.050"
    ...
    if not all(checkpoint_equivalence.values()):
        raise RuntimeError("weak2 priority baseline drifted")
    gates = {
    ...
    if not all(deterministic.values()):
        raise RuntimeError("weak2 priority replay failed")

    result = {
        "status": "weak2_vol_priority_boundary_complete_2026_not_opened",
        "candidate_budget": [f"weak2_vol_penalty_{value:.3f}" for value in PENALTIES],
        "only_change": (
            "after two consecutive weak sessions, order already-eligible exits by "
            "score minus penalty times trailing-20d volatility percentile"
        ),
        "results": results,
        "selection_gates_against_005": gates,
