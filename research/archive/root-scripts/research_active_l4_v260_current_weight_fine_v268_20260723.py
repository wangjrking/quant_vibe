...
import research_active_l4_v260_current_weight_v266_20260723 as v266
...
OUT = (
    ROOT
    / "quant/data_file/reports/strategy_agent_active_l4_v260_current_weight_fine_v268_20260723"
)
...
def stable_id(definition: dict) -> str:
    ...
    return "v268_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="V260 当前日分数权重细邻域")
    ...
    definitions = [
        v266.definition_for(protocol, value)
        for value in protocol["grid"]["current_weight"]
    ]
    rows = []
    full_rows = []
    frozen = []
    for definition in definitions:
        score, order = v266.score_pair(arrays, definition)
        ...
        if args.open_known_2026:
            for start in protocol["known_stress_starts"]:
                ...
                rows.append(
                    {
                        "case_id": case_id,
                        "current_weight": definition["current_weight"],
                        "buy_start": start,
                        **core.metrics(
