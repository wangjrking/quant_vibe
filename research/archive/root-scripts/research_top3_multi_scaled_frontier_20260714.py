...
SIGNAL_DIR = REPORT_DIR / "signals" / "top3_multi_scaled_frontier"
LOG_DIR = REPORT_DIR / "logs" / "top3_multi_scaled_frontier"
...
def main() -> None:
    ...
    for top_n in [2, 3]:
        for scale in [1.11, 1.12, 1.13, 1.14, 1.15, 1.16, 1.17, 1.18, 1.19, 1.20]:
            high, mid, low = [x * scale for x in base_weights]
            for daily_cap in [0.96, 0.98, 1.00]:
                cases.append(
                    {
                        "case": (
                            f"mf_t{top_n}_sc{int(scale*1000):04d}_cap{int(daily_cap*100):03d}"
                        ),
                        "top_n": top_n,
                        "high": high,
                        "mid": mid,
    ...
    csv_path = REPORT_DIR / "top3_multi_scaled_frontier_juejin_results_20260714.csv"
    json_path = REPORT_DIR / "top3_multi_scaled_frontier_juejin_results_20260714.json"
