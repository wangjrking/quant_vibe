...
SIGNAL_DIR = REPORT_DIR / "signals" / "micro_overlay_gb_slice_focus"
LOG_DIR = REPORT_DIR / "logs" / "micro_overlay_gb_slice_focus"
OUT_CSV = REPORT_DIR / "micro_overlay_gb_slice_focus_juejin_results_20260714.csv"
OUT_JSON = REPORT_DIR / "micro_overlay_gb_slice_focus_juejin_results_20260714.json"
REPORT_MD = REPORT_DIR / "微仓GB宽核心切片聚焦复核_20260714.md"
...
SOURCES = {
    "gb_p1hi": SOURCE_DIR / "gb_lowsum_p1hi_s010.csv",
    "gb_p1hi_sigmid": SOURCE_DIR / "gb_lowsum_p1hi_sigmid_s012.csv",
    "gb_sigmid": SOURCE_DIR / "gb_lowsum_sigmid_s010.csv",
}
SLICES = {
...
def write_slice(name: str, df: pd.DataFrame, slice_name: str, start: str, end: str) -> dict | None:
    out_df = df[(df["buy_date"] >= start) & (df["buy_date"] <= end)].copy()
    if out_df.empty:
        return None
    case = f"{name}_{slice_name}"
    out_df["strategy_variant"] = case
    out_df["filter_name"] = case
    ...
    return {
        ...
        "signal_file": str(out),
        "rows": int(len(out_df)),
        "buy_days": int(out_df["buy_date"].nunique()),
        "stock_count": int(out_df["stock_code"].nunique()),
        "max_positions": int(max(out_df.groupby("buy_date")["stock_code"].count().max(), 1)),
        "holding_days": 1,
        "max_holding_days": 3,
        "mean_daily_target_sum": float(pd.to_numeric(out_df["target_pct"], errors="coerce").groupby(out_df["buy_date"]).sum().mean()),
        "min_buy_date": str(out_df["buy_date"].min()),
        "max_buy_date": str(out_df["buy_date"].max()),
    }
...
def write_report(frame: pd.DataFrame) -> None:
    lines = [
        ...
        "## 结论",
        "",
        "- 本轮只补跑 GB 宽核心候选的全周期、2025 起始、202407 起始切片。",
        "- 用于判断宽核心是否能解决后段收益强度不足。",
        "",
        "## 掘金结果",
        "",
        "| source | slice | 年化 | Sharpe | 最大回撤 | 开仓 | 买入日 | 平均日目标仓位 |",
        "|---|---|---:|---:|---:|---:|---:|---:|",
    ]
    for _, row in frame.sort_values(["source", "slice"]).iterrows():
        lines.append(
            f"| {row.get('source')} | {row.get('slice')} | {float(row.get('pnl_ratio_annual', 0))*100:.2f}% | "
