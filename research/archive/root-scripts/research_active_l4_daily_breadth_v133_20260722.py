...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_daily_breadth_v133_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v133_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def case_protocol(protocol: dict, definition: dict) -> dict:
    ...
    result["breadth_state"] = {"breadth_count_threshold": int(definition["breadth_count_threshold"]), "high_when": "above", "low_gross": float(definition["low_gross"]), "high_gross": 1.0}
    return result


def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4每日重叠策略宽度门控研究")
    ...
    if args.open_known_2026:
        ...
        for item in frozen["candidates"]:
            ...
            for start in protocol["known_stress_buy_starts"]:
                daily = v109.simulate(full, score, order, definition, case_protocol(protocol, definition), protocol["known_stress_end"], start)
    ...
    for score_threshold, count_threshold, low_gross in product(grid["breadth_score_threshold"], grid["breadth_count_threshold"], grid["low_gross"]):
        definition = {**protocol["fixed_definition"], "breadth_score_threshold": score_threshold, "breadth_count_threshold": count_threshold, "low_gross": low_gross}
        case_id = stable_id(definition)
        definitions[case_id] = definition
        counts = np.sum(np.isfinite(score_obs) & (score_obs >= float(score_threshold)), axis=1)
        high_days = int(np.sum(counts >= int(count_threshold)))
