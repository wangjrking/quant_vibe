...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_atr_tail_filter_v229_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v229_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def run_case(arrays, score, order, definition, protocol, end, start=None):
    ...
    atr_max = definition["atr_pct_max"]
    if atr_max is not None:
        ...
        selection_mask &= valid & (atr_pct <= float(atr_max))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 ATR 极端尾部过滤研究")
    ...
    rows, definitions = [], {}
    for atr_max in protocol["grid"]["atr_pct_max"]:
        definition = {
            **protocol["fixed_definition"],
            "atr_pct_max": None if atr_max is None else float(atr_max),
        }
