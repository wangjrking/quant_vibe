...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_score_refresh_v148_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v148_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4策略分数响应速度研究")
    ...
    if args.open_known_2026:
        ...
        for item in frozen["candidates"]:
            ...
            score, order = v95.score_pair(full, 0.0, int(definition["smooth_window"]), float(definition["current_weight"]))
    ...
    for smooth_window, current_weight in product(protocol["grid"]["smooth_window"], protocol["grid"]["current_weight"]):
        ...
        score, order = v95.score_pair(observation, 0.0, int(smooth_window), float(current_weight))
