...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_sell_score_v199_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v199_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 持仓分数退出阈值研究")
    ...
    rows, definitions = [], {}
    for sell_score_below in protocol["grid"]["sell_score_below"]:
        definition = {**protocol["fixed_definition"], "sell_score_below": float(sell_score_below)}
