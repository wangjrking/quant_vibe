...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_age_adaptive_exit_v190_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v190_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def run_case(arrays, score, order, definition, protocol, end, start=None):
    return v162.run_case(
        ...
        replacement_advantage_age_bands_override=definition["replacement_advantage_age_bands"],
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 age-adaptive exit study")
    ...
    for item in protocol["grid"]:
        ...
        rows.append({"case_id": case_id, "policy_id": item["policy_id"], **metrics})
