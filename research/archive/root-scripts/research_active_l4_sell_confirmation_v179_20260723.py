...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_sell_confirmation_v179_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v179_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def run_case(arrays, score, order, definition, protocol, end, start=None):
    return v162.run_case(
        ...
        sell_confirmation_days_override=int(definition["sell_confirmation_days"]),
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 sell-confirmation study")
    ...
    rows, definitions = [], {}
    for days in protocol["grid"]["sell_confirmation_days"]:
        definition = {**protocol["fixed_definition"], "sell_confirmation_days": int(days)}
