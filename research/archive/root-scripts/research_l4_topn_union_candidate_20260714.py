...
SIGNAL_DIR = REPORT_DIR / "signals" / "l4_topn_union_candidate"
LOG_DIR = REPORT_DIR / "logs" / "l4_topn_union_candidate"
OUT_CSV = REPORT_DIR / "l4_topn_union_candidate_juejin_results_20260714.csv"
OUT_JSON = REPORT_DIR / "l4_topn_union_candidate_juejin_results_20260714.json"
POOL_PARQUET = REPORT_DIR / "l4_topn_union_candidate_pool_20260714.parquet"
...
CASES = [
    {"name": "u10_p175_g05_amt12_t2_atr12_w001", "w1": 0, "w3": 0, "w5": 0, "w10": 1, "pct": -1.75, "gap_hi": 0.5, "gap_lo": -5, "amount": 120000, "turn": 2, "atr": 12, "target": 0.46},
    {"name": "u10_p175_g05_amt20_t2_atr12_w001", "w1": 0, "w3": 0, "w5": 0, "w10": 1, "pct": -1.75, "gap_hi": 0.5, "gap_lo": -5, "amount": 200000, "turn": 2, "atr": 12, "target": 0.50},
    {"name": "u105_p175_g05_amt12_t2_atr12_w8020", "w1": 0, "w3": 0, "w5": 0.2, "w10": 0.8, "pct": -1.75, "gap_hi": 0.5, "gap_lo": -5, "amount": 120000, "turn": 2, "atr": 12, "target": 0.50},
    {"name": "u105_p175_g05_amt20_t2_atr12_w7030", "w1": 0, "w3": 0, "w5": 0.3, "w10": 0.7, "pct": -1.75, "gap_hi": 0.5, "gap_lo": -5, "amount": 200000, "turn": 2, "atr": 12, "target": 0.52},
    {"name": "u13510_p175_g05_amt12_t2_atr12_w0127", "w1": 0, "w3": 0.1, "w5": 0.2, "w10": 0.7, "pct": -1.75, "gap_hi": 0.5, "gap_lo": -5, "amount": 120000, "turn": 2, "atr": 12, "target": 0.52},
    {"name": "u13510_p25_g05_amt12_t2_atr12_w0136", "w1": 0, "w3": 0.1, "w5": 0.3, "w10": 0.6, "pct": -2.5, "gap_hi": 0.5, "gap_lo": -6, "amount": 120000, "turn": 2, "atr": 12, "target": 0.55},
    {"name": "u1510_guard_p175_g05_amt12", "w1": 0.08, "w3": 0, "w5": 0.17, "w10": 0.75, "pct": -1.75, "gap_hi": 0.5, "gap_lo": -5, "amount": 120000, "turn": 2, "atr": 12, "target": 0.50, "min_s1": 0.55},
    {"name": "u1510_guard_p175_g05_amt20", "w1": 0.08, "w3": 0, "w5": 0.17, "w10": 0.75, "pct": -1.75, "gap_hi": 0.5, "gap_lo": -5, "amount": 200000, "turn": 2, "atr": 12, "target": 0.50, "min_s1": 0.55},
    {"name": "u10_turn4_p175_g05", "w1": 0, "w3": 0, "w5": 0, "w10": 1, "pct": -1.75, "gap_hi": 0.5, "gap_lo": -5, "amount": 120000, "turn": 4, "atr": 8, "target": 0.60},
    {"name": "u105_turn4_p175_g05", "w1": 0, "w3": 0, "w5": 0.2, "w10": 0.8, "pct": -1.75, "gap_hi": 0.5, "gap_lo": -5, "amount": 120000, "turn": 4, "atr": 8, "target": 0.60},
    {"name": "u105_soft_p10_g05_amt20", "w1": 0, "w3": 0, "w5": 0.2, "w10": 0.8, "pct": -1.0, "gap_hi": 0.5, "gap_lo": -5, "amount": 200000, "turn": 2, "atr": 12, "target": 0.42},
    {"name": "u105_widegap_p175_amt15", "w1": 0, "w3": 0, "w5": 0.2, "w10": 0.8, "pct": -1.75, "gap_hi": 1.0, "gap_lo": -6, "amount": 150000, "turn": 2, "atr": 12, "target": 0.45},
]
...
def build_pool() -> pd.DataFrame:
    ...
    try:
        ...
        top1 AS (
            SELECT trade_date, stock_code FROM db1.{T1}
            QUALIFY ROW_NUMBER() OVER (PARTITION BY trade_date ORDER BY pred_prob DESC) <= 120
        ),
        top3 AS (
            SELECT trade_date, stock_code FROM db3.{T3}
            QUALIFY ROW_NUMBER() OVER (PARTITION BY trade_date ORDER BY pred_prob DESC) <= 120
        ),
        top5 AS (
            SELECT trade_date, stock_code FROM db5.{T5}
            QUALIFY ROW_NUMBER() OVER (PARTITION BY trade_date ORDER BY pred_prob DESC) <= 160
        ),
        top10 AS (
            SELECT trade_date, stock_code FROM db10.{T10}
            QUALIFY ROW_NUMBER() OVER (PARTITION BY trade_date ORDER BY pred_prob DESC) <= 220
        ),
        keys AS (
            SELECT * FROM top1 UNION SELECT * FROM top3 UNION SELECT * FROM top5 UNION SELECT * FROM top10
        )
        SELECT
    ...
    for col in ["pred_1d", "pred_3d", "pred_5d", "pred_10d"]:
        rank_col = "s" + col.split("_")[1].replace("d", "")
...
def main() -> None:
    ...
    (REPORT_DIR / "l4_topn_union_candidate_summary_20260714.json").write_text(
