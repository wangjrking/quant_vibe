...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_slow_exit_exposure_v118_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v118_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="慢评分策略卖出阈值与弱状态仓位联合研究")
    ...
    rows, definitions = [], {}
    for threshold, low_gross in product(protocol["grid"]["sell_score_below"], protocol["grid"]["low_gross"]):
        definition = {**protocol["fixed_exit"], "sell_score_below": threshold, "low_gross": low_gross}
