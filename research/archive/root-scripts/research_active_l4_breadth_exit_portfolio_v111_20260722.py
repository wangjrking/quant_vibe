...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_breadth_exit_portfolio_v111_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v111_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def case_protocol(protocol: dict, definition: dict) -> dict:
    ...
    result["fixed_strategy"] = {
        ...
        "rebalance_every": int(protocol["rebalance_every"]),
    }
    return result


def main() -> None:
    parser = argparse.ArgumentParser(description="宽度风控与独立卖出下的组合邻域研究")
    ...
    rows, definitions = [], {}
    grid = protocol["grid"]
    for top_n, max_hold, threshold in product(grid["top_n_per_rebalance"], grid["max_hold_days"], grid["sell_score_below"]):
        definition = {**protocol["fixed_exit"], "top_n_per_rebalance": top_n, "max_hold_days": max_hold, "sell_score_below": threshold}
