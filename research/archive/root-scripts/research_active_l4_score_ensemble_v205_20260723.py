...
import research_active_l4_max_positions_refine_v196_20260723 as v196
import research_active_l4_robust_objective_v56_20260721 as robust
import research_preregistered_active_l4_rank_rotation_20260721 as core


ROOT = Path(__file__).resolve().parents[2]
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_score_ensemble_v205_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v205_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def score_pair(arrays: dict, primary_weight: float) -> tuple[np.ndarray, np.ndarray]:
    primary, _ = v95.score_pair(arrays, 0.0, 7, 0.1)
    neighbor, _ = v95.score_pair(arrays, 0.0, 8, 0.0)
    score = (float(primary_weight) * primary + (1.0 - float(primary_weight)) * neighbor).astype(np.float32)
    order = np.argsort(-np.nan_to_num(score, nan=-np.inf), axis=1, kind="stable")
    return score, order


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 纯10D双评分器集成研究")
    ...
    if args.open_known_2026:
        ...
        for item in frozen["candidates"]:
            ...
            score, order = score_pair(full, float(definition["primary_weight"]))
            for start in protocol["known_stress_buy_starts"]:
                daily = v196.run_case(
                    full, score, order, definition, protocol, protocol["known_stress_end"], start
                )
                rows.append(
                    {"case_id": item["case_id"], "buy_start": start, **core.metrics(daily, start, protocol["known_stress_end"])}
                )
    ...
    rows, definitions = [], {}
    for primary_weight in protocol["grid"]["primary_weight"]:
        definition = {**protocol["fixed_definition"], "primary_weight": float(primary_weight)}
        case_id = stable_id(definition)
        definitions[case_id] = definition
        score, order = score_pair(observation, float(primary_weight))
        daily = v196.run_case(observation, score, order, definition, protocol, protocol["observation_end"])
