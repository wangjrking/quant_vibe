...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_atr_sizing_v228_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v228_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def atr_multiplier(arrays: dict, target: float | None) -> tuple[np.ndarray, np.ndarray]:
    base_mask = np.ones(arrays["atr_qfq"].shape, dtype=np.bool_)
    if target is None:
        return np.ones(arrays["atr_qfq"].shape, dtype=np.float32), base_mask
    valid = (
        np.isfinite(arrays["atr_qfq"])
        & (arrays["atr_qfq"] > 0)
        & np.isfinite(arrays["close_qfq"])
        & (arrays["close_qfq"] > 0)
    )
    atr_pct = np.full(arrays["atr_qfq"].shape, np.nan, dtype=np.float32)
    atr_pct[valid] = arrays["atr_qfq"][valid] / arrays["close_qfq"][valid]
    multiplier = np.zeros(arrays["atr_qfq"].shape, dtype=np.float32)
    multiplier[valid] = np.clip(float(target) / atr_pct[valid], 0.75, 1.25)
    return multiplier, valid


def run_case(arrays, score, order, definition, protocol, end, start=None):
    target = definition["atr_target_pct"]
    multiplier, atr_valid = atr_multiplier(arrays, target)
    selection_mask = v174.selection_mask(arrays, definition["max_rank_deterioration"])
    if target is not None:
        selection_mask &= atr_valid
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 ATR 连续仓位缩放研究")
    ...
    rows, definitions = [], {}
    for target in protocol["grid"]["atr_target_pct"]:
        definition = {
            **protocol["fixed_definition"],
            "atr_target_pct": None if target is None else float(target),
        }
