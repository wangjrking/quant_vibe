...
from research_v260_runtime import fixed10_limitup_exit_v111 as runtime


OUTPUT_ROOT = (
    REPO
    / "quant/data_file/reports/strategy_agent_v260_fixed10_limitup_exit_deferral_20260822"
)
CANDIDATES = ("sell_as_planned", "defer_sell_when_open_limit_up")
...
def main() -> None:
    ...
    if access["logical_max_date"] != round1.DEVELOPMENT_END:
        raise PermissionError("2026 data entered limit-up exit development")
    ...
    for candidate_id in CANDIDATES:
        defer = candidate_id == "defer_sell_when_open_limit_up"
        common = dict(
        ...
        windows = {
            str(offset): round1.evaluate_run(
                daily, actions, robustness.window_start_date(arrays, offset),
                round1.DEVELOPMENT_END,
            )
            for offset in (5, 20, 60)
        }
        results[candidate_id] = {
            "policy": {**policy, "defer_sell_when_open_limit_up": defer},
    ...
    current = results["sell_as_planned"]["metrics_0_30pct"]
    ...
    if not all(checkpoint_equivalence.values()):
        raise RuntimeError("limit-up current path drifted from checkpoint")
    ...
    selected_defer = selected_id == "defer_sell_when_open_limit_up"
    repeat_daily, repeat_actions = round1.run_fixed10(
        ...
        entry_block_mask_override=entry_block,
        maintenance_buy_block_mask_override=maintenance_block,
        defer_sell_on_limit_up_override=selected_defer,
    )
    deterministic = {
    ...
    if not all(deterministic.values()):
        raise RuntimeError("limit-up exit deterministic replay failed")
    ...
    result = {
        ...
        "only_change": "whether a sell is deferred when raw open is already at the board-aware upper-limit boundary",
        "results": results,
