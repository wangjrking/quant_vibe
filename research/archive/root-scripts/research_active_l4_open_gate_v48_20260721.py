...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_open_gate_v48_20260721"
...
def gated_mask(base, arrays, spec):
    ...
    gate = np.isfinite(gap) & (gap >= float(spec["min_gap"])) & (gap <= float(spec["max_gap"]))
    result["buy_clean"] &= gate
    return result
...
def main():
    ...
    try:
        rows, g = [], p["grid"]
        for gate in p["open_gate_specs"]:
            masked = gated_mask(base_masked, arrays, gate)
            for entry in g["entry_rank_min"]:
                for total in g["normal_total_pct"]:
                    for scheme in g["weight_scheme"]:
                        ...
                        rows.append({"case_id": f"{gate['id']}_{bp.case_id}", "gate_id": gate["id"], "min_gap": gate["min_gap"], "max_gap": gate["max_gap"], **bp.__dict__, **metrics})
        ...
        specs = {item["id"]: item for item in p["open_gate_specs"]}
        for _, item in frozen.iterrows():
            masked = gated_mask(base_masked, arrays, specs[str(item.gate_id)])
    ...
    (OUT / "开盘执行门控研究结论.md").write_text("\n".join(["# 开盘执行门控研究结论", "", "T日仅生成候选顺序，T+1开盘后使用未复权真实价格执行门控并按原顺序补位。", "", f"固定网格 {len(results)} 组，逐年为正 {int(results.all_year_positive.sum())} 组，强过滤后 {len(pool)} 组。", f"冻结 {len(action_rows)} 条独立掘金路径。", "", "2026验证期未打开，生产资产未修改。"]), encoding="utf-8")
