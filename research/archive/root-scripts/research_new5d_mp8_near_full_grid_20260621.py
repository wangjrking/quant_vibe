...
SOURCE_DIR = REPORT_ROOT / "new5d_dynamic_holding_fine_20260621" / "signals"
REPORT_DIR = REPORT_ROOT / "new5d_mp8_near_full_grid_20260621"
...
for source_name, source in SOURCES.items():
    for max_positions in (8, 9, 10):
        for target_total_pct in (1.00, 1.03):
            base = {
                "source_name": source_name,
                "source_signal": source,
                "max_positions": max_positions,
                "target_total_pct": target_total_pct,
                "target_pct": target_total_pct / max_positions,
            }
            GRID.append(
                {
                    **base,
                    "name": f"{source_name}_mp{max_positions}_target{str(target_total_pct).replace('.', 'p')}_base",
                ...
                GRID.append(
                    {
                        **base,
                        "name": (
                            f"{source_name}_mp{max_positions}_target{str(target_total_pct).replace('.', 'p')}"
                            f"_rankexit0p7_min{min_hold}"
                        ),
