...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_dual_market_regime_v152_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v152_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def gross_schedule(arrays: dict, definition: dict) -> np.ndarray:
    short = v134.market_momentum(arrays, int(definition["short_lookback"]))
    long = v134.market_momentum(arrays, int(definition["long_lookback"]))
    gross = np.full(len(short), float(definition["low_gross"]), dtype=float)
    medium = np.isfinite(short) & np.isfinite(long) & (short < float(definition["short_min"])) & (long >= float(definition["long_min"]))
    high = np.isfinite(short) & (short >= float(definition["short_min"]))
    gross[medium] = float(definition["medium_gross"])
    gross[high] = 1.0
    return gross
...
def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4双周期市场状态研究")
    ...
    rows, definitions = [], {}
    for long_lookback, long_min, medium_gross in product(
        protocol["grid"]["long_lookback"], protocol["grid"]["long_min"], protocol["grid"]["medium_gross"]
    ):
        definition = {
            **protocol["fixed_definition"],
            "long_lookback": long_lookback,
            "long_min": long_min,
            "medium_gross": medium_gross,
        }
    ...
    FROZEN.write_text(
        json.dumps({"status": "frozen_before_known_2026_stress", "known_2026_used_for_this_grid_selection": False, "candidates": candidates}, ensure_ascii=False, indent=2, default=str),
        encoding="utf-8",
    )
