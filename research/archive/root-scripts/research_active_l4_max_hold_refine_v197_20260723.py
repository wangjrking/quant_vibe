...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_max_hold_refine_v197_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v197_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 最大持有天数邻域研究")
    ...
    rows, definitions = [], {}
    for max_hold_days in protocol["grid"]["max_hold_days"]:
        definition = {**protocol["fixed_definition"], "max_hold_days": int(max_hold_days)}
