...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_turnover_threshold_v219_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v219_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def run_case(arrays, score, order, definition, protocol, end, start=None):
    local_protocol = copy.deepcopy(protocol)
    local_protocol["fixed_universe"]["turnover_max"] = float(definition["turnover_max"])
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 换手率上限邻域研究")
    ...
    rows, definitions = [], {}
    for turnover_max in protocol["grid"]["turnover_max"]:
        definition = {**protocol["fixed_definition"], "turnover_max": float(turnover_max)}
