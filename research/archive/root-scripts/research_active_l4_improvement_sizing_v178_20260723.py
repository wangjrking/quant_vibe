...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_improvement_sizing_v178_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v178_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def improvement_multiplier(arrays: dict, definition: dict) -> np.ndarray:
    raw = arrays["rank_10d"].astype(np.float32)
    smooth = v94.rolling_nanmean(raw, 7)
    threshold = float(definition["improvement_threshold"])
    high = float(definition["improvement_multiplier"])
    result = np.ones(raw.shape, dtype=np.float32)
    result[np.isfinite(raw) & np.isfinite(smooth) & ((raw - smooth) >= threshold)] = high
    return result
...
def run_case(arrays, score, order, definition, protocol, end, start=None):
    return v162.run_case(
        ...
        candidate_target_multiplier_override=improvement_multiplier(arrays, definition),
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 score-improvement sizing study")
