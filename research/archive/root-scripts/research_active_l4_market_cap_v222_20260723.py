...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_market_cap_v222_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v222_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def run_case(arrays, score, order, definition, protocol, end, start=None):
    local_protocol = copy.deepcopy(protocol)
    local_protocol["fixed_universe"]["mv_min"] = int(definition["mv_min"])
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 市值下限邻域研究")
    ...
    rows, definitions = [], {}
    for mv_min in protocol["grid"]["mv_min"]:
        definition = {**protocol["fixed_definition"], "mv_min": int(mv_min)}
