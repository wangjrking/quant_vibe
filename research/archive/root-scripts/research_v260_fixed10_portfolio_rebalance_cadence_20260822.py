...
from research_v260_runtime import fixed10_portfolio_rebalance_v109 as portfolio_rebalance_v109
from research_v260_runtime import fixed10_renewal_quality_v109 as renewal_quality_v109


OUTPUT_ROOT = (
    REPO
    / "quant/data_file/reports/strategy_agent_v260_fixed10_portfolio_rebalance_cadence_20260822"
)
PORTFOLIO_REBALANCE_INTERVALS = (None, 20, 10, 5)
SHARPE_PRACTICAL_EQUIVALENCE = 0.01


def candidate_id(interval: int | None) -> str:
    return "portfolio_rebalance_off" if interval is None else f"portfolio_rebalance_{interval}d"


def candidate_policy(interval: int | None) -> dict:
    current = final_round.candidate_policy(1)
    current["portfolio_rebalance_interval_days"] = interval
    return current


def rebalance_schedule(length: int, interval: int | None) -> np.ndarray:
    active = np.zeros(int(length), dtype=np.bool_)
    if interval is not None:
        active[max(int(interval) - 1, 0) :: int(interval)] = True
    return active
...
def main() -> None:
    ...
    if access["logical_max_date"] != round1.DEVELOPMENT_END:
        raise PermissionError("2026 data entered portfolio-rebalance development")
    definition = harness.production_definition(protocol)
    score, order = harness.v95.score_pair(arrays, 0.0, 7, 0.1)

    results = {}
    run_cache = {}
    for interval in PORTFOLIO_REBALANCE_INTERVALS:
        current_id = candidate_id(interval)
        current = candidate_policy(interval)
        active = rebalance_schedule(len(arrays["dates"]), interval)
        daily, actions = round1.run_fixed10(
            ...
            record_actions=True,
            simulator=portfolio_rebalance_v109.simulate,
    ...
    if not all(disabled_equivalence.values()):
        raise RuntimeError("portfolio-rebalance runtime changed the disabled path")
    ...
    selected_interval = results[selected_id]["policy"]["portfolio_rebalance_interval_days"]
    ...
    repeat_daily, repeat_actions = round1.run_fixed10(
        ...
        record_actions=True,
        simulator=portfolio_rebalance_v109.simulate,
        portfolio_rebalance_active_override=rebalance_schedule(
            len(arrays["dates"]), selected_interval
        ),
    )
    deterministic = {
    ...
    if not all(deterministic.values()):
        raise RuntimeError("portfolio-rebalance deterministic replay failed")
    ...
    result = {
        ...
        "candidate_budget": [
            ...
            for value in PORTFOLIO_REBALANCE_INTERVALS
        ],
        "selection_rule": {
