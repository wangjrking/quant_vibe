...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_breadth_score_smooth_v112_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v112_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="宽度风控与独立卖出下的10D时序平滑研究")
    ...
    rows, definitions = [], {}
    grid = protocol["grid"]
    for window, current_weight, count_threshold in product(grid["smooth_window"], grid["current_weight"], grid["breadth_count_threshold"]):
        definition = {**protocol["fixed_exit"], "smooth_window": window, "current_weight": current_weight, "breadth_count_threshold": count_threshold}
