...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_breadth_score_balance_v113_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v113_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def case_protocol(protocol: dict, definition: dict) -> dict:
    ...
    result["breadth_state"] = {**protocol["breadth_state"], "breadth_count_threshold": int(definition["breadth_count_threshold"])}
    return result


def score_pair(arrays: dict, definition: dict):
    return v95.score_pair(arrays, 0.0, int(definition["smooth_window"]), float(definition["current_weight"]))


def main() -> None:
    parser = argparse.ArgumentParser(description="观察期与已知压力窗平衡的10D平滑研究")
    ...
    rows, definitions = [], {}
    grid = protocol["grid"]
    for window, current_weight, count_threshold, sell_threshold in product(grid["smooth_window"], grid["current_weight"], grid["breadth_count_threshold"], grid["sell_score_below"]):
        definition = {**protocol["fixed_exit"], "smooth_window": window, "current_weight": current_weight, "breadth_count_threshold": count_threshold, "sell_score_below": sell_threshold}
        case_id = stable_id(definition)
        definitions[case_id] = definition
        score, order = score_pair(observation, definition)
        daily = v109.simulate(observation, score, order, definition, case_protocol(protocol, definition), protocol["observation_end"])
