...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_reentry_cooldown_v224_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v224_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def run_case(arrays, score, order, definition, protocol, end, start=None, record_actions=False):
    return v162.run_case(
        ...
        reentry_cooldown_days_override=int(definition["reentry_cooldown_days"]),
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 卖出后再买冷却研究")
    parser.add_argument("--open-known-2026", action="store_true")
    parser.add_argument("--export-observation-actions", action="store_true")
    ...
    if args.export_observation_actions:
        ...
        for cooldown in (0, 1, 2, 3):
            ...
            daily.to_csv(OUT / f"observation_daily_cooldown_{cooldown}.csv", index=False, encoding="utf-8-sig")
            actions.to_csv(OUT / f"observation_actions_cooldown_{cooldown}.csv", index=False, encoding="utf-8-sig")
            summary.append(
                {
                    "reentry_cooldown_days": cooldown,
                    "actions": len(actions),
                    "buys": int((actions["action"] == "BUY").sum()),
                    "sells": int((actions["action"] == "SELL").sum()),
                    **core.metrics(
                        daily,
                        protocol["year_folds"][0]["start"],
                        protocol["observation_end"],
                    ),
                }
            )
        pd.DataFrame(summary).to_csv(OUT / "observation_action_diagnostic_summary.csv", index=False, encoding="utf-8-sig")
        print(json.dumps({"variants": len(summary), "status": "observation_actions_exported"}, ensure_ascii=False))
        return
    ...
    order_obs = order[: len(observation["dates"])]
    rows, definitions = [], {}
    for cooldown in protocol["grid"]["reentry_cooldown_days"]:
        definition = {**protocol["fixed_definition"], "reentry_cooldown_days": int(cooldown)}
