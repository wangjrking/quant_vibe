...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_four_year_1d_exante_gate_20260627"
BASE_DAILY = DATA_DIR / "reports" / "model_agent_four_year_control_assets_20260627" / "standard_eval" / "executable_1d_open_return_four_year_control_daily_eval.csv"
CAND_DAILY = DATA_DIR / "reports" / "model_agent_four_year_adaptive_rank_fusion_vs_control_20260627" / "standard_eval" / "executable_1d_open_return_four_year_adaptive_rank_fusion_vs_control_daily_eval.csv"
BASE_TABLE = "stock_predict_data_model_agent_four_year_control_20260627_executable_1d_open_return_research"
CAND_TABLE = "stock_predict_data_model_agent_four_year_adaptive_rank_fusion_vs_control_20260627_executable_1d_open_return_research"
...
def pass_hard_for_four_year(d: dict[str, dict[str, float]], mstats: dict[str, object]) -> bool:
    return (
        d["full"]["rank_ic"] >= 0.0
        and d["full"]["top5"] >= 0.0
...
def evaluate_mask(
    ...
    hard = pass_hard_for_four_year(d, mstats)
    objective = (
        200.0 * d["full"]["top5"]
        + 80.0 * d["full"]["top1"]
        + 40.0 * d["full"]["rank_ic"]
        + 30.0 * d["recent63"]["top5"]
        + 10.0 * d["recent20"]["top5"]
        + 0.01 * int(mstats["positive_top5_months"])
        - 50.0 * max(0.0, -float(mstats["min_top5_delta"]))
    )
    row = {
        "condition": name,
...
def main() -> int:
    ...
    feature_path = REPORT_DIR / "four_year_1d_exante_features.csv"
    ...
    scan.to_csv(REPORT_DIR / "four_year_1d_exante_gate_scan.csv", index=False, encoding="utf-8-sig")
    ...
    summary = {
        ...
        "scope": "research_only_four_year_1d_exante_gate",
        ...
        "best": best_row,
        "decision": "four_year_exante_gate_found" if best_row and best_row["pass_hard"] else "continue_research_no_four_year_exante_hard_pass",
        "boundaries": {
            "research_only": True,
            "no_training": True,
    ...
    (REPORT_DIR / "four_year_1d_exante_gate_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    lines = [
        "# 四年观察期 1D 前视分数状态切换扫描",
        "",
    ...
    (REPORT_DIR / "four_year_1d_exante_gate_report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
