...
OUTPUT_ROOT = (
    ...
    "strategy_agent_v260_fixed10_price_stress_sell_priority_20260822"
)
CHECKPOINT = (
    REPO
    / "quant/data_file/reports/"
...
def run(context, policy: dict, priority_matrix: np.ndarray, cost: float):
...
def main() -> None:
    ...
    if checkpoint["validation_2026_opened"]:
        raise PermissionError("2026 data entered price-stress priority development")
    policy = copy.deepcopy(checkpoint["selected_policy"])
    context = harness.load_context(policy)
    strong = regime.strong_market_mask(context.score, context.protocol)
    baseline_weak = confirmed.confirmed_weak_mask(strong, 2)
    market_daily = state_diag.cross_sectional_median_return(
        context.arrays["close_qfq"]
    )
    market_trend = state_diag.trailing_compound_return(market_daily, WINDOW)
    price_stress = np.isfinite(market_trend) & (market_trend < STRESS_THRESHOLD)
    expanded_weak = baseline_weak | price_stress
    ...
    priorities = {
        "production_confirmed_weak2": priority.sell_priority_matrix(
            context.score, volatility_rank, ~baseline_weak, PENALTY
        ),
        "confirmed_weak2_or_price_stress": priority.sell_priority_matrix(
            context.score, volatility_rank, ~expanded_weak, PENALTY
        ),
    }
    results, cache = {}, {}
    for candidate_id, matrix in priorities.items():
        cost_metrics = {}
        for cost in COST_LEVELS:
            daily, actions = run(context, policy, matrix, cost)
    ...
    baseline_id = "production_confirmed_weak2"
    candidate_id = "confirmed_weak2_or_price_stress"
    ...
    if not all(checkpoint_equivalence.values()):
        raise RuntimeError("price-stress priority baseline drifted")
    ...
    if not all(deterministic.values()):
        raise RuntimeError("price-stress priority replay failed")
    result = {
        "status": "price_stress_sell_priority_complete_2026_not_opened",
        "only_change": (
            "extend the existing high-volatility-first sell priority to days with "
            "trailing 20-session cross-sectional median return below -5%; exit "
            "eligibility and count remain unchanged"
        ),
        "baseline_confirmed_weak_days": int(np.sum(baseline_weak)),
        "price_stress_days": int(np.sum(price_stress)),
        "new_priority_days": int(np.sum(price_stress & ~baseline_weak)),
        "results": results,
