...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_rank_deterioration_v207_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v207_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 排名恶化过滤阈值研究")
    ...
    rows, definitions = [], {}
    for max_rank_deterioration in protocol["grid"]["max_rank_deterioration"]:
        definition = {
            **protocol["fixed_definition"],
            "max_rank_deterioration": float(max_rank_deterioration),
        }
