...
from research_v260_runtime import fixed10_price_trailing_v109 as price_trailing_v109


OUTPUT_ROOT = (
    REPO
    / "quant/data_file/reports/strategy_agent_v260_fixed10_price_trailing_exit_20260822"
)
PRICE_PEAK_DRAWDOWN_THRESHOLDS = (None, 0.10, 0.15, 0.20)


def candidate_id(threshold: float | None) -> str:
    return (
        "price_peak_drawdown_off"
        if threshold is None
        else f"price_peak_drawdown_{threshold:.2f}"
    )


def candidate_policy(threshold: float | None) -> dict:
    current = daily_exit.candidate_policy(1)
    current["price_peak_drawdown_exit"] = threshold
    return current
...
def main() -> None:
    ...
    if access["logical_max_date"] != round1.DEVELOPMENT_END:
        raise PermissionError("2026 data entered price-trailing development")
    definition = harness.production_definition(protocol)
    score, order = harness.v95.score_pair(arrays, 0.0, 7, 0.1)

    results = {}
    run_cache = {}
    for threshold in PRICE_PEAK_DRAWDOWN_THRESHOLDS:
        ...
        daily, actions = round1.run_fixed10(
            ...
            record_actions=True,
            simulator=price_trailing_v109.simulate,
        )
    ...
    if not all(default_equivalence.values()):
        raise RuntimeError("price-trailing runtime changed the disabled path")
    ...
    if not all(deterministic.values()):
        raise RuntimeError("price-trailing deterministic replay failed")
    ...
    result = {
        ...
        "candidate_budget": [
            ...
            for value in PRICE_PEAK_DRAWDOWN_THRESHOLDS
        ],
        "selection_rule": (
