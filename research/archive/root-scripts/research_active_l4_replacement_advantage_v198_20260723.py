...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_replacement_advantage_v198_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v198_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 卖出替代优势阈值研究")
    ...
    rows, definitions = [], {}
    for replacement_advantage in protocol["grid"]["replacement_advantage"]:
        definition = {**protocol["fixed_definition"], "replacement_advantage": float(replacement_advantage)}
