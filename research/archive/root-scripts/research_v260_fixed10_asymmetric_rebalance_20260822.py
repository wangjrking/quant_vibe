...
OUTPUT_ROOT = (
    REPO
    / "quant/data_file/reports/strategy_agent_v260_fixed10_asymmetric_rebalance_20260822"
)
OVERWEIGHT_BANDS = (0.01, 0.02, 0.03, 10.0)
UNDERWEIGHT_BAND = 0.01


def candidate_id(overweight_band: float) -> str:
    if overweight_band >= 1.0:
        return "never_trim_overweight"
    return f"trim_above_{int(round((0.10 + overweight_band) * 100)):02d}pct"
...
def main() -> None:
    ...
    results = {}
    run_cache = {}
    for overweight_band in OVERWEIGHT_BANDS:
        current_id = candidate_id(overweight_band)
        daily, actions = round1.run_fixed10(
            ...
            portfolio_rebalance_min_weight_deviation_override=UNDERWEIGHT_BAND,
            portfolio_rebalance_overweight_deviation_override=overweight_band,
            portfolio_rebalance_underweight_deviation_override=UNDERWEIGHT_BAND,
        ...
        results[current_id] = {
            "policy": {
                **policy,
                "portfolio_rebalance_overweight_deviation": overweight_band,
                "portfolio_rebalance_underweight_deviation": UNDERWEIGHT_BAND,
            },
    ...
    current = results[candidate_id(0.01)]["metrics_0_30pct"]
    ...
    if not all(checkpoint_equivalence.values()):
        raise RuntimeError("asymmetric-rebalance current path drifted from checkpoint")
    ...
    selected_reason = "aggregate_2022_2024_training_selection; 2025 is confirmation only"
    ...
    repeat_daily, repeat_actions = round1.run_fixed10(
        ...
        portfolio_rebalance_active_override=active,
        portfolio_rebalance_min_weight_deviation_override=UNDERWEIGHT_BAND,
        portfolio_rebalance_overweight_deviation_override=selected_policy[
    ...
    if not all(deterministic.values()):
        raise RuntimeError("asymmetric-rebalance deterministic replay failed")
    ...
    result = {
        ...
        "candidate_budget": [candidate_id(value) for value in OVERWEIGHT_BANDS],
        "only_change": (
            "overweight trim band, including a structural no-trim boundary; "
            "underweight top-up band remains one percentage point"
        ),
        "results": results,
    ...
    print(json.dumps({"selected": selected_id, "results": results, "walk_forward": walk_forward}, ensure_ascii=False, default=str))
