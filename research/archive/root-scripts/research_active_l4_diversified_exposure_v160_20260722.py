...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_diversified_exposure_v160_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v160_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def run_case(arrays, score, order, definition, protocol, end, start=None, record_actions=False):
    gross, base_target = v153.schedules(arrays, definition)
    daily_top_n = max(int(definition["daily_top_n"]), 1)
    per_name_target = base_target * float(definition["cohort_exposure_multiplier"]) / daily_top_n
    return v109.simulate(
        ...
        target_pct_override=per_name_target,
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4分散持仓与总暴露研究")
    ...
    rows, definitions = [], {}
    for top_n, multiplier in product(protocol["grid"]["daily_top_n"], protocol["grid"]["cohort_exposure_multiplier"]):
        definition = {**protocol["fixed_definition"], "daily_top_n": top_n, "cohort_exposure_multiplier": multiplier}
