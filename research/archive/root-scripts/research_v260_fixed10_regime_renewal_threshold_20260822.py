...
OUTPUT_ROOT = (
    ...
    "strategy_agent_v260_fixed10_regime_renewal_threshold_20260822"
)
CHECKPOINT_PATH = (
    REPO
    / "quant/data_file/reports/"
...
CANDIDATES = ("renewal080_all_markets", "renewal080_strong_085_weak")


def renewal_threshold_schedule(strong_market_mask) -> np.ndarray:
    strong = np.asarray(strong_market_mask, dtype=np.bool_)
    return np.where(strong, 0.80, 0.85).astype(np.float64)


def run_policy(context, policy: dict, renewal_override):
    ...
    common = dict(
        ...
        max_hold_renewal_score_override=renewal_override,
    )
...
def main() -> None:
    ...
    strong = regime.strong_market_mask(context.score, context.protocol)
    schedules = {
        "renewal080_all_markets": 0.80,
        "renewal080_strong_085_weak": renewal_threshold_schedule(strong),
    }
    results, cache = {}, {}
    ...
    if not all(checkpoint_equivalence.values()):
        raise RuntimeError("regime renewal baseline drifted")
    ...
    if not all(deterministic.values()):
        raise RuntimeError("regime renewal replay failed")

    selected_policy = copy.deepcopy(policy)
    selected_policy["max_hold_renewal_score_rule"] = (
        "0.80_all_markets"
        if selected == baseline_id
        else "0.80_strong_market_0.85_weak_market"
    )
    result = {
        "status": "regime_renewal_threshold_complete_2026_not_opened",
        ...
        "only_change": (
            "raise the score required to renew a 25-session holding from 0.80 "
            "to 0.85 only in the frozen production weak-market state"
        ),
