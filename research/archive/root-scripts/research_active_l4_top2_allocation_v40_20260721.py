...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_top2_allocation_v40_20260721"
...
def main():
                    ...
                    for total in g["normal_total_pct"]:
                        for strong in g["strong_single_enabled"]:
                            for scheme in g["weight_scheme"]:
                                profile = v39.BuyProfile(top_n, entry, amount, mv, total, strong, g["fixed"]["strong_top_score_min"], g["fixed"]["strong_gap_min"], g["fixed"]["strong_target_pct"], scheme)
    ...
    (OUT / "Top2仓位邻域研究结论.md").write_text("\n".join(["# Top2仓位邻域研究结论", "", "本轮固定V36卖出，只检查Top2连续仓位和流动性邻域。", "", f"固定网格 {len(results)} 组，逐年为正 {int(results.all_year_positive.sum())} 组。", f"冻结 {len(action_rows)} 条独立掘金路径。", "", "2026验证期未打开，生产资产未修改。"]), encoding="utf-8")
