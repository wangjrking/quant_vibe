...
SMOOTHING_WINDOWS = (5, 7, 10)


def context_with_window(context, window: int):
...
def run_case(context, policy: dict, window: int, cost: float):
    case_context = context_with_window(context, window)
...
def main() -> None:
    ...
    if checkpoint["validation_2026_opened"]:
        raise PermissionError("2026 data entered smoothing optimization")
    ...
    if not all(deterministic.values()):
        raise RuntimeError("selected smoothing replay is not deterministic")

    result = {
        "status": "current_smoothing_profit_optimization_complete_2026_not_opened",
        "only_change": "trailing session count used to smooth the frozen score",
        "selection_rule": "maximize_pre2026_cumulative_return_subject_to_basic_risk_floors",
        "candidates": results,
        "selected_smoothing_window": int(selected),
        "selected_minus_current_7d": round1.numeric_delta(
