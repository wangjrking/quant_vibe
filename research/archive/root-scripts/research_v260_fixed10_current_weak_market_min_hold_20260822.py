...
OUTPUT_ROOT = (
    ...
    "strategy_agent_v260_fixed10_current_weak_market_min_hold_20260822"
)
CHECKPOINT_PATH = (
    REPO
    / "quant/data_file/reports/"
...
CANDIDATES = ("current_min_hold", "weak_market_min_hold8")


def weak_market_min_hold_schedule(strong_market: np.ndarray) -> np.ndarray:
    strong = np.asarray(strong_market, dtype=np.bool_)
    return np.where(strong, 4, 8).astype(np.int64)
...
def main() -> None:
    ...
    if checkpoint["validation_2026_opened"]:
        raise PermissionError("2026 validation is unavailable for min-hold research")
    policy = copy.deepcopy(checkpoint["selected_policy"])
    context = harness.load_context(policy)
    strong = regime.strong_market_mask(context.score, context.protocol)
    extra_min_age = weak_market_min_hold_schedule(strong)
    min_hold_schedules = {
        "current_min_hold": None,
        "weak_market_min_hold8": weak_market_min_hold_schedule(strong),
    }
    ...
    for candidate_id in CANDIDATES:
        results[candidate_id], daily, actions = age_guard.run_policy(
            ...
            policy,
            extra_min_age,
            min_hold_days_override=min_hold_schedules[candidate_id],
        )
    ...
    if not all(checkpoint_equivalence.values()):
        raise RuntimeError("weak-market min-hold baseline drifted")
    ...
    repeat, repeat_daily, repeat_actions = age_guard.run_policy(
        ...
        policy,
        extra_min_age,
        min_hold_days_override=min_hold_schedules[selected],
    )
    deterministic = {
    ...
    if not all(deterministic.values()):
        raise RuntimeError("weak-market min-hold replay failed")

    result = {
        "status": "current_weak_market_min_hold_complete_2026_not_opened",
        "candidate_budget": list(CANDIDATES),
        "only_change": (
            "all score exits require eight held sessions on production-defined "
            "weak-market days; strong-market minimum remains four sessions"
        ),
