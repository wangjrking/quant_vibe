...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_market_regime_portfolio_v135_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v135_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def case_protocol(protocol: dict, definition: dict) -> dict:
    result = copy.deepcopy(protocol)
    result["fixed_strategy"] = {"top_n_per_rebalance": int(definition["daily_top_n"]), "max_hold_days": int(definition["max_hold_days"]), "rebalance_every": 1}
    return result


def run_case(arrays, score, order, definition, protocol, end, start=None):
    return v109.simulate(arrays, score, order, definition, case_protocol(protocol, definition), end, start, gross_target_override=v134.gross_schedule(arrays, definition))


def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4历史市场状态门控下的组合结构研究")
    ...
    rows, definitions = [], {}
    grid = protocol["grid"]
    for top_n, max_hold, sell_below in product(grid["daily_top_n"], grid["max_hold_days"], grid["sell_score_below"]):
        definition = {**protocol["fixed_definition"], "daily_top_n": top_n, "max_hold_days": max_hold, "sell_score_below": sell_below}
