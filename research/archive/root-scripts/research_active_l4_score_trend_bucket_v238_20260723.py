...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_score_trend_bucket_v238_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v238_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def target_multiplier(score: np.ndarray, definition: dict) -> np.ndarray:
    lookback = int(definition["score_trend_lookback"])
    tilt = float(definition["trend_bucket_tilt"])
...
def run_case(arrays, score, order, definition, protocol, end, start=None):
    return v162.run_case(
        ...
        candidate_target_multiplier_override=target_multiplier(score, definition),
        max_positions_override=int(definition["max_positions"]),
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Bucketed score-trend sizing research")
