"""Strict 2022-2024 only, daily standardized-target 10D research candidate."""
...
OUT = DATA_DIR / "reports" / "model_agent_10d_daily_zscore_target_dev2022_2024_20260830" / "build_r1"
...
def main() -> int:
    ...
    if (
        contract["candidate_id"] != "expanding_pit_10d_daily_zscore_raw_target_v1"
        or contract["confirmation_window"] != "2025 sealed"
        or not contract["validation_2026_closed"]
        or contract["single_change"]["kind"] != "daily_cross_section_zscore_training_target"
    ):
    ...
    base.dump_json(
        OUT / "preflight.json",
        {
            ...
            "candidate_target_transform": "(target - daily_mean) / daily_population_std on mature training rows",
    ...
    try:
        for fold in folds:
            ...
            target = daily_zscore(train).astype("float32")
            ...
            model.fit(train_matrix, target, sample_weight=weights, verbose=False)
