...
OUT = ROOT / "quant" / "data_file" / "reports" / "strategy_agent_current_formal_l4_top1_gap4_open_stop_20260720"
LOGS = OUT / "logs"
...
STRATEGY = ROOT / "quant" / "data_file" / "reports" / "strategy_agent_current_formal_l4_independent_open_stop_20260720" / "research_code_snapshot"
CASES = (
    [{"name": "disabled", "stop": None, "min_hold": 1}]
    + [
        {"name": f"stop_{int(stop * 100):02d}_h{min_hold}", "stop": stop, "min_hold": min_hold}
        for stop in (0.03, 0.05, 0.07, 0.10, 0.12, 0.15)
        for min_hold in (1, 2)
    ]
)
...
def run_case(case: dict) -> dict:
    ...
    env.update({
        ...
        "GM_LIGHT_STOP_LOSS_PCT": "" if case["stop"] is None else str(case["stop"]),
        "GM_MIN_HOLDING_DAYS_BEFORE_LIGHT_STOP": str(case["min_hold"]),
    })
    command = [
