...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_5d_recent_top_condblend_scan_20260626"
...
CANDIDATES = {
    "v3_topgate_3drefined": "stock_predict_data_model_agent_5d_v3_topgate_3drefined_20260623_executable_5d_open_return_research",
    "v3_local_reblend": "stock_predict_data_model_agent_5d_v3_local_reblend_1dbest_10dstd035_20260623_executable_5d_open_return_research",
    "daily_gate_d3d1": "stock_predict_data_model_agent_5d_daily_gate_d3d1_20260623_executable_5d_open_return_research",
    "daily_gate_d3d1_v2": "stock_predict_data_model_agent_5d_daily_gate_d3d1_v2_20260623_executable_5d_open_return_research",
}
TARGET_TABLE = "stock_predict_data_model_agent_5d_recent_top_condblend_best_20260626_executable_5d_open_return_research"
...
ALPHAS = [0.15, 0.25, 0.35, 0.50, 0.70, 1.00]
...
def merge_daily(base_daily: pd.DataFrame, cand_daily: pd.DataFrame, active_dates: set[str], alpha: float) -> pd.DataFrame:
    ...
    for col in ["rank_ic", "top1", "top5", "top10", "top20", "top_bottom"]:
        out[col] = np.where(use_cand, (1.0 - alpha) * merged[f"{col}_base"] + alpha * merged[f"{col}_cand"], merged[f"{col}_base"])
    return out
...
def objective(
    ...
    return (
        ...
        + 1.1 * (cur_63["top1"] - base_63["top1"])
        + 1.0 * (cur_63["top5"] - base_63["top5"])
        + 0.9 * (cur_full["rank_ic"] - base_full["rank_ic"])
...
def write_candidate(scores: pd.DataFrame, best: pd.Series, daily_features: pd.DataFrame) -> dict[str, object]:
    ...
    with sqlite3.connect(MODEL_DB, timeout=300) as conn:
        ...
        conn.execute(f"create index if not exists idx_5d_recent_top_condblend_best_date_code on {quote(TARGET_TABLE)}(trade_date, stock_code)")
        conn.commit()
        row = conn.execute(
...
def main() -> int:
            ...
            for threshold in thresholds:
                for op in ["<=", ">="]:
                    ...
                    if len(active_dates) < 6 or len(active_dates) > 180:
                        continue
                    for alpha in ALPHAS:
                        daily = merge_daily(base_daily, cand_daily, active_dates, alpha)
                        ...
                        row["pass_basic"] = bool(
                            ...
                            and row["recent63_top5_delta"] >= -0.0005
    ...
    summary_obj = {
        ...
        "scope": "research_only_5d_recent_top_condblend_scan",
    ...
    (REPORT_DIR / "recent_top_condblend_summary.json").write_text(
