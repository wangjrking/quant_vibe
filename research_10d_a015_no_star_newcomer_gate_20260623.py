...
OUT_DIR = DATA_DIR / "reports" / "model_agent_10d_a015_no_star_newcomer_gate_20260623"
...
RESEARCH_TABLE = "stock_predict_data_model_agent_10d_a015_no_star_newcomer_gate_20260623_executable_10d_open_return_research"
...
def main() -> int:
    ...
    for trade_date, group in frame.groupby("trade_date", sort=True):
        g = group.copy()
        base_top5 = g.sort_values("base_pred_prob", ascending=False).head(5)
        source_top5 = g.sort_values("source_pred_prob", ascending=False).head(5)
        base_codes = set(base_top5["stock_code"])
        newcomers = source_top5[~source_top5["stock_code"].isin(base_codes)].copy()
        has_star_newcomer = bool((newcomers["board"] == "STAR").any())
        g["pred_prob"] = g["base_pred_prob"] if has_star_newcomer else g["source_pred_prob"]
        g["used_base_fallback"] = has_star_newcomer
        output_parts.append(g)
        daily_flags.append(
            {
                "trade_date": trade_date,
                "used_base_fallback": has_star_newcomer,
    ...
    summary = {
        ...
        "candidate_id": "model_agent_10d_a015_no_star_newcomer_gate_20260623",
        "asset_role": "research_l4_candidate_not_formal",
        "approval_status": "research_only_not_approved_for_l4_or_l5",
        "base_table": BASE_TABLE,
        "source_table": SOURCE_TABLE,
        "rule": "if candidate top5 contains newcomer STAR stock, fallback to base 10D score for that trade_date; otherwise keep a015 score",
        "base_eval": base_eval,
    ...
    manifest = {
        ...
        "notes": "10D 候选在 a015_z0015 基础上增加日级门控：若当日候选 Top5 出现新换入的 STAR(688) 股票，则整日退回 base 10D 分数。该资产仅用于 research 验证，不进入 formal L4/L5。",
    }
    ...
    lines = [
        "# 10D a015 无 STAR 新换入门控候选",
        "",
        "## 当前结论",
        "",
        "本轮不是再改单只股票排序，而是做日级门控。",
        "若当日 `a015_z0015` 的 Top5 中出现新换入的 STAR(688) 股票，则整日退回 `base 10D`；否则保留 `a015_z0015`。",
        "这个规则的目标，是保住 `a015` 在近期 `Top1/Top3` 上的优势，同时减少少量高风险 STAR 新换入日对 `Top5` 的伤害。",
        "",
        "## 关键结果",
        "",
