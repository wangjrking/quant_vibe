...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_soft_deterioration_sizing_v183_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v183_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def soft_multiplier(arrays: dict, definition: dict) -> np.ndarray:
    raw = arrays["rank_10d"].astype(np.float32)
    smooth = v94.rolling_nanmean(raw, 7)
    result = np.ones(raw.shape, dtype=np.float32)
    weak = np.isfinite(raw) & np.isfinite(smooth) & ((raw - smooth) <= -float(definition["soft_deterioration_threshold"]))
    result[weak] = float(definition["soft_target_multiplier"])
    return result
...
def run_case(arrays, score, order, definition, protocol, end, start=None):
    return v162.run_case(
        ...
        candidate_target_multiplier_override=soft_multiplier(arrays, definition),
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 soft deterioration sizing study")
