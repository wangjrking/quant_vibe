...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_exit_neighborhood_v156_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v156_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4独立卖出规则邻域研究")
    ...
    rows, definitions = [], {}
    grid = protocol["grid"]
    for min_hold, max_hold, sell_floor, advantage in product(
        grid["min_hold_days"],
        grid["max_hold_days"],
