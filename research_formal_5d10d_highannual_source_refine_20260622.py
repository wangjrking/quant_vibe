...
REPORT_DIR = (
    ...
    / "formal_5d10d_highannual_source_refine_20260622"
)
SOURCES = {
...
def _variant(
    ...
    return {
        ...
        "target_position_pct": target_cap,
        "max_positions": 5,
...
VARIANTS = []
for src, cap in [("goal_rank5_23", 0.23), ("weak_rw2422", 0.24)]:
    VARIANTS.extend(
        [
            _variant(src, "repro", target_cap=cap),
            _variant(src, "h6_mx9", hold=6, max_hold=9, target_cap=cap),
            _variant(src, "h8_mx12", hold=8, max_hold=12, target_cap=cap),
            _variant(src, "cont105", continue_ratio=1.05, target_cap=cap),
            _variant(src, "exit098_mh3", exit_ratio=0.98, min_hold_exit=3, target_cap=cap),
            _variant(src, "exit102_mh3", exit_ratio=1.02, min_hold_exit=3, target_cap=cap),
            _variant(src, "exit100_mh4", min_hold_exit=4, target_cap=cap),
            _variant(src, "mds0", max_daily_sells=0, target_cap=cap),
            _variant(src, "mds2", max_daily_sells=2, target_cap=cap),
            _variant(src, "noeqdd", target_cap=cap, extra_env={"GM_EQUITY_DD_RISK_MODE": "0"}),
            _variant(
                src,
                "defer20",
                target_cap=cap,
                extra_env={"GM_DEFER_EXITS_WITHOUT_BUY_SIGNAL": "1", "GM_NO_SIGNAL_MAX_HOLDING_DAYS": "20"},
            ),
            _variant(
                src,
                "h8_defer20",
...
def _write_report(rows: list[dict]) -> None:
    ...
    report = f"""# 高年化候选池二次调参结论

## 结论

本轮复用已存在的 formal 5D+10D 高年化研究候选信号，重新测试持有期、评分卖出、最大日卖出、账户回撤缩放和无信号延迟退出。没有加入行业过滤、月份过滤、日期过滤、外部数据或其他周期模型。
...
如果高年化候选池经过二次执行调参仍无法提升到目标，说明当前 formal 5D+10D 资产下，收益、夏普和持仓率三者存在明显冲突。未显著优于当前 L5 前，不进入生产替换。
...
"""
    (REPORT_DIR / "高年化候选池二次调参结论.md").write_text(report, encoding="utf-8")
