...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_filtered_5d_blend_v186_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v186_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def run_case(arrays, definition, protocol, end, start=None):
    score, order = v95.score_pair(arrays, float(definition["weight_5d"]), 7, 0.1)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 filtered 5D blend study")
    ...
    rows, definitions = [], {}
    for weight in protocol["grid"]["weight_5d"]:
        definition = {**protocol["fixed_definition"], "weight_5d": float(weight)}
