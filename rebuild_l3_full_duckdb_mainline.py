...
UNIFIED_PYTHON_LAUNCHER = PROJECT_ROOT / ".venv" / "Scripts" / "python.exe"
...
def _precheck(args: argparse.Namespace) -> dict[str, Any]:
    ...
    startup_memory_limit = 64 * 1024**3
    ...
    gates = {
        ...
        "startup_available_at_least_64gb": available_memory >= startup_memory_limit,
...
def _configure_worker_duckdb(conn: duckdb.DuckDBPyConnection, memory_limit: str) -> None:
...
def _build_raw_bucket(task: dict[str, Any]) -> dict[str, Any]:
    ...
    with closing(duckdb.connect(str(source_path), read_only=True)) as source, closing(duckdb.connect(str(output_path))) as target:
        _configure_worker_duckdb(source, task["duckdb_memory_limit"])
        _configure_worker_duckdb(target, task["duckdb_memory_limit"])
        created = False
        for code in codes:
            frame = source.execute(
                f"SELECT * FROM {_quote_ident(source_table)} WHERE stock_code = ? ORDER BY trade_date",
                [code],
            ).fetchdf()
            if frame.empty:
                continue
            ...
            try:
                if not created:
                    target.execute("CREATE TABLE raw_factor AS SELECT * FROM factor_frame")
                    created = True
                else:
                    target.execute("INSERT INTO raw_factor BY NAME SELECT * FROM factor_frame")
            finally:
...
def _merge_shards(shards: Iterable[Path], output_path: Path, table: str, source_table: str) -> dict[str, Any]:
    started = time.perf_counter()
    output_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    with closing(duckdb.connect(str(temp_path))) as target:
        ...
        for index, shard in enumerate(sorted(shards)):
            ...
            if not created:
                target.execute(
                    f"CREATE TABLE {_quote_ident(table)} AS SELECT * FROM {_quote_ident(alias)}.{_quote_ident(source_table)}"
                )
                created = True
            else:
                target.execute(
                    f"INSERT INTO {_quote_ident(table)} BY NAME SELECT * FROM {_quote_ident(alias)}.{_quote_ident(source_table)}"
                )
            total += count
            target.execute(f"DETACH {_quote_ident(alias)}")
        if not created:
            raise RuntimeError(f"no shards available for {table}")
        target.execute(f"CREATE INDEX idx_{table}_key ON {_quote_ident(table)}(trade_date, stock_code)")
    ...
    if target_rows != total or duplicate:
        temp_path.unlink(missing_ok=True)
        raise RuntimeError(f"shard merge integrity failure table={table} source={total} target={target_rows} duplicate={duplicate}")
    os.replace(temp_path, output_path)
    return {"path": str(output_path), "table": table, "row_count": target_rows, "duplicate_key_groups": duplicate, "elapsed_seconds": round(time.perf_counter() - started, 3)}


def _build_raw_stage(precheck: dict[str, Any], args: argparse.Namespace, workspace: Path, logger) -> dict[str, Any]:
    policy = MemoryPolicy(workers=args.workers)
    ...
    merged = _merge_shards([Path(item["path"]) for item in results], output_path, "raw_factor", "raw_factor")
...
def _build_gtja_chunk(task: dict[str, Any]) -> dict[str, Any]:
    ...
    with closing(duckdb.connect(str(raw_path), read_only=True)) as conn:
        _configure_worker_duckdb(conn, task["duckdb_memory_limit"])
        frame = conn.execute(
    ...
    with closing(duckdb.connect(str(output_path))) as target:
        ...
        try:
            target.execute("CREATE TABLE gtja_factor AS SELECT * FROM gtja_frame")
            duplicate = int(target.execute(
...
def _build_gtja_stage(raw_stage: dict[str, Any], args: argparse.Namespace, workspace: Path, logger) -> dict[str, Any]:
    ...
    merged = _merge_shards([Path(item["path"]) for item in results], output_path, "gtja_factor", "gtja_factor")
...
def _candidate_table_metrics(path: Path, table: str, target_date: str) -> dict[str, Any]:
    with closing(duckdb.connect(str(path), read_only=True)) as conn:
        metrics = _key_metrics(conn, table, target_date)
        schema = _schema(conn, table)
    return {**metrics, "column_count": len(schema), "schema_hash": _schema_hash(schema), "columns": [item["name"] for item in schema]}
...
def _build_feature_candidate(
    ...
    with closing(duckdb.connect(str(temp_path))) as conn:
        ...
        conn.execute(f"CREATE INDEX idx_feature_key ON {_quote_ident(table)}(trade_date, stock_code)")
...
def _build_label_candidate(precheck: dict[str, Any], raw_stage: dict[str, Any], args: argparse.Namespace, workspace: Path, logger) -> dict[str, Any]:
    ...
    projections = ["CAST(trade_date AS VARCHAR) AS trade_date", "stock_code", *[_label_expr(label, raw_columns) for label in labels]]
    candidate_dir = workspace / "candidates"
    candidate_dir.mkdir(parents=True, exist_ok=True)
    ...
    with closing(duckdb.connect(str(temp_path))) as conn:
        ...
        conn.execute(f"CREATE INDEX idx_label_key ON {_quote_ident(table)}(trade_date, stock_code)")
        conn.execute("DETACH rawsrc")
    os.replace(temp_path, candidate_path)
    metrics = _candidate_table_metrics(candidate_path, table, args.target_trade_date)
    columns = metrics.pop("columns")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--gtja-date-chunk-days", type=int, default=60)
    parser.add_argument("--gtja-alpha-batch-size", type=int, default=4)
    parser.add_argument("--workers", type=int, default=2)
    parser.add_argument("--report-dir", required=True)
    parser.add_argument("--workspace-dir")
    parser.add_argument("--expected-l2-sha256")
    parser.add_argument("--feature-contract-version", choices=("v1", "v2"), default="v1")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.workers > 2:
        raise ValueError("full-history L3 short-lived scheduler allows at most 2 workers")
    ...
    if args.mode == "build" and workspace.exists() and any(workspace.rglob("*")):
        raise RuntimeError(f"existing non-empty workspace prohibits resume: {workspace}")
