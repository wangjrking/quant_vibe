...
OUT_DIR = DATA_DIR / "reports" / "model_agent_10d_with_1dgate092_alpha012_20260623"
...
RESEARCH_TABLE = "stock_predict_data_model_agent_10d_dategate_3dformal_plus_1dgate092_a012_20260623_executable_10d_open_return_research"
...
def main() -> int:
    ...
    summary = {
        ...
        "candidate_id": "model_agent_10d_dategate_3dformal_plus_1dgate092_a012_20260623",
        "asset_role": "research_l4_candidate_not_formal",
        "approval_status": "research_only_not_approved_for_l4_or_l5",
        "base_table": BASE_TABLE,
        "aux_table": AUX_TABLE,
        "score_formula": f"if trade_date >= {START_DATE} and base_rank >= {threshold:.3f}: base_rank + {ALPHA:.3f} * aux_rank else base_rank",
        "base_eval": base_eval,
    ...
    manifest = {
        ...
        "notes": "10D 基于已存在的 dategate+3D formal research 候选，在近期头部区域引入 1D gate092 研究分数做局部重排，并将 alpha 提高到 0.012。该资产仅供 research 验证，不进入 formal L4/L5。",
    }
    ...
    lines = [
        "# 10D 叠加 1D Gate092 Alpha012 研究候选",
        "",
        "## 当前结论",
        "",
        "本轮在现有 `10D dategate + 3D formal` 候选基础上，只对近期头部区域叠加 `1D gate092` 分数，并把 `alpha` 从 `0.01` 提到 `0.012`。",
        "结果显示，这版相对上一版更均衡，`recent126 Top5` 转正，`recent63 Top1/Top3` 也继续改善，值得保留为新的 research-only 候选。",
        "",
        "## 参数",
        "",
        f"- 起始日期：`{START_DATE}`",
        f"- 头部区域：`top {ZONE_PCT:.1%}`",
        f"- 叠加系数：`{ALPHA}`",
