...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_position_concentration_v140_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v140_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4市场门控策略集中度压力研究")
    ...
    rows, definitions = [], {}
    for target_cohorts in protocol["grid"]["target_cohorts"]:
        definition = {**protocol["fixed_definition"], "target_cohorts": target_cohorts}
