...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_research_5d_current_vs_fixed4y_daygate_scan_20260626"
...
def enrich_scores(base: pd.DataFrame, alt: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for trade_date, group in scores.groupby("trade_date", sort=True):
        base_order = group.nlargest(min(20, len(group)), "base_rank")["stock_code"]
        alt_order = group.nlargest(min(20, len(group)), "alt_rank")["stock_code"]
        top10_base = set(base_order.head(10))
        top10_alt = set(alt_order.head(10))
        top20_base = set(base_order)
        top20_alt = set(alt_order)
        daily_rows.append(
            {
...
def main() -> int:
    ...
    result = pd.DataFrame(rows).sort_values(["objective", "top5", "top1"], ascending=False).reset_index(drop=True)
    best = result.iloc[0].to_dict()

    summary = {
        "generated_at": now_iso(),
        "scope": "research_only_5d_current_vs_fixed4y_daygate_fold08",
        "label": LABEL,
    ...
    lines = [
        "# 5D 当前主线对近四年训练候选的日级门控扫描报告（20260626）",
        "",
        "## 当前结论",
        "",
        ...
        "- 如果最优门控仍然打不过当前主线，说明近四年训练候选在当前 5D 主线上没有形成足够稳定的增益日特征。",
        "",
        "## 产物",
        "",
