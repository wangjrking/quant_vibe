...
OUT_DIR = DATA / "reports" / "strategy_agent_adaptive70w_fouryear_1d_gate_20260629"
...
MANIFEST_1D = MAIN / "config" / "prediction_manifests" / "executable_1d_open_return_l4_formal_20260619.json"
...
GATES = [
    {"name": "g00", "rank_floor": 0.00},
    {"name": "g55", "rank_floor": 0.55},
    {"name": "g60", "rank_floor": 0.60},
    {"name": "g65", "rank_floor": 0.65},
    {"name": "g70", "rank_floor": 0.70},
    {"name": "g75", "rank_floor": 0.75},
]
...
def _load_1d_source() -> dict[str, str]:
    source = load_prediction_source_manifest(MANIFEST_1D, require_approved=True, allow_legacy=False)
    ...
    if market_db_path and market_db_path.resolve() != MARKET_DUCKDB.resolve():
        raise RuntimeError(f"{MANIFEST_1D} market_db_path mismatch: {market_db_path} != {MARKET_DUCKDB}")
    return {"db_path": str(db_path), "table": str(source["table"])}


SOURCE_1D = _load_1d_source()


def _case_key(target: float, gate: dict[str, Any]) -> str:
    return f"{BASE_CASE}__amt150000_mv300000_pc150_pos{int(target * 100)}_cool2d20_1d{gate['name']}"


def _ensure_signal(target: float, gate: dict[str, Any]) -> dict[str, Any]:
    case_key = _case_key(target, gate)
    ...
    try:
        ...
        con.execute(f"ATTACH '{Path(SOURCE_1D['db_path']).as_posix()}' AS l4_1d (READ_ONLY)")
        ...
        scored AS (
            ...
            LEFT JOIN rank_1d r1
              ON s.trade_date = r1.trade_date AND s.stock_code = r1.stock_code
            WHERE NOT (s.stock_code LIKE '%.BJ' OR substr(s.stock_code, 1, 1) IN ('4', '8'))
              ...
              AND coalesce(r1.rank_1d, 0.0) >= {gate["rank_floor"]}
        ),
        with_buy_checks AS (
            SELECT
                s.*,
...
def _run(case_meta: dict[str, Any], target: float, gate: dict[str, Any], tag: str, start: str, end: str) -> dict[str, Any]:
...
def main() -> None:
    ...
    for target in TARGETS:
        for gate in GATES:
            case_meta = _ensure_signal(target, gate)
            rows = []
            for tag, start, end in SLICES:
                row = _run(case_meta, target, gate, tag, start, end)
                detail.append(row)
                rows.append(row)
                _write_rows(OUT_DIR / "gate_detail.csv", detail)
            ...
            _write_rows(OUT_DIR / "gate_summary.csv", list(summary.values()))
