...
import research_v260_fixed10_min_hold_20260822 as round8
...
OUTPUT_ROOT = (
    REPO
    / "quant/data_file/reports/strategy_agent_v260_fixed10_reentry_cooldown_20260822"
)
REENTRY_COOLDOWN_DAYS = (0, 2, 5)


def candidate_policy(cooldown_days: int) -> dict:
    current = round8.candidate_policy(4)
    current["reentry_cooldown_days"] = int(cooldown_days)
    return current
...
def main() -> None:
    ...
    if access["logical_max_date"] != round1.DEVELOPMENT_END:
        raise PermissionError("2026 data entered reentry-cooldown development")
    definition = harness.production_definition(protocol)
    score, order = harness.v95.score_pair(arrays, 0.0, 7, 0.1)

    results = {}
    run_cache = {}
    for cooldown_days in REENTRY_COOLDOWN_DAYS:
        candidate_id = f"reentry_cooldown_{cooldown_days}d"
        current = candidate_policy(cooldown_days)
    ...
    if not all(deterministic.values()):
        raise RuntimeError("reentry-cooldown deterministic replay failed")
    ...
    result = {
        ...
        "candidate_budget": [int(value) for value in REENTRY_COOLDOWN_DAYS],
        "selection_rule": (
