...
OUT_DIR = DATA_DIR / "reports" / "model_agent_10d_proxy5d_lite_daygate_std035_20260623"
...
PREVIOUS_BEST_TABLE = "stock_predict_data_model_agent_10d_proxy5d_lite_daygate_std004_20260623_executable_10d_open_return_research"
RESEARCH_TABLE = "stock_predict_data_model_agent_10d_proxy5d_lite_daygate_std035_20260623_executable_10d_open_return_research"
...
def main() -> int:
    ...
    payload = {
        ...
        "candidate_id": "model_agent_10d_proxy5d_lite_daygate_std035_20260623",
    ...
    manifest = {
        ...
        "notes": "10D 研究候选沿用原先的 5D proxy + lite top-zone 重排方案，但将日级 gate 从 std<0.04 放宽到 std<0.35。这个变更不依赖标签，只依赖当日预测分布，用于放大 lite 方案在更多交易日的有效增益。",
    }
    ...
    report_lines = [
        ...
        "## 当前结论",
        "",
        f"- 上一版最优候选：`{PREVIOUS_BEST_TABLE}`",
        f"- 新研究候选：`{RESEARCH_TABLE}`",
        f"- 上一版 Focus：`{payload['previous_focus']:.12f}`",
        f"- 新候选 Focus：`{payload['selected_focus']:.12f}`",
        f"- Focus 增量：`{payload['selected_minus_previous_focus']:.12f}`",
        "",
        "## 规则",
        "",
        "- 默认仍使用 5D proxy 作为 10D 排序基底。",
        "- lite 候选内部规则不变，只调整日级 gate 阈值。",
        f"- 原 gate：`std(rank10d of base top5) < 0.04`",
        f"- 新 gate：`std(rank10d of base top5) < {STD_GATE}`",
        "",
        "## 窗口结果",
        "",
