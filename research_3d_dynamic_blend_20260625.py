...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_3d_dynamic_blend_research_20260625"
TARGET_TABLE = "stock_predict_data_model_agent_3d_dynamic_blend_20260625_executable_3d_open_return_research"
...
SOURCES = {
    ...
    "guard3": "stock_predict_data_model_agent_dynamic_practical_std_guard_20260625_executable_3d_open_return_research",
    ...
    "cross5": "stock_predict_data_model_agent_5d_cross_horizon_guard_20260625_executable_5d_open_return_research",
...
def load_scores() -> pd.DataFrame:
    with sqlite3.connect(MODEL_DB) as conn:
        out = read_source(conn, "formal3", SOURCES["formal3"])
    ...
    for alias in SOURCES:
        ...
        out[score_col] = out[score_col].fillna(out["formal3_score"])
...
def build_features(scores: pd.DataFrame) -> pd.DataFrame:
    ...
    for trade_date, group in scores.groupby("trade_date", sort=True):
        base10 = top_set(group, "formal3_rank", 10)
        base50 = top_set(group, "formal3_rank", 50)
        ...
        for alias in SOURCES:
            ...
            if alias != "formal3":
                row[f"corr_formal3_{alias}"] = float(group["formal3_rank"].corr(group[rank_col]))
                row[f"overlap10_formal3_{alias}"] = len(base10 & top_set(group, rank_col, 10))
                row[f"overlap50_formal3_{alias}"] = len(base50 & top_set(group, rank_col, 50))
        rows.append(row)
...
def scan(scores: pd.DataFrame, labels: pd.DataFrame, features: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    base = metric_by_source[metric_by_source["source"] == "formal3"].set_index("trade_date")
    feature_idx = features.set_index("trade_date")
    rows = []
    daily_rows = []
    for alt in [a for a in SOURCES if a != "formal3"]:
...
def write_candidate(scores: pd.DataFrame, features: pd.DataFrame, best: pd.Series) -> dict:
    ...
    active_dates = set(active[active].index.astype(str))
    out = scores[["trade_date", "stock_code", "formal3_score", "formal3_rank", f"{alt}_score", f"{alt}_rank"]].copy()
    out["guard_active"] = out["trade_date"].isin(active_dates)
    out["pred_prob"] = np.where(out["guard_active"], out[f"{alt}_rank"], out["formal3_rank"])
    out["score_formula"] = f"if {feature} {op} {threshold:.12g} then {alt}_rank else formal3_rank"
    out = out.rename(columns={"formal3_score": "formal3", f"{alt}_score": alt})
    keep = ["trade_date", "stock_code", "pred_prob", "formal3", alt, "formal3_rank", f"{alt}_rank", "guard_active", "score_formula"]
...
def write_outputs(scan_result: pd.DataFrame, daily: pd.DataFrame, source_metrics: pd.DataFrame, stats: dict | None) -> None:
    ...
    lines = ["# 3D 跨周期动态守卫 V3 研究报告（20260625）", "", "## 当前结论", ""]
    if stats:
        lines.extend(
            [
                "本轮找到一个通过基础模型评价门槛的 3D cross-horizon V2 research-only 候选。",
                "",
                "候选规则：",
                "",
                "```text",
        ...
        lines.append("本轮未找到通过基础模型评价门槛的 3D cross-horizon 候选。")
    lines.extend(
        [
            "",
            "## 证据路径",
...
def scan(scores: pd.DataFrame, labels: pd.DataFrame, features: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    rows = []
    daily_rows = []
    alt_sources = ["cross5v2"]
    alpha_grid = [0.25, 0.50, 0.75, 1.00]
    allowed_features = ["corr_formal3_cross5v2", "overlap50_formal3_cross5v2"]
    threshold_quantiles = [0.10, 0.25, 0.33, 0.50]

    for alt in alt_sources:
        alt_rank_col = f"{alt}_rank"
        active_eval = eval_frame[["trade_date", "stock_code", "formal3_rank", alt_rank_col, LABEL]].copy()
        candidate_features = [c for c in allowed_features if c in feature_idx.columns]
        for feature in candidate_features:
            vals = feature_idx[feature].replace([np.inf, -np.inf], np.nan).dropna()
            if vals.nunique() < 3:
                continue
            thresholds = sorted(set(float(vals.quantile(q)) for q in threshold_quantiles))
            for threshold in thresholds:
                for op in ["<=", ">="]:
                    active = feature_idx[feature] <= threshold if op == "<=" else feature_idx[feature] >= threshold
                    active_dates = set(active[active].index.astype(str))
                    if len(active_dates) < 10 or len(active_dates) > int(0.55 * len(feature_idx)):
                        continue
                    active_eval_dates = [d for d in active_dates if d in base.index]
                    if len(active_eval_dates) < 10:
                        continue
                    active_frame = active_eval[active_eval["trade_date"].isin(active_eval_dates)].reset_index(drop=True)
                    if active_frame.empty:
                        continue
                    for alpha in alpha_grid:
                        active_frame["blend_rank"] = (1.0 - alpha) * active_frame["formal3_rank"] + alpha * active_frame[alt_rank_col]
                        active_daily = source_daily_metrics(
                            active_frame.rename(columns={"blend_rank": "blend_rank"}),
                            "blend",
                        ).set_index("trade_date")
                        cand = base.copy()
                        cand.loc[active_eval_dates, :] = active_daily.loc[active_eval_dates, cand.columns]
                        cand = cand.reset_index()
                        merged = cand.merge(base.reset_index(), on="trade_date", suffixes=("", "_base"))
                        for metric in ["rank_ic", "pearson_ic", "top_bottom", "top1", "top3", "top5", "top10", "top20", "top50"]:
                            merged[f"{metric}_delta"] = merged[metric] - merged[f"{metric}_base"]
...
def write_candidate(scores: pd.DataFrame, features: pd.DataFrame, best: pd.Series) -> dict:
    ...
    out = scores[["trade_date", "stock_code", "formal3_score", "formal3_rank", f"{alt}_score", f"{alt}_rank"]].copy()
    out["guard_active"] = out["trade_date"].isin(active_dates)
    out["blend_rank"] = (1.0 - alpha) * out["formal3_rank"] + alpha * out[f"{alt}_rank"]
    out["pred_prob"] = np.where(out["guard_active"], out["blend_rank"], out["formal3_rank"])
    out["alpha"] = alpha
    out["score_formula"] = f"if {feature} {op} {threshold:.12g} then (1-{alpha:.2f})*formal3_rank + {alpha:.2f}*{alt}_rank else formal3_rank"
    out = out.rename(columns={"formal3_score": "formal3", f"{alt}_score": alt})
    keep = [
        "trade_date",
        "stock_code",
...
def write_outputs(scan_result: pd.DataFrame, daily: pd.DataFrame, source_metrics: pd.DataFrame, stats: dict | None) -> None:
    REPORT_DIR.mkdir(parents=True, exist_ok=True)
    scan_path = REPORT_DIR / "dynamic_blend_scan_results.csv"
    daily_path = REPORT_DIR / "dynamic_blend_daily_deltas.csv"
    source_path = REPORT_DIR / "dynamic_blend_source_daily_metrics.csv"
    scan_result.to_csv(scan_path, index=False, encoding="utf-8-sig")
    daily.to_csv(daily_path, index=False, encoding="utf-8-sig")
    source_metrics.to_csv(source_path, index=False, encoding="utf-8-sig")
    best = scan_result.iloc[0].to_dict() if not scan_result.empty else None
    ...
    manifest_path = REPORT_DIR / "dynamic_blend_research_manifest.json"
    manifest_path.write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")
    lines = [
        "# 3D 动态混合权重研究报告（20260625）",
        "",
        "## 当前结论",
        "",
    ]
    if stats:
        lines.extend(
            [
                "本轮找到一个通过基础模型评价门槛的 3D research-only 候选。",
                "",
                "候选规则：",
                "",
                "```text",
        ...
        lines.append("本轮未找到通过基础模型评价门槛的 3D 动态混合候选。")
    lines.extend(
        [
            "",
            "## 证据路径",
    ...
    (REPORT_DIR / "dynamic_blend_research_report.md").write_text("\n".join(lines), encoding="utf-8")
