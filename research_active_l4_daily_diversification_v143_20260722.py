...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_daily_diversification_v143_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v143_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def run_case(arrays, score, order, definition, protocol, end, start=None):
    return v109.simulate(
        ...
        start,
        gross_target_override=v134.gross_schedule(arrays, definition),
        target_cohorts_override=int(definition["target_cohorts"]),
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4策略每日选股分散度研究")
    ...
    rows, definitions = [], {}
    for daily_top_n, target_cohorts in product(protocol["grid"]["daily_top_n"], protocol["grid"]["target_cohorts"]):
        definition = {**protocol["fixed_definition"], "daily_top_n": daily_top_n, "target_cohorts": target_cohorts}
