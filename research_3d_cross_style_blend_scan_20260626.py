...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_3d_cross_style_blend_scan_20260626"
...
TARGET_TABLE = "stock_predict_data_model_agent_3d_rankic_recent_focus_topnblend_20260626_executable_3d_open_return_research"
SOURCES = {
    "base": BASE_TABLE,
    "guarded_lowvol": "stock_predict_data_model_agent_3d_guarded_lowvol_20260623_executable_3d_open_return_research",
    "proxy_bestmix": "stock_predict_data_model_agent_3d_proxy10d5d_bestmix_20260623_executable_3d_open_return_research",
    "stability_anchor": "stock_predict_data_model_agent_3d_stability_anchor_fast_20260624_executable_3d_open_return_research",
}
...
def write_candidate(scores: pd.DataFrame, best: pd.Series) -> dict[str, object]:
    ...
    with sqlite3.connect(MODEL_DB, timeout=300) as conn:
        ...
        conn.execute(f"create index if not exists idx_3d_cross_style_blend_date_code on {quote(TARGET_TABLE)}(trade_date, stock_code)")
        conn.commit()
        row = conn.execute(
...
def main() -> int:
    ...
    rows = []
    for alt in ["guarded_lowvol", "proxy_bestmix", "stability_anchor"]:
        for w in [0.05, 0.10, 0.15, 0.20, 0.25, 0.30]:
            rank_col = f"blend_{alt}_{int(w*100):02d}"
            ...
            row["pass_basic"] = bool(
                row["recent20_top5_delta"] >= 0.00050
                and row["recent63_top5_delta"] >= 0.00020
                and row["full_rank_ic_delta"] >= -0.00100
            )
    ...
    summary_obj = {
        ...
        "scope": "research_only_3d_cross_style_blend_scan",
