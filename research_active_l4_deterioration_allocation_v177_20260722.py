...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_deterioration_allocation_v177_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v177_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 deterioration allocation study")
    ...
    rows, definitions = [], {}
    for target in protocol["grid"]["normal_target_pct"]:
        definition = {**protocol["fixed_definition"], "normal_target_pct": float(target), "strong_target_pct": min(float(target) * 1.5, 0.55)}
    ...
    frame["eligible"] = ((frame["min_year_cumulative_return"] >= 0) & (frame["full_linear_annual_proxy"] >= 0.08) & (frame["full_sharpe"] >= 0.5) & (frame["full_max_drawdown"] <= 0.45) & (frame["recent60_linear_annual_proxy"] >= 0) & (frame["recent120_linear_annual_proxy"] >= 0.1))
