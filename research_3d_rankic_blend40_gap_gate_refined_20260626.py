...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_3d_rankic_blend40_gap_gate_refined_20260626"
...
TARGET_TABLE = "stock_predict_data_model_agent_3d_rankic_blend40_gap_gate_refined_20260626_executable_3d_open_return_research"
ALT_PATH = (
    DATA_DIR
    / "reports"
...
FEATURE = "mean_abs_rank_gap"
THRESHOLD = 0.38658796692427944
        ...
        gate_rows.append(
            {
                ...
                FEATURE: float((group["base_rank"] - group["alt_rank"]).abs().mean()),
            }
        )
...
def evaluate(scored: pd.DataFrame, labels: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, dict[str, object]]:
    ...
    candidate = daily_metrics(eval_frame, "pred_prob", "blend40_gap_gate_refined")
    formal = daily_metrics(eval_frame, "formal_rank", "formal")
    base = daily_metrics(eval_frame, "base_rank", "base_rankic_v5")
...
def write_outputs(
    ...
    metrics_path = REPORT_DIR / "gap_gate_refined_daily_metrics.csv"
    delta_formal_path = REPORT_DIR / "gap_gate_refined_delta_vs_formal.csv"
    delta_base_path = REPORT_DIR / "gap_gate_refined_delta_vs_base.csv"
    manifest_path = REPORT_DIR / "gap_gate_refined_research_manifest.json"
    report_path = REPORT_DIR / "gap_gate_refined_research_report.md"
    ...
    manifest = {
        ...
        "candidate_id": "model_agent_3d_rankic_blend40_gap_gate_refined_20260626",
        "score_formula": stats["formula"],
        "research_hypothesis": "use_blend40_60_on_refined_low_rank_gap_days_with_current_3d_mainline",
    ...
    lines = [
        "# 3D rankic 主线与 blend40_60 的 refined rank-gap 门控研究报告（20260626）",
        "",
        "## 当前结论",
        "",
        "这是一个 research-only 的 3D 全覆盖候选，不是 formal L4 或 L5 生产资产。",
        f"它只在与 blend40_60 候选重叠的 {stats['overlap_days']} 个交易日里，当 `mean_abs_rank_gap <= {THRESHOLD:.15f}` 时切到 blend40_60，其余日期保持当前 3D 主线不变。",
        "",
        "## 评分公式",
        "",
        "```text",
        ...
        "## 关键评价摘要",
        "",
        f"- 相对当前 3D 主线：full objective delta `{summary['full_candidate_objective'] - summary['full_base_objective']:.12f}`，full RankIC delta `{summary['full_vs_base_rank_ic_delta']:.12f}`，full Top5 delta `{summary['full_vs_base_top5_delta']:.12f}`。",
        f"- 相对当前 3D 主线：recent63 Top1 delta `{summary['recent63_vs_base_top1_delta']:.12f}`，recent63 Top5 delta `{summary['recent63_vs_base_top5_delta']:.12f}`，recent20 Top5 delta `{summary['recent20_vs_base_top5_delta']:.12f}`。",
        ...
        "## 当前判断",
        "",
        "- 这条分支是 3D repair candidate 在 full-window refine 之后的更优版本。",
