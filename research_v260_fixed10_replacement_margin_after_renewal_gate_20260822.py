...
import research_v260_fixed10_max_hold_after_renewal_gate_20260822 as max_hold_recheck
...
OUTPUT_ROOT = (
    REPO
    / "quant/data_file/reports/strategy_agent_v260_fixed10_replacement_margin_after_renewal_gate_20260822"
)
REPLACEMENT_MARGINS = (0.03, 0.05, 0.08)


def candidate_policy(replacement_advantage: float) -> dict:
    current = max_hold_recheck.candidate_policy(25)
    current["replacement_advantage"] = float(replacement_advantage)
    return current
...
def main() -> None:
    ...
    if access["logical_max_date"] != round1.DEVELOPMENT_END:
        raise PermissionError("2026 data entered replacement-margin-after-renewal recheck")
    definition = harness.production_definition(protocol)
    score, order = harness.v95.score_pair(arrays, 0.0, 7, 0.1)

    results = {}
    run_cache = {}
    for margin in REPLACEMENT_MARGINS:
        candidate_id = f"replacement_margin_{int(round(margin * 100)):02d}pct"
        current = candidate_policy(margin)
    ...
    if not all(deterministic.values()):
        raise RuntimeError("replacement-margin-after-renewal replay failed")
    ...
    result = {
        ...
        "candidate_budget": [float(value) for value in REPLACEMENT_MARGINS],
        "selection_rule": (
