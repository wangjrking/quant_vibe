...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_research_3d_current_vs_blend40_daygate_scan_20260626"
...
def main() -> int:
    ...
    summary = {
        ...
        "scope": "research_only_3d_current_vs_blend40_daygate_fold08",
        "label": LABEL,
    ...
    lines = [
        "# 3D 当前主线对 blend40_60 候选的日级门控扫描报告（20260626）",
        "",
        "## 当前结论",
        "",
        f"- 基准主线 objective：`{base_objective:.12f}`",
        f"- blend40_60 候选 objective：`{alt_objective:.12f}`",
        ...
        "- 如果最优门控仍然打不过当前主线，说明 3D 在当前候选集合里仍缺乏可利用的局部切换结构。",
        "",
        "## 产物",
        "",
