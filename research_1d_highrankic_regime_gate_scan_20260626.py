...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_1d_highrankic_regime_gate_scan_20260626"
...
CANDIDATES = {
    ...
    "dense_guard_smallcap_vr": "stock_predict_data_model_agent_1d_dense_guard_smallcap_vr_20260623_executable_1d_open_return_research",
...
TARGET_TABLE = "stock_predict_data_model_agent_1d_highrankic_regime_gate_best_20260626_executable_1d_open_return_research"
...
def merge_daily_by_gate(base_daily: pd.DataFrame, cand_daily: pd.DataFrame, active_dates: set[str]) -> pd.DataFrame:
...
def objective(
    ...
    return (
        ...
        + 2.1 * (cur_20["top5"] - base_20["top5"])
        + 1.3 * (cur_63["top1"] - base_63["top1"])
        + 1.1 * (cur_63["top5"] - base_63["top5"])
        + 0.7 * (cur_full["rank_ic"] - base_full["rank_ic"])
        + 0.3 * (cur_full["top5"] - base_full["top5"])
...
def write_candidate(scores: pd.DataFrame, best: pd.Series, daily_features: pd.DataFrame) -> dict[str, object]:
    ...
    out["pred_prob"] = np.where(out["trade_date"].isin(active_dates), out[f"{alias}_rank"], out["base_rank"])
    out["score_formula"] = f"if {feature} {op} {threshold:.12g} then {alias}_rank else base_rank"
    ...
    with sqlite3.connect(MODEL_DB, timeout=300) as conn:
        ...
        conn.execute(f"create index if not exists idx_1d_highrankic_gate_best_date_code on {quote(TARGET_TABLE)}(trade_date, stock_code)")
        conn.commit()
        row = conn.execute(
...
def main() -> int:
            ...
            for threshold in thresholds:
                for op in ["<=", ">="]:
                    ...
                    if len(active_dates) < 6 or len(active_dates) > 180:
                        continue
                    daily = merge_daily_by_gate(base_daily, candidate_daily, active_dates)
                    ...
                    row["pass_basic"] = bool(
                        ...
                        and row["recent63_top5_delta"] >= -0.00020
                        and row["full_rank_ic_delta"] >= -0.00020
                    )
                    rows.append(row)
    ...
    result.to_csv(REPORT_DIR / "highrankic_regime_gate_results.csv", index=False, encoding="utf-8-sig")
    ...
    summary_obj = {
        ...
        "scope": "research_only_1d_highrankic_regime_gate_scan",
    ...
    (REPORT_DIR / "highrankic_regime_gate_summary.json").write_text(
