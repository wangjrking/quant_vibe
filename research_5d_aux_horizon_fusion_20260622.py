...
OUT_DIR = DATA_DIR / "reports" / "model_agent_5d_aux_fusion_tune_20260622"
...
def materialize(best_name: str, merged: pd.DataFrame, score: pd.Series) -> dict:
    ...
    manifest = {
        ...
        "notes": "5D formal 与辅助周期预测分数rank融合研究资产；不作为L5正式入口。",
    }
...
def main() -> int:
    ...
    with sqlite3.connect(MODEL_DB) as conn:
        merged = read_prediction(conn, BASE_TABLE, "formal_5d_pred_prob", keep_payload=True)
    ...
    score_cols = ["formal_5d_pred_prob"] + [f"{alias}_pred_prob" for alias in AUX_TABLES]
    ...
    for col in aux_rank_cols:
        merged[col] = merged[col].fillna(merged["rank_formal_5d"])

    candidates: dict[str, pd.Series] = {
        "formal_5d_raw": merged["formal_5d_pred_prob"],
        "formal_5d_rank": merged["rank_formal_5d"],
    }
    for alias in AUX_TABLES:
        candidates[f"{alias}_rank"] = merged[f"rank_{alias}"]
        for weight in [0.90, 0.85, 0.75, 0.65, 0.50]:
            rest = 1.0 - weight
            candidates[f"rank_blend_5dw{str(weight).replace('.', 'p')}_{alias}w{str(rest).replace('.', 'p')}"] = (
                merged["rank_formal_5d"] * weight + merged[f"rank_{alias}"] * rest
            )

    candidates["rank_avg_5d_1d3d10daux"] = merged[
        ["rank_formal_5d", "rank_research_1d_aux", "rank_research_3d_aux", "rank_research_10d_aux"]
    ].mean(axis=1)
    candidates["rank_avg_1d3d10daux"] = merged[
        ["rank_research_1d_aux", "rank_research_3d_aux", "rank_research_10d_aux"]
    ].mean(axis=1)
    candidates["rank_weighted_5d_0p70_1daux_0p15_10daux_0p15"] = (
        merged["rank_formal_5d"] * 0.70 + merged["rank_research_1d_aux"] * 0.15 + merged["rank_research_10d_aux"] * 0.15
    )
    candidates["rank_weighted_5d_0p60_1daux_0p20_10daux_0p20"] = (
        merged["rank_formal_5d"] * 0.60 + merged["rank_research_1d_aux"] * 0.20 + merged["rank_research_10d_aux"] * 0.20
    )
    candidates["rank_weighted_5d_0p50_1daux_0p25_10daux_0p25"] = (
        merged["rank_formal_5d"] * 0.50 + merged["rank_research_1d_aux"] * 0.25 + merged["rank_research_10d_aux"] * 0.25
    )
    candidates["rank_consensus_max_5d_1daux_10daux"] = merged[
        ["rank_formal_5d", "rank_research_1d_aux", "rank_research_10d_aux"]
    ].max(axis=1)
    candidates["rank_consensus_min_5d_1daux_10daux"] = merged[
        ["rank_formal_5d", "rank_research_1d_aux", "rank_research_10d_aux"]
    ].min(axis=1)
    ...
    baseline = by_name["formal_5d_raw"]
    ...
    def strict_gate(row: dict) -> bool:
        ...
        return (
            ...
            and row_delta["top_k_mean_returns"]["5"] >= -0.001
        )
    ...
    def front_rank_gate(row: dict) -> bool:
        ...
        return (
            row["name"] != "formal_5d_raw"
            and row_delta["front_selection_score"] > 0.001
            and row_delta["daily_rank_ic_mean"] >= -0.002
    ...
    payload = {
        ...
        "baseline": baseline,
        "best": best,
        "best_delta_vs_formal_5d": best_delta,
        "selected": selected,
        "selected_delta_vs_formal_5d": selected_delta,
        ...
        "selection_rule": (
            "先尝试严格门槛：front_selection_score 改善且 RankIC/Top1/Top3/Top5 不明显回撤；"
            "若严格门槛无候选，则允许 front-rank 研究门槛：Top1 明显改善且 Top3 不明显回撤。"
            "辅助分数缺失日回退到5D基线rank。"
        ),
    }
    ...
    lines.extend(
        [
            ...
            "- 辅助分数缺失日回退到 5D 基线 rank，避免缩短最新预测覆盖。",
            "",
            "## 证据路径",
            "",
