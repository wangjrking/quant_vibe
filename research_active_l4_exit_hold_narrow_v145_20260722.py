...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_exit_hold_narrow_v145_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v145_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4持有期与卖出规则窄邻域研究")
    ...
    rows, definitions = [], {}
    grid = protocol["grid"]
    for max_hold, min_hold, sell_below, advantage in product(
        grid["max_hold_days"], grid["min_hold_days"], grid["sell_score_below"], grid["replacement_advantage"]
    ):
        definition = {
