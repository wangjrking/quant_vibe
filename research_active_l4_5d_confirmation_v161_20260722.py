...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_5d_confirmation_v161_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v161_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def score_pair(arrays, definition):
    return v95.score_pair(arrays, float(definition["weight_5d"]), 7, 0.1)


def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4 5D轻量确认研究")
