...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_adaptive_exit_allocation_v165_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v165_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4自适应卖出与强势仓位联合研究")
    ...
    rows, definitions = [], {}
    for threshold, target in product(protocol["grid"]["strong_return_min"], protocol["grid"]["strong_target_pct"]):
        definition = {**protocol["fixed_definition"], "strong_return_min": threshold, "strong_target_pct": target}
