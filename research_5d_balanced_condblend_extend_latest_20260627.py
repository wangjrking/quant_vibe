...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_5d_balanced_condblend_latest_20260627"
...
CANDIDATE_TABLE = "stock_predict_data_model_agent_5d_v3_topgate_3drefined_20260623_executable_5d_open_return_research"
REFERENCE_AGGRESSIVE_TABLE = "stock_predict_data_model_agent_5d_recent_top_condblend_latest_20260626_executable_5d_open_return_research"
TARGET_TABLE = "stock_predict_data_model_agent_5d_balanced_condblend_latest_20260627_executable_5d_open_return_research"

THRESHOLD = 0.002736976576015071
...
def build_balanced_scores(scores: pd.DataFrame, daily_gap: pd.DataFrame) -> pd.DataFrame:
    ...
    out["score_formula"] = np.where(
        use_candidate,
        f"cand_top20_gap <= {THRESHOLD:.16g}: v3_topgate_3drefined_rank",
        "base_rank_fallback",
    )
...
def main() -> int:
    ...
    with sqlite3.connect(MODEL_DB, timeout=300) as conn:
        ...
        conn.execute(f"create index if not exists idx_5d_balanced_condblend_latest_date_code on {quote(TARGET_TABLE)}(trade_date, stock_code)")
        conn.commit()
        row = conn.execute(
    ...
    summary = {
        ...
        "scope": "research_only_5d_balanced_condblend_latest",
        "target_table": TARGET_TABLE,
        "base_table": BASE_TABLE,
        "candidate_table": CANDIDATE_TABLE,
        "reference_aggressive_table": REFERENCE_AGGRESSIVE_TABLE,
        "formula": f"candidate dates: if cand_top20_gap <= {THRESHOLD:.16g} then v3_topgate_3drefined_rank else base_rank; candidate-missing dates: base_rank",
        "coverage": {
            "row_count": int(row[0]),
        ...
        "active_candidate_days": int((daily_gap["cand_top20_gap"] <= THRESHOLD).sum()),
    ...
    (REPORT_DIR / "balanced_condblend_summary.json").write_text(
    ...
    lines = [
        ...
        "",
        "它相对当前 5D 激进版牺牲近期 Top1，但换取更高的 RankIC 稳定性；用途是保留一条稳健备选线。",
        "",
        "## 边界",
        "",
        "- 未训练模型",
        ...
        "## 证据",
        "",
        "- `balanced_condblend_summary.json`",
        "- `candidate_daily_gap.csv`",
    ]
    (REPORT_DIR / "balanced_condblend_report.md").write_text("\n".join(lines), encoding="utf-8")
    return 0
