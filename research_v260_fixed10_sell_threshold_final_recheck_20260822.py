...
import research_v260_fixed10_replacement_margin_final_recheck_20260822 as round14
...
OUTPUT_ROOT = (
    REPO
    / "quant/data_file/reports/strategy_agent_v260_fixed10_sell_threshold_final_recheck_20260822"
)
SELL_THRESHOLDS = (0.80, 0.85, 0.90)


def candidate_policy(sell_threshold: float) -> dict:
    current = round14.candidate_policy(0.05)
...
def main() -> None:
    ...
    if access["logical_max_date"] != round1.DEVELOPMENT_END:
        raise PermissionError("2026 data entered final sell-threshold recheck")
    ...
    if not all(deterministic.values()):
        raise RuntimeError("final sell-threshold deterministic replay failed")
