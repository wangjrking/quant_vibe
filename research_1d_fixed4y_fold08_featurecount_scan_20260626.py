...
BASE_PREDICTION = (
    DATA_DIR
    / "reports"
    / "model_agent_research_1d_fixed4y_fold08_selfeatures_scan_20260626"
    / "self_top3_reg16"
    / "fold08"
    / "fold_predictions"
    / "fold08.parquet"
...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_research_1d_fixed4y_fold08_featurecount_scan_20260626"
...
def write_feature_subset_files() -> list[Candidate]:
    ...
    counts = [24, 40, 56]
    candidates = [Candidate("base_fs64", 64, FEATURES_PATH, False)]
    for count in counts:
        ...
        candidates.append(Candidate(f"top3reg16_fs{count}", count, path, True))
    return candidates


def run_candidate(candidate: Candidate) -> dict:
    if not candidate.retrain:
        return {"stdout": "reused existing base fs64 artifact", "stderr": "", "returncode": 0, "reused": True}
...
def main() -> int:
    ...
    for candidate in candidates:
        ...
        for baseline_name in [*BASELINE_TABLES.keys(), "base_fs64"]:
            if baseline_name == "base_fs64":
                base = next(row for row in rows if row["asset"] == "base_fs64")
            else:
    ...
    packet = {
        ...
        "scope": "research_only_1d_fixed4y_fold08_featurecount_scan",
        "label": LABEL,
