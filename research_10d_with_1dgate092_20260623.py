...
OUT_DIR = DATA_DIR / "reports" / "model_agent_10d_with_1dgate092_20260623"
...
RESEARCH_TABLE = "stock_predict_data_model_agent_10d_dategate_3dformal_plus_1dgate092_20260623_executable_10d_open_return_research"
...
def main() -> int:
    ...
    summary = {
        ...
        "candidate_id": "model_agent_10d_dategate_3dformal_plus_1dgate092_20260623",
    ...
    manifest = {
        ...
        "notes": "10D 基于已存在的 dategate+3D formal research 候选，在近期头部区域引入 1D gate092 研究分数做局部重排。该资产仅供 research 验证，不进入 formal L4/L5。",
    }
    ...
    lines = [
        "# 10D 叠加 1D Gate092 研究候选",
        "",
        "## 当前结论",
        "",
        "本轮在现有 `10D dategate + 3D formal` 候选基础上，只对近期头部区域叠加 `1D gate092` 分数。",
        "结果显示，这个方向对 `recent63` 和 `recent126` 的前排指标有实质改善，值得保留为新的 research-only 候选。",
        "",
        "## 参数",
        "",
        f"- 起始日期：`{START_DATE}`",
