...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_daily_topn_v206_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v206_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def run_case(arrays, score, order, definition, protocol, end, start=None, record_actions=False):
    ...
    case_protocol["fixed_strategy"] = {
        "top_n_per_rebalance": int(definition["daily_top_n"]),
        "max_hold_days": int(definition["max_hold_days"]),
        "rebalance_every": 1,
    }
    gross, target = v153.schedules(arrays, definition)
    target = target / float(definition["daily_top_n"])
    return v109.simulate(
        ...
        min_hold_days_override=v162.min_hold_schedule(arrays, definition["min_hold_policy"]),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 每日TopN等总仓位分散研究")
    ...
    rows, definitions = [], {}
    for daily_top_n in protocol["grid"]["daily_top_n"]:
        definition = {**protocol["fixed_definition"], "daily_top_n": int(daily_top_n)}
