...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_1d_recent_enhance_v4_research_20260625"
TARGET_TABLE = "stock_predict_data_model_agent_1d_recent_enhance_v4_20260625_executable_1d_open_return_research"
...
SOURCES = {
    ...
    "base1": "stock_predict_data_model_agent_1d_broad_stability_guard_20260625_executable_1d_open_return_research",
    "dyn1": "stock_predict_data_model_agent_1d_dynamic_guard_20260625_executable_1d_open_return_research",
    "strict1": "stock_predict_data_model_agent_1d_dynamic_guard_strict_20260625_executable_1d_open_return_research",
    "cross1": "stock_predict_data_model_agent_1d_cross_horizon_guard_20260625_executable_1d_open_return_research",
    "formal3": "stock_predict_data_model_agent_best_full_20260623_executable_3d_open_return_formal",
    "recent3d": "stock_predict_data_model_agent_3d_recent_enhance_v4_20260625_executable_3d_open_return_research",
    "base3": "stock_predict_data_model_agent_3d_cross_horizon_guard_v3_20260625_executable_3d_open_return_research",
    "formal5": "stock_predict_data_model_agent_best_full_20260623_executable_5d_open_return_formal",
    "recent5d": "stock_predict_data_model_agent_5d_recent_enhance_v4_20260625_executable_5d_open_return_research",
    "formal10": "stock_predict_data_model_agent_best_full_20260623_executable_10d_open_return_formal",
    "recent10d": "stock_predict_data_model_agent_10d_recent_enhance_v4_20260625_executable_10d_open_return_research",
}
...
def load_scores() -> pd.DataFrame:
    with sqlite3.connect(MODEL_DB) as conn:
        out = read_source(conn, "base1", SOURCES["base1"])
        for alias, table in SOURCES.items():
            if alias == "base1":
                continue
            out = out.merge(read_source(conn, alias, table), on=["trade_date", "stock_code"], how="left", validate="one_to_one")
    for alias in SOURCES:
        score_col = f"{alias}_score"
        out[score_col] = out[score_col].fillna(out["base1_score"])
...
def build_features(scores: pd.DataFrame) -> pd.DataFrame:
    ...
    for trade_date, group in scores.groupby("trade_date", sort=True):
        base10 = top_set(group, "formal10_rank", 10)
        base50 = top_set(group, "formal10_rank", 50)
        ...
        for alias in SOURCES:
            ...
            if alias != "formal10":
                row[f"corr_formal10_{alias}"] = float(group["formal10_rank"].corr(group[rank_col]))
                row[f"overlap10_formal10_{alias}"] = len(base10 & top_set(group, rank_col, 10))
                row[f"overlap50_formal10_{alias}"] = len(base50 & top_set(group, rank_col, 50))
        rows.append(row)
...
def summarize_daily(daily: pd.DataFrame, dates: list[str], n: int | None) -> dict:
    ...
    return {c: float(sub[c].mean()) for c in ["rank_ic", "pearson_ic", "top_bottom", "top1", "top3", "top5", "top10", "top20", "top50"]}
...
def scan(scores: pd.DataFrame, labels: pd.DataFrame, features: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    base = metric_by_source[metric_by_source["source"] == "formal10"].set_index("trade_date")
    feature_idx = features.set_index("trade_date")
    rows = []
    daily_rows = []
    for alt in [a for a in SOURCES if a != "formal10"]:
        ...
        for feature in candidate_features:
            ...
            if vals.nunique() < 3:
                continue
            thresholds = sorted(set(float(vals.quantile(q)) for q in [0.05, 0.1, 0.15, 0.2, 0.25, 0.33, 0.5, 0.67, 0.75, 0.8, 0.85, 0.9, 0.95]))
...
def write_candidate(scores: pd.DataFrame, features: pd.DataFrame, best: pd.Series) -> dict:
    ...
    active_dates = set(active[active].index.astype(str))
    out = scores[["trade_date", "stock_code", "formal10_score", "formal10_rank", f"{alt}_score", f"{alt}_rank"]].copy()
    out["guard_active"] = out["trade_date"].isin(active_dates)
    out["pred_prob"] = np.where(out["guard_active"], out[f"{alt}_rank"], out["formal10_rank"])
    out["score_formula"] = f"if {feature} {op} {threshold:.12g} then {alt}_rank else formal10_rank"
    out = out.rename(columns={"formal10_score": "formal10", f"{alt}_score": alt})
    keep = ["trade_date", "stock_code", "pred_prob", "formal10", alt, "formal10_rank", f"{alt}_rank", "guard_active", "score_formula"]
    with sqlite3.connect(MODEL_DB, timeout=300) as conn:
        ...
        conn.commit()
        row = conn.execute(f"select count(*), min(trade_date), max(trade_date), count(distinct trade_date), sum(case when pred_prob is null then 1 else 0 end) from {quote(TARGET_TABLE)}").fetchone()
        dup = conn.execute(f"select count(*) from (select trade_date, stock_code, count(*) c from {quote(TARGET_TABLE)} group by trade_date, stock_code having c > 1)").fetchone()[0]
        latest = conn.execute(f"select trade_date, count(*), count(distinct stock_code) from {quote(TARGET_TABLE)} group by trade_date order by trade_date desc limit 10").fetchall()
    return {
        "table": TARGET_TABLE,
...
def write_outputs(scan_result: pd.DataFrame, daily: pd.DataFrame, source_metrics: pd.DataFrame, stats: dict | None) -> None:
    ...
    (REPORT_DIR / "cross_horizon_guard_research_manifest.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")
    lines = ["# 10D 跨周期动态守卫研究报告（20260625）", "", "## 当前结论", ""]
    if stats:
        lines.extend(
            [
                "本轮找到一个通过基础模型评价门槛的 10D cross-horizon research-only 候选。",
                "",
                "候选规则：",
                "",
                "```text",
        ...
        lines.append("本轮未找到通过基础模型评价门槛的 10D cross-horizon 候选。")
    lines.extend(
        [
            "",
            "## 证据路径",
            ...
            f"- 研究 manifest：`{(REPORT_DIR / 'cross_horizon_guard_research_manifest.json').as_posix()}`",
            "",
            "## 边界说明",
            "",
            "- 未训练模型。",
...
def build_features(scores: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for trade_date, group in scores.groupby("trade_date", sort=True):
        base10 = top_set(group, "base1_rank", 10)
        base20 = top_set(group, "base1_rank", 20)
        base50 = top_set(group, "base1_rank", 50)
        row: dict[str, float | str] = {"trade_date": trade_date}
        for alias in SOURCES:
            score_col = f"{alias}_score"
            rank_col = f"{alias}_rank"
            ...
            row[f"{alias}_top50_score_mean"] = float(group.nlargest(min(50, len(group)), rank_col)[score_col].mean())
            if alias != "base1":
                row[f"corr_base1_{alias}"] = float(group["base1_rank"].corr(group[rank_col]))
                row[f"overlap10_base1_{alias}"] = len(base10 & top_set(group, rank_col, 10))
                row[f"overlap20_base1_{alias}"] = len(base20 & top_set(group, rank_col, 20))
                row[f"overlap50_base1_{alias}"] = len(base50 & top_set(group, rank_col, 50))
        rows.append(row)
    return pd.DataFrame(rows)
...
def scan(scores: pd.DataFrame, labels: pd.DataFrame, features: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    metric_by_source = pd.concat([source_daily_metrics(eval_frame, alias) for alias in SOURCES], ignore_index=True)
    dates = sorted(metric_by_source["trade_date"].unique().tolist())
    base = metric_by_source[metric_by_source["source"] == "base1"].set_index("trade_date")
    feature_idx = features.set_index("trade_date")
    rows = []
    daily_rows = []
    alt_sources = ["dyn1", "strict1", "cross1", "recent3d", "base3", "formal3", "recent5d", "formal5", "recent10d", "formal10"]
    for alt in alt_sources:
        alt_daily = metric_by_source[metric_by_source["source"] == alt].set_index("trade_date")
        candidate_features = [c for c in feature_idx.columns if c.startswith(f"{alt}_") or c.endswith(f"_{alt}") or c.startswith("base1_")]
        for feature in candidate_features:
            vals = feature_idx[feature].replace([np.inf, -np.inf], np.nan).dropna()
            if vals.nunique() < 3:
                continue
            thresholds = sorted(set(float(vals.quantile(q)) for q in [0.10, 0.20, 0.33, 0.50, 0.67, 0.80, 0.90]))
            for threshold in thresholds:
                for op in ["<=", ">="]:
                    active = feature_idx[feature] <= threshold if op == "<=" else feature_idx[feature] >= threshold
                    active_dates = set(active[active].index.astype(str))
                    if len(active_dates) < 10 or len(active_dates) > int(0.45 * len(feature_idx)):
                        continue
                    active_eval_dates = [d for d in active_dates if d in alt_daily.index]
                    cand = base.copy()
                    cand.loc[active_eval_dates, :] = alt_daily.loc[active_eval_dates, cand.columns]
                    ...
                    for prefix, cur, base_sum in [("full", full, base_full), ("recent63", r63, b63), ("recent20", r20, b20)]:
                        for metric in ["rank_ic", "top1", "top3", "top5", "top10", "top20", "top50"]:
                            row[f"{prefix}_{metric}_delta_vs_base1"] = cur[metric] - base_sum[metric]
                    row.update(period_stats(merged))
                    row["pass_basic"] = bool(
                        row["recent63_active_days"] >= 5
                        and row["recent63_top1_delta_vs_base1"] > 0
                        and row["recent63_top5_delta_vs_base1"] > 0
                        and row["recent63_top10_delta_vs_base1"] >= -0.0005
                        and row["recent20_top5_delta_vs_base1"] >= 0
                        and row["full_top5_delta_vs_base1"] >= 0
                        and row["full_rank_ic_delta_vs_base1"] >= -0.0005
                        and row["min_period_top5_delta"] >= -0.00025
                        and row["positive_top5_periods"] >= 2
                    )
                    row["objective"] = (
                        1.2 * row["recent63_top1_delta_vs_base1"]
                        + 2.0 * row["recent63_top5_delta_vs_base1"]
                        + row["recent20_top5_delta_vs_base1"]
                        + 0.8 * row["full_top5_delta_vs_base1"]
                        + 0.5 * row["full_rank_ic_delta_vs_base1"]
                        + 1.2 * row["min_period_top5_delta"]
                        + 0.01 * row["positive_top5_periods"]
...
def write_candidate(scores: pd.DataFrame, features: pd.DataFrame, best: pd.Series) -> dict:
    ...
    active = active <= threshold if op == "<=" else active >= threshold
    active_dates = set(active[active].index.astype(str))
    out = scores[["trade_date", "stock_code", "base1_score", "base1_rank", f"{alt}_score", f"{alt}_rank"]].copy()
    out["guard_active"] = out["trade_date"].isin(active_dates)
    out["pred_prob"] = np.where(out["guard_active"], out[f"{alt}_rank"], out["base1_rank"])
    out["score_formula"] = f"if {feature} {op} {threshold:.12g} then {alt}_rank else base1_rank"
    out = out.rename(columns={"base1_score": "base1", f"{alt}_score": alt})
    keep = ["trade_date", "stock_code", "pred_prob", "base1", alt, "base1_rank", f"{alt}_rank", "guard_active", "score_formula"]
    with sqlite3.connect(MODEL_DB, timeout=300) as conn:
        conn.execute("PRAGMA busy_timeout=300000")
        conn.execute(f"drop table if exists {quote(TARGET_TABLE)}")
        out[keep].to_sql(TARGET_TABLE, conn, if_exists="replace", index=False)
...
def write_outputs(scan_result: pd.DataFrame, daily: pd.DataFrame, source_metrics: pd.DataFrame, stats: dict | None) -> None:
    REPORT_DIR.mkdir(parents=True, exist_ok=True)
    scan_path = REPORT_DIR / "recent_enhance_scan_results.csv"
    daily_path = REPORT_DIR / "recent_enhance_daily_deltas.csv"
    source_path = REPORT_DIR / "recent_enhance_source_daily_metrics.csv"
    scan_result.to_csv(scan_path, index=False, encoding="utf-8-sig")
    daily.to_csv(daily_path, index=False, encoding="utf-8-sig")
    source_metrics.to_csv(source_path, index=False, encoding="utf-8-sig")
    best = scan_result.iloc[0].to_dict() if not scan_result.empty else None
    manifest = {
        ...
        "generated_at": now_iso(),
        "decision": "candidate" if stats else "rejected",
        "baseline_for_scan": "research_1d_broad_stability_guard",
        "research_hypothesis": "recent_metric_enhancement_with_limited_period_tradeoff",
        "best_scan": best,
        "asset_stats": stats,
        "promotion_requires_user_confirmation": True,
        "audit_required_before_formal": True,
    ...
    manifest_path = REPORT_DIR / "recent_enhance_research_manifest.json"
    manifest_path.write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")
    report_path = REPORT_DIR / "recent_enhance_research_report.md"
    lines = [
        "# 1D 近期增强研究报告（20260625）",
        "",
        "## 当前结论",
        "",
    ]
    if stats:
        lines.extend(
            [
                "本轮找到一个相对 1D 当前研究基准改善近期 Top 指标的 research-only 候选；它不是稳定化发布候选，仍需后续评价和审批。",
                "",
                "候选规则：",
                "",
                "```text",
                ...
                "## 相对 1D 当前研究基准的扫描摘要",
                "",
                f"- 近 63 日 Top1 增量：`{best['recent63_top1_delta_vs_base1']:.6f}`",
                f"- 近 63 日 Top5 增量：`{best['recent63_top5_delta_vs_base1']:.6f}`",
                f"- 近 63 日 Top10 增量：`{best['recent63_top10_delta_vs_base1']:.6f}`",
                f"- 近 20 日 Top5 增量：`{best['recent20_top5_delta_vs_base1']:.6f}`",
                f"- 全样本 Top5 增量：`{best['full_top5_delta_vs_base1']:.6f}`",
                f"- 全样本 RankIC 增量：`{best['full_rank_ic_delta_vs_base1']:.6f}`",
                f"- 正 Top5 自然阶段数：`{int(best['positive_top5_periods'])}/4`",
                f"- 最差自然阶段 Top5 增量：`{best['min_period_top5_delta']:.6f}`",
            ]
        )
    else:
        lines.append("本轮未找到相对 1D 当前研究基准通过近期增强门槛的候选。")
    lines.extend(
        [
            "",
            "## 证据路径",
    ...
    if stats:
        lines.extend(
            [
                "本轮找到一个相对 10D 当前最优基准通过稳定化门槛的 research-only 候选。",
                "",
                "候选规则：",
                "",
                "```text",
                ...
                "## 相对 10D 当前最优基准的扫描摘要",
                "",
                f"- 近 63 日 Top1 增量：`{best['recent63_top1_delta_vs_base10']:.6f}`",
                f"- 近 63 日 Top5 增量：`{best['recent63_top5_delta_vs_base10']:.6f}`",
                f"- 近 20 日 Top5 增量：`{best['recent20_top5_delta_vs_base10']:.6f}`",
                f"- 全样本 Top5 增量：`{best['full_top5_delta_vs_base10']:.6f}`",
                f"- 正 Top5 自然阶段数：`{int(best['positive_top5_periods'])}/4`",
                f"- 最差自然阶段 Top5 增量：`{best['min_period_top5_delta']:.6f}`",
            ]
        )
    else:
        lines.append("本轮未找到相对 10D 当前最优基准通过稳定化门槛的候选。")
    lines.extend(
        [
            "",
            "## 证据路径",
    ...
    (REPORT_DIR / "recent_enhance_research_report.md").write_text("\n".join(lines), encoding="utf-8")
