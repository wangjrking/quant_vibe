...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_1d_top5_safe_gate_v59_20260627"
SOURCE_FEATURES = DATA_DIR / "reports" / "model_agent_1d_topzone_second_gate_scan_20260627" / "date_score_features.csv"
...
TOPZONE_TABLE = "stock_predict_data_model_agent_1d_topzone_confirmation_best_20260627_executable_1d_open_return_research"
TARGET_TABLE = "stock_predict_data_model_agent_1d_top5_safe_gate_v59_20260627_executable_1d_open_return_research"

THRESHOLD = 0.00149488006929
...
def load_scores() -> pd.DataFrame:
    with sqlite3.connect(MODEL_DB, timeout=300) as conn:
        ...
        topzone = read_table(conn, TOPZONE_TABLE, "topzone")
    scores = base.merge(topzone, on=["trade_date", "stock_code"], how="inner", validate="one_to_one")
    scores["base_rank"] = scores.groupby("trade_date")["base_score"].rank(method="average", pct=True)
    scores["topzone_rank"] = scores.groupby("trade_date")["topzone_score"].rank(method="average", pct=True)
    features = pd.read_csv(SOURCE_FEATURES, usecols=["trade_date", "mean_abs_rank_gap"])
...
def build_candidate(scores: pd.DataFrame) -> pd.DataFrame:
    out = scores[["trade_date", "stock_code", "base_rank", "topzone_rank", "mean_abs_rank_gap"]].copy()
    use_topzone = out["mean_abs_rank_gap"] <= THRESHOLD
    out["pred_prob"] = out["base_rank"].where(~use_topzone, out["topzone_rank"])
    out["score_formula"] = "base_rank"
    out.loc[use_topzone, "score_formula"] = f"topzone_rank when mean_abs_rank_gap <= {THRESHOLD:.12g}"
    return out[["trade_date", "stock_code", "pred_prob", "base_rank", "topzone_rank", "mean_abs_rank_gap", "score_formula"]]
...
def month_stability(daily: pd.DataFrame, base_daily: pd.DataFrame) -> dict[str, object]:
    ...
    return {
        ...
        "worst_top1_delta": float(df["top1_delta"].min()),
...
def write_table(out: pd.DataFrame) -> dict[str, object]:
    with sqlite3.connect(MODEL_DB, timeout=300) as conn:
        ...
        conn.execute(f"create index if not exists idx_1d_top5_safe_gate_v59_date_code on {quote(TARGET_TABLE)}(trade_date, stock_code)")
        conn.commit()
        row = conn.execute(
...
def main() -> int:
    ...
    active_days = int(scores[["trade_date", "mean_abs_rank_gap"]].drop_duplicates()["mean_abs_rank_gap"].le(THRESHOLD).sum())
    summary = {
        "generated_at": now_iso(),
        "scope": "research_only_1d_top5_safe_gate_v59",
        "label": LABEL,
        "base_table": BASE_TABLE,
        "topzone_table": TOPZONE_TABLE,
        "target_table": TARGET_TABLE,
        "source_features": str(SOURCE_FEATURES),
        "formula": f"topzone_rank when mean_abs_rank_gap <= {THRESHOLD:.12g}; otherwise base_rank",
        ...
        "reason": "strict 1D gate keeps full/recent126/recent63/recent20 Top1 and Top5 nonnegative versus active 1D",
        "boundaries": {
            "research_only": True,
            "no_training": True,
    ...
    scores[["trade_date", "mean_abs_rank_gap"]].drop_duplicates().to_csv(
    ...
    lines = [
        ...
        "## 结论",
        "",
        "本轮生成一条 research-only 1D 候选，用于在保留 Top1 提升的同时避免 Top5 退化。",
        "",
        "## 规则",
        "",
        f"- 基准表：`{BASE_TABLE}`",
        f"- 候选表：`{TOPZONE_TABLE}`",
        f"- 门控：`mean_abs_rank_gap <= {THRESHOLD:.12g}` 时使用候选 rank，否则使用基准 rank",
        "",
        "## 相对基准增量",
        "",
