...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_3d_recent_top_daygate_scan_20260627"
...
TARGET_TABLE = "stock_predict_data_model_agent_3d_recent_top_daygate_best_20260627_executable_3d_open_return_research"
...
def build_candidate(scores: pd.DataFrame, ff: pd.DataFrame, best: pd.Series) -> pd.DataFrame:
    ...
    out["score_formula"] = np.where(use_cand, f"if {feature} {op} {threshold:.12g}: {alias}_rank", "base_rank")
    return out


def write_candidate(scores: pd.DataFrame, ff: pd.DataFrame, best: pd.Series) -> dict[str, object]:
    alias = str(best["candidate"])
    out = build_candidate(scores, ff, best)
    with sqlite3.connect(MODEL_DB, timeout=300) as conn:
        conn.execute(f"drop table if exists {quote(TARGET_TABLE)}")
        out[["trade_date", "stock_code", "pred_prob", "base_rank", f"{alias}_rank", "score_formula"]].to_sql(TARGET_TABLE, conn, if_exists="replace", index=False)
        conn.execute(f"create index if not exists idx_3d_recent_top_daygate_best_date_code on {quote(TARGET_TABLE)}(trade_date, stock_code)")
        conn.commit()
        row = conn.execute(
...
def main() -> int:
    ...
    result.to_csv(REPORT_DIR / "recent_top_daygate_results.csv", index=False, encoding="utf-8-sig")
    ...
    summary = {
        ...
        "scope": "research_only_3d_recent_top_daygate_scan",
    ...
    (REPORT_DIR / "recent_top_daygate_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    return 0
