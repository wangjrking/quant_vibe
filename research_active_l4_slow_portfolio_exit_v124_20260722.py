...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_slow_portfolio_exit_v124_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v124_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4慢分数单批组合独立卖出研究")
    ...
    if args.open_known_2026:
        ...
        for item in frozen["candidates"]:
            ...
            for start in protocol["known_stress_buy_starts"]:
                daily = v109.simulate(full, score, order, definition, current, protocol["known_stress_end"], start)
    ...
    for min_hold, sell_below, advantage in product(grid["min_hold_days"], grid["sell_score_below"], grid["replacement_advantage"]):
        definition = {**protocol["fixed_definition"], "min_hold_days": min_hold, "sell_score_below": sell_below, "replacement_advantage": advantage}
    ...
    selected = frame[frame["eligible"]].drop_duplicates(subset=["full_linear_annual_proxy", "full_sharpe", "full_max_drawdown"]).head(int(protocol["freeze_count"]))
