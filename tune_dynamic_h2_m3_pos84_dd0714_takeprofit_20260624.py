...
REPORT_DIR = BASE_DIR / "dynamic_h2_m3_pos84_dd0714_takeprofit_20260624"
BASE_SIGNAL_FILE = BASE_DIR / "dynamic_h2_m3_tp060_position_20260624" / "signals" / "tp060_pos84.csv"
...
TAKE_PROFITS = [None, 0.04, 0.05, 0.055, 0.06, 0.065, 0.07, 0.08, 0.10, 0.12]
...
def _case_name(take_profit: float | None) -> str:
    if take_profit is None:
        return "pos84_dd0714_tpnone"
    return f"pos84_dd0714_tp{int(round(take_profit * 1000)):03d}"
...
def _signal_for_case(take_profit: float | None) -> Path:
    ...
    for row in rows:
        ...
        row["strategy_variant"] = f"dynamic_h2_m3_pos84_dd0714_takeprofit_{take_profit or 'none'}"
...
def _write_report(summary: list[dict], audit: dict) -> None:
    ...
    for row in ranked:
        take_profit_text = "无" if row["take_profit"] in (None, "") else f"{_f(row['take_profit']):.1%}"
        lines.append(
