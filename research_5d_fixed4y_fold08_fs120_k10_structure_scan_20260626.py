...
FEATURES_PATH = (
    DATA_DIR
    / "reports"
    / "model_agent_research_5d_fixed4y_fold08_featurecount_scan_20260626"
    / "selected_features"
    / "selected_features_top120.json"
)
BASE_PREDICTION = (
    DATA_DIR
    / "reports"
    / "model_agent_research_5d_fixed4y_fold08_fs120_validation_scan_20260626"
    / "k10_t126"
    / "fold08"
    / "fold_predictions"
    / "fold08.parquet"
...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_research_5d_fixed4y_fold08_fs120_k10_structure_scan_20260626"
...
CANDIDATES = [
    Candidate("base_k10_t126", 3, 8.0, 0.2, 0.003, 6000, False),
    Candidate("d2_l12_a03", 2, 12.0, 0.3, 0.003, 6000, True),
    Candidate("d3_l12_a03", 3, 12.0, 0.3, 0.003, 6000, True),
    Candidate("d3_l8_a01_lr004_n5000", 3, 8.0, 0.1, 0.004, 5000, True),
    Candidate("d4_l8_a02", 4, 8.0, 0.2, 0.003, 6000, True),
]
...
def prediction_path(candidate: Candidate) -> Path:
    if candidate.name == "base_k10_t126":
...
def run_candidate(candidate: Candidate) -> dict:
    if not candidate.retrain:
        return {"stdout": "reused existing k10_t126 artifact", "stderr": "", "returncode": 0, "reused": True}
    ...
    command = [
        ...
        "--output-table",
        f"stock_predict_data_model_agent_{candidate.name}_5d_20260626",
...
def main() -> int:
    ...
    for candidate in CANDIDATES:
        ...
        for baseline_name in ["research5d_current", "formal5d_current", "base_k10_t126"]:
            if baseline_name == "base_k10_t126":
                base = next(row for row in rows if row["asset"] == "base_k10_t126")
            else:
    ...
    summary_path = REPORT_DIR / "structure_scan_summary.csv"
    delta_path = REPORT_DIR / "structure_scan_deltas.csv"
    logs_path = REPORT_DIR / "structure_scan_run_logs.json"
    packet_path = REPORT_DIR / "structure_scan_packet.json"
    ...
    packet = {
        ...
        "scope": "research_only_5d_fixed4y_fold08_fs120_k10_structure_scan",
        "label": LABEL,
        ...
        "baselines": {**BASELINE_TABLES, "base_k10_t126": str(BASE_PREDICTION)},
        "boundaries": {
            "research_only": True,
    ...
    lines = [
        ...
        "## 实验目的",
        "",
        "围绕当前 5D 近四年训练分支的新底座 `fs120 + k10 + tail126`，继续验证结构参数调整是否还能形成增益。",
        "",
        "## 结果摘要",
        "",
        f"- 当前排序第一：`{best['asset']}`",
        ...
        "## 产物",
        "",
        "- 汇总表：`structure_scan_summary.csv`",
        "- 差异表：`structure_scan_deltas.csv`",
        "- 运行日志：`structure_scan_run_logs.json`",
        "- 实验清单：`structure_scan_packet.json`",
    ]
