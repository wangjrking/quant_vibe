...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_adaptive_allocation_refine_v166_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v166_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4自适应策略常态弱市仓位微调")
    ...
    rows, definitions = [], {}
    for normal_target, weak_target in product(protocol["grid"]["normal_target_pct"], protocol["grid"]["weak_target_pct"]):
        definition = {**protocol["fixed_definition"], "normal_target_pct": normal_target, "weak_target_pct": weak_target}
