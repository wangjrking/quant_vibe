...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_10d_v5shrink_fixed4y_top20zero_gate_20260626"
...
TARGET_TABLE = "stock_predict_data_model_agent_10d_v5shrink_fixed4y_top20zero_gate_20260626_executable_10d_open_return_research"
ALT_PATH = (
    DATA_DIR
    / "reports"
...
def add_gate(scores: pd.DataFrame) -> pd.DataFrame:
    ...
    for trade_date, group in overlap.groupby("trade_date", sort=True):
        ...
        union = top20_base | top20_alt
        overlap_score = float(len(top20_base & top20_alt) / max(1, len(union)))
        gate_rows.append({"trade_date": trade_date, FEATURE: overlap_score})
    ...
    out["score_formula"] = f"if {FEATURE} <= {THRESHOLD:.1f}: alt_rank else base_rank"
    return out
...
def evaluate(scored: pd.DataFrame, labels: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, dict[str, object]]:
    ...
    candidate = daily_metrics(eval_frame, "pred_prob", "fixed4y_top20zero_gate")
    formal = daily_metrics(eval_frame, "formal_rank", "formal")
    base = daily_metrics(eval_frame, "base_rank", "base_v5_shrink")
...
def write_outputs(
    ...
    metrics_path = REPORT_DIR / "top20zero_gate_daily_metrics.csv"
    delta_formal_path = REPORT_DIR / "top20zero_gate_delta_vs_formal.csv"
    delta_base_path = REPORT_DIR / "top20zero_gate_delta_vs_base.csv"
    manifest_path = REPORT_DIR / "top20zero_gate_research_manifest.json"
    report_path = REPORT_DIR / "top20zero_gate_research_report.md"
    ...
    manifest = {
        ...
        "candidate_id": "model_agent_10d_v5shrink_fixed4y_top20zero_gate_20260626",
        "score_formula": stats["formula"],
        "research_hypothesis": "use_fixed4y_candidate_when_top20_has_zero_overlap_with_current_10d_mainline",
    ...
    lines = [
        "# 10D v5_shrink 与近四年候选的 top20 zero-overlap 门控研究报告（20260626）",
        "",
        "## 当前结论",
        "",
        "这是一个 research-only 的 10D 全覆盖候选，不是 formal L4 或 L5 生产资产。",
        f"它只在与近四年候选重叠的 {stats['overlap_days']} 个交易日里，当 `top20_overlap <= {THRESHOLD:.1f}` 时切到近四年候选，其余日期保持当前 10D 主线不变。",
        "",
        "## 评分公式",
        "",
        "```text",
        ...
        "## 关键评价摘要",
        "",
        f"- 相对当前 10D 主线：full objective delta `{summary['full_candidate_objective'] - summary['full_base_objective']:.12f}`，full RankIC delta `{summary['full_vs_base_rank_ic_delta']:.12f}`，full Top5 delta `{summary['full_vs_base_top5_delta']:.12f}`。",
        f"- 相对当前 10D 主线：recent63 Top1 delta `{summary['recent63_vs_base_top1_delta']:.12f}`，recent63 Top5 delta `{summary['recent63_vs_base_top5_delta']:.12f}`，recent20 Top5 delta `{summary['recent20_vs_base_top5_delta']:.12f}`。",
        ...
        "## 当前判断",
        "",
        "- 这条分支说明：近四年训练候选虽然整体远弱于当前 10D 主线，但当双方 top20 完全错位时，近四年候选更可能提供额外的信息量。",
        "- 这是当前 10D 主线的增量修补候选，不是新的独立训练主线。",
        "",
        "## 边界",
        "",
        "- 未训练模型。",
