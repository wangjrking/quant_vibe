...
def main() -> None:
    ...
    if checkpoint["validation_2026_opened"]:
        raise PermissionError("2026 validation is unavailable for age recheck")
    policy = copy.deepcopy(checkpoint["selected_policy"])
    context = harness.load_context(policy)
    strong = regime.strong_market_mask(context.score, context.protocol)
    weak = confirmed.confirmed_weak_mask(strong, 2)
    ...
    score_priority = priority.sell_priority_matrix(
        context.score, volatility_rank, ~weak, PENALTY
    )

    results, cache = {}, {}
    for weak_age in WEAK_AGES:
        candidate_id = f"strong4_weak{weak_age}"
        extra_age = age_boundary.pressure_age_schedule(strong, weak_age)
        ...
        cache[candidate_id] = (daily, actions, extra_age)

    baseline_id = "strong4_weak10"
    ...
    if not all(checkpoint_equivalence.values()):
        raise RuntimeError("weak2 pressure-age baseline drifted")
    gates = {
    ...
    if not all(deterministic.values()):
        raise RuntimeError("weak2 pressure-age replay failed")
    result = {
        "status": "confirmed_weak2_pressure_age_boundary_complete_2026_not_opened",
        "candidate_budget": [f"strong4_weak{age}" for age in WEAK_AGES],
        "only_change": (
            "weak-market minimum holding age for the second same-day pressure exit; "
            "the ordinary first score exit remains at four sessions"
        ),
        "results": results,
        "selection_gates_against_weak10": gates,
