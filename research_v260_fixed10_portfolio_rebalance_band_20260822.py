...
OUTPUT_ROOT = (
    REPO
    / "quant/data_file/reports/strategy_agent_v260_fixed10_portfolio_rebalance_band_20260822"
)
PORTFOLIO_REBALANCE_BANDS = (0.00, 0.01, 0.02, 0.03)
PORTFOLIO_REBALANCE_INTERVAL_DAYS = 20
SHARPE_PRACTICAL_EQUIVALENCE = 0.01


def candidate_id(band: float) -> str:
    return f"portfolio_rebalance_band_{int(round(band * 100)):02d}pct"


def candidate_policy(band: float) -> dict:
    current = cadence.candidate_policy(PORTFOLIO_REBALANCE_INTERVAL_DAYS)
    current["portfolio_rebalance_min_weight_deviation"] = float(band)
    return current
...
def main() -> None:
    ...
    for band in PORTFOLIO_REBALANCE_BANDS:
        current_id = candidate_id(band)
        current = candidate_policy(band)
        daily, actions = round1.run_fixed10(
            ...
            portfolio_rebalance_min_weight_deviation_override=band,
        )
    ...
    base_policy = cadence.candidate_policy(PORTFOLIO_REBALANCE_INTERVAL_DAYS)
    ...
    disabled_daily, disabled_actions = run_cache[candidate_id(0.00)]
    ...
    if not all(disabled_equivalence.values()):
        raise RuntimeError("portfolio-rebalance band changed the zero-band path")
    ...
    selected_band = results[selected_id]["policy"][
    ...
    repeat_daily, repeat_actions = round1.run_fixed10(
        ...
        simulator=rebalance_band_v109.simulate,
        portfolio_rebalance_active_override=active,
        portfolio_rebalance_min_weight_deviation_override=selected_band,
    )
    deterministic = {
    ...
    if not all(deterministic.values()):
        raise RuntimeError("portfolio-rebalance band deterministic replay failed")
    ...
    result = {
        ...
        "candidate_budget": [float(value) for value in PORTFOLIO_REBALANCE_BANDS],
        "selection_rule": {
