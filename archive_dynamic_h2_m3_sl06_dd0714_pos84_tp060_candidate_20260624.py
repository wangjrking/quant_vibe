...
STRATEGY_ID = "candidate_dynamic_h2m3_sl06dd0714_pos84_tp060_v20260624"
...
VALIDATION_DIR = BASE_REPORT_DIR / "dynamic_h2_m3_sl06_dd0714_pos84_tp060_validation_20260624"
TAKEPROFIT_DIR = BASE_REPORT_DIR / "dynamic_h2_m3_pos76_takeprofit_20260624"
POSITION_DIR = BASE_REPORT_DIR / "dynamic_h2_m3_tp060_position_20260624"
DD_DIR = BASE_REPORT_DIR / "dynamic_h2_m3_tp060_pos84_dd_neighborhood_20260624"
SIGNAL_FILE = DD_DIR / "signals" / "tp060_pos84_dd0714_s8060.csv"
...
def _trading_rules() -> dict:
    return {
        ...
        "strategy_name": "动态持有 Top1 三周期融合 pos84 6%止盈 dd0714 增强",
...
def _manifest_payload(validation: dict, signal_rows: list[dict]) -> dict:
    ...
    return {
        "schema_version": 1,
        "strategy_id": STRATEGY_ID,
        "name": "动态持有 Top1 三周期融合 pos84 6%止盈 dd0714 增强",
        "code_name": "dynamic_h2_m3_c098_w78_5d12_3d10_pos84_e097_mh1_sl06_dd0714_tp060",
        "status": "production_candidate_pending_audit_not_published",
        "created_at": "2026-06-24",
        "created_by": "strategy-agent",
        "definition": "基于 formal L4 10D/5D/3D 资产，按 0.78*10D rank + 0.12*5D rank + 0.10*3D rank 选 Top1，目标仓位 84%，动态持有 2-3 日，日内 6% 止损，账户回撤缩放 soft=7% / hard=14% / scale=0.80/0.60，并新增 6% 止盈。",
        "input_contract": {
            "source_mode": "formal_l4_3d5d10d_only",
            "approval_required": "approved_for_l5",
            "legacy_odb_forbidden_by_default": True,
        ...
        "admission_status": {
            ...
            "known_risks": [
            "recent60 年化低于无止盈主线，且最大回撤接近 40%，属于收益增强与风险边界折中项，需要审计复核。",
    ...
    return {
        ...
        "strong_gate_precheck": {
            ...
            "takeprofit_neighborhood": {
                ...
                "best_case": "tp060_pos84_dd0714_s8060",
                "evidence": "research/dd_neighborhood_report.md",
            },
...
def main() -> int:
    ...
    _copy(SIGNAL_FILE, ARCHIVE_DIR / "signals" / "historical_sl06_dd0714_pos84_tp060.csv")
    ...
    for name in ["summary.csv", "summary_by_annual.csv", "position_report.md", "hard_gate_audit.json"]:
        dst = "position_hard_gate_audit.json" if name == "hard_gate_audit.json" else name
        _copy(POSITION_DIR / name, ARCHIVE_DIR / "research" / dst)
    for name in ["summary.csv", "summary_by_annual.csv", "dd_neighborhood_report.md", "hard_gate_audit.json"]:
        dst = f"dd_neighborhood_{name}" if name != "dd_neighborhood_report.md" else name
        _copy(DD_DIR / name, ARCHIVE_DIR / "research" / dst)
    for log_name in [
        "time_full.log",
    ...
    for script in [
        ...
        "tune_dynamic_h2_m3_tp060_position_20260624.py",
        "tune_dynamic_h2_m3_tp060_pos84_dd_neighborhood_20260624.py",
        "validate_dynamic_h2_m3_sl06_dd0714_pos84_tp060_20260624.py",
        "archive_dynamic_h2_m3_sl06_dd0714_pos84_tp060_candidate_20260624.py",
    ]:
