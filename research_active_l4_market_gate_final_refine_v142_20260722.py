...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_market_gate_final_refine_v142_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v142_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4策略市场门控最终窄邻域研究")
    ...
    if args.open_known_2026:
        ...
        for item in frozen["candidates"]:
            for start in protocol["known_stress_buy_starts"]:
                daily = v139.run_case(full, score, order, item["definition"], protocol, protocol["known_stress_end"], start)
    ...
    rows, definitions = [], {}
    for lookback, return_min in product(protocol["grid"]["market_lookback"], protocol["grid"]["market_return_min"]):
        definition = {**protocol["fixed_definition"], "market_lookback": lookback, "market_return_min": return_min}
        case_id = stable_id(definition)
        definitions[case_id] = definition
        daily = v139.run_case(observation, score_obs, order_obs, definition, protocol, protocol["observation_end"])
