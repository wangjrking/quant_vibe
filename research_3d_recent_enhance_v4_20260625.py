...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_3d_recent_enhance_v4_research_20260625"
TARGET_TABLE = "stock_predict_data_model_agent_3d_recent_enhance_v4_20260625_executable_3d_open_return_research"
...
SOURCES = {
    ...
    "blend3": "stock_predict_data_model_agent_3d_dynamic_blend_20260625_executable_3d_open_return_research",
...
def build_features(scores: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for trade_date, group in scores.groupby("trade_date", sort=True):
        base10 = top_set(group, "base3_rank", 10)
        base20 = top_set(group, "base3_rank", 20)
        base50 = top_set(group, "base3_rank", 50)
        row: dict[str, float | str] = {"trade_date": trade_date}
        for alias in SOURCES:
            score_col = f"{alias}_score"
            rank_col = f"{alias}_rank"
            ...
            if alias != "base3":
                ...
                row[f"overlap10_base3_{alias}"] = len(base10 & top_set(group, rank_col, 10))
                row[f"overlap20_base3_{alias}"] = len(base20 & top_set(group, rank_col, 20))
                row[f"overlap50_base3_{alias}"] = len(base50 & top_set(group, rank_col, 50))
        rows.append(row)
    return pd.DataFrame(rows)
...
def scan(scores: pd.DataFrame, labels: pd.DataFrame, features: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    metric_by_source = pd.concat([source_daily_metrics(eval_frame, alias) for alias in SOURCES], ignore_index=True)
    dates = sorted(metric_by_source["trade_date"].unique().tolist())
    base = metric_by_source[metric_by_source["source"] == "base3"].set_index("trade_date")
    feature_idx = features.set_index("trade_date")
    rows = []
    daily_rows = []
    alt_sources = ["cross3v2", "cross3", "blend3", "recent5d", "stable5v3", "formal5", "recent10d", "cross10", "formal10", "stable1", "formal1"]
    for alt in alt_sources:
        alt_daily = metric_by_source[metric_by_source["source"] == alt].set_index("trade_date")
        candidate_features = [c for c in feature_idx.columns if c.startswith(f"{alt}_") or c.endswith(f"_{alt}") or c.startswith("base3_")]
        for feature in candidate_features:
            vals = feature_idx[feature].replace([np.inf, -np.inf], np.nan).dropna()
            if vals.nunique() < 3:
                continue
            ...
            for threshold in thresholds:
                for op in ["<=", ">="]:
                    ...
                    for prefix, cur, base_sum in [("full", full, base_full), ("recent63", r63, b63), ("recent20", r20, b20)]:
                        for metric in ["rank_ic", "top1", "top3", "top5", "top10", "top20", "top50"]:
                            row[f"{prefix}_{metric}_delta_vs_base3"] = cur[metric] - base_sum[metric]
                    row.update(period_stats(merged))
                    row["pass_basic"] = bool(
                        row["recent63_active_days"] >= 5
                        and row["recent63_top1_delta_vs_base3"] > 0
                        and row["recent63_top5_delta_vs_base3"] > 0
                        and row["recent63_top10_delta_vs_base3"] > 0
                        and row["recent20_top5_delta_vs_base3"] >= 0
                        and row["full_top5_delta_vs_base3"] >= 0
                        and row["min_period_top5_delta"] >= -0.00025
                        and row["positive_top5_periods"] >= 2
                    )
                    row["objective"] = (
                        1.2 * row["recent63_top1_delta_vs_base3"]
                        + 2.0 * row["recent63_top5_delta_vs_base3"]
                        + row["recent20_top5_delta_vs_base3"]
                        + 0.8 * row["full_top5_delta_vs_base3"]
...
def write_candidate(scores: pd.DataFrame, features: pd.DataFrame, best: pd.Series) -> dict:
    ...
    active = active <= threshold if op == "<=" else active >= threshold
    active_dates = set(active[active].index.astype(str))
    out = scores[["trade_date", "stock_code", "base3_score", "base3_rank", f"{alt}_score", f"{alt}_rank"]].copy()
    out["guard_active"] = out["trade_date"].isin(active_dates)
    ...
    keep = ["trade_date", "stock_code", "pred_prob", "base3", alt, "base3_rank", f"{alt}_rank", "guard_active", "score_formula"]
    with sqlite3.connect(MODEL_DB, timeout=300) as conn:
        conn.execute("PRAGMA busy_timeout=300000")
        conn.execute(f"drop table if exists {quote(TARGET_TABLE)}")
        out[keep].to_sql(TARGET_TABLE, conn, if_exists="replace", index=False)
...
def write_outputs(scan_result: pd.DataFrame, daily: pd.DataFrame, source_metrics: pd.DataFrame, stats: dict | None) -> None:
    ...
    if stats:
        lines.extend(
            [
                "本轮找到一个相对 3D 当前研究基准改善近期 Top 指标的 research-only 候选；它不是稳定化发布候选，仍需后续评价和审批。",
                "",
                "候选规则：",
                "",
                "```text",
                ...
                "## 相对 3D 当前研究基准的扫描摘要",
                "",
                f"- 近 63 日 Top1 增量：`{best['recent63_top1_delta_vs_base3']:.6f}`",
                f"- 近 63 日 Top5 增量：`{best['recent63_top5_delta_vs_base3']:.6f}`",
                f"- 近 63 日 Top10 增量：`{best['recent63_top10_delta_vs_base3']:.6f}`",
        ...
        lines.append("本轮未找到相对 3D 当前研究基准通过近期增强门槛的候选。")
    lines.extend(
        [
            "",
            "## 证据路径",
