...
SIGNAL_ROOT = DATA / "reports" / "strategy_agent_adaptive70w_fouryear_pos60_daydrop96_target_grid_20260629"
OUT_DIR = DATA / "reports" / "strategy_agent_adaptive70w_fouryear_pos70_sell_micro_20260629"
...
SIGNAL_FILE = SIGNAL_ROOT / "signals" / f"{CASE_KEY}__daydrop96__pos70.csv"
...
def _ensure_case_signal(case: dict[str, Any], base_rows: list[dict[str, Any]]) -> Path:
    signal_path = OUT_DIR / "signals" / f"{CASE_KEY}__pos70__{case['name']}.csv"
...
def _run_case(case: dict[str, Any], signal_file: Path, tag: str, start: str, end: str, signal_rows: int) -> dict[str, Any]:
    log_file = OUT_DIR / "logs" / f"{CASE_KEY}__pos70__{case['name']}__{tag}.log"
...
def main() -> None:
    ...
    for case in SELL_CASES:
        signal_file = _ensure_case_signal(case, base_rows)
        signal_rows = _signal_rows(signal_file)
        rows = [_run_case(case, signal_file, tag, start, end, signal_rows) for tag, start, end in SLICES]
        detail.extend(rows)
