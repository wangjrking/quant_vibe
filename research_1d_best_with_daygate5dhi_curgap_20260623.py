...
OUT_DIR = DATA_DIR / "reports" / "model_agent_1d_best_with_daygate5dhi_curgap_20260623"
...
CURRENT_TABLE = "stock_predict_data_model_agent_1d_best_with_daygate5dhi_20260623_executable_1d_open_return_research"
RESEARCH_TABLE = "stock_predict_data_model_agent_1d_best_with_daygate5dhi_curgap_20260623_executable_1d_open_return_research"
...
def main() -> int:
    ...
    for trade_date, group in frame.groupby("trade_date", sort=True):
        ...
        use_daygate = newcomer_mean_5d_pct >= NEWCOMER_MEAN_5D_PCT_THRESHOLD and cur_top1_gap <= CUR_TOP1_GAP_MAX
    ...
    payload = {
        ...
        "candidate_id": "model_agent_1d_best_with_daygate5dhi_curgap_20260623",
        ...
        "rule": {
            "type": "daily_gate_daygate_and_curgap",
        ...
        "governance_notes": [
            ...
            "designed to preserve current recent63 front metrics while further repairing q2 weak window",
            "no training",
    ...
    compare_windows(best_eval, selected_eval).to_csv(
    ...
    manifest = {
        ...
        "notes": "1D 研究候选以当前最强候选为默认排序，仅当 daygate 的 newcomer_mean_5d_pct 足够高且 current 1D 的 cur_top1_gap 不大时，才切换到 daygate。目标是在不额外伤 recent63 前排的前提下，继续修复弱窗口。",
    }
    ...
    report_lines = [
        "# 1D 当前最强候选叠加 daygate 高 5D 一致性 + curgap 双门控",
        "",
        "## 当前结论",
        "",
        ...
        f"- 门控 1：`newcomer_mean_5d_pct >= {NEWCOMER_MEAN_5D_PCT_THRESHOLD}`",
        f"- 门控 2：`cur_top1_gap <= {CUR_TOP1_GAP_MAX}`",
        ...
        "",
        "- 默认沿用当前最强 1D 候选。",
        "- 只有当 daygate 的 5D 一致性足够高，且 current 1D 当日头部优势没有明显拉开时，才切到 daygate。",
        "- 这个双门控的目标，是保住 recent63 前排不继续回落，同时把 daygate 对 Q2 弱窗口的修复继续保留下来。",
        "",
        "## 窗口结果",
        "",
