...
OUT_DIR = DATA_DIR / "reports" / "model_agent_5d_topgate_fusion_tune_20260622"
...
RESEARCH_TABLE = (
    "stock_predict_data_model_agent_5d_topgate_fusion_20260622_"
    "executable_5d_open_return_research"
)
...
def materialize(merged: pd.DataFrame, selected: dict, selected_delta: dict) -> dict:
    ...
    manifest = {
        ...
        "score_formula": (
            "base_5d_aux_rank + 0.001 * formal_5d_rank "
            "when base_5d_aux_rank >= 0.9975; otherwise base_5d_aux_rank"
        ),
        ...
        "selected_delta_vs_base_5d_aux": selected_delta,
        "notes": "5D aux 前排内部微重排研究资产；不作为 L5 正式入口。",
    }
...
def main() -> int:
    ...
    with sqlite3.connect(MODEL_DB) as conn:
        merged = read_prediction(conn, BASE_TABLE, "base_5d_aux_pred_prob", keep_payload=True)
        if "formal_5d_pred_prob" not in merged.columns:
            aux = read_prediction(conn, AUX_TABLE, "formal_5d_pred_prob")
            merged = merged.merge(aux, on=["trade_date", "stock_code"], how="left", validate="one_to_one")

    add_rank(merged, "base_5d_aux_pred_prob", "rank_base_5d_aux")
    add_rank(merged, "formal_5d_pred_prob", "rank_formal_5d")
    merged["rank_formal_5d"] = merged["rank_formal_5d"].fillna(merged["rank_base_5d_aux"])
    merged["topgate_score"] = merged["rank_base_5d_aux"]
    merged["topgate_applied"] = merged["rank_base_5d_aux"] >= BASE_RANK_THRESHOLD
    merged.loc[merged["topgate_applied"], "topgate_score"] = (
        merged.loc[merged["topgate_applied"], "rank_base_5d_aux"]
        + AUX_ADD_ALPHA * merged.loc[merged["topgate_applied"], "rank_formal_5d"]
    )

    baseline, baseline_daily = evaluate("base_5d_aux_raw", merged, "base_5d_aux_pred_prob")
    selected, selected_daily = evaluate("topgate_base_5d_aux_plus_formal_5d", merged, "topgate_score")
    ...
    materialize_asset = (
        ...
        and selected_delta["top_k_mean_returns"]["1"] > 0.001
        ...
        and selected_delta["top_k_mean_returns"]["20"] >= -0.00005
    )
    ...
    payload = {
        ...
        "manifest": manifest,
        "selection_rule": (
            "仅在当前 5D aux 每日最前 0.25% 内用 formal 5D rank 微重排；"
            "要求 Top1/Top3/Top5/Top10 均不低于当前 5D aux，且 RankIC 不明显回撤。"
        ),
    }
    ...
    if materialize_asset:
        lines.append("候选 `topgate_base_5d_aux_plus_formal_5d` 满足研究落库门槛，已生成 research-only L4 资产。")
    else:
        lines.append("候选 `topgate_base_5d_aux_plus_formal_5d` 未满足研究落库门槛，未生成新资产。")
    lines.extend(
        [
            "",
            "## 相对当前 5D aux 的变化",
