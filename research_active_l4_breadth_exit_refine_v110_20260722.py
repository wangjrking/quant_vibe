...
def stable_id(definition: dict) -> str:
    ...
    return "v110_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="独立卖出分数阈值邻域研究")
    ...
    if args.open_known_2026:
        ...
        for item in frozen["candidates"]:
            for start in protocol["known_stress_buy_starts"]:
                daily = v109.simulate(full, score, order, item["definition"], protocol, protocol["known_stress_end"], start)
    ...
    rows, definitions = [], {}
    for threshold in protocol["grid"]["sell_score_below"]:
        definition = {**protocol["fixed_exit"], "sell_score_below": threshold}
        case_id = stable_id(definition)
        definitions[case_id] = definition
        daily = v109.simulate(observation, score_obs, order_obs, definition, protocol, protocol["observation_end"])
