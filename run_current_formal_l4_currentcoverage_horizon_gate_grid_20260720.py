...
FEATURE_DB = (
    ROOT / "quant/data_file/reports/strategy_agent_current_formal_l4_persistent_edge_20260720/persistent_edge_features.duckdb"
)
SCORE_DB = (
    ROOT / "quant/data_file/reports/strategy_agent_current_formal_l4_cap5_rank_hold_refine_20260720/score_assets/w10_100.duckdb"
)
OUT = ROOT / "quant/data_file/reports/strategy_agent_current_formal_l4_currentcoverage_horizon_gate_grid_20260720"
SIGNALS = OUT / "signals"
LOGS = OUT / "logs"

CASES = [
    {"name": f"gate5_{int(threshold * 100)}", "rank3_min": 0.0, "rank5_min": threshold}
    for threshold in (0.0, 0.20, 0.40, 0.50, 0.60, 0.70, 0.80)
] + [
    {"name": f"gate3_{int(threshold * 100)}_gate5_50", "rank3_min": threshold, "rank5_min": 0.50}
    for threshold in (0.20, 0.40, 0.50, 0.60, 0.70, 0.80)
]
...
def build_signal(con: duckdb.DuckDBPyConnection, case: dict) -> Path:
    frame = con.execute(
        ...
        WITH eligible AS (
            ...
            WHERE rank_10d >= 0.98
              AND rank_3d >= {float(case['rank3_min'])}
        ...
        SELECT * FROM ranked WHERE pick_rank = 1 AND rank_1d >= 0.88 ORDER BY trade_date
        """
    ).fetchdf()
    ...
    frame["filter_name"] = "current_formal_l4_horizon_gate_grid"
...
def run_case(con: duckdb.DuckDBPyConnection, case: dict) -> dict:
    ...
    return {
        **case, "signal_days": int(signal_frame["signal_date"].nunique()),
