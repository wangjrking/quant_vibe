...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_research_10d_current_vs_fixed4y_daygate_scan_20260626"
...
def main() -> int:
    ...
    result = pd.DataFrame(rows).sort_values(["objective", "top1", "top3"], ascending=False).reset_index(drop=True)
    best = result.iloc[0].to_dict()
    summary = {
        "generated_at": now_iso(),
        "scope": "research_only_10d_current_vs_fixed4y_daygate_fold08",
        "label": LABEL,
    ...
    lines = [
        "# 10D 当前主线对近四年训练候选的日级门控扫描报告（20260626）",
        "",
        "## 当前结论",
        "",
        ...
        "- 如果最优门控仍然打不过当前主线，说明近四年训练候选在当前 10D 主线上没有形成足够稳定的增益日特征。",
        "",
        "## 产物",
        "",
