"""Evaluate the frozen Top10-internal HGB rank ordering research candidate."""
...
CONTRACT = Path("quant/data_file/reports/model_agent_10d_hist_top10_internal_rank_20260830/training_contract.json")
...
DEFAULT_OUTPUT = Path("quant/data_file/reports/model_agent_10d_hist_top10_internal_rank_20260830/build_r1")
PREFIX = 10


def project_one_day(group: pd.DataFrame) -> pd.DataFrame:
    production = group.sort_values(["baseline_pred_prob", "stock_code"], ascending=[False, True], kind="mergesort").copy()
    prefix = production.head(PREFIX).sort_values(["candidate_pred_prob", "stock_code"], ascending=[False, True], kind="mergesort")
    tail = production.iloc[PREFIX:]
    projected = pd.concat([prefix, tail], ignore_index=True)
    if set(projected.head(PREFIX)["stock_code"]) != set(production.head(PREFIX)["stock_code"]):
    ...
    projected["baseline_rank"] = projected["stock_code"].map({code: pos + 1 for pos, code in enumerate(production["stock_code"])}).astype("int32")
...
def project(source: pd.DataFrame) -> pd.DataFrame:
    return pd.concat([project_one_day(g) for _, g in source.groupby("trade_date", sort=True)], ignore_index=True).sort_values(["trade_date", "stock_code"], kind="mergesort").reset_index(drop=True)
...
def run(output: Path) -> int:
    ...
    dump_json(output / "preflight.json", {"candidate_id": CANDIDATE_ID, "contract_sha256": sha256_file(CONTRACT), "hgb_oof_sha256": sha256_file(HGB_OOF), "development_window": ["20220101", "20241231"], "sealed_windows": {"2025": "not_read", "2026_plus": "not_read"}, "production_unchanged": True})
    replays = [project(source) for _ in range(3)]
    replay_hashes = [canonical_frame_hash(x, ["trade_date", "stock_code", "candidate_raw_score"]) for x in replays]
    ...
    for fold_id, group in oof.groupby("fold_id", sort=True):
        ...
        folds[str(fold_id)] = {"rows": int(len(group)), "baseline_metrics": fold_base, "candidate_metrics": fold_candidate, "failed_gates": compare_metrics(fold_base, fold_candidate)}
    failures = compare_metrics(base_metrics, candidate_metrics) + [failure for result in folds.values() for failure in result["failed_gates"]]
    ...
    summary = {"candidate_id": CANDIDATE_ID, "status": "completed_waiting_for_fixed_readonly_audit" if passed else "reject_no_further_search", "same_key_rows": int(len(oof)), "same_key_duplicate_groups": int(oof.duplicated(["trade_date", "stock_code"]).sum()), "bj_rows": int(oof.stock_code.astype(str).str.endswith(".BJ").sum()), "null_or_nonfinite_rows": int((~np.isfinite(oof[["target", "baseline_pred_prob", "candidate_raw_score"]].to_numpy(dtype="float64")).all(axis=1)).sum()), "projection": {"mutable_prefix_size": PREFIX, "changed_rank_row_share": changed_share, "top10_membership_identical": True}, "aggregate": {"baseline_metrics": base_metrics, "candidate_metrics": candidate_metrics, "failed_gates": compare_metrics(base_metrics, candidate_metrics)}, "folds": folds, "hard_gate_passed": passed, "failed_gates": failures, "deterministic": {"passed": True, "replay_candidate_score_sha256": replay_hashes}, "baseline_score_sha256": canonical_frame_hash(base, ["trade_date", "stock_code", "pred_prob"]), "candidate_score_sha256": canonical_frame_hash(candidate, ["trade_date", "stock_code", "pred_prob"]), "production_unchanged": True, "allow_next_layer_continue": False}
    ...
    dump_json(output / "hash_inventory.json", {"script_sha256": sha256_file(Path(__file__)), "contract_sha256": sha256_file(CONTRACT), "source_hgb_oof_sha256": sha256_file(HGB_OOF), "oof_sha256": sha256_file(output / "baseline_candidate_same_key_oof.parquet"), "summary_sha256": sha256_file(output / "evaluation_summary.json")})
    return 0
