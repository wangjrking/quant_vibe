...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_5d_v6shrink_fixed4y_rankcorr_gate_20260626"
...
TARGET_TABLE = "stock_predict_data_model_agent_5d_v6shrink_fixed4y_rankcorr_gate_20260626_executable_5d_open_return_research"
ALT_PATH = (
    DATA_DIR
    / "reports"
...
def add_gate(scores: pd.DataFrame) -> pd.DataFrame:
        ...
        gate_rows.append(
            {
                ...
                FEATURE: float(group["base_rank"].corr(group["alt_rank"])),
            }
        )
    ...
    out[FEATURE] = out[FEATURE].fillna(np.nan)
...
def evaluate(scored: pd.DataFrame, labels: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, dict[str, object]]:
    ...
    candidate = daily_metrics(eval_frame, "pred_prob", "fixed4y_rankcorr_gate")
...
def write_outputs(
    ...
    metrics_path = REPORT_DIR / "rankcorr_gate_daily_metrics.csv"
    delta_formal_path = REPORT_DIR / "rankcorr_gate_delta_vs_formal.csv"
    delta_base_path = REPORT_DIR / "rankcorr_gate_delta_vs_base.csv"
    manifest_path = REPORT_DIR / "rankcorr_gate_research_manifest.json"
    report_path = REPORT_DIR / "rankcorr_gate_research_report.md"
    ...
    manifest = {
        ...
        "candidate_id": "model_agent_5d_v6shrink_fixed4y_rankcorr_gate_20260626",
        "score_formula": stats["formula"],
        "research_hypothesis": "use_fixed4y_candidate_only_on_high_rankcorr_days_with_current_5d_mainline",
    ...
    lines = [
        "# 5D v6_shrink 与近四年候选的 rank_corr 门控研究报告（20260626）",
        "",
        "## 当前结论",
        "",
        "这是一个 research-only 的 5D 全覆盖候选，不是 formal L4 或 L5 生产资产。",
        f"它只在与近四年候选重叠的 {stats['overlap_days']} 个交易日里，挑出 `rank_corr >= {THRESHOLD:.15f}` 的交易日切到近四年候选，其余日期保持当前 5D 主线不变。",
        "",
        "## 评分公式",
        "",
        "```text",
        ...
        "## 当前判断",
        "",
        "- 这条分支验证了一个关键点：近四年训练候选虽然整体远弱于当前 5D 主线，但在极少数与主线高度同向的交易日上，切换到近四年候选仍可能带来增益。",
        "- 这是增量修补型候选，不是新的独立训练主线。",
        "",
        "## 边界",
        "",
        "- 未训练模型。",
