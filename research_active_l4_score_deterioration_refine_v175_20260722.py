...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_score_deterioration_refine_v175_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v175_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 score deterioration refinement")
    ...
    if args.export_actions:
        ...
        actions.to_csv(OUT / "best_actions_through_20260720.csv", index=False, encoding="utf-8-sig")
        daily.to_csv(OUT / "best_daily_through_20260720.csv", index=False, encoding="utf-8-sig")
        print(json.dumps({"case_id": item["case_id"], "actions": len(actions), "days": len(daily)}, ensure_ascii=False))
        return
    ...
    if args.open_known_2026:
        ...
        for item in frozen["candidates"]:
            for start in protocol["known_stress_buy_starts"]:
                daily = v174.run_case(full, score, order, item["definition"], protocol, protocol["known_stress_end"], start)
    ...
    rows, definitions = [], {}
    for threshold in protocol["grid"]["max_rank_deterioration"]:
        definition = {**protocol["fixed_definition"], "max_rank_deterioration": float(threshold)}
        case_id = stable_id(definition)
        definitions[case_id] = definition
        daily = v174.run_case(observation, score_obs, order_obs, definition, protocol, protocol["observation_end"])
