...
import research_active_l4_deterioration_window_v182_20260723 as v182
import research_active_l4_robust_objective_v56_20260721 as robust
import research_preregistered_active_l4_rank_rotation_20260721 as core


ROOT = Path(__file__).resolve().parents[2]
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_filtered_score_smoothing_v185_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v185_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def run_case(arrays, definition, protocol, end, start=None):
    ...
    return v162.run_case(
        ...
        start,
        selection_mask_override=v182.selection_mask(arrays, definition),
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 filtered score-smoothing study")
    ...
    rows, definitions = [], {}
    for window, current_weight in product(protocol["grid"]["smooth_window"], protocol["grid"]["current_weight"]):
        definition = {**protocol["fixed_definition"], "smooth_window": int(window), "deterioration_window": int(window), "current_weight": float(current_weight)}
