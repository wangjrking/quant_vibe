...
REPORT_DIR = BASE_DIR / "dynamic_h2_m3_pos76_score_takeprofit_20260624"
BASE_SIGNAL_FILE = BASE_DIR / "dynamic_h2_m3_sl06_dd0612_position_20260624" / "signals" / "sl06_dd0612_pos76.csv"
...
SCORE_TAKE_PROFIT_RATIOS = [None, 0.98, 1.00, 1.02, 1.05]
...
def _case_name(ratio: float | None) -> str:
    if ratio is None:
        return "pos76_sl06_dd0612_scoretp_none"
    return f"pos76_sl06_dd0612_scoretp{int(round(ratio * 1000)):04d}"


def _signal_for_case(ratio: float | None) -> Path:
    if not BASE_SIGNAL_FILE.exists():
        raise FileNotFoundError(BASE_SIGNAL_FILE)
    path = REPORT_DIR / "signals" / f"{_case_name(ratio)}.csv"
    if path.exists():
        return path
    ...
    for row in rows:
        ...
        row["signal_score_take_profit_ratio"] = "" if ratio is None else f"{ratio:.5f}"
        row["strategy_variant"] = f"dynamic_h2_m3_pos76_sl06_dd0612_score_takeprofit_{ratio or 'none'}"
...
def _run(ratio: float | None, signal_file: Path, start_name: str, start: str, end: str) -> dict:
    case_name = _case_name(ratio)
    ...
    if indicator is None:
        ...
        if ratio is not None:
            command.extend(["--score-take-profit-ratio", str(ratio)])
...
def _summarize(detail: list[dict], signal_stats: dict[str, dict]) -> list[dict]:
    ...
    for case_name in sorted({row["case_name"] for row in detail}):
        ...
        out.append(
            {
                "case_name": case_name,
                "score_take_profit_ratio": rows[0]["score_take_profit_ratio"],
...
def _write_report(summary: list[dict], audit: dict) -> None:
    ...
    lines = [
        ...
        "## 当前结论",
        "",
        "本轮固定入场信号、目标仓位 76%、6% 日内止损、`dd0612_s8060` 账户回撤缩放和动态持有规则，只调整 `score_take_profit_ratio`。该规则只在持仓盈利且当前 10D 分数相对入场分数走弱时卖出，不改变股票池。",
        "",
        "| 候选 | 分数止盈比例 | 全周期年化 | Sharpe | 最大回撤 | recent60 年化 | recent60 Sharpe | recent60 最大回撤 | 近期开仓最差年化 |",
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for row in ranked:
        ratio_text = "无" if row["score_take_profit_ratio"] in (None, "") else f"{_f(row['score_take_profit_ratio']):.0%}"
        lines.append(
            f"| {row['case_name']} | {ratio_text} | {_f(row['full_annual']):.2%} | {_f(row['full_sharpe']):.2f} | "
    ...
    (REPORT_DIR / "score_takeprofit_report.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> int:
    ...
    for ratio in SCORE_TAKE_PROFIT_RATIOS:
        signal_file = _signal_for_case(ratio)
        ...
        for start_name, start, end in STARTS:
            row = _run(ratio, signal_file, start_name, start, end)
            detail.append(row)
