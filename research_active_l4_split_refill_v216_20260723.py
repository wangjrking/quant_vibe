...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_split_refill_v216_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v216_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def run_case(arrays, score, order, definition, protocol, end, start=None):
    refill = int(definition["max_refill_buys"])
    return v162.run_case(
        ...
        refill_after_sells_override=refill > 1,
        max_refill_buys_override=refill,
        split_candidate_target_override=refill > 1,
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 固定总预算多候选补位研究")
    ...
    rows, definitions = [], {}
    for refill in protocol["grid"]["max_refill_buys"]:
        definition = {**protocol["fixed_definition"], "max_refill_buys": int(refill)}
