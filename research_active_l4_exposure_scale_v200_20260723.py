...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_exposure_scale_v200_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v200_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 正常与强势仓位尺度研究")
    ...
    rows, definitions = [], {}
    for exposure_scale in protocol["grid"]["exposure_scale"]:
        definition = {
            **protocol["fixed_definition"],
            "exposure_scale": float(exposure_scale),
            "normal_target_pct": 0.25 * float(exposure_scale),
            "strong_target_pct": 0.375 * float(exposure_scale),
        }
