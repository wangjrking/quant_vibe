...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_weak_target_v201_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v201_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 弱势市场单票仓位研究")
    ...
    rows, definitions = [], {}
    for weak_target_pct in protocol["grid"]["weak_target_pct"]:
        definition = {**protocol["fixed_definition"], "weak_target_pct": float(weak_target_pct)}
