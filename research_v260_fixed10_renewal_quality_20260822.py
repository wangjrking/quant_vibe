...
OUTPUT_ROOT = (
    REPO
    / "quant/data_file/reports/strategy_agent_v260_fixed10_renewal_quality_20260822"
)
MAX_HOLD_RENEWAL_SCORE_THRESHOLDS = (None, 0.80, 0.85, 0.90)


def candidate_id(threshold: float | None) -> str:
    return (
        "renewal_score_gate_off"
        if threshold is None
        else f"renewal_score_gate_{threshold:.2f}"
    )


def candidate_policy(threshold: float | None) -> dict:
    current = daily_exit.candidate_policy(1)
    current["max_hold_renewal_score"] = threshold
    return current
...
def main() -> None:
    ...
    if access["logical_max_date"] != round1.DEVELOPMENT_END:
        raise PermissionError("2026 data entered renewal-quality development")
    definition = harness.production_definition(protocol)
    score, order = harness.v95.score_pair(arrays, 0.0, 7, 0.1)

    results = {}
    run_cache = {}
    for threshold in MAX_HOLD_RENEWAL_SCORE_THRESHOLDS:
    ...
    if not all(default_equivalence.values()):
        raise RuntimeError("renewal-quality runtime changed the disabled path")
    ...
    if not all(deterministic.values()):
        raise RuntimeError("renewal-quality deterministic replay failed")
    ...
    result = {
        ...
        "candidate_budget": [
            ...
            for value in MAX_HOLD_RENEWAL_SCORE_THRESHOLDS
        ],
        "selection_rule": (
