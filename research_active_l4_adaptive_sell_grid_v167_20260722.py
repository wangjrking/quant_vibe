...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_adaptive_sell_grid_v167_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v167_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4自适应策略卖出邻域研究")
    ...
    if args.open_known_2026:
        ...
        for item in frozen["candidates"]:
            for start in protocol["known_stress_buy_starts"]:
                daily = v162.run_case(full, score, order, item["definition"], protocol, protocol["known_stress_end"], start)
    ...
    rows, definitions = [], {}
    for max_hold, sell_floor, advantage in product(protocol["grid"]["max_hold_days"], protocol["grid"]["sell_score_below"], protocol["grid"]["replacement_advantage"]):
        definition = {**protocol["fixed_definition"], "max_hold_days": max_hold, "sell_score_below": sell_floor, "replacement_advantage": advantage}
        case_id = stable_id(definition)
        definitions[case_id] = definition
        daily = v162.run_case(observation, score_obs, order_obs, definition, protocol, protocol["observation_end"])
