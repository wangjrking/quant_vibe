...
OUT = (
    ROOT
    / "quant/data_file/reports/strategy_agent_active_l4_v260_exposure_surface_v279_20260723"
)
...
def stable_id(definition: dict) -> str:
    ...
    return "v279_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def definitions(protocol: dict) -> list[dict]:
    ...
    return [
        {
            **base,
            "normal_target_pct": float(normal_target_pct),
            "strong_target_pct": float(strong_target_pct),
        }
        for normal_target_pct, strong_target_pct in product(
            protocol["grid"]["normal_target_pct"],
            protocol["grid"]["strong_target_pct"],
        )
    ]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="V260 常态与强势暴露二维邻域")
    ...
    if args.open_known_2026:
        ...
        (
            frame.groupby(["normal_target_pct", "strong_target_pct"])
            .agg(
