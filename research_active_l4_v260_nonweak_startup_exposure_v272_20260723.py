...
OUT = (
    ROOT
    / "quant/data_file/reports/strategy_agent_active_l4_v260_nonweak_startup_exposure_v272_20260723"
)
...
def stable_id(definition: dict) -> str:
    ...
    return "v272_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def definition_for(protocol: dict, nonweak_startup_scale: float) -> dict:
    return {
        **v260.definition_for(protocol, 50),
        "startup_exposure_days": 50,
        "nonweak_startup_scale": float(nonweak_startup_scale),
    }
...
def target_multiplier(arrays, score, definition, protocol, start):
    ...
    scale = float(definition["nonweak_startup_scale"])
...
def main() -> None:
    parser = argparse.ArgumentParser(description="V260 非弱势冷启动仓位倍率研究")
    ...
    definitions = [
        ...
        for value in protocol["grid"]["nonweak_startup_scale"]
    ]
    rows = []
    full_rows = []
    frozen = []
        ...
        if args.open_known_2026:
            for start in protocol["known_stress_starts"]:
                ...
                rows.append(
                    {
                        "case_id": case_id,
                        "nonweak_startup_scale": definition[
                            "nonweak_startup_scale"
                        ],
                        "buy_start": start,
                        **core.metrics(
    ...
    (
        frame.groupby("nonweak_startup_scale")
        .agg(
