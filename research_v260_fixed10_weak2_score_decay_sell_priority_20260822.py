...
OUTPUT_ROOT = (
    ...
    "strategy_agent_v260_fixed10_weak2_score_decay_sell_priority_20260822"
)
CHECKPOINT_PATH = (
    REPO
    / "quant/data_file/reports/"
...
DECAY_WEIGHTS = (0.0, 0.25, 0.50)
DECAY_LAG = 3
MAX_MDD_TOLERANCE = 0.001


def score_change(score: np.ndarray, lag: int) -> np.ndarray:
    values = np.asarray(score, dtype=np.float64)
    if values.ndim != 2 or lag < 1:
        raise ValueError("score matrix and lag are invalid")
    result = np.zeros(values.shape, dtype=np.float64)
    valid = np.isfinite(values[lag:]) & np.isfinite(values[:-lag])
    change = np.zeros(values[lag:].shape, dtype=np.float64)
    change[valid] = values[lag:][valid] - values[:-lag][valid]
    result[lag:] = change
    return result


def decay_priority(
    baseline_priority: np.ndarray,
    change: np.ndarray,
    confirmed_weak: np.ndarray,
    weight: float,
) -> np.ndarray:
    base = np.asarray(baseline_priority, dtype=np.float64)
    delta = np.asarray(change, dtype=np.float64)
    weak = np.asarray(confirmed_weak, dtype=np.bool_)
    if base.shape != delta.shape or weak.shape != (base.shape[0],):
        raise ValueError("score-decay priority inputs do not align")
    result = base.copy()
    result[weak] = base[weak] + float(weight) * delta[weak]
    return result
...
def main() -> None:
    ...
    if checkpoint["validation_2026_opened"]:
        raise PermissionError("2026 validation is unavailable for decay priority")
    ...
    baseline_priority = priority.sell_priority_matrix(
        context.score, volatility_rank, ~confirmed_weak, 0.05
    )
    change = score_change(context.score, DECAY_LAG)

    results, cache = {}, {}
    for weight in DECAY_WEIGHTS:
        candidate_id = f"weak2_score_decay_weight_{weight:.2f}"
        matrix = decay_priority(baseline_priority, change, confirmed_weak, weight)
    ...
    baseline_id = "weak2_score_decay_weight_0.00"
    ...
    if not all(checkpoint_equivalence.values()):
        raise RuntimeError("score-decay priority baseline drifted")
    gates = {
    ...
    if not all(deterministic.values()):
        raise RuntimeError("score-decay priority replay failed")

    result = {
        "status": "weak2_score_decay_sell_priority_complete_2026_not_opened",
        "candidate_budget": [
            f"weak2_score_decay_weight_{value:.2f}" for value in DECAY_WEIGHTS
        ],
        "only_change": (
            "after two consecutive weak sessions, use three-session score change only "
            "to order already-eligible exits; no exit eligibility or count changes"
        ),
        "results": results,
        "selection_gates_against_current_priority": gates,
