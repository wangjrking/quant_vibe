...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_entry_persistence_v45_20260721"
PROTOCOL = OUT / "preregistered_protocol.json"
RATIOS = {"ratio80": 0.80, "ratio90": 0.90}
...
def apply_persistence(masked, score, spec):
    result = {key: value.copy() for key, value in masked.items()}
    lookback = int(spec["lookback"])
    if lookback == 0:
        return result
    threshold = float(spec["previous_score_min"])
    required = int(spec["required_days"])
    confirmed = np.zeros(score.shape, dtype=bool)
    for t in range(lookback, score.shape[0]):
        history = score[t - lookback:t]
        confirmed[t] = np.sum(np.isfinite(history) & (history >= threshold), axis=0) >= required
    result["signal_clean"] &= confirmed
    return result


def profile(entry, amount, mv, total, scheme):
    return v39.BuyProfile(2, float(entry), int(amount), int(mv), float(total), False, 0.99, 0.005, 1.0, str(scheme))
...
def main():
    ...
    try:
        rows = []
        g = p["grid"]
        for persistence in p["persistence_specs"]:
            masked = apply_persistence(base_masked, buy_score, persistence)
                    ...
                    for mv in g["mv_min"]:
                        for total in g["normal_total_pct"]:
                            for scheme in g["weight_scheme"]:
                                bp = profile(entry, amount, mv, total, scheme)
                                ...
                                case_id = f"{persistence['id']}_{bp.case_id}"
                                rows.append({"case_id": case_id, **persistence, **bp.__dict__, **metrics})
        ...
        specs = {item["id"]: item for item in p["persistence_specs"]}
        for _, item in frozen.iterrows():
            masked = apply_persistence(base_masked, buy_score, specs[item["id"]])
            quality = health.candidate_quality(masked, buy_score, order, 3)
            model_exp = continuous.exposure_series(quality, int(e["health_lookback"]), float(e["health_floor_fraction"]), float(e["health_scale_denominator"]))
            exposure = np.minimum(model_exp, market_exp).astype(np.float32)
    ...
    summary = {
        ...
        "all_year_positive": int(results.all_year_positive.sum()),
        "eligible_cases": len(pool),
    ...
    (OUT / "入场持续性研究结论.md").write_text("\n".join([
        "# 入场持续性研究结论",
        "",
        "本轮只用T日及此前评分，检验过去一至两个交易日的高分持续性。",
        "",
        f"固定网格 {len(results)} 组，逐年为正 {int(results.all_year_positive.sum())} 组，强过滤后 {len(pool)} 组。",
