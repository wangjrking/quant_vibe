...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_cohort_allocation_v173_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v173_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 cohort and allocation study")
    ...
    rows, definitions = [], {}
    for cohorts, normal_target in product(
        protocol["grid"]["target_cohorts"],
        protocol["grid"]["normal_target_pct"],
    ):
        definition = {
            **protocol["fixed_definition"],
            "target_cohorts": int(cohorts),
            "normal_target_pct": float(normal_target),
            "strong_target_pct": min(float(normal_target) * 1.5, 0.50),
        }
