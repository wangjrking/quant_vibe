...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_entry_cadence_v184_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v184_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def run_case(arrays, score, order, definition, protocol, end, start=None):
    ...
    every = int(definition["entry_every"])
    offset = int(definition["entry_offset"])
    active[offset::every] = True
    return v109.simulate(
        ...
        rebalance_active_override=active,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 entry-cadence study")
