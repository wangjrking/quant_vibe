"""Build one frozen 3D-plus-1D label candidate under strict pre-2025 PIT.
...
training label: mean of the same-day 3D and 1D cross-sectional percentile
...
CANDIDATE_ID = "v261_3d_plus_1d_daily_rank_label_v1"
...
DEFAULT_OUTPUT = Path("quant/data_file/reports/model_agent_3d_plus_1d_daily_rank_label_20260830")
...
def combined_target(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    valid["candidate_training_target"] = 0.5 * valid["rank_1d"] + 0.5 * valid["rank_3d"]
...
def run(output: Path) -> int:
    ...
    contract = {
        ...
        "training_target": "0.5 * daily_percentile_rank(executable_3d_open_return) + 0.5 * daily_percentile_rank(executable_1d_open_return)",
