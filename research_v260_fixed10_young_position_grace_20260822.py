...
OUTPUT_ROOT = (
    REPO
    / "quant/data_file/reports/strategy_agent_v260_fixed10_young_position_grace_20260822"
)
CANDIDATES = ("constant_exit_085", "young_exit_080_then_085")
YOUNG_POSITION_SESSIONS = 10
YOUNG_EXIT_THRESHOLD = 0.80
MATURE_EXIT_THRESHOLD = 0.85


def threshold_age_bands(candidate_id: str) -> list[tuple[int, float]] | None:
    if candidate_id == "constant_exit_085":
        return None
    if candidate_id == "young_exit_080_then_085":
        return [
            (0, YOUNG_EXIT_THRESHOLD),
            (YOUNG_POSITION_SESSIONS, MATURE_EXIT_THRESHOLD),
        ]
...
def main() -> None:
    ...
    if policy["sell_score_below"] != MATURE_EXIT_THRESHOLD:
        raise ValueError("mature threshold must reuse the frozen exit threshold")
    if policy["max_hold_renewal_score"] != YOUNG_EXIT_THRESHOLD:
        raise ValueError("young threshold must reuse the frozen renewal threshold")

    harness, protocol, rules, manifests, arrays, access = round1.load_arrays(
        round1.DEVELOPMENT_END
    )
    if access["logical_max_date"] != round1.DEVELOPMENT_END:
        raise PermissionError("2026 data entered young-position grace development")
    ...
    for candidate_id in CANDIDATES:
        age_bands = threshold_age_bands(candidate_id)
        common = dict(
            ...
            sell_score_below_age_bands_override=age_bands,
        )
        ...
        results[candidate_id] = {
            "policy": {**policy, "sell_score_below_age_bands": age_bands},
    ...
    current = results["constant_exit_085"]["metrics_0_30pct"]
    ...
    if not all(checkpoint_equivalence.values()):
        raise RuntimeError("young-position current path drifted from checkpoint")
    ...
    repeat_daily, repeat_actions = round1.run_fixed10(
        ...
        entry_block_mask_override=entry_block,
        maintenance_buy_block_mask_override=maintenance_block,
        sell_score_below_age_bands_override=threshold_age_bands(selected_id),
    )
    deterministic = {
    ...
    if not all(deterministic.values()):
        raise RuntimeError("young-position grace deterministic replay failed")
    ...
    result = {
        ...
        "only_change": (
            "reuse the frozen 0.80 threshold for positions younger than 10 sessions, "
            "then restore the frozen 0.85 exit threshold"
        ),
        "results": results,
