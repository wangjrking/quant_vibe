...
OUT_DIR = DATA_DIR / "reports" / "model_agent_10d_proxy5dbest_lite_daygate_std035_20260623"
...
CURRENT_TABLE = "stock_predict_data_model_agent_10d_daygate_joint_refine_20260623_executable_10d_open_return_research"
PREVIOUS_PROXY_TABLE = "stock_predict_data_model_agent_10d_proxy5dv3_lite_daygate_std035_20260623_executable_10d_open_return_research"
RESEARCH_TABLE = "stock_predict_data_model_agent_10d_proxy5dbest_lite_daygate_std035_20260623_executable_10d_open_return_research"
...
def compare_windows(base_windows: dict, previous_windows: dict, current_windows: dict, new_windows: dict) -> pd.DataFrame:
    rows: list[dict] = []
...
def main() -> int:
    ...
    with sqlite3.connect(MODEL_DB) as conn:
        ...
        previous = pd.read_sql_query(
            f"select trade_date, stock_code, pred_prob as pred_previous from '{PREVIOUS_PROXY_TABLE}'",
            conn,
        )
    ...
    compare_windows(base_eval, previous_eval, current_eval, selected_eval).to_csv(
    ...
    payload = {
        ...
        "candidate_id": "model_agent_10d_proxy5dbest_lite_daygate_std035_20260623",
        ...
        "previous_proxy_table": PREVIOUS_PROXY_TABLE,
        "lite_rule": {
    ...
    manifest = {
        ...
        "previous_proxy_ref": f"MODEL_PREDICTIONS.db::{PREVIOUS_PROXY_TABLE}",
        "notes": "10D research 候选沿用当前 lite/day-gate std035 结构，但把 5D proxy 直接替换为当前 5D 最优 research 资产，用于验证 10D 是否仍受旧 proxy 底座约束。",
    }
    ...
    report_lines = [
        "# 10D 使用当前 5D 最优作为 proxy 的 std035 研究候选",
        "",
        "## 当前结论",
        "",
        f"- 当前正式对比候选：`{CURRENT_TABLE}`",
        f"- 上一版 5D-v3 proxy 候选：`{PREVIOUS_PROXY_TABLE}`",
        f"- 新研究候选：`{RESEARCH_TABLE}`",
        f"- 5D proxy：`{BASE5_TABLE}`",
        f"- 相对当前 10D 最优 Focus 增量：`{payload['selected_minus_current_focus']:.12f}`",
        f"- 相对上一版 proxy 候选 Focus 增量：`{payload['selected_minus_previous_focus']:.12f}`",
        "",
        "## 规则",
        "",
        "- lite/day-gate 结构保持不变，只替换 5D proxy 为当前 5D 最优 research 资产。",
