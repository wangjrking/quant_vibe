...
OUT = ROOT / "quant" / "data_file" / "reports" / "strategy_agent_current_formal_l4_3d_gate_top3_independent_20260720"
...
BASE_HIGH_SIGNAL = ROOT / "quant" / "data_file" / "reports" / "strategy_agent_current_formal_l4_5d10d_top3_independent_sell_refine_20260720" / "signals" / "r1ge88_top3_min12_max13_ratio100_edge0.csv"
FLOORS = (0.20, 0.40, 0.50, 0.60, 0.70, 0.80)
for floor in FLOORS:
    MODULE.FILTERS[f"g3_{int(floor*100)}"] = (
        f"least(rank_5d, rank_10d) >= 0.50 AND rank_3d >= {floor} AND signal_pct_chg_raw <= 3.0"
    )


def execute(name: str, frame: pd.DataFrame, score_db: Path) -> dict:
    frame = frame.copy()
    frame["target_pct"], frame["holding_days"], frame["max_holding_days"] = 1.0 / 3.0, 12, 13
...
def main() -> None:
    ...
    try:
        ...
        base = pd.read_csv(BASE_HIGH_SIGNAL)
        for floor in FLOORS:
            no_refill = base[base["rank_3d"] >= floor].copy()
            result = execute(f"no_refill_g3_{int(floor*100)}", no_refill, score_db)
            rows.append(result)
            case = {"blend": "w10_100", "score": "plain", "filter": f"g3_{int(floor*100)}", "threshold": 0.98, "topn": 1, "hold": 12}
            _, refill_path, _, _ = MODULE.build_signal(con, case)
            refill = pd.read_csv(refill_path)
            refill = refill[refill["rank_1d"] >= 0.88].copy()
            rows.append(execute(f"refill_g3_{int(floor*100)}", refill, score_db))
