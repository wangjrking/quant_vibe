...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_research_10d_fixed4y_fold08_featurecount_scan_20260626"
...
def write_feature_subset_files() -> list[Candidate]:
    ...
    counts = [40, 80, 120]
    candidates = [
        Candidate("self_ref_fs160", 160, FEATURES_PATH, False),
    ]
    for count in counts:
        ...
        candidates.append(Candidate(f"self_ref_fs{count}", count, path, True))
    return candidates


def run_candidate(candidate: Candidate) -> dict:
    if not candidate.retrain:
        return {"stdout": "reused existing self_ref fs160 artifact", "stderr": "", "returncode": 0, "reused": True}
...
def main() -> int:
    ...
    packet = {
        ...
        "scope": "research_only_10d_fixed4y_fold08_featurecount_scan",
        "label": LABEL,
