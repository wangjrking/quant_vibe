...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_entry_persistence_v231_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v231_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def run_case(arrays, score, order, definition, protocol, end, start=None):
    ...
    previous_score_min = definition["previous_score_min"]
    if previous_score_min is not None:
        previous = np.full(score.shape, np.nan, dtype=np.float32)
        previous[1:] = score[:-1]
        selection_mask &= np.isfinite(previous) & (previous >= float(previous_score_min))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 前一日分数持续性研究")
    ...
    rows, definitions = [], {}
    for previous_score_min in protocol["grid"]["previous_score_min"]:
        definition = {
            **protocol["fixed_definition"],
            "previous_score_min": None if previous_score_min is None else float(previous_score_min),
        }
