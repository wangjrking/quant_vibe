...
OUT_DIR = REPORT_DIR / "repro500_target_frontier"
SIGNAL_DIR = OUT_DIR / "signals"
LOG_DIR = OUT_DIR / "juejin_runs"
SUMMARY_FILE = OUT_DIR / "target_frontier_summary.csv"
...
def case_grid() -> list[dict]:
    cases: list[dict] = []
    for weak_rule in ["crowded", "late_rebound", "late_or_crowded"]:
        for base in [0.4425, 0.4450, 0.4475, 0.4500, 0.4550, 0.4600]:
            for weak in [0.385, 0.400, 0.415, 0.425]:
                cases.append(
                    {
                        "name": (
                            f"b{base:.4f}_w{weak:.3f}_sbase_{weak_rule}"
                        ).replace(".", "p"),
                        "base": base,
                        "weak": weak,
                        "strong": base,
        ...
        for base, weak, boost in [
            ...
            strong = min(base + boost, 0.49)
            cases.append(
                {
                    "name": (
                        f"b{base:.4f}_w{weak:.3f}_s{strong:.4f}_{weak_rule}"
                    ).replace(".", "p"),
                    "base": base,
                    "weak": weak,
                    "strong": strong,
