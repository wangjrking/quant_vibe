...
OUTPUT_ROOT = (
    ...
    "strategy_agent_v260_fixed10_pressure_cooldown_recheck_20260822"
)
CHECKPOINT_PATH = (
    REPO
    / "quant/data_file/reports/"
...
CANDIDATES = {"cooldown_0d": 0, "cooldown_3d": 3, "cooldown_5d": 5}


def run_policy(context, policy: dict, cooldown_days: int):
    ...
    common = dict(
        ...
        score_sell_pressure_cooldown_days_override=int(cooldown_days),
    )
    daily, actions = round1.run_fixed10(
        context.harness, context.arrays, context.protocol, context.definition,
        context.score, context.order, policy, round1.DEVELOPMENT_END,
        slip=round1.BASELINE_COST, record_actions=True, **common,
    )
    stress_daily, stress_actions = round1.run_fixed10(
        context.harness, context.arrays, context.protocol, context.definition,
        context.score, context.order, policy, round1.DEVELOPMENT_END,
        slip=round1.STRESS_COST, record_actions=True, **common,
    )
    return {
...
def main() -> None:
    ...
    if checkpoint["validation_2026_opened"]:
        raise PermissionError("2026 validation is not available for cooldown development")
    policy = copy.deepcopy(checkpoint["selected_policy"])
    context = harness.load_context(policy)
    results, cache = {}, {}
    for candidate_id, cooldown_days in CANDIDATES.items():
        results[candidate_id], daily, actions = run_policy(
            context, policy, cooldown_days
        )
        cache[candidate_id] = (daily, actions)

    baseline = results["cooldown_0d"]
    ...
    if not all(checkpoint_equivalence.values()):
        raise RuntimeError("pressure cooldown baseline drifted")
    gates = {
        candidate_id: selection_gates(item, baseline)
        for candidate_id, item in results.items()
        if candidate_id != "cooldown_0d"
    }
    passing = [key for key, value in gates.items() if all(value.values())]
    selected = max(
        passing,
        key=lambda key: (
    ...
    if not all(deterministic.values()):
        raise RuntimeError("pressure cooldown replay failed")
    selected_policy = copy.deepcopy(policy)
    selected_policy["score_sell_pressure_cooldown_days"] = CANDIDATES[selected]
    result = {
        "status": "pressure_cooldown_recheck_complete_2026_not_opened",
        ...
        "only_change": (
            "after an executed second pressure score-exit, suppress another extra "
            "pressure exit for a fixed number of official signal sessions"
        ),
        "results": results,
        "selection_gates": gates,
        "selected_candidate": selected,
        "selected_policy": selected_policy,
        "changed_from_checkpoint": selected != "cooldown_0d",
