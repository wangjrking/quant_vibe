...
SPEC = importlib.util.spec_from_file_location("open_gap_sell", SOURCE)
MODULE = importlib.util.module_from_spec(SPEC)
assert SPEC and SPEC.loader
SPEC.loader.exec_module(MODULE)

OUT = ROOT / "quant" / "data_file" / "reports" / "strategy_agent_current_formal_l4_open_gap_takeprofit_20260720"
LOGS = OUT / "logs"
CASES = [
    {"name": "disabled", "gap_min_pct": None, "early_min_hold": 1},
    *[
        {"name": f"gap_p{int(gap)}_h{hold}", "gap_min_pct": gap, "early_min_hold": hold}
...
def run_case(case: dict) -> dict:
    ...
    env.update({
        ...
        "GM_INDEPENDENT_EARLY_OPEN_GAP_MIN_PCT": "" if case["gap_min_pct"] is None else str(case["gap_min_pct"]),
        "GM_INDEPENDENT_EARLY_OPEN_GAP_MIN_HOLD": str(case["early_min_hold"]),
        "GM_RESIZE_HELD_ON_SIGNAL": "0",
    })
    command = [
        sys.executable, str(MODULE.RUNNER), "--strategy-dir", str(MODULE.STRATEGY),
        "--signal-file", str(MODULE.SIGNAL), "--log-file", str(log),
