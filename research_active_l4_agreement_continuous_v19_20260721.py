...
REPORT_DIR = ROOT / "quant" / "data_file" / "reports" / "strategy_agent_active_l4_agreement_continuous_v19_20260721"
...
REPORT_PATH = REPORT_DIR / "一致性连续仓位研究结论.md"
...
def case_id(profile, shadow_top_n):
    raw = json.dumps({"profile": asdict(profile), "shadow_top_n": shadow_top_n}, sort_keys=True, separators=(",", ":"))
    return "ac_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def main():
    ...
    qualities = {top_n: candidate_health.candidate_quality(masked, score, order, top_n) for top_n in protocol["health_to_exposure"]["shadow_top_n"]}
    exposures = {(shadow, lookback, floor, denom): continuous.exposure_series(qualities[shadow], lookback, floor, denom) for shadow in qualities for lookback in protocol["health_to_exposure"]["rolling_lookback"] for floor in protocol["health_to_exposure"]["floor_fraction"] for denom in protocol["health_to_exposure"]["scale_denominator"]}
    fixed = protocol["stage1"]["fixed"]
    rows = []
    for shadow_top_n in qualities:
                    ...
                    for amount_min in protocol["stage1"]["amount_min"]:
                        for top_n in protocol["stage1"]["top_n"]:
                            for entry_rank_min in protocol["stage1"]["entry_rank_min"]:
                                profile = continuous.ExposureProfile(protocol["score"]["id"], amount_min, 200000, top_n, entry_rank_min, lookback, floor, denom, **fixed)
                                daily = continuous.simulate(masked, score, order, exposures[(shadow_top_n, lookback, floor, denom)], profile, protocol["observation_end"])
                                rows.append({"case_id": case_id(profile, shadow_top_n), "shadow_top_n": shadow_top_n, **asdict(profile), **evaluate(daily, protocol["year_folds"])})
                ...
                for sell_rank_below in grid["sell_rank_below"]:
                    for replacement_advantage in grid["replacement_advantage"]:
                        for invested_ratio in grid["invested_ratio"]:
                            ...
                            exposure = exposures[(int(seed.shadow_top_n), profile.health_lookback, profile.floor_fraction, profile.scale_denominator)]
                            daily = continuous.simulate(masked, score, order, exposure, profile, protocol["observation_end"])
                            rows.append({"case_id": case_id(profile, int(seed.shadow_top_n)), "seed_case_id": str(seed.case_id), "shadow_top_n": int(seed.shadow_top_n), **asdict(profile), **evaluate(daily, protocol["year_folds"])})
    ...
    for item in payload["profiles"]:
        ...
        exposure = exposures[(int(item["shadow_top_n"]), profile.health_lookback, profile.floor_fraction, profile.scale_denominator)]
        _, actions = continuous.simulate(masked, score, order, exposure, profile, protocol["observation_end"], record_actions=True)
    ...
    REPORT_PATH.write_text("\n".join(["# 一致性连续仓位研究结论", "", "- 参数选择仅使用 `20220606-20251231`。", "- 一致性候选和仓位缩放均只读取当时可用信息。", "- `2026` 未参与筛选。", "- 本地只作预筛，正式结果以掘金为准。", "", f"第一阶段 {len(stage1)} 组，逐年为正 {int(stage1.all_year_positive.sum())} 组。", f"第二阶段 {len(stage2)} 组，逐年为正 {int(stage2.all_year_positive.sum())} 组。", best_line, "", "本轮为 research-only，未修改生产策略或正式信号。"]), encoding="utf-8")
