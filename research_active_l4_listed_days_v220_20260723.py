...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_listed_days_v220_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v220_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def run_case(arrays, score, order, definition, protocol, end, start=None):
    local_protocol = copy.deepcopy(protocol)
    local_protocol["fixed_universe"]["listed_days_min"] = int(definition["listed_days_min"])
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 最短上市天数邻域研究")
    ...
    rows, definitions = [], {}
    for listed_days_min in protocol["grid"]["listed_days_min"]:
        definition = {**protocol["fixed_definition"], "listed_days_min": int(listed_days_min)}
