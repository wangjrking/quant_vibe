...
def _run(case: dict[str, Any], sell_limit: int, tag: str, start: str, end: str) -> dict[str, Any]:
    case_key = case["case_key"]
    variant = f"{case_key}__selllimit{sell_limit}"
    ...
    if indicator is None:
        env = os.environ.copy()
        env.update(BASE_ENV)
        env["GM_MAX_DAILY_SELLS"] = str(sell_limit)
...
def main() -> None:
    ...
    for case in CASES:
        for sell_limit in SELL_LIMITS:
            rows = [_run(case, sell_limit, tag, start, end) for tag, start, end in SLICES]
            detail.extend(rows)
            by_slice = {row["slice"]: row for row in rows}
            out = {"variant": rows[0]["variant"], "case_key": case["case_key"], "sell_limit": sell_limit}
            ...
            _write_rows(REPORT_DIR / "adaptive70w_fouryear_sell_unlimited_detail.csv", detail)
            _write_rows(REPORT_DIR / "adaptive70w_fouryear_sell_unlimited_summary.csv", summary)
