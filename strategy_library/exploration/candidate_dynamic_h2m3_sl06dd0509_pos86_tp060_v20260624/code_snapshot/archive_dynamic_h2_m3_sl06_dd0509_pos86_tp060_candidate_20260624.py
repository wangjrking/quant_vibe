...
STRATEGY_ID = "candidate_dynamic_h2m3_sl06dd0509_pos86_tp060_v20260624"
...
VALIDATION_DIR = BASE_REPORT_DIR / "dynamic_h2_m3_sl06_dd0509_pos86_tp060_validation_20260624"
HIGH_POSITION_DIR = BASE_REPORT_DIR / "dynamic_h2_m3_pos86_tp060_dd_neighborhood_20260624"
SIGNAL_FILE = HIGH_POSITION_DIR / "signals" / "tp060_pos86_dd0509_s7050.csv"
...
def trading_rules() -> dict:
    return {
        ...
        "strategy_name": "动态持有 Top1 三周期融合 pos86 强风控止损止盈候选",
...
def validation_payload(summary: dict, audit: dict) -> dict:
    ...
    return {
        ...
        "strict_admission": {
            "passed": False,
            "reason": "收益指标仅为弱准入。该候选硬过滤、近期开仓、低路径依赖和贡献压力测试表现较好，且最大回撤从 dd0714 的 39.94% 降至 36.89%；但尚未完成审计智能体复核和用户生产发布审批。",
...
def manifest_payload(validation: dict, signal_rows: list[dict]) -> dict:
    ...
    return {
        "schema_version": 1,
        "strategy_id": STRATEGY_ID,
        "name": "动态持有 Top1 三周期融合 pos86 强风控止损止盈候选",
        "code_name": "dynamic_h2_m3_c098_w78_5d12_3d10_pos86_e097_mh1_sl06_dd0509_tp060",
        "status": "exploration_candidate_pending_audit_not_published",
        "created_at": "2026-06-24",
        "created_by": "strategy-agent",
        "definition": "基于 formal L4 10D/5D/3D 资产，按 0.78*10D rank + 0.12*5D rank + 0.10*3D rank 选 Top1，目标仓位 86%，动态持有 2-3 日，日内 6% 止损，6% 止盈，账户回撤缩放 soft=5% / hard=9% / scale=0.70/0.50。",
        "validation": {
            "platform": "juejin",
    ...
    return {
        ...
        "strong_gate_precheck": {
            ...
            "parameter_sensitivity": {
                ...
                "chosen_case": "tp060_pos86_dd0509_s7050",
                "evidence": "research/dd_neighborhood_summary.csv",
                "reason": "dd0714 年化更高但最大回撤 39.94% 贴近 40% 红线；dd0509 将最大回撤降至 36.89%，recent60 也更好，更接近强准入口径。",
            },
        ...
        "production_publish_blockers": [
            "需要审计智能体复核强准入证据，尤其是收益下降换取回撤安全边际的取舍是否符合生产策略口径。",
...
def write_report(validation: dict) -> None:
    ...
    lines = [
        ...
        f"- 贡献压力：去掉最高 1% 代理贡献信号后年化仍为 {float(validation['contribution_stress']['drop_top_1pct']['annual_return']):.2%}。",
...
def main() -> int:
    ...
    for script in [
        "tune_dynamic_h2_m3_pos86_tp060_dd_neighborhood_20260624.py",
        "validate_dynamic_h2_m3_sl06_dd0509_pos86_tp060_20260624.py",
        "archive_dynamic_h2_m3_sl06_dd0509_pos86_tp060_candidate_20260624.py",
    ]:
