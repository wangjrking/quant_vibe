...
REPORT_DIR = BASE_DIR / "dynamic_h2_m3_pos76_takeprofit_20260624"
...
TAKE_PROFITS = [None, 0.06, 0.07, 0.075, 0.08, 0.085, 0.09, 0.10, 0.12, 0.15, 0.20]
...
def _case_name(take_profit: float | None) -> str:
    if take_profit is None:
        return "pos76_sl06_dd0612_tpnone"
    return f"pos76_sl06_dd0612_tp{int(round(take_profit * 1000)):03d}"


def _signal_for_case(take_profit: float | None) -> Path:
    if not BASE_SIGNAL_FILE.exists():
        raise FileNotFoundError(BASE_SIGNAL_FILE)
    path = REPORT_DIR / "signals" / f"{_case_name(take_profit)}.csv"
    if path.exists():
        return path
    ...
    for row in rows:
        ...
        row["signal_take_profit_pct"] = "" if take_profit is None else f"{take_profit:.5f}"
        row["strategy_variant"] = f"dynamic_h2_m3_pos76_sl06_dd0612_takeprofit_{take_profit or 'none'}"
...
def _run(take_profit: float | None, signal_file: Path, start_name: str, start: str, end: str) -> dict:
    case_name = _case_name(take_profit)
    ...
    if indicator is None:
        ...
        if take_profit is not None:
            command.extend(["--take-profit-pct", str(take_profit)])
...
def _summarize(detail: list[dict], signal_stats: dict[str, dict]) -> list[dict]:
    ...
    for case_name in sorted({row["case_name"] for row in detail}):
        ...
        out.append(
            {
                "case_name": case_name,
                "take_profit": rows[0]["take_profit"],
...
def _write_report(summary: list[dict], audit: dict) -> None:
    ...
    for row in ranked:
        take_profit_text = "无" if row["take_profit"] in (None, "") else f"{_f(row['take_profit']):.0%}"
        lines.append(
            f"| {row['case_name']} | {take_profit_text} | {_f(row['full_annual']):.2%} | {_f(row['full_sharpe']):.2f} | "
    ...
    (REPORT_DIR / "takeprofit_report.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> int:
    ...
    for take_profit in TAKE_PROFITS:
        signal_file = _signal_for_case(take_profit)
        ...
        for start_name, start, end in STARTS:
            row = _run(take_profit, signal_file, start_name, start, end)
            detail.append(row)
