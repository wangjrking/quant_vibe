...
import tune_dynamic_h2_m3_dd0714_tp060_high_position_20260624 as hp


BASE_DIR = hp.BASE_DIR
REPORT_DIR = BASE_DIR / "dynamic_h2_m3_dd0509_tp060_high_position_20260624"
...
TARGET_PCTS = [0.84, 0.86, 0.88, 0.90, 0.91, 0.92, 0.93, 0.94, 0.95, 0.96]
...
def _signal_for_case(target_pct: float) -> Path:
    ...
    for row in rows:
        ...
        row["signal_stop_loss_pct"] = f"{hp.STOP_LOSS:.5f}"
        row["signal_take_profit_pct"] = f"{hp.TAKE_PROFIT:.5f}"
...
def _run(target_pct: float, signal_file: Path, start_name: str, start: str, end: str) -> dict:
    ...
    try:
        ...
        return hp._run(target_pct, signal_file, start_name, start, end)
    finally:
...
def _write_report(summary: list[dict], audit: dict) -> None:
    ranked = sorted(
        summary,
        key=lambda row: (_f(row["full_max_drawdown"]) <= 0.40, _f(row["full_annual"])),
        reverse=True,
    )
    lines = [
        "# dd0509 强风控高仓位邻域验证",
...
def main() -> int:
    ...
    for target_pct in TARGET_PCTS:
        ...
        for start_name, start, end in hp.STARTS:
    ...
    audit = hp.liq._audit_signals(signal_files)
