...
REPORT_DIR = BASE_DIR / "dynamic_h2_m3_tp060_pos84_dd_neighborhood_20260624"
...
DD_CASES = [
    ...
    {
        "case_name": "tp060_pos84_dd0509_s7050",
        "risk_mode": 1,
        "soft": 0.05,
        "hard": 0.09,
        "recover": 0.03,
    ...
    {
        "case_name": "tp060_pos84_dd0510_s8060",
        "risk_mode": 1,
        "soft": 0.05,
        "hard": 0.10,
        "recover": 0.03,
    ...
    {
        "case_name": "tp060_pos84_dd0612_s8060",
        "risk_mode": 1,
        "soft": 0.06,
        "hard": 0.12,
        "recover": 0.03,
    ...
    {
        "case_name": "tp060_pos84_dd0612_s8565",
        "risk_mode": 1,
        "soft": 0.06,
        "hard": 0.12,
        "recover": 0.03,
    ...
    {
        "case_name": "tp060_pos84_dd0714_s8060",
        "risk_mode": 1,
        "soft": 0.07,
        "hard": 0.14,
        "recover": 0.04,
    ...
    {
        "case_name": "tp060_pos84_dd0814_s8565",
        "risk_mode": 1,
        "soft": 0.08,
        "hard": 0.14,
        "recover": 0.04,
...
def _signal_for_case(case: dict) -> Path:
    ...
    for row in rows:
        ...
        row["strategy_variant"] = f"dynamic_h2_m3_tp060_pos84_{case['case_name']}"
...
def _apply_dd_env(env: dict[str, str], case: dict) -> None:
    env["GM_EQUITY_DD_RISK_MODE"] = str(case["risk_mode"])
    if not case["risk_mode"]:
        return
...
def _run(case: dict, signal_file: Path, start_name: str, start: str, end: str) -> dict:
    ...
    return {
        "case_name": case_name,
        "dd_risk_mode": case["risk_mode"],
        "dd_soft": case["soft"],
        "dd_hard": case["hard"],
        "dd_recover": case["recover"],
        "dd_soft_scale": case["soft_scale"],
        "dd_hard_scale": case["hard_scale"],
...
def _summarize(detail: list[dict], signal_stats: dict[str, dict]) -> list[dict]:
    ...
    for case_name in sorted({row["case_name"] for row in detail}):
        ...
        out.append(
            {
                "case_name": case_name,
                "dd_risk_mode": first["dd_risk_mode"],
...
def _write_report(summary: list[dict], audit: dict) -> None:
    ...
    for row in ranked:
        if int(_f(row["dd_risk_mode"])) == 0:
            risk_text = "关闭"
        else:
            risk_text = f"{_f(row['dd_soft']):.0%}/{_f(row['dd_hard']):.0%}, scale {_f(row['dd_soft_scale']):.0%}/{_f(row['dd_hard_scale']):.0%}"
        lines.append(
            f"| {row['case_name']} | {risk_text} | {_f(row['full_annual']):.2%} | {_f(row['full_sharpe']):.2f} | "
