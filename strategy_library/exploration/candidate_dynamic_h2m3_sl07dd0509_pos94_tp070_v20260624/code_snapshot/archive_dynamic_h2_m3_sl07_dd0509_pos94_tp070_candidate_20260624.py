...
STRATEGY_ID = "candidate_dynamic_h2m3_sl07dd0509_pos94_tp070_v20260624"
...
VALIDATION_DIR = BASE_REPORT_DIR / "dynamic_h2_m3_sl07_dd0509_pos94_tp070_validation_20260624"
POSITION_DIR = BASE_REPORT_DIR / "dynamic_h2_m3_pos94_dd0509_exit_risk_20260624"
SIGNAL_FILE = POSITION_DIR / "signals" / "pos94_sl07_tp07.csv"
...
def trading_rules() -> dict:
    return {
        ...
        "strategy_name": "动态持有 Top1 三周期融合 pos94 7%止损止盈收益增强候选",
...
def manifest_payload(validation: dict, signal_rows: list[dict]) -> dict:
    ...
    return {
        "schema_version": 1,
        "strategy_id": STRATEGY_ID,
        "name": "动态持有 Top1 三周期融合 pos94 7%止损止盈收益增强候选",
        "code_name": "dynamic_h2_m3_c098_w78_5d12_3d10_pos94_e097_mh1_sl07_dd0509_tp070",
        "status": "exploration_candidate_pending_audit_not_published",
        "created_at": "2026-06-24",
        "created_by": "strategy-agent",
        "definition": "formal L4 10D/5D/3D 融合 Top1，目标仓位 94%，动态持有 2-3 日，7% 止损，7% 止盈，账户回撤缩放 soft=5% / hard=9% / scale=0.70/0.50。",
        "validation": {
            "platform": "juejin",
    ...
    return {
        ...
        "strong_gate_precheck": {
            ...
            "parameter_sensitivity": {
                ...
                "chosen_case": "pos94_sl07_tp07",
                "evidence": "research/exit_risk_summary.csv",
                "reason": "止损/止盈 5%-7% 邻域中 sl07/tp07 在未提高最大回撤的情况下提升年化和 Sharpe；但最大回撤仍接近 40% 红线。",
            },
...
def main() -> int:
    ...
    for name in ["summary.csv", "summary_by_annual.csv", "detail.csv", "exit_risk_report.md", "hard_gate_audit.json"]:
        archive_name = "exit_risk_hard_gate_audit.json" if name == "hard_gate_audit.json" else f"exit_risk_{name}"
    ...
    for script in [
        "tune_dynamic_h2_m3_pos94_dd0509_exit_risk_20260624.py",
        "validate_dynamic_h2_m3_sl07_dd0509_pos94_tp070_20260624.py",
        "archive_dynamic_h2_m3_sl07_dd0509_pos94_tp070_candidate_20260624.py",
    ]:
