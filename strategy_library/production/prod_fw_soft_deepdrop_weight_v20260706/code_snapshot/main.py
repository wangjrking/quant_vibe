...
def _artifact_enabled():
    return bool(str(os.environ.get("GM_BACKTEST_ARTIFACT_DIR", "") or "").strip())


def _artifact_path(filename):
    if not _artifact_enabled():
        return None
    out_dir = Path(str(os.environ.get("GM_BACKTEST_ARTIFACT_DIR", "") or "").strip())
...
def _append_jsonl(filename, payload):
    ...
    with path.open("a", encoding="utf-8") as file:
        file.write(json.dumps(_json_ready(payload), ensure_ascii=False) + "\n")
...
ADAPTIVE_LIQUIDITY_SLIPPAGE = str(os.environ.get("GM_ADAPTIVE_LIQUIDITY_SLIPPAGE", "0") or "0").strip().lower() in {
    "1",
    "true",
    "yes",
}
ADAPTIVE_SLIPPAGE_ORDER_VALUE = float(os.environ.get("GM_ADAPTIVE_SLIPPAGE_ORDER_VALUE", "700000"))
ADAPTIVE_SLIPPAGE_BASE = float(os.environ.get("GM_ADAPTIVE_SLIPPAGE_BASE", "0.0015"))
ADAPTIVE_SLIPPAGE_IMPACT_COEF = float(os.environ.get("GM_ADAPTIVE_SLIPPAGE_IMPACT_COEF", "0.0200"))
ADAPTIVE_SLIPPAGE_CAP = float(os.environ.get("GM_ADAPTIVE_SLIPPAGE_CAP", "0.0065"))
ADAPTIVE_SLIPPAGE_AMOUNT_UNIT = float(os.environ.get("GM_ADAPTIVE_SLIPPAGE_AMOUNT_UNIT", "1000"))
ADAPTIVE_BUY_SLIPPAGE_MULT = float(os.environ.get("GM_ADAPTIVE_BUY_SLIPPAGE_MULT", "1.0"))
ADAPTIVE_SELL_SLIPPAGE_MULT = float(os.environ.get("GM_ADAPTIVE_SELL_SLIPPAGE_MULT", "1.0"))
...
EQUITY_DD_STRICT_WHEN_DRAWDOWN = str(
    os.environ.get("GM_EQUITY_DD_STRICT_WHEN_DRAWDOWN", "0") or "0"
).strip().lower() in {
    "1",
    "true",
    "yes",
}
EQUITY_DD_STRICT_TRIGGER = float(os.environ.get("GM_EQUITY_DD_STRICT_TRIGGER", str(EQUITY_DD_SOFT_TRIGGER)))
EQUITY_DD_STRICT_SOFT_TRIGGER = float(os.environ.get("GM_EQUITY_DD_STRICT_SOFT_TRIGGER", "0.06"))
EQUITY_DD_STRICT_HARD_TRIGGER = float(os.environ.get("GM_EQUITY_DD_STRICT_HARD_TRIGGER", "0.10"))
EQUITY_DD_STRICT_RECOVER_TRIGGER = float(os.environ.get("GM_EQUITY_DD_STRICT_RECOVER_TRIGGER", "0.03"))
EQUITY_DD_STRICT_SOFT_SCALE = float(os.environ.get("GM_EQUITY_DD_STRICT_SOFT_SCALE", "0.65"))
EQUITY_DD_STRICT_HARD_SCALE = float(os.environ.get("GM_EQUITY_DD_STRICT_HARD_SCALE", "0.45"))
...
def _adaptive_liquidity_slippage(market_row, order_value=None):
    if not ADAPTIVE_LIQUIDITY_SLIPPAGE:
        return 0.0
    open_price = _as_float((market_row or {}).get("open"))
    amount = _as_float((market_row or {}).get("amount"))
    if open_price in (None, 0) or amount in (None, 0):
        return ADAPTIVE_SLIPPAGE_CAP
    if order_value is None:
        volume = int(float(ADAPTIVE_SLIPPAGE_ORDER_VALUE) / float(open_price) / 100) * 100
        if volume < 100:
            volume = 100
        order_value = float(volume) * float(open_price)
    amount_rmb = float(amount) * ADAPTIVE_SLIPPAGE_AMOUNT_UNIT
    if amount_rmb <= 0:
        return ADAPTIVE_SLIPPAGE_CAP
    participation = max(0.0, order_value / amount_rmb)
    impact = ADAPTIVE_SLIPPAGE_IMPACT_COEF * (participation ** 0.5)
    return min(ADAPTIVE_SLIPPAGE_CAP, max(ADAPTIVE_SLIPPAGE_BASE, ADAPTIVE_SLIPPAGE_BASE + impact))
...
def _update_equity_drawdown_risk(context):
    ...
    strict_mode = EQUITY_DD_STRICT_WHEN_DRAWDOWN and drawdown >= EQUITY_DD_STRICT_TRIGGER
    soft_trigger = EQUITY_DD_STRICT_SOFT_TRIGGER if strict_mode else EQUITY_DD_SOFT_TRIGGER
    hard_trigger = EQUITY_DD_STRICT_HARD_TRIGGER if strict_mode else EQUITY_DD_HARD_TRIGGER
    recover_trigger = EQUITY_DD_STRICT_RECOVER_TRIGGER if strict_mode else EQUITY_DD_RECOVER_TRIGGER
    soft_scale = EQUITY_DD_STRICT_SOFT_SCALE if strict_mode else EQUITY_DD_SOFT_SCALE
    hard_scale = EQUITY_DD_STRICT_HARD_SCALE if strict_mode else EQUITY_DD_HARD_SCALE
    if drawdown >= hard_trigger:
        scale = hard_scale
    elif drawdown >= soft_trigger:
        scale = soft_scale
    elif drawdown <= recover_trigger:
        scale = 1.0
...
def _get_market_rows_for_date(context, trade_date):
    ...
    cursor = conn.execute(
        """
        SELECT stock_code, open, high, low, close, pre_close, amount, turnover_rate, total_mv
    ...
    rows = {}
    market_rows = cursor.fetchall() if hasattr(cursor, "fetchall") else cursor
    for stock_code, open_price, high_price, low_price, close_price, pre_close, amount, turnover_rate, total_mv in market_rows:
        rows[str(stock_code)] = {
            ...
            "turnover_rate": _as_float(turnover_rate),
            "total_mv": _as_float(total_mv),
        }
...
def _preload_market_rows(context):
    ...
    try:
        ...
        for offset in range(0, len(stock_codes), chunk_size):
            ...
            sql = f"""
                SELECT trade_date, stock_code, open, high, low, close, pre_close, amount, turnover_rate, total_mv
                FROM STOCK_DAILY_DATA
            ...
            market_rows = cursor.fetchall() if hasattr(cursor, "fetchall") else cursor
            for trade_date, stock_code, open_price, high_price, low_price, close_price, pre_close, amount, turnover_rate, total_mv in market_rows:
...
def _submit_buy_order(context, symbol, target_pct, trade_date):
    ...
    if volume < 100:
        ...
        return False
    dynamic_slippage = _adaptive_liquidity_slippage(market_row, float(volume) * float(open_price)) * ADAPTIVE_BUY_SLIPPAGE_MULT
    order_price = float(open_price) * (1.0 + dynamic_slippage)
    if VERBOSE_TRADES:
        print(
            "BUY_ATTEMPT {} {} volume={} price={} open={} adaptive_slippage={:.6f} target={:.6f}".format(
                trade_date, symbol, volume, order_price, open_price, dynamic_slippage, float(target_pct)
            )
        )
    order_volume(
        symbol=symbol,
...
def _position_sellable_volume(context, symbol):
    account_getter = getattr(context, "account", None)
    if account_getter is None:
        return 0
    for position in account_getter().positions():
        if position.get("symbol") != symbol:
            continue
        for key in ("available_now", "available", "available_today", "volume"):
...
def _submit_topup_order(context, symbol, target_total_pct, trade_date):
    ...
    if VERBOSE_TRADES:
        print(
            "BUY_TOPUP {} {} volume={} price={} open={} adaptive_slippage={:.6f} target_total={:.6f}".format(
                trade_date, symbol, volume, order_price, open_price, dynamic_slippage, float(target_total_pct)
            )
        )
    order_volume(
        symbol=symbol,
...
def _submit_sell_order(context, symbol, trade_date):
    ...
    if FORCE_SELL_MARKET_ORDER:
        sell_volume = _position_sellable_volume(context, symbol)
        if sell_volume > 0:
            if VERBOSE_TRADES:
                print(
                    "SELL_ATTEMPT {} {} volume={} mode=market reason=sellable_volume".format(
                        trade_date, symbol, sell_volume
                    )
                )
            order_volume(
                symbol=symbol,
            ...
            return True
        if VERBOSE_TRADES:
            print("SELL_ATTEMPT {} {} target=0 mode=market open={} reason=no_sellable_volume".format(trade_date, symbol, market_row.get("open")))
        order_target_percent(symbol=symbol, percent=0, order_type=OrderType_Market, position_side=PositionSide_Long)
        return True
    open_price = _as_float(market_row.get("open"))
    if open_price in (None, 0):
    ...
    for position in positions:
        if position.get("symbol") != symbol:
            continue
        volume = _position_sellable_volume(context, symbol)
        if volume <= 0:
            if VERBOSE_TRADES:
                print("SELL_ATTEMPT {} {} target=0 mode=market reason=volume_le_0".format(trade_date, symbol))
            order_target_percent(symbol=symbol, percent=0, order_type=OrderType_Market, position_side=PositionSide_Long)
            return True
        dynamic_slippage = _adaptive_liquidity_slippage(market_row, float(volume) * float(open_price)) * ADAPTIVE_SELL_SLIPPAGE_MULT
        order_price = float(open_price) * (1.0 - dynamic_slippage)
        if VERBOSE_TRADES:
            print(
                "SELL_ATTEMPT {} {} volume={} price={} open={} adaptive_slippage={:.6f}".format(
                    trade_date, symbol, volume, order_price, open_price, dynamic_slippage
                )
            )
        order_volume(
            symbol=symbol,
...
def on_execution_report(context, report):
    ...
    payload = {
        ...
        "report": report,
        "trade_date": getattr(context, "last_trade_date", None),
    }
    _append_jsonl("execution_reports.jsonl", payload)
...
def on_order_status(context, order):
    ...
    _append_jsonl("order_status.jsonl", payload)
...
def on_backtest_finished(context, indicator=None, *args, **kwargs):
    ...
    artifacts = {
        ...
        "indicator": indicator,
        "held_symbols": getattr(context, "held_symbols", {}),
    }
    try:
        account_getter = getattr(context, "account", None)
        if account_getter is not None:
            account_obj = account_getter()
            cash_obj = getattr(account_obj, "cash", None)
            if callable(cash_obj):
                cash_obj = cash_obj()
            artifacts["cash"] = cash_obj
            artifacts["positions"] = account_obj.positions()
    except Exception as exc:
        artifacts["cash_positions_error"] = str(exc)
    try:
        artifacts["execution_reports"] = get_execution_reports()
    except Exception as exc:
        artifacts["execution_reports_error"] = str(exc)
    try:
        artifacts["orders"] = get_orders()
    except Exception as exc:
        artifacts["orders_error"] = str(exc)
    try:
        artifacts["unfinished_orders"] = get_unfinished_orders()
    except Exception as exc:
        artifacts["unfinished_orders_error"] = str(exc)
    _write_json("backtest_trade_artifacts.json", artifacts)
