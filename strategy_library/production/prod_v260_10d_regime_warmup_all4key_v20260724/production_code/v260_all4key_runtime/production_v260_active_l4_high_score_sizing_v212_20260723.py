...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_high_score_sizing_v212_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v212_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def score_multiplier(score: np.ndarray, definition: dict) -> np.ndarray:
    threshold = float(definition["high_score_threshold"])
    multiplier = float(definition["high_score_multiplier"])
    result = np.ones(score.shape, dtype=np.float32)
    result[np.isfinite(score) & (score >= threshold)] = multiplier
    return result
...
def run_case(arrays, score, order, definition, protocol, end, start=None, record_actions=False):
    return v162.run_case(
        ...
        candidate_target_multiplier_override=score_multiplier(score, definition),
        max_positions_override=int(definition["max_positions"]),
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 高分候选动态仓位研究")
    ...
    if args.export_selected_actions:
        ...
        selected = next(
            ...
            if abs(float(item["definition"]["high_score_threshold"]) - 0.995) < 1e-12
            and abs(float(item["definition"]["high_score_multiplier"]) - 1.10) < 1e-12
        )
        ...
        actions_path = OUT / "replay_actions_highscore995_mult110_through_latest.csv"
        daily_path = OUT / "daily_local_highscore995_mult110_through_latest.csv"
