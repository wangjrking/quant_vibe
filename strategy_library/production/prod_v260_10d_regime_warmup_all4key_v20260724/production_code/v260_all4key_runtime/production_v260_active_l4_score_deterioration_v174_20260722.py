...
from . import production_v260_active_l4_rank_smoothing_v94_20260722 as v94
...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_score_deterioration_v174_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v174_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def selection_mask(arrays: dict, max_deterioration: float) -> np.ndarray:
    raw = arrays["rank_10d"].astype(np.float32)
    smooth = v94.rolling_nanmean(raw, 7)
    return np.isfinite(raw) & np.isfinite(smooth) & ((raw - smooth) >= -float(max_deterioration))
...
def run_case(arrays, score, order, definition, protocol, end, start=None, record_actions=False):
    return v162.run_case(
        ...
        selection_mask_override=selection_mask(arrays, definition["max_rank_deterioration"]),
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 score deterioration study")
