...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_10d_stability_guard_v4_research_20260625"
TARGET_TABLE = "stock_predict_data_model_agent_10d_stability_guard_v4_20260625_executable_10d_open_return_research"
        ...
        for feature in candidate_features:
            ...
            for threshold in thresholds:
                for op in ["<=", ">="]:
                    ...
                    row["pass_basic"] = bool(
                        row["recent63_active_days"] >= 5
                        and row["recent63_top1_delta_vs_base10"] >= -0.002
                        and row["recent63_top5_delta_vs_base10"] >= -0.001
                        and row["recent20_top5_delta_vs_base10"] >= -0.001
                        and row["full_top5_delta_vs_base10"] >= -0.0005
                        and row["positive_top5_periods"] >= 4
                        and row["min_period_top5_delta"] >= -0.00005
                    )
                    row["objective"] = (
...
def write_outputs(scan_result: pd.DataFrame, daily: pd.DataFrame, source_metrics: pd.DataFrame, stats: dict | None) -> None:
    REPORT_DIR.mkdir(parents=True, exist_ok=True)
    scan_path = REPORT_DIR / "stability_guard_scan_results.csv"
    daily_path = REPORT_DIR / "stability_guard_daily_deltas.csv"
    source_path = REPORT_DIR / "stability_guard_source_daily_metrics.csv"
    scan_result.to_csv(scan_path, index=False, encoding="utf-8-sig")
    daily.to_csv(daily_path, index=False, encoding="utf-8-sig")
    source_metrics.to_csv(source_path, index=False, encoding="utf-8-sig")
    best = scan_result.iloc[0].to_dict() if not scan_result.empty else None
    ...
    manifest_path = REPORT_DIR / "stability_guard_research_manifest.json"
    ...
    (REPORT_DIR / "stability_guard_research_report.md").write_text("\n".join(lines), encoding="utf-8")
