...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_10d_risk_balanced_v5_research_20260625"
TARGET_TABLE = "stock_predict_data_model_agent_10d_risk_balanced_v5_20260625_executable_10d_open_return_research"
LABEL = "executable_10d_open_return"

SOURCES = {
    "base10": "stock_predict_data_model_agent_10d_recent_enhance_v4_20260625_executable_10d_open_return_research",
...
def scan(scores: pd.DataFrame, labels: pd.DataFrame, features: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    rows = []
    daily_rows = []
    alt_sources = ["formal10", "cross10", "cond5", "formal5", "rank3", "formal3", "stable1", "dyn10"]
        ...
        for feature in sorted(set(candidate_features)):
            ...
            for threshold in thresholds:
                for op in ["<=", ">="]:
                    ...
                    row["pass_basic"] = bool(
                        row["full_rank_ic_delta_vs_base10"] >= 0.0
                        and row["min_period_rank_ic_delta"] >= -0.0008
                        and row["recent63_top1_delta_vs_base10"] >= -0.005
                        and row["recent63_top5_delta_vs_base10"] >= -0.002
                        and row["recent20_top5_delta_vs_base10"] >= -0.004
                        and row["full_top5_delta_vs_base10"] >= -0.0005
                        and row["min_period_top5_delta"] >= -0.0005
                        and row["positive_top5_periods"] >= 2
                    )
                    row["objective"] = (
                        1.4 * row["full_rank_ic_delta_vs_base10"]
                        + 1.0 * row["min_period_rank_ic_delta"]
                        + 0.8 * row["avg_period_rank_ic_delta"]
                        + 1.0 * row["full_top5_delta_vs_base10"]
                        + 0.8 * row["recent63_top5_delta_vs_base10"]
                        + 0.5 * row["recent20_top5_delta_vs_base10"]
...
def write_outputs(scan_result: pd.DataFrame, daily: pd.DataFrame, source_metrics: pd.DataFrame, stats: dict | None) -> None:
    ...
    scan_path = REPORT_DIR / "risk_balanced_scan_results.csv"
    daily_path = REPORT_DIR / "risk_balanced_daily_deltas.csv"
    source_path = REPORT_DIR / "risk_balanced_source_daily_metrics.csv"
    ...
    manifest = {
        ...
        "baseline_for_scan": "research_10d_recent_enhance_v4",
        "research_hypothesis": "reduce rank_ic_and_period_penalty_without_material_topn_loss",
        "best_scan": best,
        "asset_stats": stats,
    ...
    manifest_path = REPORT_DIR / "risk_balanced_research_manifest.json"
    manifest_path.write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")
    lines = [
        "# 10D 风险平衡研究报告（20260625）",
        "",
        "## 当前结论",
        "",
    ]
    if stats:
        lines.extend(
            [
                "本轮找到一个相对当前 10D 最强研究基线通过风险平衡门槛的 research-only 候选。",
                "",
                "候选规则：",
                "",
                "```text",
        ...
        lines.append("本轮未找到相对当前 10D 最强研究基线通过风险平衡门槛的候选；不写入新的预测研究表。")
    lines.extend(
        [
            "",
            "## 证据路径",
    ...
    (REPORT_DIR / "risk_balanced_research_report.md").write_text("\n".join(lines), encoding="utf-8")
