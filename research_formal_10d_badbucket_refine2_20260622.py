...
REPORT_DIR = (
    ...
    / "formal_10d_badbucket_refine2_20260622"
)
SOURCE_SIGNAL = (
    DATA_DIR
    / "reports"
...
def _variant(
    ...
    *,
    ...
    env.update(base_grid.EQDD_PROFILES["mid"])
    if extra_env:
...
def _rule(field: str, op: str, scale: float, **kwargs) -> dict:
    out = {"field": field, "op": op, "scale": scale}
    out.update(kwargs)
    return out


P10_12_17 = _rule("pred_10d", "between", 1.0, low=0.12, high=0.17)
P10_14_17 = _rule("pred_10d", "between", 1.0, low=0.14, high=0.17)
GAP_07_09 = _rule("pred_gap", "between", 1.0, low=0.07, high=0.09)
AMT_8_15 = _rule("amount", "between", 1.0, low=8000.0, high=15000.0)
GOOD_P10_17 = _rule("pred_10d", "ge", 1.0, threshold=0.17)
GOOD_AMT_LT8 = _rule("amount", "lt", 1.0, threshold=8000.0)
GOOD_TURN_LT1 = _rule("turnover_rate", "lt", 1.0, threshold=1.0)


def _with_scale(template: dict, scale: float) -> dict:
    out = dict(template)
    out["scale"] = scale
    return out
...
VARIANTS = [
    _variant("p10_12_17_down70", [_with_scale(P10_12_17, 0.70)]),
    _variant("p10_12_17_down55", [_with_scale(P10_12_17, 0.55)]),
    _variant("p10_14_17_down60", [_with_scale(P10_14_17, 0.60)]),
    _variant("gap_07_09_down70", [_with_scale(GAP_07_09, 0.70)]),
    _variant("gap_07_09_down55", [_with_scale(GAP_07_09, 0.55)]),
    _variant("amt_8_15_down75", [_with_scale(AMT_8_15, 0.75)]),
    _variant("amt_8_15_down60", [_with_scale(AMT_8_15, 0.60)]),
    _variant("p10_gap_soft", [_with_scale(P10_12_17, 0.72), _with_scale(GAP_07_09, 0.78)]),
    _variant("p10_gap_amt_soft", [_with_scale(P10_12_17, 0.72), _with_scale(GAP_07_09, 0.78), _with_scale(AMT_8_15, 0.82)]),
    _variant("bad_down_good_up_soft", [
        _with_scale(P10_12_17, 0.72),
        _with_scale(GAP_07_09, 0.78),
        _with_scale(AMT_8_15, 0.82),
        _with_scale(GOOD_P10_17, 1.12),
        _with_scale(GOOD_AMT_LT8, 1.10),
        _with_scale(GOOD_TURN_LT1, 1.08),
    ]),
    _variant("bad_down_good_up_mid", [
        _with_scale(P10_12_17, 0.62),
        _with_scale(GAP_07_09, 0.70),
        _with_scale(AMT_8_15, 0.75),
        _with_scale(GOOD_P10_17, 1.20),
        _with_scale(GOOD_AMT_LT8, 1.15),
        _with_scale(GOOD_TURN_LT1, 1.12),
    ], cap=0.55),
    _variant("bad_down_good_up_grow", [
        _with_scale(P10_12_17, 0.62),
        _with_scale(GAP_07_09, 0.70),
        _with_scale(AMT_8_15, 0.75),
        _with_scale(GOOD_P10_17, 1.20),
...
def _write_signal(cfg: dict, signal_file: Path) -> dict:
    ...
    for field in ["refine2_scale", "refine2_source_target"]:
    ...
    for row in rows:
        ...
        row["refine2_source_target"] = f"{base_target:.5f}"
        row["refine2_scale"] = f"{scale:.6f}"
    ...
    for row in rows:
        ...
        if abs((_to_float(row.get("refine2_scale"), 1.0) or 1.0) - 1.0) > 1e-9:
            scale_hits += 1
    return {
        "signal_count": len(rows),
        "scaled_rows": scale_hits,
