...
OUT = (
    ...
    / "data_file"
    / "reports"
    / "strategy_agent_current_formal_l4_currentcoverage_sell_neighborhood_20260720"
)
LOGS = OUT / "logs"

CASES = [
    {"min_hold": min_hold, "max_hold": max_hold, "ratio": ratio}
    for min_hold, max_hold in ((11, 12), (11, 13), (11, 14), (12, 13), (12, 14), (12, 15), (13, 14), (13, 15))
...
def run_case(case: dict) -> dict:
    name = (
        f"min{case['min_hold']}_max{case['max_hold']}"
        f"_ratio{str(case['ratio']).replace('.', 'p')}"
    )
    log = LOGS / f"{name}.log"
    env = os.environ.copy()
    env.update(
