...
OUT_DIR = DATA_DIR / "reports" / "model_agent_10d_no_star_r3gate991_r5gate98_20260623"
...
TARGET_TABLE = "stock_predict_data_model_agent_10d_no_star_r3gate991_r5gate98_20260623_executable_10d_open_return_research"
...
def main() -> int:
    ...
    for trade_date, group in frame.groupby("trade_date", sort=True):
        ...
        keep_source = star_count == 0 and mean_rank3d >= 0.991 and mean_rank5d >= 0.98
        if keep_source:
    ...
    payload = {
        ...
        "candidate_id": "model_agent_10d_no_star_r3gate991_r5gate98_20260623",
        ...
        "rule": "if source candidate top5 contains no STAR(688) stock and source_top5_mean_rank3d >= 0.991 and source_top5_mean_rank5d >= 0.98, keep source score for that trade_date; otherwise fallback to base 10D score",
        ...
        json.dumps(
            {
                ...
                "notes": "10D 研究候选在 no-star 与 5D/3D 一致性门控基础上，把 3D 分位门槛从 0.99 收紧到 0.991，用于剔除一小部分近期扰动较大的换源日；仅用于 research 验证，不进入 formal L4/L5。",
            },
            ensure_ascii=False,
            indent=2,
        ),
    ...
    report_lines = [
        "# 10D no-star + r3>=0.991 + r5>=0.98 研究候选",
        "",
        "## 当前结论",
        "",
        f"- 新候选 `10d_no_star_r3gate991_r5gate98` 的 focus 分数为 `{payload['selected_focus_score']:.12f}`。",
        f"- 相比上一版 `10d_no_star_r3gate099_r5gate098`，本版通过再收紧 3D 门槛，换源日从 `21` 天降到 `{len(keep_source_days)}` 天。",
        f"- 本版剔除了 `20260317` 这一个换源日，其余换源日维持不变。",
        "",
        "## 规则",
        "",
        "- 条件 1：source candidate top5 中不含 STAR(688) 股票。",
        "- 条件 2：`source_top5_mean_rank3d >= 0.991`。",
