...
EXPERIMENT_DB = DATA_DIR / "experimental_assets" / "model-agent" / "l4_predictions" / "l4_10d_front_combo_calibrator_20260714.duckdb"
TABLE = "stock_predict_data_model_agent_four_year_10d_front_combo_calibrator_20260714_research"
...
SOURCE_REPORT = REPORT_DIR / "front_combo_calibrator_10d_report.json"
...
def _sha256_keys(df: pd.DataFrame) -> str:
    h = hashlib.sha256()
    for row in df[["trade_date", "stock_code", "pred_prob"]].itertuples(index=False):
        h.update(f"{row.trade_date}|{row.stock_code}|{float(row.pred_prob):.12g}\n".encode("utf-8"))
    return h.hexdigest()
...
def _prepare_full_base() -> pd.DataFrame:
...
def _materialize() -> tuple[pd.DataFrame, dict[str, Any]]:
    frame = _prepare_full_base()
    ...
    out["score_source"] = "research_10d_front_combo_calibrator_20260714"
    out["model_label"] = "executable_10d_open_return"
...
def _quality(df: pd.DataFrame) -> dict[str, Any]:
    ...
    return {
        ...
        "pred_prob_sha256": _sha256_keys(df.sort_values(["trade_date", "stock_code"]).reset_index(drop=True)),
    }
...
def main() -> None:
    ...
    manifest = {
        ...
        "score_formula": "pred_10d_rank + 0.03*(front_top5_rank-0.5) - 0.01*(front_top3_rank-0.5), applied only when pred_10d_rank >= 0.90",
    ...
    manifest_path = REPORT_DIR / "research_candidate_manifest_10d_front_combo.json"
    report_path = REPORT_DIR / "materialized_research_asset_report.json"
    md_path = REPORT_DIR / "materialized_research_asset_report.md"
    manifest_path.write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")
    report = {
        "generated_at": now_iso(),
        "actor": "model-agent",
        "scope": "materialize_research_10d_front_combo_calibrator",
        "db_path": str(EXPERIMENT_DB),
        "table": TABLE,
        "quality": quality,
        "score_source_usage": usage,
    ...
    lines = [
        "# 10D 前排组合校准 Research L4 资产物化报告",
        "",
        "## 结论",
        "",
        "已将 10D 前排组合校准候选物化为 research-only L4 评分资产。该资产不属于 formal，不是 `approved_for_l5`，不能进入生产主线。",
        "",
        "## 资产",
        "",
        ...
        f"- 交易日：`{quality['trade_days']}`",
        f"- 最新日行数 / 股票数：`{quality['latest_day_rows']}` / `{quality['latest_day_stock_count']}`",
        f"- `.BJ` 行数：`{quality['bj_rows']}`",
        f"- `pred_prob` 空值：`{quality['null_pred_prob']}`",
    ...
    md_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(json.dumps({"report": str(report_path), "manifest": str(manifest_path), "db": str(EXPERIMENT_DB), "table": TABLE}, ensure_ascii=False, indent=2))
