...
base.REPORT_DIR = (
    ...
    / "reports"
    / "strategy_agent_goal_high_annual_20260623"
    / "current_formal_top1keep_targetpct"
)
base.FUSION_DB = base.REPORT_DIR / "fusion_top1keep_targetpct.db"
base.SCORE_DB = base.REPORT_DIR / "scores_top1keep_targetpct.db"


def _cfg(name: str, *, target_pct_each: float, top1_exit_ratio: float = 0.91, top1_holding_days: int = 7) -> dict:
    return {
        "name": name,
        "w10": 0.80,
        "w5": 0.20,
...
base.VARIANTS = [
    _cfg("tp0320", target_pct_each=0.3200),
    _cfg("tp0324", target_pct_each=0.3240),
    _cfg("tp0328", target_pct_each=0.3280),
    _cfg("tp0330", target_pct_each=0.3300),
    _cfg("tp0332", target_pct_each=0.3320),
    _cfg("tp0334", target_pct_each=0.3340),
    _cfg("tp0336", target_pct_each=0.3360),
    _cfg("tp0338", target_pct_each=0.3380),
    _cfg("tp0340", target_pct_each=0.3400),
]
