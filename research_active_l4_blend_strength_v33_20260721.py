...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_blend_strength_v33_20260721"
...
def main():
    ...
    contexts, rows = {}, []
    g = p["stage1"]
    for blend in p["score_blends"]:
        score = core.blend_scores(arrays, blend)
        ...
        contexts[blend["id"]] = (score, order, exposure)
        for entry in g["entry_rank_min"]:
            for top in g["strong_top_score_min"]:
                for gap in g["strong_gap_min"]:
                    ...
                    cid = blend["id"] + "_" + v30.case_id(profile)
                    rows.append({"case_id": cid, "blend_id": blend["id"], **asdict(profile), **metrics})
    ...
    for _, seed in seeds.iterrows():
        score, order, exposure = contexts[str(seed.blend_id)]
        for exit_rule in p["stage2_exit_profiles"]:
            ...
            daily = v30.simulate(arrays, masked, score, order, exposure, profile, p["observation_end"])
            metrics = v30.evaluate(daily, p["year_folds"])
            cid = str(seed.blend_id) + "_" + v30.case_id(profile)
            rows.append({"case_id": cid, "blend_id": seed.blend_id, "exit_profile_id": exit_rule["id"], **asdict(profile), **metrics})
    ...
    for _, item in frozen.iterrows():
        score, order, exposure = contexts[str(item.blend_id)]
        ...
        action_rows.append({"case_id": item.case_id, "blend_id": item.blend_id, "path": str(path.relative_to(ROOT)).replace("\\", "/"), "sha256": v30.digest(path), "rows": len(actions), "buy_rows": int((actions.action == "BUY").sum()), "max_positions": 2})
    ...
    (OUT / "融合强度研究结论.md").write_text("\n".join(["# 融合强度研究结论", "", "本轮仅调整3D、5D、10D融合和强信号阈值，全部参数运行前冻结。", "", f"第一阶段 {len(stage1)} 组，逐年为正 {int(stage1.all_year_positive.sum())} 组。", f"第二阶段 {len(stage2)} 组，逐年为正 {int(stage2.all_year_positive.sum())} 组。", f"冻结 {len(action_rows)} 条独立掘金路径。", "", "2026数据未参与筛选，生产资产未修改。"]), encoding="utf-8")
