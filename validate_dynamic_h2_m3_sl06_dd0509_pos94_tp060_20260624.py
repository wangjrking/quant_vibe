...
CASE_NAME = "dynamic_h2_m3_c098_w78_5d12_3d10_pos94_e097_mh1_sl06_dd0509_tp060"
SOURCE_DIR = liq.SOURCE_DIR / "strict_sync_liquidity_neighborhood_20260624"
REPORT_DIR = SOURCE_DIR / "formal_horizon_entry_confirmation_20260624" / "dynamic_h2_m3_sl06_dd0509_pos94_tp060_validation_20260624"
SIGNAL_FILE = (
    SOURCE_DIR
    / "formal_horizon_entry_confirmation_20260624"
    / "dynamic_h2_m3_dd0509_tp060_high_position_20260624"
    / "signals"
    / "dd0509_tp060_pos94.csv"
)
...
def _write_report(time_rows: list[dict], nearby_summary: list[dict], stress_summary: list[dict], proxy_stats: dict, audit: dict) -> None:
    lines = [
        ...
        "## 当前结论",
        "",
        f"验证对象：`{CASE_NAME}`。该版本固定 formal L4 多周期模型入场、Top1 单票、目标仓位 `94%`、动态持有 `h2_m3_c098`、分数退出 `0.97`、日内 `6%` 止损、账户回撤缩放 `soft=5% / hard=9% / recover=3% / scale=0.70/0.50`，新增通用 `6%` 止盈。",
        "",
