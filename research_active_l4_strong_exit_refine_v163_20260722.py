...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_strong_exit_refine_v163_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v163_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def case_protocol(protocol: dict) -> dict:
    result = copy.deepcopy(protocol)
    result["fixed_strategy"] = {"top_n_per_rebalance": 1, "max_hold_days": 20, "rebalance_every": 1}
    return result
...
def min_hold_schedule(arrays: dict, definition: dict) -> np.ndarray:
    ...
    strong = np.isfinite(momentum) & (momentum >= float(definition["exit_strong_return_min"]))
    result[strong] = int(definition["strong_min_hold_days"])
    return result
...
def run_case(arrays, score, order, definition, protocol, end, start=None, record_actions=False):
    ...
    return v109.simulate(
        ...
        min_hold_days_override=min_hold_schedule(arrays, definition),
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4强势市场卖出频率邻域研究")
    ...
    rows, definitions = [], {}
    for threshold, min_hold in product(protocol["grid"]["exit_strong_return_min"], protocol["grid"]["strong_min_hold_days"]):
        definition = {**protocol["fixed_definition"], "exit_strong_return_min": threshold, "strong_min_hold_days": min_hold}
