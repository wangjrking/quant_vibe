...
import research_active_l4_score_gap_sizing_v169_20260722 as v169
import research_preregistered_active_l4_rank_rotation_20260721 as core


ROOT = Path(__file__).resolve().parents[2]
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_deterioration_gap_v176_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v176_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def run_case(arrays, score, order, definition, protocol, end, start=None):
    return v162.run_case(
        arrays, score, order, definition, protocol, end, start,
        selection_mask_override=v174.selection_mask(arrays, definition["max_rank_deterioration"]),
        candidate_target_multiplier_override=v169.gap_multiplier(score, order, definition),
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 deterioration and score-gap study")
