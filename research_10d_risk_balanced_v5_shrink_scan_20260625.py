...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_10d_risk_balanced_v5_shrink_scan_20260625"
TARGET_TABLE = (
    "stock_predict_data_model_agent_10d_risk_balanced_v5_shrink_20260625_"
    "executable_10d_open_return_research"
)
LABEL = "executable_10d_open_return"

SOURCES = {
    "base10": "stock_predict_data_model_agent_10d_risk_balanced_v5_20260625_executable_10d_open_return_research",
    "formal10": "stock_predict_data_model_agent_best_full_20260623_executable_10d_open_return_formal",
    "recent10": "stock_predict_data_model_agent_10d_recent_enhance_v4_20260625_executable_10d_open_return_research",
    "cross10": "stock_predict_data_model_agent_10d_cross_horizon_guard_v3_20260625_executable_10d_open_return_research",
    "cond5": "stock_predict_data_model_agent_5d_condition_gate_v6_20260625_executable_5d_open_return_research",
    "formal5": "stock_predict_data_model_agent_best_full_20260623_executable_5d_open_return_formal",
    "rank3": "stock_predict_data_model_agent_3d_rankic_balanced_v5_20260625_executable_3d_open_return_research",
    "stable1": "stock_predict_data_model_agent_1d_new5d_condition_gate_v6_20260625_executable_1d_open_return_research",
}
...
def load_scores() -> pd.DataFrame:
    with sqlite3.connect(MODEL_DB) as conn:
        out = read_source(conn, "base10", SOURCES["base10"])
    ...
    for alias in SOURCES:
        ...
        out[score_col] = out[score_col].fillna(out["base10_score"])
...
def build_features(scores: pd.DataFrame) -> pd.DataFrame:
    ...
    for trade_date, group in scores.groupby("trade_date", sort=True):
        base_top10 = top_set(group, "base10_rank", 10)
        base_top20 = top_set(group, "base10_rank", 20)
        base_top50 = top_set(group, "base10_rank", 50)
        row: dict[str, float | str] = {"trade_date": trade_date}
        row["base10_top1_top5_gap"] = float(
            group.nlargest(1, "base10_rank")["base10_rank"].mean()
            - group.nlargest(5, "base10_rank")["base10_rank"].min()
        )
        row["base10_top5_top10_gap"] = float(
            group.nlargest(5, "base10_rank")["base10_rank"].mean()
            - group.nlargest(10, "base10_rank")["base10_rank"].mean()
        )
        for alias in SOURCES:
            score_col = f"{alias}_score"
            ...
            if alias != "base10":
                ...
                row[f"overlap10_base10_{alias}"] = len(base_top10 & top_set(group, rank_col, 10))
                row[f"overlap20_base10_{alias}"] = len(base_top20 & top_set(group, rank_col, 20))
                row[f"overlap50_base10_{alias}"] = len(base_top50 & top_set(group, rank_col, 50))
        rows.append(row)
    ...
    *,
    ...
    for prefix, current, base in [
        ...
        for metric in ["rank_ic", "top1", "top3", "top5", "top10", "top20", "top50"]:
            row[f"{prefix}_{metric}_delta_vs_base10"] = current[metric] - base[metric]
...
def scan(scores: pd.DataFrame, labels: pd.DataFrame, features: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    base_daily = source_metrics[source_metrics["source"] == "base10"].set_index("trade_date")
    feature_idx = features.set_index("trade_date")
    rows = []
    daily_rows = []
    alt_sources = ["formal10", "recent10", "cross10", "cond5", "formal5", "rank3", "stable1"]
    ...
    for alt in alt_sources:
        ...
        candidate_features = [
            ...
            if col.startswith("base10_")
            or col.startswith(f"{alt}_")
            or col.endswith(f"_{alt}")
            or col.startswith("recent10_")
            or col.startswith("cross10_")
            or col.startswith("cond5_")
            or col.startswith("rank3_")
            or col.startswith("stable1_")
        ]
        for feature in sorted(set(candidate_features)):
            ...
            for threshold in thresholds:
                for op in ["<=", ">="]:
                    ...
                    if len(active_dates) < 6 or len(active_dates) > int(0.35 * len(feature_idx)):
                        continue
                    ...
                    row["pass_basic"] = bool(
                        row["full_rank_ic_delta_vs_base10"] >= -0.00035
                        and row["min_period_rank_ic_delta"] >= -0.0009
                        and row["recent63_top1_delta_vs_base10"] >= -0.01
                        and row["recent63_top5_delta_vs_base10"] >= 0.0
                        and row["recent20_top5_delta_vs_base10"] >= 0.0
                        and row["full_top5_delta_vs_base10"] >= 0.0
                        and row["min_period_top5_delta"] >= -0.0003
                        and row["positive_top5_periods"] >= 3
                    )
                    row["objective"] = (
                        1.4 * row["full_top5_delta_vs_base10"]
                        + 1.0 * row["recent63_top5_delta_vs_base10"]
                        + 0.8 * row["recent20_top5_delta_vs_base10"]
                        + 0.8 * row["full_top1_delta_vs_base10"]
                        + 0.6 * row["recent63_top1_delta_vs_base10"]
                        + 0.4 * row["recent20_top1_delta_vs_base10"]
                        + 0.8 * row["full_rank_ic_delta_vs_base10"]
                        + 0.6 * row["min_period_rank_ic_delta"]
                        + 0.004 * row["positive_top5_periods"]
                        - 0.003 * row["active_ratio"]
                    )
                    rows.append(row)
                    if row["pass_basic"]:
...
def write_candidate(scores: pd.DataFrame, features: pd.DataFrame, best: pd.Series) -> dict[str, object]:
    ...
    active_dates = set(active[active].index.astype(str))
    out = scores[
        ["trade_date", "stock_code", "base10_score", "base10_rank", f"{alt}_score", f"{alt}_rank"]
    ].copy()
...
def write_outputs(scan_result: pd.DataFrame, daily: pd.DataFrame, source_metrics: pd.DataFrame, stats: dict[str, object] | None) -> None:
    ...
    scan_path = REPORT_DIR / "risk_balanced_v5_shrink_scan_results.csv"
    daily_path = REPORT_DIR / "risk_balanced_v5_shrink_daily_deltas.csv"
    source_path = REPORT_DIR / "risk_balanced_v5_shrink_source_daily_metrics.csv"
    ...
    manifest = {
        ...
        "baseline_for_scan": "research_10d_risk_balanced_v5",
        "research_hypothesis": "shrink_high_risk_days_of_current_best_10d_candidate_without_losing_topn_strength",
        "best_scan": best,
        "asset_stats": stats,
    ...
    manifest_path = REPORT_DIR / "risk_balanced_v5_shrink_manifest.json"
    manifest_path.write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")

    lines = [
        "# 10D risk_balanced_v5 收缩扫描报告（20260625）",
        "",
        "## 当前结论",
        "",
    ]
    if stats:
        lines.extend(
            [
                "本轮在 research-only 范围内找到一个相对 `risk_balanced_v5` 更稳的收缩候选。",
                "",
                "候选规则：",
                "",
                "```text",
                ...
                "",
                "## 相对 10D 当前最强研究基线的扫描摘要",
                "",
                f"- 全样本 RankIC 增量：`{best['full_rank_ic_delta_vs_base10']:.6f}`",
                f"- 全样本 Top1 增量：`{best['full_top1_delta_vs_base10']:.6f}`",
        ...
        lines.append("本轮没有找到相对 `risk_balanced_v5` 更优、且满足收缩约束的候选，因此不写入新的研究预测表。")
    lines.extend(
        [
            "",
            "## 证据路径",
    ...
    (REPORT_DIR / "risk_balanced_v5_shrink_report.md").write_text("\n".join(lines), encoding="utf-8")
