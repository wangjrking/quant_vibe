...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_research_10d_fixed4y_fold08_fs40d3_feature_neighborhood_scan_20260626"
...
def write_feature_subset_files() -> list[Candidate]:
    ...
    raw = json.loads(BASE_FEATURES_PATH.read_text(encoding="utf-8"))
    features = raw["features"] if isinstance(raw, dict) else raw
    counts = [24, 32, 48, 64]
    candidates = [Candidate("base_fs40_d3_l8", 40, FEATURE_DIR / "selected_features_top40.json", False)]
    # persist top40 for manifest completeness
    top40_payload = {"label": LABEL, "features": features[:40]}
    (FEATURE_DIR / "selected_features_top40.json").write_text(
        json.dumps(top40_payload, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    for count in counts:
        ...
        candidates.append(Candidate(f"fs{count}_d3_l8", count, path, True))
    return candidates


def run_candidate(candidate: Candidate) -> dict:
    if not candidate.retrain:
        return {"stdout": "reused existing base fs40_d3_l8 artifact", "stderr": "", "returncode": 0, "reused": True}
...
def main() -> int:
    ...
    for candidate in candidates:
        ...
        for baseline_name in [*BASELINE_TABLES.keys(), "base_fs40_d3_l8"]:
    ...
    summary_path = REPORT_DIR / "feature_neighborhood_scan_summary.csv"
    delta_path = REPORT_DIR / "feature_neighborhood_scan_deltas.csv"
    logs_path = REPORT_DIR / "feature_neighborhood_run_logs.json"
    packet_path = REPORT_DIR / "feature_neighborhood_scan_packet.json"
    ...
    packet = {
        ...
        "scope": "research_only_10d_fixed4y_fold08_fs40d3_feature_neighborhood_scan",
        "label": LABEL,
