...
OUT_DIR = DATA_DIR / "reports" / "model_agent_5d_local_reblend_1d10d_20260623"
...
RESEARCH_TABLE = "stock_predict_data_model_agent_5d_local_reblend_1d10d_20260623_executable_5d_open_return_research"
...
ZONES = [0.002, 0.003, 0.004, 0.005, 0.006, 0.008]
GRID = [
    (0.0, 0.002),
    (0.0, 0.005),
    (0.0, 0.01),
...
def main() -> int:
    ...
    for start in STARTS:
        for zone in ZONES:
            for alpha1, alpha10 in GRID:
                mask = (frame["trade_date"] >= start) & (frame["rank_base"] >= 1.0 - zone)
                score = frame["rank_base"].copy()
                score.loc[mask] = (
                    frame.loc[mask, "rank_base"]
                    + alpha1 * frame.loc[mask, "rank1"]
                    + alpha10 * frame.loc[mask, "rank10"]
                )
                variant = frame[["trade_date", "stock_code", LABEL_COL]].copy()
                variant["score"] = score
                summary, _ = evaluate(variant, "score")
                score = focus_score(summary)
                row = {
                    "start": start,
                    "zone": zone,
                    "alpha1": alpha1,
                ...
                rows.append(row)
                if (
                    score > best_focus + 1e-15
                    or (
                        abs(score - best_focus) <= 1e-15
                        and best_rule is not None
                        and (start > best_rule["start"] or (start == best_rule["start"] and int(mask.sum()) < best_rule["active_rows"]))
                    )
                ):
                    best_focus = score
                    best_rule = row
    ...
    manifest = {
        ...
        "label": LABEL_COL,
        "candidate_id": "model_agent_5d_local_reblend_1d10d_20260623",
        ...
        "notes": "5D 研究候选在当前 3D dategate 候选基础上，叠加 1D/10D 的局部重排，仅用于 research 验证，不进入 formal L4/L5。",
    }
