...
OUT = ROOT / "quant" / "data_file" / "reports" / "strategy_agent_current_formal_l4_style_regularization_juejin_20260720"
SIGNALS = OUT / "signals"
LOGS = OUT / "logs"
...
FILTERS = {
    "base": "least(rank_5d, rank_10d) >= 0.50 AND signal_pct_chg_raw <= 3.0",
    "mv500": "least(rank_5d, rank_10d) >= 0.50 AND signal_pct_chg_raw <= 3.0 AND total_mv >= 500000",
    "mv800": "least(rank_5d, rank_10d) >= 0.50 AND signal_pct_chg_raw <= 3.0 AND total_mv >= 800000",
    "amt150": "least(rank_5d, rank_10d) >= 0.50 AND signal_pct_chg_raw <= 3.0 AND amount >= 150000",
    "amt200": "least(rank_5d, rank_10d) >= 0.50 AND signal_pct_chg_raw <= 3.0 AND amount >= 200000",
    "amt150_mv500": "least(rank_5d, rank_10d) >= 0.50 AND signal_pct_chg_raw <= 3.0 AND amount >= 150000 AND total_mv >= 500000",
    "atr80": "least(rank_5d, rank_10d) >= 0.50 AND signal_pct_chg_raw <= 3.0 AND atr_rank <= 0.80",
}
for name, expression in FILTERS.items():
    MODULE.FILTERS[f"style_{name}"] = expression


def run_case(con: duckdb.DuckDBPyConnection, name: str, score_db: Path) -> dict:
    case = {"blend": "w10_100", "score": "plain", "filter": f"style_{name}", "threshold": 0.98, "topn": 1, "hold": 12}
...
def main() -> None:
    for path in (OUT, SIGNALS, LOGS, SCORES):
        path.mkdir(parents=True, exist_ok=True)
    MODULE.REPORT_DIR, MODULE.SIGNAL_DIR, MODULE.LOG_DIR, MODULE.SCORE_DIR = OUT, SIGNALS, LOGS, SCORES
    con = duckdb.connect(str(MODULE.FEATURE_DB), read_only=True)
    rows = []
    try:
        score_db = MODULE.build_score(con, "w10_100")
        for name in FILTERS:
            result = run_case(con, name, score_db)
            rows.append(result)
