...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_score_momentum_order_v242_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v242_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def momentum_order(score: np.ndarray, lookback: int, tilt: float) -> np.ndarray:
    if tilt <= 0:
...
def run_case(arrays, score, definition, protocol, end, start=None):
    order = momentum_order(
        score,
        int(definition["score_momentum_lookback"]),
        float(definition["score_momentum_tilt"]),
    )
    return v195.run_case(arrays, score, order, definition, protocol, end, start)


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 分数动量候选排序研究")
                ...
                rows.append(
                    {
                        ...
                        "score_momentum_lookback": definition["score_momentum_lookback"],
                        "score_momentum_tilt": definition["score_momentum_tilt"],
                        "buy_start": start,
