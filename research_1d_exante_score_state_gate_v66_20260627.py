...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_1d_exante_score_state_gate_v66_20260627"
BASE_DAILY = DATA_DIR / "reports" / "model_agent_current_best_promotion_review_20260627_v60" / "executable_1d_open_return_baseline_daily_eval.csv"
CAND_DAILY = DATA_DIR / "reports" / "model_agent_current_best_promotion_review_20260627_v60" / "executable_1d_open_return_candidate_daily_eval.csv"
...
def main() -> int:
    ...
    for (_, name_a, mask_a), (_, name_b, mask_b) in combinations(top_single, 2):
        ...
        if not bool(mask.any()):
            continue
        row, daily, month_df = gate.evaluate_mask(f"({name_a}) AND ({name_b})", mask, base_daily, cand_diff, base_summary)
        rows.append(row)
        if better(row, best_row):
            best_row = row
            best_daily = daily
    ...
    summary = {
        ...
        "scope": "research_only_1d_exante_score_state_gate_v66",
