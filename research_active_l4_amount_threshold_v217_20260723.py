...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_amount_threshold_v217_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v217_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 成交额门槛邻域研究")
    ...
    rows, definitions = [], {}
    for amount_min in protocol["grid"]["amount_min"]:
        definition = {**protocol["fixed_definition"], "amount_min": int(amount_min)}
