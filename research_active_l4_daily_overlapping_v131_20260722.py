...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_daily_overlapping_v131_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v131_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4慢分数每日小批次重叠持仓研究")
    ...
    rows, definitions = [], {}
    grid = protocol["grid"]
    for daily_top_n, max_hold, sell_below in product(grid["daily_top_n"], grid["max_hold_days"], grid["sell_score_below"]):
        definition = {**protocol["fixed_definition"], "daily_top_n": daily_top_n, "max_hold_days": max_hold, "sell_score_below": sell_below}
