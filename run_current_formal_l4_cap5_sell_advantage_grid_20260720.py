...
OUT = ROOT / "quant" / "data_file" / "reports" / "strategy_agent_current_formal_l4_cap5_sell_advantage_grid_20260720"
...
CASES = [
    {"min_hold": min_hold, "max_hold": 16, "edge": edge}
    for min_hold in (4, 6, 8, 10)
    for edge in (0.01, 0.02, 0.03)
]


def case_name(case: dict) -> str:
    return f"min{case['min_hold']}_max{case['max_hold']}_edge{int(case['edge'] * 100):02d}"
...
def run_case(base: pd.DataFrame, case: dict) -> dict:
    ...
    env.update({
        ...
        "GM_INDEPENDENT_REPLACE_RATIO": "1.0", "GM_INDEPENDENT_REPLACE_EDGE": str(case["edge"]),
