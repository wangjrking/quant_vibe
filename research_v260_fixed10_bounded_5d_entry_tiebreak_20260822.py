...
OUTPUT_ROOT = (
    ...
    "strategy_agent_v260_fixed10_bounded_5d_entry_tiebreak_20260822"
)
CHECKPOINT = (
    REPO
    / "quant/data_file/reports/"
...
def bounded_entry_order(
    baseline_order: np.ndarray,
    alternative_score: np.ndarray,
    pool_size: int,
) -> np.ndarray:
    order = np.asarray(baseline_order, dtype=np.int64)
    score = np.asarray(alternative_score, dtype=np.float64)
    if order.shape != score.shape:
        raise ValueError("entry order and alternative score shapes do not align")
    limit = min(max(int(pool_size), 10), order.shape[1])
    result = order.copy()
    base_position = np.arange(limit, dtype=np.int64)
    for day in range(order.shape[0]):
        head = order[day, :limit]
        values = score[day, head]
        safe = np.where(np.isfinite(values), values, -np.inf)
        permutation = np.lexsort((base_position, -safe))
        result[day, :limit] = head[permutation]
    return result


def run(context, policy: dict, order, priority_matrix, cost: float):
    case_context = copy.copy(context)
    case_context.order = order
...
def main() -> None:
    ...
    if checkpoint["validation_2026_opened"]:
        raise PermissionError("2026 data entered bounded entry-score development")
    ...
    score_5d, _ = score_tools.smooth_score(
        context.arrays["rank_5d"],
        score_tools.SMOOTHING_WINDOW,
        score_tools.RAW_ALPHA,
    )
    orders = {
        "current_pure10d_order": context.order,
        "control_5d_tiebreak_within_top15_10d": bounded_entry_order(
            context.order, score_5d, 15
        ),
        "candidate_5d_tiebreak_within_top20_10d": bounded_entry_order(
            context.order, score_5d, 20
        ),
        "control_5d_tiebreak_within_top30_10d": bounded_entry_order(
            context.order, score_5d, 30
        ),
    }
    results, cache = {}, {}
    for candidate_id, order in orders.items():
        ...
        for cost in COST_LEVELS:
            daily, actions = run(
                context, policy, order, priority_matrix, cost
            )
    ...
    baseline_id = "current_pure10d_order"
    candidate_id = "candidate_5d_tiebreak_within_top20_10d"
    ...
    if not all(equivalent.values()):
        raise RuntimeError("bounded entry-score baseline drifted")
    ...
    gates = {
        ...
        "both_pool_controls_improve_training_sharpe": all(
            ...
            > baseline["train_2022_2024"]["sharpe"]
            for control in (
                "control_5d_tiebreak_within_top15_10d",
                "control_5d_tiebreak_within_top30_10d",
            )
        ),
    ...
    repeat_daily, repeat_actions = run(
        context, policy, orders[selected], priority_matrix, 0.0030
    )
    deterministic = {
    ...
    if not all(deterministic.values()):
        raise RuntimeError("bounded entry-score replay failed")
    result = {
        ...
        "only_change": (
            "keep the production 10D score for eligibility and exits, but reorder "
            "only the top-20 10D entry pool by the smoothed 5D score"
        ),
        "results": results,
        ...
        "selected_policy": {
            **policy,
            "entry_order_rule": None
            if selected == baseline_id
            else "smoothed_5d_tiebreak_within_top20_smoothed_10d_pool",
        },
