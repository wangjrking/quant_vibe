...
import research_v260_fixed10_max_hold_final_recheck_20260822 as round13
...
OUTPUT_ROOT = (
    REPO
    / "quant/data_file/reports/strategy_agent_v260_fixed10_replacement_margin_final_recheck_20260822"
)
REPLACEMENT_MARGINS = (0.03, 0.05, 0.08, 0.10)


def candidate_policy(replacement_advantage: float) -> dict:
    current = round13.candidate_policy(25)
...
def main() -> None:
    ...
    if access["logical_max_date"] != round1.DEVELOPMENT_END:
        raise PermissionError("2026 data entered final replacement-margin recheck")
    ...
    if not all(deterministic.values()):
        raise RuntimeError("final replacement-margin deterministic replay failed")
