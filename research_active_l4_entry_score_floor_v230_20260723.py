...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_entry_score_floor_v230_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v230_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def run_case(arrays, score, order, definition, protocol, end, start=None):
    ...
    score_min = definition["entry_score_min"]
    if score_min is not None:
        selection_mask &= np.isfinite(score) & (score >= float(score_min))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 绝对分数入场下限研究")
    ...
    rows, definitions = [], {}
    for score_min in protocol["grid"]["entry_score_min"]:
        definition = {
            **protocol["fixed_definition"],
            "entry_score_min": None if score_min is None else float(score_min),
        }
