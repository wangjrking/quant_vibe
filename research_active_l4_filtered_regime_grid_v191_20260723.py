...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_filtered_regime_grid_v191_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v191_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 filtered regime-grid study")
    ...
    rows, definitions = [], {}
    for normal_min, strong_min, weak_target in product(protocol["grid"]["market_return_min"], protocol["grid"]["strong_return_min"], protocol["grid"]["weak_target_pct"]):
        definition = {**protocol["fixed_definition"], "market_return_min": float(normal_min), "strong_return_min": float(strong_min), "weak_target_pct": float(weak_target)}
