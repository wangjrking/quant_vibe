...
OUTPUT_ROOT = (
    REPO
    / "quant/data_file/reports/strategy_agent_v260_fixed10_replacement_margin_20260822"
)
REPLACEMENT_MARGINS = (0.03, 0.05, 0.08, 0.10)


def candidate_policy(replacement_advantage: float) -> dict:
    current = round3.candidate_policy(25, "no_score_exit_rebalance")
...
def main() -> None:
    ...
    if access["logical_max_date"] != round1.DEVELOPMENT_END:
        raise PermissionError("2026 data entered replacement-margin development")
    ...
    if not all(deterministic.values()):
        raise RuntimeError("replacement-margin deterministic replay failed")
