...
SIGNAL_ROOT = REPORT_DIR / "signals" / "market_regime_scaling"
SIGNAL_DIR = REPORT_DIR / "signals" / "market_regime_hit_fragility"
LOG_DIR = REPORT_DIR / "logs" / "market_regime_hit_fragility"
OUT_CSV = REPORT_DIR / "market_regime_hit_fragility_juejin_results_20260714.csv"
OUT_JSON = REPORT_DIR / "market_regime_hit_fragility_juejin_results_20260714.json"
PROXY_CSV = REPORT_DIR / "market_regime_hit_proxy_contribution_20260714.csv"
SUMMARY_JSON = REPORT_DIR / "market_regime_hit_fragility_summary_20260714.json"
...
SOURCES = [
    SIGNAL_ROOT / "mkt_panic_deep_x115.csv",
    SIGNAL_ROOT / "mkt_panic_deep_x125.csv",
]
...
def cases_for_source(proxy: pd.DataFrame, source_case: str) -> list[dict]:
    ...
    return [
        {
            "case": f"{source_case}_remove_top1_month",
            "source_case": source_case,
            "remove_months": top_months[:1],
            "remove_years": [],
            "remove_stocks": [],
        },
        {
            "case": f"{source_case}_remove_top3_month",
            "source_case": source_case,
            "remove_months": top_months,
            "remove_years": [],
            "remove_stocks": [],
        },
        {
            "case": f"{source_case}_remove_top1_stock",
            "source_case": source_case,
            "remove_months": [],
            "remove_years": [],
            "remove_stocks": top_stocks[:1],
        },
        {
            "case": f"{source_case}_remove_top3_stock",
            "source_case": source_case,
            "remove_months": [],
            "remove_years": [],
            "remove_stocks": top_stocks,
        },
        {
            "case": f"{source_case}_remove_2024",
            "source_case": source_case,
            "remove_months": [],
            "remove_years": ["2024"],
            "remove_stocks": [],
...
def main() -> None:
    ...
    sources = {path.stem: load_signal(path) for path in SOURCES}
    all_df = pd.concat(sources.values(), ignore_index=True)
    proxy = add_proxy_returns(all_df)
    cases = []
    for source_case in sources:
        cases.extend(cases_for_source(proxy, source_case))
    manifest = [write_variant(sources[case["source_case"]], case) for case in cases]
    results = []
    for row in manifest:
        result = run_or_parse(row)
        results.append(result)
        pd.DataFrame(results).sort_values(
            ["source_case", "case"], ascending=[True, True], na_position="last"
