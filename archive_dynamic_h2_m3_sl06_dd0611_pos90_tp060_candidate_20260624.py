...
STRATEGY_ID = "candidate_dynamic_h2m3_sl06dd0611_pos90_tp060_v20260624"
...
VALIDATION_DIR = BASE_REPORT_DIR / "dynamic_h2_m3_sl06_dd0611_pos90_tp060_validation_20260624"
POSITION_DIR = BASE_REPORT_DIR / "dynamic_h2_m3_dd0611_tp060_position_20260624"
SIGNAL_FILE = POSITION_DIR / "signals" / "dd0611_tp060_pos90.csv"
...
def trading_rules() -> dict:
    return {
        ...
        "strategy_name": "动态持有 Top1 三周期融合 pos90 高收益贴边候选",
...
def validation_payload(summary: dict) -> dict:
    ...
    return {
        ...
        "strong_admission_status": {
            ...
            "reason": "收益指标属于弱准入。该候选完整掘金验证、低路径依赖、贡献压力和硬过滤预检通过；但最大回撤贴近 40% 红线，需审计复核后才能讨论生产。",
            "residual_risks": [
                "最大回撤 39% 以上，离 40% 风险边界很近。",
...
def main() -> int:
    ...
    latest = sorted(signal_rows, key=lambda row: (row.get("signal_date") or "", row.get("buy_date") or ""))[-1]
