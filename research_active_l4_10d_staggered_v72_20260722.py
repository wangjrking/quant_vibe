...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_10d_staggered_v72_20260722"
...
class Profile:
    ...
    sell_review_interval: int

    @property
    def case_id(self) -> str:
        raw = json.dumps(asdict(self), sort_keys=True, separators=(",", ":"))
        return "s10_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def simulate(
    ...
    for t, signal_date in enumerate(dates[:-1]):
        ...
        for idx in list(shares):
            ...
            review_due = age >= profile.min_hold and (
                (age - profile.min_hold) % profile.sell_review_interval == 0
            )
            rank_exit = review_due and held_score < profile.sell_rank_below
            replacement_exit = review_due and (
                best_unheld_score - held_score >= profile.replacement_advantage
            )
            if age >= profile.max_hold or rank_exit or replacement_exit:
...
def is_eligible(frame: pd.DataFrame) -> pd.Series:
    return (
...
def main() -> None:
    ...
    for amount_min in grid["amount_min"]:
        for mv_min in grid["mv_min"]:
            for turnover_max in grid["turnover_max"]:
    ...
    stage1["eligible"] = is_eligible(stage1)
    ...
    for _, seed in promoted.iterrows():
        ...
        for min_hold in grid2["min_hold"]:
            for max_hold in grid2["max_hold"]:
                for sell_rank_below in grid2["sell_rank_below"]:
                    for sell_review_interval in grid2["sell_review_interval"]:
                        profile = Profile(
                            base.amount_min,
                            base.mv_min,
                            base.turnover_max,
                            ...
                            int(min_hold),
                            int(max_hold),
                            float(sell_rank_below),
                            float(grid2["replacement_advantage"]),
                            int(sell_review_interval),
                        )
    ...
    if not stage2.empty:
        ...
        stage2["eligible"] = is_eligible(stage2)
