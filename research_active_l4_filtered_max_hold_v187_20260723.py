...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_filtered_max_hold_v187_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v187_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 filtered max-hold study")
    ...
    rows, definitions = [], {}
    for days in protocol["grid"]["max_hold_days"]:
        definition = {**protocol["fixed_definition"], "max_hold_days": int(days)}
