...
OUT_DIR = DATA_DIR / "reports" / "model_agent_1d_aux_fusion_tune_20260622"
...
def materialize(best_name: str, merged: pd.DataFrame, score: pd.Series, best: dict) -> dict:
    ...
    manifest = {
        ...
        "notes": "1D gate7 与多周期预测分数rank融合研究资产；不作为L5正式入口。",
    }
...
def main() -> int:
    ...
    with sqlite3.connect(MODEL_DB) as conn:
        merged = read_prediction(conn, BASE_TABLE, "gate7_pred_prob", keep_payload=True)
    ...
    score_cols = ["gate7_pred_prob"] + [f"{alias}_pred_prob" for alias in AUX_TABLES]
    for col in score_cols:
        add_rank(merged, col, f"rank_{col.removesuffix('_pred_prob')}")

    rank_cols = ["rank_gate7"] + [f"rank_{alias}" for alias in AUX_TABLES]
    candidates: dict[str, pd.Series] = {
        "gate7_raw": merged["gate7_pred_prob"],
        "gate7_rank": merged["rank_gate7"],
    }
    for alias in AUX_TABLES:
        candidates[f"{alias}_rank"] = merged[f"rank_{alias}"]
        for weight in [0.85, 0.75, 0.65, 0.50]:
            rest = 1.0 - weight
            name = f"rank_blend_gate7w{str(weight).replace('.', 'p')}_{alias}w{str(rest).replace('.', 'p')}"
            candidates[name] = merged["rank_gate7"] * weight + merged[f"rank_{alias}"] * rest

    aux_rank_cols = [f"rank_{alias}" for alias in AUX_TABLES]
    candidates["rank_avg_gate7_3d5d10d"] = merged[["rank_gate7", "rank_formal_3d", "rank_formal_5d", "rank_formal_10d"]].mean(axis=1)
    candidates["rank_avg_gate7_3d5dblend10d"] = merged[["rank_gate7", *aux_rank_cols]].mean(axis=1)
    candidates["rank_consensus_min_gate7_3d5d10d"] = merged[["rank_gate7", "rank_formal_3d", "rank_formal_5d", "rank_formal_10d"]].min(axis=1)
    candidates["rank_consensus_max_gate7_3d5d10d"] = merged[["rank_gate7", "rank_formal_3d", "rank_formal_5d", "rank_formal_10d"]].max(axis=1)
    candidates["rank_weighted_gate7_0p55_5dblend_0p25_10d_0p20"] = (
        merged["rank_gate7"] * 0.55 + merged["rank_blend_5d_research"] * 0.25 + merged["rank_formal_10d"] * 0.20
    )
    candidates["rank_weighted_gate7_0p60_3d_0p20_5dblend_0p20"] = (
        merged["rank_gate7"] * 0.60 + merged["rank_formal_3d"] * 0.20 + merged["rank_blend_5d_research"] * 0.20
    )
    candidates["rank_weighted_gate7_0p70_5dblend_0p30"] = (
        merged["rank_gate7"] * 0.70 + merged["rank_blend_5d_research"] * 0.30
    )
