...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_current_best_second_or_expansion_v96_20260627"
SNAPSHOT = DATA_DIR / "reports" / "model_agent_current_best_research_snapshot_20260627_v94" / "current_best_research_snapshot_v94.json"
...
TARGET_TABLES = {
    "executable_1d_open_return": "stock_predict_data_model_agent_1d_second_or_expansion_v96_20260627_executable_1d_open_return_research",
    "executable_3d_open_return": "stock_predict_data_model_agent_3d_second_or_expansion_v96_20260627_executable_3d_open_return_research",
    "executable_5d_open_return": "stock_predict_data_model_agent_5d_second_or_expansion_v96_20260627_executable_5d_open_return_research",
    "executable_10d_open_return": "stock_predict_data_model_agent_10d_second_or_expansion_v96_20260627_executable_10d_open_return_research",
}
RANK_IC_FLOORS = {
...
QUANTILES = [0.03, 0.05, 0.10, 0.20, 0.33, 0.50, 0.67, 0.80, 0.90, 0.95, 0.97]
MAX_ACTIVE_DAYS = {
    "executable_1d_open_return": 340,
    "executable_3d_open_return": 320,
    "executable_5d_open_return": 280,
    "executable_10d_open_return": 320,
}
...
def scan_label(label: str, base_active: set[str], features: pd.DataFrame, pair: pd.DataFrame, formal_daily: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, object]]:
    ...
    base_row = make_row(label, "v94_base_condition", len(base_active), base_active, base_result)
    ...
    max_active = MAX_ACTIVE_DAYS[label]
    for feature in guarded.FEATURES:
        ...
        for threshold in thresholds:
            for op in ["<=", ">="]:
                ...
                if len(active_dates) <= len(base_active) or len(active_dates) > max_active:
                    continue
                condition = f"v94_base OR {feature} {op} {threshold:.12g}"
...
def run_label(label: str, item: dict[str, object], config: dict[str, object], expected_latest: str, expected_rows: int) -> dict[str, object]:
    ...
    asset_prefix = label.replace("executable_", "").replace("_open_return", "")
    spec = {
        "asset": f"research_{asset_prefix}_second_or_expansion_v96_20260627",
    ...
    scan.to_csv(output_dir / "second_or_expansion_scan.csv", index=False, encoding="utf-8-sig")
    ...
    candidate = {
        ...
        "prefer_simpler_formula": "v94 current best condition plus one additional TopRank-focused OR expansion filter",
        "prefer_fewer_active_days": row["active_days"],
        "prefer_fewer_upstream_sources": "formal+candidate score tables",
        "prefer_clearer_roll_back_path": "drop research-only v96 table and keep current formal unchanged",
    }
    ...
    manifest = {
        ...
        "feature_input": "quant/data_file/production_factor_parts/",
        "label_input": "quant/data_file/prediction_label_parts/",
        "formula": "v94 current best condition plus one additional TopRank-focused OR expansion filter",
    ...
    (output_dir / f"{label}_v96_summary.json").write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    return payload
...
def main() -> int:
    ...
    payload = {
        ...
        "scope": "research_only_current_best_second_or_expansion_v96",
    ...
    matrix.to_csv(REPORT_DIR / "v96_summary_matrix.csv", index=False, encoding="utf-8-sig")
    payload["summary_matrix"] = str(REPORT_DIR / "v96_summary_matrix.csv")
    (REPORT_DIR / "v96_run_summary.json").write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    lines = [
        "# 当前最优第二层 OR 扩展研究 v96",
        "",
        "## 结论",
        "",
        "本轮以 v94 当前最优研究资产为 base，只扫描一条额外 OR 条件；不训练模型，不修改 formal/production manifest，不生成信号，不运行回测。",
        "",
    ]
    for row in rows:
        lines.extend(
    ...
    (REPORT_DIR / "v96_report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
