...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_weak_entry_cadence_v213_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v213_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def entry_schedule(arrays: dict, definition: dict) -> np.ndarray:
    momentum = v134.market_momentum(arrays, 10)
    active = np.ones(len(momentum), dtype=np.bool_)
    weak = np.isfinite(momentum) & (momentum < 0.0)
    every = int(definition["weak_entry_every"])
    offset = int(definition["weak_entry_offset"])
    if every > 1:
        indices = np.arange(len(momentum))
        active[weak] = (indices[weak] % every) == offset
    return active
...
def run_case(arrays, score, order, definition, protocol, end, start=None):
    ...
    return v109.simulate(
        ...
        rebalance_active_override=entry_schedule(arrays, definition),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 弱市入场频率研究")
