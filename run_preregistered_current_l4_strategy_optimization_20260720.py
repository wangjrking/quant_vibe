...
CONFIG = MAIN / "config" / "strategy_research" / "current_l4_preregistered_optimization_20260720.json"
OUT = ROOT / "quant" / "data_file" / "reports" / "strategy_agent_preregistered_current_l4_optimization_20260720"
...
def entry_cases(config: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    values = itertools.product(
        ...
        grid["signal_pct_band"], grid["buy_open_gate"],
    )
    for (blend, weights), topn, rank_min, support, amount, mv, pct, open_gate in values:
        name = (
            f"{blend}_n{topn}_r{str(rank_min).replace('.', 'p')}_{support['name']}_"
            f"a{int(amount)}_{pct['name']}_{open_gate['name']}"
        )
        rows.append({
            "name": name, "blend": blend, "weights": weights, "topn": topn,
            "blend_rank_min": rank_min, "support": support, "amount_min": amount,
            "mv_min": mv, "pct": pct, "open_gate": open_gate,
        })
    return rows
...
def load_blend_frame(
    ...
    source_columns = "* EXCLUDE(next_open_raw, next_open_return_raw)" if include_holdout else "*"
    return con.execute(
        f'''
        WITH scored AS (
...
def period_metrics(
    ...
    part = part.dropna(subset=["next_open_return_raw"])
    if part.empty:
        return {"annual": -1.0, "sharpe": -99.0, "mdd": 1.0, "buy_days": 0, "rows": 0}
    part["weight"] = 0.9 / topn * part["open_scale"]
    part["net"] = part["weight"] * (part["next_open_return_raw"] - cost)
    ...
    try:
        ...
        for case in all_cases:
            ...
            for prefix, window in (
                ...
                metrics = period_metrics(frame, calendar, window[0], window[1], cost, int(case["topn"]))
...
def run_juejin(signal: Path, score_db: Path, name: str, start: str, end: str, slippage: float, exit_case: dict[str, Any], market: Path, max_positions: int) -> dict[str, Any]:
    log = LOGS / f"{name}.log"
    env = os.environ.copy()
    env.update({
    ...
    command = [
        ...
        "--log-file", str(log), "--max-positions", str(max_positions), "--holding-days", str(exit_case["min_hold"]),
