...
def _variant(name: str, rules: list[dict], *, cap: float = 0.42, env: dict[str, str] | None = None) -> dict:
    out_env = dict(BASE_ENV)
    if env:
        out_env.update(env)
    return {"name": name, "rules": rules, "cap": cap, "env": out_env}
...
VARIANTS = [
    ...
    _variant(
        "avgmv_low75_mid108",
        [
            {"field": "avg_mv", "op": "lt", "threshold": 132800.0, "scale": 0.75},
            {"field": "avg_mv", "op": "between", "low": 132800.0, "high": 171600.0, "scale": 1.08},
        ],
    ),
    _variant(
        "avgmv_low65_mid112",
        [
            {"field": "avg_mv", "op": "lt", "threshold": 132800.0, "scale": 0.65},
            {"field": "avg_mv", "op": "between", "low": 132800.0, "high": 171600.0, "scale": 1.12},
        ],
    ),
    _variant(
        "avgp10_high80_low108",
        [
            {"field": "avg_p10", "op": "ge", "threshold": 0.0703, "scale": 0.80},
            {"field": "avg_p10", "op": "lt", "threshold": 0.0119, "scale": 1.08},
        ],
    ),
    _variant(
        "avggap_high80_low108",
        [
            {"field": "avg_gap", "op": "ge", "threshold": 0.0635, "scale": 0.80},
            {"field": "avg_gap", "op": "lt", "threshold": 0.0109, "scale": 1.08},
        ],
    ),
    _variant(
        "strong5_boost108_weakmany_down90",
        [
            {"field": "strong_cnt", "op": "ge", "threshold": 5.0, "scale": 1.08},
            {"field": "weak_cnt", "op": "ge", "threshold": 3.0, "scale": 0.90},
        ],
    ),
    _variant(
        "mv_p10_combo_soft",
        [
            {"field": "avg_mv", "op": "lt", "threshold": 132800.0, "scale": 0.82},
            {"field": "avg_mv", "op": "between", "low": 132800.0, "high": 171600.0, "scale": 1.06},
            {"field": "avg_p10", "op": "ge", "threshold": 0.0703, "scale": 0.88},
            {"field": "avg_p10", "op": "lt", "threshold": 0.0119, "scale": 1.05},
        ],
    ),
    _variant(
        "mv_gap_combo_soft",
        [
            {"field": "avg_mv", "op": "lt", "threshold": 132800.0, "scale": 0.82},
            {"field": "avg_mv", "op": "between", "low": 132800.0, "high": 171600.0, "scale": 1.06},
            {"field": "avg_gap", "op": "ge", "threshold": 0.0635, "scale": 0.88},
            {"field": "avg_gap", "op": "lt", "threshold": 0.0109, "scale": 1.05},
        ],
    ),
    _variant(
        "mv_gap_strong_combo",
        [
            {"field": "avg_mv", "op": "lt", "threshold": 132800.0, "scale": 0.80},
            {"field": "avg_mv", "op": "between", "low": 132800.0, "high": 171600.0, "scale": 1.07},
            {"field": "avg_gap", "op": "ge", "threshold": 0.0635, "scale": 0.90},
            {"field": "avg_gap", "op": "lt", "threshold": 0.0109, "scale": 1.04},
            {"field": "strong_cnt", "op": "ge", "threshold": 5.0, "scale": 1.05},
        ],
    ),
    _variant(
        "mv_p10_gap_strong_combo",
        [
            {"field": "avg_mv", "op": "lt", "threshold": 132800.0, "scale": 0.80},
            {"field": "avg_mv", "op": "between", "low": 132800.0, "high": 171600.0, "scale": 1.07},
            {"field": "avg_p10", "op": "ge", "threshold": 0.0703, "scale": 0.90},
            {"field": "avg_p10", "op": "lt", "threshold": 0.0119, "scale": 1.04},
            {"field": "avg_gap", "op": "ge", "threshold": 0.0635, "scale": 0.92},
            {"field": "avg_gap", "op": "lt", "threshold": 0.0109, "scale": 1.03},
            {"field": "strong_cnt", "op": "ge", "threshold": 5.0, "scale": 1.04},
        ],
    ),
    _variant(
        "mv_combo_no_dd",
...
def _day_features(rows: list[dict]) -> dict[str, dict]:
    ...
    for day, day_rows in grouped.items():
        targets = [_to_float(row.get("target_pct"), 0.0) or 0.0 for row in day_rows]
        gaps = [_to_float(row.get("pred_gap")) for row in day_rows]
        gaps = [value for value in gaps if value is not None]
        p10s = [_to_float(row.get("pred_10d")) for row in day_rows]
        p10s = [value for value in p10s if value is not None]
        mvs = [_to_float(row.get("total_mv")) for row in day_rows]
        mvs = [value for value in mvs if value is not None]
        features[day] = {
            "day_target": sum(targets),
            "avg_gap": sum(gaps) / len(gaps) if gaps else None,
            "avg_p10": sum(p10s) / len(p10s) if p10s else None,
            "avg_mv": sum(mvs) / len(mvs) if mvs else None,
            "strong_cnt": sum(1 for row in day_rows if row.get("pool_role") == "strong"),
            "weak_cnt": sum(1 for row in day_rows if row.get("pool_role") == "weak"),
        }
    return features
...
def _day_scale(features: dict, rules: list[dict]) -> float:
    scale = 1.0
    for rule in rules:
        if _matches(_to_float(features.get(rule["field"])), rule):
...
def _write_signal(cfg: dict, signal_file: Path) -> dict:
    ...
    for field in ["day_state_scale", "day_avg_mv", "day_avg_gap", "day_avg_p10", "day_strong_cnt", "day_weak_cnt"]:
    ...
    for row in rows:
        ...
        feat = features[day]
        scale = _day_scale(feat, cfg["rules"])
        base_target = _to_float(row.get("target_pct"), 0.0) or 0.0
        row["target_pct"] = f"{min(base_target * scale, float(cfg['cap'])):.5f}"
        row["day_state_scale"] = f"{scale:.6f}"
        row["day_avg_mv"] = f"{_to_float(feat.get('avg_mv'), 0.0) or 0.0:.6f}"
        row["day_avg_gap"] = f"{_to_float(feat.get('avg_gap'), 0.0) or 0.0:.6f}"
        row["day_avg_p10"] = f"{_to_float(feat.get('avg_p10'), 0.0) or 0.0:.6f}"
        row["day_strong_cnt"] = str(int(feat.get("strong_cnt") or 0))
        row["day_weak_cnt"] = str(int(feat.get("weak_cnt") or 0))
...
def main() -> int:
    ...
    _write_rows(
        ...
        [row for row in results if _metric(row, "annual") >= 3.0 and _metric(row, "sharpe") >= 4.0 and _metric(row, "avg_invested_pct") >= 0.80],
    )
    return 0
