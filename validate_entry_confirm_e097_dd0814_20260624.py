...
CASE_NAME = "entry_confirm_w78_5d12_3d10_pos56_h3_e097_mh1_dd0814_s8565"
SOURCE_DIR = liq.SOURCE_DIR / "strict_sync_liquidity_neighborhood_20260624"
REPORT_DIR = (
    SOURCE_DIR
    / "formal_horizon_entry_confirmation_20260624"
    / "entry_confirm_w78_5d12_3d10_pos56_h3_e097_dd0814_validation_20260624"
)
SIGNAL_FILE = (
    SOURCE_DIR
    / "formal_horizon_entry_confirmation_20260624"
...
TIME_SLICES = validator.TIME_SLICES
MAIN_STARTS = validator.MAIN_STARTS
RISK_ENV = {
...
def _write_report(time_rows: list[dict], nearby_summary: list[dict], stress_summary: list[dict], proxy_stats: dict, audit: dict) -> None:
    lines = [
        "# entry_confirm e097 账户回撤缩放完整验证",
        "",
        "## 当前结论",
        "",
        f"验证对象：`{CASE_NAME}`。本版本在 `entry_confirm_w78_5d12_3d10_pos56_h3_e097_mh1` 上启用账户回撤缩放 `dd_08_14_s85_65`，不改变选股池，不使用日期、月份、行业过滤。",
        "",
        "该版本降低全周期最大回撤，但收益和近期开仓最差年化低于不启用回撤缩放版本。是否采用应作为风险偏好选择，不能视为已经解决全部强准入问题。",
        "",
        "## 时间切片",
        "",
...
def main() -> int:
    ...
    try:
        ...
        validator.HOLDING_DAYS = HOLDING_DAYS
