...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_breadth_universe_v115_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v115_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def case_protocol(protocol: dict, definition: dict) -> dict:
    result = copy.deepcopy(protocol)
    result["fixed_universe"] = {
        **protocol["fixed_universe"],
        "amount_min": int(definition["amount_min"]),
        "mv_min": int(definition["mv_min"]),
        "turnover_max": float(definition["turnover_max"]),
    }
    return result


def main() -> None:
    parser = argparse.ArgumentParser(description="收益优先宽度策略的流动性股票池研究")
    ...
    rows, definitions = [], {}
    grid = protocol["grid"]
    for amount_min, mv_min, turnover_max in product(grid["amount_min"], grid["mv_min"], grid["turnover_max"]):
        definition = {**protocol["fixed_exit"], "amount_min": amount_min, "mv_min": mv_min, "turnover_max": turnover_max}
