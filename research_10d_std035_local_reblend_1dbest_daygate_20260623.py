...
OUT_DIR = DATA_DIR / "reports" / "model_agent_10d_std035_local_reblend_1dbest_daygate_20260623"
...
RESEARCH_TABLE = "stock_predict_data_model_agent_10d_std035_local_reblend_1dbest_daygate_20260623_executable_10d_open_return_research"
...
CURRENT_ZONE = 0.0015
CURRENT_ALPHA = 0.01
STARTS = ["20251201", "20260115", "20260201", "20260225"]
STD_THRESHOLDS = [0.005, 0.01, 0.015, 0.02, 0.03, 0.05, 0.1, 0.2, 0.35, 0.45]
...
def main() -> int:
    ...
    current_summary, _ = evaluate(frame.assign(score=frame["pred_prob"] if "pred_prob" in frame.columns else frame["base_pred_prob"]), "score")
    current_best_score = frame["rank_base"].copy()
    active_best = (frame["trade_date"] >= "20260201") & (frame["rank_base"] >= 1.0 - CURRENT_ZONE)
    current_best_score.loc[active_best] = frame.loc[active_best, "rank_base"] + CURRENT_ALPHA * frame.loc[active_best, "rank1"]
    current_summary, _ = evaluate(frame.assign(score=current_best_score), "score")
    ...
    for start in STARTS:
        for threshold in STD_THRESHOLDS:
            active_mask = (
                ...
                & (frame["rank_base"] >= 1.0 - CURRENT_ZONE)
                & (frame["std_r10_top5_base"] < threshold)
            )
            score = frame["rank_base"].copy()
            score.loc[active_mask] = frame.loc[active_mask, "rank_base"] + CURRENT_ALPHA * frame.loc[active_mask, "rank1"]
    ...
    payload = {
        ...
        "candidate_id": "model_agent_10d_std035_local_reblend_1dbest_daygate_20260623",
        ...
        "governance_notes": [
            "research only",
            "no training",
            "adds day-level gate on top of current 10d best local rerank",
            "gate uses ex-ante std_r10_top5_base from existing std035 day-gate diagnostics",
    ...
    manifest = {
        ...
        "notes": "10D research 候选在当前 10D best 的窄区局部重排规则上增加日级门控，用于检查是否应仅在部分交易日启用局部重排。",
    }
    ...
    lines = [
        "# 10D current-best 加日级 gate 研究候选",
        "",
        "## 当前结论",
        "",
