...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_turnover_hold_v61_20260721"
...
def apply_turnover_filter(state, arrays, turnover_max):
...
def rebuild_exposure(state, arrays, floor, protocol):
    fixed = dict(protocol["fixed"])
    fixed["exposure_floor"] = floor
    rebuilt = robust.build_state(
        arrays,
        protocol["fixed"]["score_weights"],
        protocol["fixed"]["agreement"],
        protocol["fixed"]["entry"]["risk_off"],
        protocol["fixed"]["entry"]["risk_on"],
        fixed,
    )
    return rebuilt[0], rebuilt[1], state[2], rebuilt[3]


def run_case(arrays, protocol, state, top_n, exit_profile, record_actions=False):
    buy = simulator.BuyProfile(
        int(top_n), -1.0, 90000, 200000, 1.0, False, 0.99, 0.005, 1.0, "equal"
    )
...
def main():
    ...
    for turnover_max in p["stage1_grid"]["turnover_max"]:
        filtered = apply_turnover_filter(base_state, arrays, turnover_max)
        for exposure_floor in p["stage1_grid"]["exposure_floor"]:
            state = rebuild_exposure(filtered, arrays, exposure_floor, p)
            for top_n in p["stage1_grid"]["top_n"]:
                for exit_values in p["stage1_grid"]["exit_profiles"]:
                    ...
                    daily = run_case(arrays, p, state, top_n, exit_profile)
                    ...
                    case_id = "th1_" + stable_id(definition)
                    stage1_defs[case_id] = definition
                    stage1_rows.append({"case_id": case_id, **definition, **values})
    ...
    for _, seed in promoted.iterrows():
        ...
        for agreement in p["stage2_grid"]["agreement_profiles"]:
            for entry in p["stage2_grid"]["entry_rank_pairs"]:
                state_key = stable_id({
                    "turnover_max": seed_def["turnover_max"],
                    "exposure_floor": seed_def["exposure_floor"],
                    "agreement": agreement,
                    "entry": entry,
                })
                if state_key not in state_cache:
                    local_fixed = dict(fixed)
                    local_fixed["exposure_floor"] = seed_def["exposure_floor"]
                    ...
                    state_cache[state_key] = apply_turnover_filter(
                        state, arrays, seed_def["turnover_max"]
                    )
                ...
                for sell_rank_below in p["stage2_grid"]["sell_rank_below"]:
                    for replacement_advantage in p["stage2_grid"]["replacement_advantage"]:
                        ...
                        daily = run_case(arrays, p, state, seed_def["top_n"], exit_profile)
                        values = robust.evaluate_robust(daily, p)
                        definition = {
                        ...
                        case_id = "th2_" + stable_id(definition)
    ...
    for _, item in frozen.iterrows():
        ...
        _, actions = run_case(
            arrays, p, state, definition["top_n"], exit_profile, record_actions=True
        )
