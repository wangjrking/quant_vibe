...
def stable_id(definition: dict) -> str:
    ...
    return "v120_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4慢分数策略的组合结构研究")
    ...
    for top_n, max_hold, every in product(grid["top_n_per_rebalance"], grid["max_hold_days"], grid["rebalance_every"]):
        ...
        current = case_protocol(protocol, definition)
        daily = v109.simulate(observation, score_obs, order_obs, definition, current, protocol["observation_end"])
