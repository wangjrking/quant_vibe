...
REPORT_DIR = BASE_DIR / "dynamic_h2_m3_signal_field_refine_20260625"
SOURCE_SIGNAL = (
    BASE_DIR
...
TARGET_VARIANTS = [
    {"tag": "pos8975_scale7555", "target_pct": 0.8975, "dd_soft_scale": 0.75, "dd_hard_scale": 0.55},
    {"tag": "pos90_scale7454", "target_pct": 0.90, "dd_soft_scale": 0.74, "dd_hard_scale": 0.54},
]
EXIT_RATIOS = [0.968, 0.970, 0.972, 0.974]
CONTINUE_RATIOS = [0.972, 0.975, 0.978]

CASES = []
for base, exit_ratio, continue_ratio in product(TARGET_VARIANTS, EXIT_RATIOS, CONTINUE_RATIOS):
    name = (
        f"{base['tag']}_e{str(exit_ratio).replace('.', 'p')}"
        f"_c{str(continue_ratio).replace('.', 'p')}"
    )
    CASES.append(
        {
            **BASE_CASE,
...
def _run_case(case: dict, signal_file: Path, start_name: str, start: str, end: str) -> dict:
    ...
    try:
        ...
        core.MIN_HOLD = int(case["min_hold"])
...
def _summarize(detail: list[dict], signal_stats: dict[str, dict]) -> list[dict]:
    ...
    for case in CASES:
        ...
        late = [_f(by_start[key]["annual"]) for key in ["late_20251009", "late_20260105"] if key in by_start]
...
def _write_report(summary: list[dict], audit: dict) -> None:
    ...
    lines = [
        ...
        "## 当前结论",
        "",
        "本轮固定 formal L4 输入、Top1 单票股票池、BJ/ST/退市/开盘涨停硬约束和现有执行风控，只微调信号字段 `target_pct / score_exit_entry_ratio / score_continue_entry_ratio`。",
        "",
        "| 候选 | 年化 | Sharpe | 最大回撤 | recent60 年化 | 近端最差年化 | 平均持仓率 | 参数 |",
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | --- |",
    ]
    for row in ranked:
        params = (
            f"pos={_f(row['target_pct']):.2%}, "
            f"exit={_f(row['exit_ratio']):.3f}, "
            f"cont={_f(row['continue_ratio']):.3f}, "
    ...
    (REPORT_DIR / "signal_field_refine_report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
