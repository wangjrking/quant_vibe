...
import research_v260_fixed10_age_replacement_margin_20260822 as round12
...
OUTPUT_ROOT = (
    REPO
    / "quant/data_file/reports/strategy_agent_v260_fixed10_max_hold_final_recheck_20260822"
)
MAX_HOLD_DAYS = (20, 25, 30)


def candidate_policy(max_hold_days: int) -> dict:
    current = round12.candidate_policy(None)
...
def main() -> None:
    ...
    if access["logical_max_date"] != round1.DEVELOPMENT_END:
        raise PermissionError("2026 data entered final max-hold recheck")
    ...
    if not all(deterministic.values()):
        raise RuntimeError("final max-hold deterministic replay failed")
