...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_score_breadth_refine_v105_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v105_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="10D分数广度动态仓位边界扩展")
    ...
    if args.open_2026:
        ...
        for item in frozen["candidates"]:
            for start in protocol["validation_buy_starts"]:
                daily = v103.simulate(full, score, order, item["definition"], protocol, protocol["validation_end"], start)
    ...
    grid = protocol["grid"]
    rows, definitions = [], {}
    for threshold, low, high in product(grid["breadth_count_threshold"], grid["low_gross"], grid["high_gross"]):
        definition = {"breadth_count_threshold": threshold, "high_when": "above", "low_gross": low, "high_gross": high}
        case_id = stable_id(definition)
        definitions[case_id] = definition
        daily = v103.simulate(observation, score_obs, order_obs, definition, protocol, protocol["observation_end"])
