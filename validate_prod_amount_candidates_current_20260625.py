...
REPORT_DIR = DATA / "reports" / "strategy_agent_prod_amount_candidates_current_validation_20260625"
...
CANDIDATES = [
    {
        "name": "amt8p5w_mv20w",
        "signal_file": filt.REPORT_DIR / "signals" / "prod_filter_amt8p5w_mv20w.csv",
    },
    {
        "name": "amt9w_mv20w",
        "signal_file": filt.REPORT_DIR / "signals" / "prod_filter_amt9w_mv20w.csv",
    },
    {
        "name": "amt9p5w_mv20w",
        "signal_file": filt.REPORT_DIR / "signals" / "prod_filter_amt9p5w_mv20w.csv",
    },
]
...
def _run(signal_file: Path, case_name: str, tag: str, start: str, end: str = filt.SLICES[0][2]) -> dict:
    log_file = REPORT_DIR / case_name / "logs" / f"{tag}.log"
...
def _build_stress_variants(rows: list[dict], case_dir: Path) -> tuple[list[dict], dict]:
    ...
    _write_rows(case_dir / "proxy_signal_contribution.csv", [{k: v for k, v in row.items() if not k.startswith("_")} for row in enriched])
...
def _validate_candidate(candidate: dict) -> dict:
    ...
    for slice_name, start, end in TIME_SLICES:
        row = _run(signal_file, name, f"time_{slice_name}", start, end)
        row["slice"] = slice_name
        time_rows.append(row)
    _write_rows(case_dir / "time_slices.csv", time_rows)

    nearby_rows = []
    for item in _nearby_starts():
        tag = f"nearby_{item['anchor']}_{item['offset']}_{item['start_date']}".replace("-", "m")
        row = _run(signal_file, name, tag, item["start_time"], filt.SLICES[0][2])
        row.update(item)
        nearby_rows.append(row)
    nearby_summary = _summarize_nearby(nearby_rows)
    _write_rows(case_dir / "nearby_cold_starts.csv", nearby_rows)
    _write_rows(case_dir / "nearby_summary.csv", nearby_summary)

    variants, proxy_stats = _build_stress_variants(signal_rows, case_dir)
    stress_rows = []
    signal_files = [signal_file]
    for variant in variants:
        variant_path = case_dir / "signals" / f"{variant['name']}.csv"
        _write_rows(variant_path, variant["rows"])
        signal_files.append(variant_path)
        for start_name, start in MAIN_STARTS:
            row = _run(variant_path, name, f"stress_{variant['name']}_{start_name}", start, filt.SLICES[0][2])
            row["variant"] = variant["name"]
            row["start_name"] = start_name
            stress_rows.append(row)
    stress_summary = _stress_summary(stress_rows, proxy_stats)
    _write_rows(case_dir / "stress_cases.csv", stress_rows)
    _write_rows(case_dir / "stress_summary.csv", stress_summary)
    (case_dir / "proxy_summary.json").write_text(json.dumps(proxy_stats, ensure_ascii=False, indent=2), encoding="utf-8")

    audit = filt.liq._audit_signals(signal_files)
    (case_dir / "hard_gate_audit.json").write_text(json.dumps(audit, ensure_ascii=False, indent=2), encoding="utf-8")
    summary = {
        "candidate": name,
    ...
    (case_dir / "validation_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")

    full = next((row for row in time_rows if row["slice"] == "full"), None) or time_rows[0]
    recent60 = next((row for row in time_rows if row["slice"] == "slice_recent60"), None) or {}
    ...
    return {
        ...
        "recent60_annual": _f(recent60.get("annual")),
        "late_min_annual": late_min,
        "top_stock_share": _f(proxy_stats.get("top_stock_share")),
        "top_month_share": _f(proxy_stats.get("top_month_share")),
        "drop_top_1pct_full_annual": _f(next((r["full_annual"] for r in stress_summary if r["variant"] == "drop_top_1pct"), None)),
        "audit_failed_files": audit.get("failed_files"),
...
def main() -> int:
    ...
    for candidate in CANDIDATES:
        print(f"validating {candidate['name']}", flush=True)
        rows.append(_validate_candidate(candidate))
    _write_rows(REPORT_DIR / "summary.csv", rows)
    (REPORT_DIR / "summary.json").write_text(json.dumps(rows, ensure_ascii=False, indent=2), encoding="utf-8")
    return 0
