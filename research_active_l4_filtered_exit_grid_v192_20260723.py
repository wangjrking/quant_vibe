...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_filtered_exit_grid_v192_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v192_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 filtered exit-grid study")
    ...
    rows, definitions = [], {}
    for sell_below, advantage in product(protocol["grid"]["sell_score_below"], protocol["grid"]["replacement_advantage"]):
        definition = {**protocol["fixed_definition"], "sell_score_below": float(sell_below), "replacement_advantage": float(advantage)}
