...
OUT_DIR = DATA_DIR / "reports" / "model_agent_5d_topgate_3dformal_dategate_refine_20260623"
...
RESEARCH_TABLE = "stock_predict_data_model_agent_5d_topgate_3dformal_dategate_refine_20260623_executable_5d_open_return_research"
...
def materialize(merged: pd.DataFrame, selected_summary: dict, window_summaries: dict) -> dict:
    ...
    manifest = {
        ...
        "score_formula": (
            "if trade_date >= 20260201 and base_5d_rank >= 0.996: "
        ...
        "notes": "5D 使用 3D formal 做更窄阈值日期门控顶部局部重排的研究资产，仅供研究验证，不进入 formal 或 L5。",
    }
...
def main() -> int:
    ...
    selected, selected_daily = evaluate("topgate_3d_formal_dategate_refine", merged, "topgate_score")
    ...
    payload = {
        ...
        "manifest": manifest,
        "selection_rule": (
            "固定使用 20260201 起生效的 5D 顶部 0.4% + 3D formal rank 局部重排，"
            "作为 research-only 候选保留，不切换 formal。"
        ),
    }
    (OUT_DIR / "topgate_3dformal_dategate_refine_summary.json").write_text(
        json.dumps(payload, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )

    lines = [
        "# 5D 顶部 3D formal 日期门控重排微调研究",
        "",
        "## 当前结论",
        "",
        "本轮将日期门控方案的阈值从 0.995 缩窄到 0.996，观察是否进一步改善近期窗口且保持全窗口稳定。",
        "",
        "## 参数",
        "",
        f"- 基线表：`{BASE_TABLE}`",
        ...
        f"- recent1y Top1：`{baseline_windows['recent1y']['top1']:.8f} -> {selected_windows['recent1y']['top1']:.8f}`",
        ...
        "## 证据路径",
        "",
        f"- 汇总 JSON：`{(OUT_DIR / 'topgate_3dformal_dategate_refine_summary.json').as_posix()}`",
        f"- research manifest：`{(OUT_DIR / 'research_prediction_manifest.json').as_posix()}`",
        f"- research DB 表：`{MODEL_DB.as_posix()}::{RESEARCH_TABLE}`",
    ]
    (OUT_DIR / "topgate_3dformal_dategate_refine_report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
