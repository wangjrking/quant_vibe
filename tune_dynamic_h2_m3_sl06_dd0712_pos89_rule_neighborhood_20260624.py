...
REPORT_DIR = BASE_DIR / "dynamic_h2_m3_sl06_dd0712_pos89_rule_neighborhood_20260624"
BASE_SIGNAL_FILE = (
    BASE_DIR
    / "dynamic_h2_m3_dd0712_tp060_position_20260624"
    / "signals"
    / "dd0712_tp060_pos89.csv"
)
...
BASE_CASE = {
    "case_name": "base_pos89_e970_c980_mh3_sl06_tp06_dd0712",
    "target_pct": 0.89,
    "exit_ratio": 0.970,
    "continue_ratio": 0.980,
    "holding_days": 2,
    "max_holding_days": 3,
    "stop_loss": 0.06,
    "take_profit": 0.06,
...
CASES = [
    BASE_CASE,
    {**BASE_CASE, "case_name": "exit_e965", "exit_ratio": 0.965},
    {**BASE_CASE, "case_name": "exit_e975", "exit_ratio": 0.975},
    {**BASE_CASE, "case_name": "continue_c975", "continue_ratio": 0.975},
    {**BASE_CASE, "case_name": "continue_c985", "continue_ratio": 0.985},
    {**BASE_CASE, "case_name": "hold_mh2", "max_holding_days": 2},
    {**BASE_CASE, "case_name": "hold_mh4", "max_holding_days": 4},
    {**BASE_CASE, "case_name": "stop_sl05", "stop_loss": 0.05},
    {**BASE_CASE, "case_name": "stop_sl07", "stop_loss": 0.07},
    {**BASE_CASE, "case_name": "take_tp05", "take_profit": 0.05},
    {**BASE_CASE, "case_name": "take_tp07", "take_profit": 0.07},
    {**BASE_CASE, "case_name": "dd0611", "dd_soft": 0.06, "dd_hard": 0.11},
    {**BASE_CASE, "case_name": "dd0813", "dd_soft": 0.08, "dd_hard": 0.13},
]
...
def _summarize(detail: list[dict], signal_stats: dict[str, dict]) -> list[dict]:
    out: list[dict] = []
    for case_name in [case["case_name"] for case in CASES]:
        ...
        case_params = next(case for case in CASES if case["case_name"] == case_name)
        out.append(
            {
                **case_params,
...
def _write_report(summary: list[dict], audit: dict) -> None:
    ranked = sorted(
        ...
        key=lambda row: (
            ...
            _f(row["late_min_annual"]) >= 1.0,
            _f(row["recent60_annual"]) >= 0.0,
            _f(row["full_annual"]),
            _f(row["full_sharpe"]),
        ),
        reverse=True,
    ...
    for row in ranked:
        params = (
            f"e={_f(row['exit_ratio']):.3f}, c={_f(row['continue_ratio']):.3f}, "
            f"mh={int(float(row['max_holding_days']))}, sl={_f(row['stop_loss']):.2%}, "
            f"tp={_f(row['take_profit']):.2%}, dd={_f(row['dd_soft']):.0%}/{_f(row['dd_hard']):.0%}"
        )
        lines.append(
            f"| `{row['case_name']}` | {_f(row['full_annual']):.2%} | {_f(row['full_sharpe']):.2f} | "
            f"{_f(row['full_max_drawdown']):.2%} | {_f(row['recent60_annual']):.2%} | "
            f"{_f(row['late_min_annual']):.2%} | {params} |"
        )
    lines.extend(
        [
            "",
    ...
    (REPORT_DIR / "rule_neighborhood_report.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
