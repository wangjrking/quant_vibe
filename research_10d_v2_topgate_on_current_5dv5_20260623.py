...
OUT_DIR = DATA_DIR / "reports" / "model_agent_10d_v2_topgate_on_current_5dv5_20260623"

BASE_TABLE = "stock_predict_data_model_agent_10d_daygate_joint_refine_20260623_executable_10d_open_return_research"
AUX5_TABLE = "stock_predict_data_model_agent_5d_v5_topgate_on_v4_3drefined_20260623_executable_5d_open_return_research"
RESEARCH_TABLE = "stock_predict_data_model_agent_10d_v2_topgate_on_current_5dv5_20260623_executable_10d_open_return_research"
...
DATE_GATES = ["20260225", "20260301", "20260401"]
BASE_RANK_THRESHOLDS = [0.995, 0.996, 0.997, 0.998, 0.999]
AUX_ALPHAS = [0.002, 0.003, 0.005, 0.008]
...
def main() -> int:
    ...
    with sqlite3.connect(MODEL_DB) as conn:
        ...
        aux5_rows = pd.read_sql_query(
            f"select trade_date, stock_code, pred_prob as aux5_pred_prob "
            f"from '{AUX5_TABLE}' where trade_date >= ? and trade_date <= ?",
            conn,
            params=[DATE_FROM, DATE_TO],
        )

    for df in (base_rows, aux5_rows):
        df["trade_date"] = df["trade_date"].astype(str)

    merged = base_rows.merge(aux5_rows, on=["trade_date", "stock_code"], how="left", validate="one_to_one")
    merged["rank_base"] = merged.groupby("trade_date")["base_pred_prob"].rank(method="average", pct=True)
    merged["rank5"] = merged.groupby("trade_date")["aux5_pred_prob"].rank(method="average", pct=True)
    merged["rank5"] = merged["rank5"].fillna(merged["rank_base"])
    ...
    for date_gate in DATE_GATES:
        for threshold in BASE_RANK_THRESHOLDS:
            for alpha in AUX_ALPHAS:
                ...
                score.loc[topgate_mask] = score.loc[topgate_mask] + alpha * merged.loc[topgate_mask, "rank5"]
                eval_summary, daily_eval = evaluate(merged.assign(score=score), "score")
                focus = focus_score(eval_summary)
                pass_guard = (
                    eval_summary["recent63"]["top3"] >= base_recent63_top3 - 0.0001
                    and eval_summary["recent63"]["top5"] >= base_recent63_top5 - 0.0001
                )
                row = {
                    "date_gate": date_gate,
                    "base_rank_threshold": threshold,
                    "aux5_add_alpha": alpha,
                ...
                if pass_guard and not best_passed:
                    ...
                    if focus > best_focus + 1e-15:
                        ...
                        if row["recent63_top1"] > best_rule["recent63_top1"] + 1e-15:
                            better = True
                        elif abs(row["recent63_top1"] - best_rule["recent63_top1"]) <= 1e-15 and row["recent63_top3"] > best_rule["recent63_top3"] + 1e-15:
                            better = True
    ...
    merged.loc[merged["topgate_applied"], "topgate_reason"] = (
        ...
        + ";alpha5="
        + str(best_rule["aux5_add_alpha"])
    )

    result_df = pd.DataFrame(grid_rows).sort_values(
        ["guard_pass", "focus", "recent63_top1", "recent63_top3", "recent126_top3"],
    ...
    payload = {
        ...
        "candidate_id": "model_agent_10d_v2_topgate_on_current_5dv5_20260623",
        ...
        "governance_notes": [
            "research only",
            "no training",
            "10d current best receives a very light 5d v5 rerank only inside the top bucket after a late date gate",
            "selection prefers candidates that preserve recent63 top3/top5",
    ...
    manifest = {
        ...
        "approval_status": "research_only_not_approved_for_l4_or_l5",
        "strategy_scope": "research_only",
        "label_name": LABEL_COL,
        "db_path": "../../../data_file/model_predictions/MODEL_PREDICTIONS.db",
        "table": RESEARCH_TABLE,
        "generated_at": payload["generated_at"],
        "notes": "10D v2 研究候选：在当前 10D 最优底座上，仅对尾段日期的极头部样本叠加 5D v5 局部重排，用于验证是否还能继续提升评价指标；不进入 formal L4/L5。",
    }
    (OUT_DIR / "research_candidate_manifest.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")

    report_lines = [
        "# 10D v2 头部门控研究候选",
        "",
        "- 候选 ID：`model_agent_10d_v2_topgate_on_current_5dv5_20260623`",
        f"- 基座表：`{BASE_TABLE}`",
        f"- 辅助表：`{AUX5_TABLE}`",
        f"- 最优规则：`date_gate={best_rule['date_gate']}`、`base_rank_threshold={best_rule['base_rank_threshold']}`、`aux5_add_alpha={best_rule['aux5_add_alpha']}`",
        f"- Focus：`{best_focus:.12f}`，相对基座变化：`{best_focus - base_focus:+.12f}`",
        f"- recent63 守卫：Top3 最低 `>= {base_recent63_top3 - 0.0001:.12f}`，Top5 最低 `>= {base_recent63_top5 - 0.0001:.12f}`，本次结果通过：`{best_passed}`",
        "",
        "## 窗口表现",
        "",
        f"- full Top1/Top3/Top5：`{best_summary['full']['top1']:.12f}` / `{best_summary['full']['top3']:.12f}` / `{best_summary['full']['top5']:.12f}`",
        f"- recent126 Top1/Top3/Top5：`{best_summary['recent126']['top1']:.12f}` / `{best_summary['recent126']['top3']:.12f}` / `{best_summary['recent126']['top5']:.12f}`",
        f"- recent63 Top1/Top3/Top5：`{best_summary['recent63']['top1']:.12f}` / `{best_summary['recent63']['top3']:.12f}` / `{best_summary['recent63']['top5']:.12f}`",
        "",
        "## 边界",
        "",
        "- 仅研究用途。",
