...
OUT = ROOT / "quant" / "data_file" / "reports" / "strategy_agent_current_formal_l4_cap5_selection_neighborhood_20260720"
...
CASES = [
    {"rank10": rank10, "agreement": agreement}
    for rank10 in (0.96, 0.97, 0.98, 0.985, 0.99)
    for agreement in (0.40, 0.50)
]


def run_case(con: duckdb.DuckDBPyConnection, case: dict, score_db: Path) -> dict:
    name = f"r10_{str(case['rank10']).replace('.', 'p')}_agree_{int(case['agreement'] * 100)}"
    filter_name = f"cap5_agree_{int(case['agreement'] * 100)}"
    MODULE.FILTERS[filter_name] = (
        f"least(rank_5d, rank_10d) >= {case['agreement']} AND signal_pct_chg_raw <= 5.0"
    )
    params = {
        "blend": "w10_100", "score": "plain", "filter": filter_name,
        "threshold": case["rank10"], "topn": 1, "hold": 12,
    }
