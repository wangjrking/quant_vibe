...
SIGNAL_DIR = REPORT_DIR / "signals" / "deepdrop_boundary_micro"
LOG_DIR = REPORT_DIR / "logs" / "deepdrop_boundary_micro"
OUT_CSV = REPORT_DIR / "deepdrop_boundary_micro_juejin_results_20260714.csv"
OUT_JSON = REPORT_DIR / "deepdrop_boundary_micro_juejin_results_20260714.json"
...
CASES = []
for deep_scale in [0.86, 0.87, 0.88, 0.89]:
    for global_scale in [1.065, 1.070, 1.075]:
        CASES.append(
            {
                "case": f"ddb_p45_x{int(deep_scale * 100):02d}_g{int(global_scale * 1000):04d}",
                "deep_pct": -4.5,
...
def write_variant(source: pd.DataFrame, case: dict) -> dict:
    ...
    scale = pd.Series(float(case["global_scale"]), index=df.index).mask(
        deep_mask, float(case["global_scale"]) * float(case["deep_scale"])
    )
