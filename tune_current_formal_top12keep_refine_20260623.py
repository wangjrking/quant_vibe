...
base.REPORT_DIR = (
    ...
    / "reports"
    / "strategy_agent_goal_high_annual_20260623"
    / "current_formal_top12keep_refine"
)
base.FUSION_DB = base.REPORT_DIR / "fusion_top12keep_refine.db"
base.SCORE_DB = base.REPORT_DIR / "scores_top12keep_refine.db"
...
def _cfg(
    ...
    *,
    ...
    top2_exit_ratio: float | None = None,
    top2_holding_days: int | None = None,
    top2_min_hold: int | None = None,
    ...
    if top2_exit_ratio is not None or top2_holding_days is not None or top2_min_hold is not None:
        rule = {}
        if top2_exit_ratio is not None:
            rule["score_exit_entry_ratio"] = top2_exit_ratio
        if top2_holding_days is not None:
            rule["holding_days"] = top2_holding_days
        if top2_min_hold is not None:
            rule["min_holding_days_before_score_exit"] = top2_min_hold
        rank_rules[2] = rule
    return {
        "name": name,
        "w10": 0.80,
...
base.VARIANTS = [
    _cfg("base_top1keep"),
    _cfg("top2_r093_h6", top2_exit_ratio=0.93, top2_holding_days=6),
    _cfg("top2_r094_h6", top2_exit_ratio=0.94, top2_holding_days=6),
    _cfg("top2_r092_h6", top2_exit_ratio=0.92, top2_holding_days=6),
    _cfg("top2_r093_h7", top2_exit_ratio=0.93, top2_holding_days=7),
    _cfg("top2_r094_h7", top2_exit_ratio=0.94, top2_holding_days=7),
    _cfg("top2_r093_h6_mh3", top2_exit_ratio=0.93, top2_holding_days=6, top2_min_hold=3),
    _cfg("top2_r094_h6_mh3", top2_exit_ratio=0.94, top2_holding_days=6, top2_min_hold=3),
    _cfg("top2_r093_h5_mh1", top2_exit_ratio=0.93, top2_holding_days=5, top2_min_hold=1),
    _cfg("top2_r092_h5_mh1", top2_exit_ratio=0.92, top2_holding_days=5, top2_min_hold=1),
    _cfg("top2_r091_h6", top2_exit_ratio=0.91, top2_holding_days=6),
    _cfg("top2_r091_h7", top2_exit_ratio=0.91, top2_holding_days=7),
]
...
def _build_signals(cfg: dict) -> list[dict]:
    ...
    try:
        ...
        conn.close()
    rank_rules = cfg.get("rank_rules") or {}
    signals = []
    for row in rows:
        item = dict(row)
        ...
        for key, value in (rank_rules.get(rank) or {}).items():
            signal[key] = value
