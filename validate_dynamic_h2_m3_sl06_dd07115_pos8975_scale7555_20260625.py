...
CASE_NAME = "dynamic_h2_m3_c0975_w78_5d12_3d10_pos8975_e097_mh1_sl06_dd07115_tp070_scale7555"
REPORT_DIR = (
    core.SOURCE_DIR
    / "formal_horizon_entry_confirmation_20260624"
    / "dynamic_h2_m3_sl06_dd07115_pos8975_scale7555_validation_20260625"
)
SIGNAL_FILE = (
    core.SOURCE_DIR
    / "formal_horizon_entry_confirmation_20260624"
...
def _rewrite_report() -> None:
    ...
    lines = [
        ...
        "",
        f"- 验证对象：`{CASE_NAME}`。",
        "- 该版本只在当前生产策略基础上把账户回撤 hard gate 从 `0.12` 收紧到 `0.115`，其余执行口径保持不变。",
        "- 输入仍为当前正式信号文件与当前生产分数表；不引入行业、月份、日期排除，不使用 1D 模型，不使用最新状态回填。",
    ...
    lines.extend(
        [
            ...
            "",
            "| 压力变体 | 说明 | 年化 | Sharpe | 最大回撤 | 近端启动最差年化 |",
            "| --- | --- | ---: | ---: | ---: | ---: |",
        ]
    )
