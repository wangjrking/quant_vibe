...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_3d_risk_balanced_v7_research_20260625"
...
CURRENT_TABLE = "stock_predict_data_model_agent_3d_rankic_balanced_v5_20260625_executable_3d_open_return_research"
ALT_TABLE = "stock_predict_data_model_agent_10d_risk_balanced_v5_20260625_executable_10d_open_return_research"
TARGET_TABLE = "stock_predict_data_model_agent_3d_risk_balanced_v7_20260625_executable_3d_open_return_research"

FEATURE = "best10_top50_mean"
THRESHOLD = 0.995253
...
def load_scores() -> pd.DataFrame:
    with sqlite3.connect(MODEL_DB, timeout=300) as conn:
        ...
        alt = read_scores(conn, ALT_TABLE, "best10_score")
    ...
    for prefix in ["formal", "current", "best10"]:
        ...
        rows.append(
            {
                ...
                FEATURE: float(group.nlargest(min(50, len(group)), "best10_rank")["best10_score"].mean()),
            }
        )
    gates = pd.DataFrame(rows)
    gates["gate_active"] = gates[FEATURE] >= THRESHOLD
    out = scores.merge(gates, on="trade_date", how="left", validate="many_to_one")
    out["pred_prob"] = np.where(out["gate_active"], out["best10_rank"], out["current_rank"])
    out["score_formula"] = f"if {FEATURE} >= {THRESHOLD:.6f}: best10_rank else current_3d_rank"
    return out
...
def evaluate(scored: pd.DataFrame, labels: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, dict]:
    ...
    current = daily_metrics(eval_frame, "current_rank", "current_3d_v5")
    best10 = daily_metrics(eval_frame, "best10_rank", "best10")
    ...
    return pd.concat([formal, current, best10, candidate], ignore_index=True), delta_formal, summary
...
def write_outputs(stats: dict, summary: dict, metrics: pd.DataFrame, delta_formal: pd.DataFrame) -> None:
    ...
    manifest = {
        ...
        "decision": "candidate",
        "research_hypothesis": "use_10d_risk_balanced_rank_on_broadly_stable_10d_days_to_repair_3d_rankic",
        "asset_stats": stats,
    ...
    lines = [
        "# 3D 风险平衡 v7 研究报告（20260625）",
        "",
        "## 当前结论",
        "",
        "本资产是 research-only 的 3D 候选，不是 formal L4 或 L5 生产资产。它在 10D 风险平衡模型处于稳定高分布状态的交易日，使用 10D rank 替代当前 3D v5 rank，目标是修复 3D 当前候选的 RankIC 罚分，同时尽量保留 TopN 改善。",
        "",
        "## 评分公式",
        "",
        "```text",
        ...
        "## 关键评价摘要",
        "",
        f"- 相对 formal 3D：full RankIC delta `{summary['full_vs_formal_rank_ic_delta']}`，recent63 Top1 delta `{summary['recent63_vs_formal_top1_delta']}`，recent63 Top5 delta `{summary['recent63_vs_formal_top5_delta']}`，recent20 Top5 delta `{summary['recent20_vs_formal_top5_delta']}`，full Top5 delta `{summary['full_vs_formal_top5_delta']}`。",
        f"- 相对当前 3D v5：full RankIC delta `{summary['full_vs_current_rank_ic_delta']}`，min-period RankIC delta `{summary['current_min_period_rank_ic_delta']}`，recent63 Top5 delta `{summary['recent63_vs_current_top5_delta']}`。",
