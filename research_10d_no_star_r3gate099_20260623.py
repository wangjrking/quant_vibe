...
OUT_DIR = DATA_DIR / "reports" / "model_agent_10d_no_star_r3gate099_20260623"
...
TARGET_TABLE = "stock_predict_data_model_agent_10d_no_star_r3gate099_20260623_executable_10d_open_return_research"
...
def main() -> int:
    ...
    for trade_date, group in frame.groupby("trade_date", sort=True):
        ...
        keep_source = star_count == 0 and mean_rank3d >= 0.99
        if keep_source:
    ...
    payload = {
        ...
        "candidate_id": "model_agent_10d_no_star_r3gate099_20260623",
        "asset_role": "research_l4_candidate_not_formal",
        "approval_status": "research_only_not_approved_for_l4_or_l5",
        "base_table": BASE_TABLE,
        "source_table": SOURCE_TABLE,
        "rule": "if source candidate top5 contains no STAR(688) stock and source_top5_mean_rank3d >= 0.99, keep source score for that trade_date; otherwise fallback to base 10D score",
        ...
        json.dumps(
            {
                ...
                "notes": "10D 候选在 no-star gate 基础上再加一层 3D 一致性门控：仅当 source top5 无 STAR 且 source_top5_mean_rank3d >= 0.99 时保留 source，否则回退到 base。该资产仅用于 research 验证，不进入 formal L4/L5。",
            },
            ensure_ascii=False,
            indent=2,
        ),
    ...
    report_lines = [
        "# 10D no-star + r3 门控研究候选",
        "",
        "## 当前结论",
        "",
        f"- 新候选 `10d_no_star_r3gate099` 的 focus 分数为 `{payload['selected_focus_score']:.12f}`。",
        f"- 相比上一版 source 候选，focus 由 `{payload['source_focus_score']:.12f}` 提升到 `{payload['selected_focus_score']:.12f}`。",
        ...
        "- 仅当两个条件同时满足时保留 source；否则整日回退到 base。",
        "",
        "## 关键窗口",
        "",
