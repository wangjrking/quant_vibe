...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_3d_recent_focus_recent_scan_20260626"
...
TARGET_TABLE = "stock_predict_data_model_agent_3d_recent_focus_recent_scan_20260626_executable_3d_open_return_research"

SOURCES = {
    "base": BASE_TABLE,
    "balanced_v5": "stock_predict_data_model_agent_3d_rankic_balanced_v5_20260625_executable_3d_open_return_research",
    "cross_v3": "stock_predict_data_model_agent_3d_cross_horizon_guard_v3_20260625_executable_3d_open_return_research",
    "recent_v4": "stock_predict_data_model_agent_3d_recent_enhance_v4_20260625_executable_3d_open_return_research",
    "risk_v7": "stock_predict_data_model_agent_3d_risk_balanced_v7_20260625_executable_3d_open_return_research",
...
def objective(row: dict[str, float | int]) -> float:
    return (
        ...
        + 0.7 * row["full_top5_delta"]
        + 1.0 * row["recent63_top1_delta"]
        + 0.8 * row["recent20_top1_delta"]
        + 0.4 * row["full_top1_delta"]
        + 0.7 * row["recent63_rank_ic_delta"]
        + 0.5 * row["recent20_rank_ic_delta"]
        + 0.25 * row["full_rank_ic_delta"]
        + 0.006 * row["positive_top5_periods"]
        - 0.005 * row["active_ratio"]
    )
...
def scan(scores: pd.DataFrame, labels: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, pd.DataFrame], pd.DataFrame]:
    ...
    for alt in ["balanced_v5", "cross_v3", "recent_v4", "risk_v7", "formal3"]:
        ...
        for feature in FEATURES:
            ...
            for threshold in thresholds:
                for op in ["<=", ">="]:
                    ...
                    row["pass_basic"] = bool(
                        row["full_rank_ic_delta"] >= -0.00030
                        ...
                        and row["recent63_top1_delta"] >= -0.0005
    ...
    if not result.empty:
        result = result.sort_values(
            ["pass_basic", "objective", "recent63_top5_delta", "recent20_rank_ic_delta"],
...
def main() -> int:
    ...
    summary = {
        ...
        "scope": "research_only_3d_recent_focus_recent_scan",
