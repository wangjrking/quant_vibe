...
base.REPORT_DIR = (
    ...
    / "reports"
    / "strategy_agent_goal_high_annual_20260623"
    / "current_formal_target_profile"
)
base.FUSION_DB = base.REPORT_DIR / "fusion_target_profile.db"
base.SCORE_DB = base.REPORT_DIR / "scores_target_profile.db"
...
base.VARIANTS = [
    _cfg("top4_ctrl_current", top_k=4, targets={1: 0.27, 2: 0.25, 3: 0.22, 4: 0.18}, scale=1.29, cap=0.415),
    _cfg("top4_flat125", top_k=4, targets={1: 0.25, 2: 0.25, 3: 0.25, 4: 0.25}, scale=1.25, cap=0.36),
    _cfg("top4_frontheavy_a", top_k=4, targets={1: 0.34, 2: 0.29, 3: 0.22, 4: 0.15}, scale=1.16, cap=0.42),
    _cfg("top4_frontheavy_b", top_k=4, targets={1: 0.32, 2: 0.29, 3: 0.23, 4: 0.16}, scale=1.20, cap=0.41),
    _cfg("top4_smooth_a", top_k=4, targets={1: 0.28, 2: 0.26, 3: 0.24, 4: 0.22}, scale=1.18, cap=0.36),
    _cfg("top5_p4like_a", top_k=5, targets={1: 0.30, 2: 0.27, 3: 0.22, 4: 0.16, 5: 0.08}, scale=1.20, cap=0.40),
    _cfg("top5_p4like_b", top_k=5, targets={1: 0.32, 2: 0.28, 3: 0.22, 4: 0.14, 5: 0.06}, scale=1.20, cap=0.42),
    _cfg("top5_balanced_a", top_k=5, targets={1: 0.26, 2: 0.23, 3: 0.20, 4: 0.17, 5: 0.14}, scale=1.22, cap=0.36),
    _cfg("top5_balanced_b", top_k=5, targets={1: 0.28, 2: 0.25, 3: 0.21, 4: 0.17, 5: 0.13}, scale=1.18, cap=0.36),
    _cfg("top5_ultra_front", top_k=5, targets={1: 0.35, 2: 0.27, 3: 0.18, 4: 0.12, 5: 0.08}, scale=1.18, cap=0.42),
]
...
def _build_signals(cfg: dict) -> list[dict]:
    ...
    try:
        rows = conn.execute(
            ...
            """,
            (base.START_DATE, base.END_DATE, int(cfg["top_k"])),
        ).fetchall()
    finally:
        conn.close()
    signals = []
