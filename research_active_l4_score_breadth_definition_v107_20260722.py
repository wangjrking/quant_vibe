...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_score_breadth_definition_v107_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v107_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def case_protocol(protocol: dict, definition: dict) -> dict:
    result = copy.deepcopy(protocol)
    result["breadth"] = {"score_threshold": float(definition["score_threshold"])}
    return result


def main() -> None:
    parser = argparse.ArgumentParser(description="10D 分数宽度定义邻域研究")
    ...
    if args.open_known_2026:
        ...
        for item in frozen["candidates"]:
            ...
            for start in protocol["known_stress_buy_starts"]:
                daily = v103.simulate(full, score, order, item["definition"], current_protocol, protocol["known_stress_end"], start)
    ...
    grid = protocol["grid"]
    rows, definitions = [], {}
    for score_threshold, count_threshold, low, high in product(
        grid["score_threshold"], grid["breadth_count_threshold"], grid["low_gross"], grid["high_gross"]
    ):
        definition = {
            "score_threshold": score_threshold,
            "breadth_count_threshold": count_threshold,
            "high_when": "above",
            "low_gross": low,
            "high_gross": high,
        }
        ...
        daily = v103.simulate(observation, score_obs, order_obs, definition, case_protocol(protocol, definition), protocol["observation_end"])
