...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_10d_smoothing_v204_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v204_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 纯10D平滑窗口与当日权重研究")
    ...
    if args.open_known_2026:
        ...
        for item in frozen["candidates"]:
            ...
            score, order = v95.score_pair(
                full, 0.0, int(definition["smooth_window"]), float(definition["current_weight"])
            )
    ...
    rows, definitions = [], {}
    for smooth_window, current_weight in product(
        protocol["grid"]["smooth_window"], protocol["grid"]["current_weight"]
    ):
        definition = {
