...
import research_v260_fixed10_replacement_margin_20260822 as round4
...
OUTPUT_ROOT = (
    REPO
    / "quant/data_file/reports/strategy_agent_v260_fixed10_sell_threshold_20260822"
)
SELL_THRESHOLDS = (0.80, 0.85, 0.90)


def candidate_policy(sell_threshold: float) -> dict:
    current = round4.candidate_policy(0.05)
...
def main() -> None:
    ...
    if access["logical_max_date"] != round1.DEVELOPMENT_END:
        raise PermissionError("2026 data entered sell-threshold development")
    ...
    if not all(deterministic.values()):
        raise RuntimeError("sell-threshold deterministic replay failed")
    ...
    result = {
        ...
        "selection_rule": (
            ...
            "drawdown and turnover; 2025 is forward confirmation only"
        ),
        "results": results,
