...
CONFIG = ROOT / "quant/main/config/model_research_10d_high_liquidity_probe_20260721.json"
...
OUT = ROOT / "quant/data_file/reports/model_agent_10d_high_liquidity_probe_20260721"
...
def evaluate(frame: pd.DataFrame, score_col: str, start: int, end: int) -> dict:
    ...
    part["position"] = part.groupby("trade_date")[score_col].rank(ascending=False, method="first")
...
def main() -> None:
    ...
    if cfg["status"] != "frozen_research_only" or manifest["source_type"] != "duckdb_table":
        raise RuntimeError("research or formal contract failed")
    ...
    frame = con.execute(
        ...
        WITH source AS (
            SELECT
                ...
                p.pred_prob,
                f.amount,
                l.executable_10d_open_return AS label_value
        ...
        SELECT
            trade_date::INTEGER AS trade_date,
            percent_rank() OVER (PARTITION BY trade_date ORDER BY pred_prob)::FLOAT AS rank_score,
            percent_rank() OVER (PARTITION BY trade_date ORDER BY amount)::FLOAT AS rank_amount,
            label_value::FLOAT AS label_value,
            percent_rank() OVER (PARTITION BY trade_date ORDER BY label_value)::FLOAT AS label_rank
        FROM source
        ORDER BY trade_date, stock_code
        """,
        [cfg["train_start"], cfg["evaluation_end"], cfg["amount_min"]],
    ).fetchdf()
    con.close()
    ...
    weights = (
        1.0 + float(cfg["daily_top_1pct_bonus"]) * (train.label_rank >= 0.99).astype("float32")
    ).to_numpy(dtype="float32", copy=False)
    model = xgb.XGBRegressor(**cfg["model_params"])
    model.fit(
        train[features].to_numpy(dtype="float32", copy=False),
        train.label_value.to_numpy(dtype="float32", copy=False),
        sample_weight=weights,
        verbose=False,
    )
    model_path = OUT / "research_10d_high_liquidity_probe_model.json"
    model.save_model(model_path)
    frame["candidate_score"] = model.predict(
        frame[features].to_numpy(dtype="float32", copy=False)
    ).astype("float32")

    evaluations = {}
    for period, bounds in cfg["frozen_evaluation_periods"].items():
        start, end = map(int, bounds)
        baseline = evaluate(frame, "rank_score", start, end)
        candidate = evaluate(frame, "candidate_score", start, end)
        evaluations[period] = {
            "baseline": baseline,
            "candidate": candidate,
            "delta": {key: candidate[key] - baseline[key] for key in baseline if key != "trade_days"},
        }

    top3_deltas = [item["delta"]["top3"] for item in evaluations.values()]
    top1_deltas = [item["delta"]["top1"] for item in evaluations.values()]
    rank_deltas = [item["delta"]["rank_ic"] for item in evaluations.values()]
    checks = {
        "min_top3_delta": float(min(top3_deltas)),
        "min_top1_delta": float(min(top1_deltas)),
        "mean_rank_ic_delta": float(np.mean(rank_deltas)),
        "period_deltas": {period: item["delta"] for period, item in evaluations.items()},
    }
    gate = cfg["probe_gate"]
    passed = (
        checks["min_top3_delta"] >= gate["all_period_top3_delta_min"]
        and checks["min_top1_delta"] >= gate["all_period_top1_delta_floor"]
        and checks["mean_rank_ic_delta"] >= gate["mean_rank_ic_delta_min"]
    )
    report = {
        ...
        "probe_passed": passed,
        "subdomain": {"amount_min": cfg["amount_min"], "no_bj": True},
        "selection_contract": "single frozen high-liquidity model; 2024-2026 never used for training or tuning",
