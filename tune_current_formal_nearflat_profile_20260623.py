...
base.REPORT_DIR = (
    ...
    / "reports"
    / "strategy_agent_goal_high_annual_20260623"
    / "current_formal_nearflat_profile"
)
base.FUSION_DB = base.REPORT_DIR / "fusion_nearflat_profile.db"
base.SCORE_DB = base.REPORT_DIR / "scores_nearflat_profile.db"


def _cfg(name: str, targets: Dict[int, float]) -> dict:
    return {
        "name": name,
        "w10": 0.80,
        "w5": 0.20,
...
base.VARIANTS = [
    _cfg("nearflat_equal", {1: 0.25, 2: 0.25, 3: 0.25, 4: 0.25}),
    _cfg("nearflat_26252424", {1: 0.26, 2: 0.25, 3: 0.25, 4: 0.24}),
    _cfg("nearflat_27252424", {1: 0.27, 2: 0.25, 3: 0.24, 4: 0.24}),
    _cfg("nearflat_28242424", {1: 0.28, 2: 0.24, 3: 0.24, 4: 0.24}),
    _cfg("nearflat_265255245235", {1: 0.265, 2: 0.255, 3: 0.245, 4: 0.235}),
    _cfg("nearflat_27272323", {1: 0.27, 2: 0.27, 3: 0.23, 4: 0.23}),
    _cfg("nearflat_28262224", {1: 0.28, 2: 0.26, 3: 0.22, 4: 0.24}),
    _cfg("nearflat_30242323", {1: 0.30, 2: 0.24, 3: 0.23, 4: 0.23}),
]
...
def _build_signals(cfg: dict) -> list[dict]:
    ...
    for row in rows:
        ...
        if base._is_st_like(buy_market) or base._is_limit_buy(buy_market):
            continue
        rank = int(item["rn"])
        target = float(targets.get(rank, 0.0)) * float(cfg["scale"])
        signals.append(
            {
