...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_max_positions_refine_v196_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v196_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 最大并发持仓数邻域研究")
    ...
    rows, definitions = [], {}
    for max_positions in protocol["grid"]["max_positions"]:
        definition = {**protocol["fixed_definition"], "max_positions": int(max_positions)}
