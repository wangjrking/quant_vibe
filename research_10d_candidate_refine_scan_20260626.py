...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_10d_candidate_refine_scan_20260626"
...
TARGET_TABLE = "stock_predict_data_model_agent_10d_v5shrink_fixed4y_top20zero_blend15_20260626_executable_10d_open_return_research"
...
def write_blend(scores: pd.DataFrame, w: float) -> dict[str, object]:
    out = scores[["trade_date", "stock_code", "base_rank", "cand_rank"]].copy()
    out["pred_prob"] = (1.0 - w) * out["base_rank"] + w * out["cand_rank"]
    out["score_formula"] = f"{1.0 - w:.2f}*base_rank + {w:.2f}*cand_rank"
    with sqlite3.connect(MODEL_DB, timeout=300) as conn:
        ...
        conn.execute(f"create index if not exists idx_10d_candidate_refine_blend_date_code on {quote(TARGET_TABLE)}(trade_date, stock_code)")
        conn.commit()
        row = conn.execute(
...
def main() -> int:
    ...
    cand_daily = daily_metrics(eval_frame, "cand_rank")
    base_full = summary(base_daily, None)
    base_63 = summary(base_daily, 63)
    base_20 = summary(base_daily, 20)
    cand_full = summary(cand_daily, None)
    cand_63 = summary(cand_daily, 63)
    cand_20 = summary(cand_daily, 20)
    ...
    for w in [0.10, 0.15, 0.20, 0.25, 0.30, 0.35]:
        rank_col = f"blend_{int(w*100):02d}"
        eval_frame[rank_col] = (1.0 - w) * eval_frame["base_rank"] + w * eval_frame["cand_rank"]
        ...
        rows.append(
            {
                ...
                "objective": objective(base_full, base_63, base_20, cur_full, cur_63, cur_20),
            }
        )

    result = pd.DataFrame(rows)
    result["objective"] = result["objective"].fillna(
        objective(base_full, base_63, base_20, cand_full, cand_63, cand_20)
    )
    result["pass_basic"] = (
        (result["full_rank_ic_delta"] >= 0.0)
        & (result["recent63_top5_delta"] >= 0.0)
        & (result["recent20_top5_delta"] >= -0.0050)
    )
    result = result.sort_values(
        ["pass_basic", "objective", "full_rank_ic_delta", "recent63_top5_delta", "recent20_top5_delta"],
        ascending=[False, False, False, False, False],
    ).reset_index(drop=True)
    result.to_csv(REPORT_DIR / "candidate_refine_results.csv", index=False, encoding="utf-8-sig")

    best = result.iloc[0].to_dict() if not result.empty else None
    asset_stats = None
    if best and best["mode"] == "blend" and best["pass_basic"] and best["objective"] > 0:
        asset_stats = write_blend(scores, float(best["weight_candidate"]))

    summary_obj = {
        "generated_at": now_iso(),
        "scope": "research_only_10d_candidate_refine_scan",
        "base_table": BASE_TABLE,
        "candidate_table": CANDIDATE_TABLE,
        "rows": int(len(result)),
        "best": best,
        "base_metrics": {"full": base_full, "recent63": base_63, "recent20": base_20},
        "candidate_metrics": {"full": cand_full, "recent63": cand_63, "recent20": cand_20},
    ...
    (REPORT_DIR / "candidate_refine_summary.json").write_text(json.dumps(summary_obj, ensure_ascii=False, indent=2), encoding="utf-8")
    return 0
