...
OUT = (
    ROOT
    / "quant/data_file/reports/strategy_agent_active_l4_v260_entry_exit_surface_v277_20260723"
)
...
def stable_id(definition: dict) -> str:
    ...
    return "v277_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def definitions(protocol: dict) -> list[dict]:
    ...
    return [
        {
            **base,
            "sell_score_below": float(sell_score_below),
            "max_rank_deterioration": float(max_rank_deterioration),
        }
        for sell_score_below, max_rank_deterioration in product(
            protocol["grid"]["sell_score_below"],
            protocol["grid"]["max_rank_deterioration"],
        )
    ]
...
def metric_row(definition: dict) -> dict:
    return {
        ...
        "max_rank_deterioration": definition["max_rank_deterioration"],
    }


def main() -> None:
    parser = argparse.ArgumentParser(description="V260 买入衰退与卖出分数二维邻域")
    ...
    if args.open_known_2026:
        ...
        (
            frame.groupby(["sell_score_below", "max_rank_deterioration"])
            .agg(
