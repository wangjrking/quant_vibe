...
SIGNAL_DIR = OUT_DIR / "top2_1d_confirm_exit_signals"
RUN_DIR = OUT_DIR / "top2_1d_confirm_exit_grid"
SUMMARY = OUT_DIR / "top2_1d_confirm_exit_summary.csv"
...
SCORE_1D_DB = OUT_DIR / "score_1d_candidate_symbols.duckdb"


# name, min_1d_rank, score_exit_ratio, min_hold_exit, row_scale, cap
CASES = [
    ("no1d_e90", 0.0, 0.90, 1, 1.50, 0.82),
    ("r1d40_e90", 0.40, 0.90, 1, 1.50, 0.82),
    ("r1d50_e90", 0.50, 0.90, 1, 1.50, 0.82),
    ("r1d60_e90", 0.60, 0.90, 1, 1.50, 0.82),
    ("r1d40_e95", 0.40, 0.95, 1, 1.50, 0.82),
    ("r1d50_e95", 0.50, 0.95, 1, 1.50, 0.82),
    ("r1d40_e85", 0.40, 0.85, 1, 1.50, 0.82),
    ("r1d40_e90_h2", 0.40, 0.90, 2, 1.50, 0.82),
]
...
def load_base_with_1d() -> pd.DataFrame:
    con = duckdb.connect(read_only=False)
    try:
        con.execute(f"ATTACH '{str(CACHE_DB)}' AS cache (READ_ONLY)")
        con.execute(f"ATTACH '{str(L4_1D_DB)}' AS l4_1d (READ_ONLY)")
        return con.execute(
            f"""
            WITH r1d AS (
                SELECT
                    ...
                    cume_dist() OVER (PARTITION BY trade_date ORDER BY pred_prob) AS rank_1d
                FROM l4_1d.{L4_1D_TABLE}
            ),
            picked AS (
                SELECT
                    a.*,
                    ...
                    r.rank_1d,
                    row_number() OVER (
                        PARTITION BY a.buy_date
                        ORDER BY a.buy_open_gap_raw_pct ASC, a.pred_prob DESC, a.stock_code
                    ) AS pick_rank
                FROM cache.active_l4_wide a
                JOIN r1d r
                  ON r.signal_date = a.signal_date
                 AND r.stock_code = a.stock_code
                WHERE a.score_pct_rank >= 0.95
                  AND a.signal_amount >= 50000
                  AND a.buy_amount >= 50000
                  AND a.ret_h2 IS NOT NULL
                  AND a.buy_open_gap_raw_pct >= -3.0
                  AND a.buy_open_gap_raw_pct <= -0.5
                  AND NOT (a.buy_open_gap_raw_pct > -2.5 AND a.buy_open_gap_raw_pct <= -1.5)
                  AND NOT (a.signal_atr_qfq >= 2.0 AND a.signal_atr_qfq < 4.0)
            )
            SELECT
                signal_date,
                buy_date,
...
def ensure_score_1d_db(symbols: list[str]) -> None:
    con = duckdb.connect(str(SCORE_1D_DB), read_only=False)
    try:
        con.execute(f"ATTACH '{str(L4_1D_DB)}' AS l4_1d (READ_ONLY)")
        con.execute("CREATE OR REPLACE TEMP TABLE candidate_symbols(stock_code VARCHAR)")
        con.executemany("INSERT INTO candidate_symbols VALUES (?)", [(s,) for s in sorted(set(symbols))])
        con.execute(
            f"""
            ...
            SELECT
                ...
                pred_prob
            FROM l4_1d.{L4_1D_TABLE}
            WHERE stock_code IN (SELECT stock_code FROM candidate_symbols)
            """
        )
    finally:
        con.close()
...
def export_signal(base: pd.DataFrame, case: tuple[str, float, float, int, float, float]) -> tuple[Path, dict[str, object]]:
    name, min_1d_rank, score_exit_ratio, min_hold_exit, row_scale, cap = case
    df = base.loc[base["rank_1d"] >= min_1d_rank].copy()
    if df.empty:
    ...
    mult.loc[(df["signal_pct_chg"] > -2.0) & (df["signal_pct_chg"] <= 0.0)] *= 0.5
    mult.loc[(df["signal_total_mv"] > 300000) & (df["signal_total_mv"] <= 1000000)] *= 0.5
    ...
    mult.loc[df["up_ratio"] < 0.40] *= 0.62
    mult.loc[df["up_ratio"] >= 0.60] *= 1.08
    df["target_pct"] = (df["target_pct"] * mult * row_scale).clip(lower=0.03, upper=cap)
    df["pred_prob"] = df["pred_1d"]
    df["holding_days"] = 2
    df["max_holding_days"] = 4
    df["score_exit_entry_ratio"] = score_exit_ratio
    df["min_holding_days_before_score_exit"] = min_hold_exit
...
def run_case(base: pd.DataFrame, case: tuple[str, float, float, int, float, float]) -> dict[str, object]:
    name, min_1d_rank, score_exit_ratio, min_hold_exit, row_scale, cap = case
    signal_file, meta = export_signal(base, case)
    ...
    return {
        "name": name,
        "min_1d_rank": min_1d_rank,
        "score_exit_ratio": score_exit_ratio,
        "min_hold_exit": min_hold_exit,
        "row_scale": row_scale,
        "cap": cap,
        **meta,
...
def main() -> int:
    ...
    base = load_base_with_1d()
    ensure_score_1d_db(base["stock_code"].dropna().astype(str).unique().tolist())
    rows: list[dict[str, object]] = []
    for case in CASES:
        print(f"RUN {case[0]}", flush=True)
        rows.append(run_case(base, case))
    rows.sort(
        key=lambda r: (
