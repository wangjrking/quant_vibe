...
def simulate(
    ...
    score_peak_drop_exit_override=None,
):
    ...
    for t, signal_date in enumerate(dates[:-1]):
        ...
        for idx in sorted(shares, key=lambda item: stocks[item]):
            ...
            if np.isfinite(current):
                peak_score[idx] = max(peak_score.get(idx, current), current)
            score_level_exit_condition = (
                age >= min_hold_days
                and current < sell_score_below
                and best_unheld - current >= replacement_advantage
            )
            score_decay_exit_condition = (
                score_peak_drop_exit_override is not None
                and age >= min_hold_days
                and peak_score.get(idx, current) - current
                >= float(score_peak_drop_exit_override)
                and best_unheld - current >= replacement_advantage
            )
            score_exit_condition = (
                score_level_exit_condition or score_decay_exit_condition
            )
            ...
            if age >= max_hold and renewal_policy in {
                "top1",
                "top1_daily",
                "top1_rebalance",
            }:
        ...
        if max_daily_score_sells_override is not None:
            limit = max(int(max_daily_score_sells_override), 0)
            score_sells = sorted(
                score_sells,
                key=lambda item: (
        ...
        for idx, candidate_target_pct in zip(candidates, candidate_target_pcts):
            ...
            sell_streak[idx] = 0
            peak_score[idx] = float(score[t, idx])
