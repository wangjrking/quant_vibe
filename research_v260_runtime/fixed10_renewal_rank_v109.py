...
def simulate(
    ...
    for t, signal_date in enumerate(dates[:-1]):
        ...
        if daily_exit_candidates:
            ...
            ranked_exit = ranked
        renewal_rank = {idx: rank for rank, idx in enumerate(ranked_exit, start=1)}
        ...
        for idx in sorted(shares, key=lambda item: stocks[item]):
            ...
            if age >= max_hold and max_hold_renewal_rank_override is not None:
                renewal_rank_threshold = (
                    int(max_hold_renewal_rank_override)
                    if np.ndim(max_hold_renewal_rank_override) == 0
                    else int(max_hold_renewal_rank_override[t])
                )
                if renewal_rank_threshold < 1:
                    raise ValueError("最大持有期续持排名必须大于等于1")
                renew_at_max_hold = renewal_rank.get(idx, renewal_rank_threshold + 1) <= renewal_rank_threshold
            elif age >= max_hold and max_hold_renewal_score_override is not None:
        ...
        for idx in rebalance_indices:
            ...
            if portfolio_rebalance_active:
                ...
                if (
                    abs(current_weight - target_pct)
                    < float(portfolio_rebalance_min_weight_deviation_override)
                ):
                    continue
