...
OUT_DIR = DATA_DIR / "reports" / "model_agent_10d_daygate_joint_scan_20260623"
...
RESEARCH_TABLE = "stock_predict_data_model_agent_10d_daygate_joint_scan_20260623_executable_10d_open_return_research"
...
STARTS = ["20251118", "20251201", "20260115", "20260201", "20260225"]
STD_THRESHOLDS = [0.005, 0.01, 0.015, 0.02, 0.03, 0.05, 0.1, 0.2, 0.35]
ZONES = [0.001, 0.0015, 0.002, 0.003]
ALPHAS = [0.005, 0.008, 0.01, 0.012, 0.015]
...
def main() -> int:
    ...
    current_mask = (
        (frame["trade_date"] >= "20251201")
        & (frame["rank_base"] >= 1.0 - 0.0015)
        & (frame["std_r10_top5_base"] < 0.02)
    )
    current_best_score.loc[current_mask] = frame.loc[current_mask, "rank_base"] + 0.01 * frame.loc[current_mask, "rank1"]
    ...
    payload = {
        ...
        "candidate_id": "model_agent_10d_daygate_joint_scan_20260623",
        ...
        "governance_notes": [
            "research only",
            "no training",
            "jointly retunes the day gate and the local rerank zone alpha on top of the current 10d structure",
    ...
    manifest = {
        ...
        "notes": "10D 研究候选在当前 best 结构上联调 day gate 与局部重排 zone/alpha，仅用于 research 验证，不进入 formal L4/L5。",
    }
