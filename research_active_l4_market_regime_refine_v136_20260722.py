...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_market_regime_refine_v136_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v136_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4市场状态门控Top1组合窄邻域研究")
    ...
    rows, definitions = [], {}
    grid = protocol["grid"]
    for max_hold, min_hold, sell_below in product(grid["max_hold_days"], grid["min_hold_days"], grid["sell_score_below"]):
        if min_hold >= max_hold:
            continue
        definition = {**protocol["fixed_definition"], "max_hold_days": max_hold, "min_hold_days": min_hold, "sell_score_below": sell_below}
