...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_1d_new5d_condition_gate_v6_research_20260625"
TARGET_TABLE = "stock_predict_data_model_agent_1d_new5d_condition_gate_v6_20260625_executable_1d_open_return_research"
...
SOURCES = {
    ...
    "cross1": "stock_predict_data_model_agent_1d_cross_horizon_guard_v2_20260625_executable_1d_open_return_research",
    ...
    "recent5": "stock_predict_data_model_agent_5d_recent_enhance_v4_20260625_executable_5d_open_return_research",
...
def build_features(scores: pd.DataFrame) -> pd.DataFrame:
    ...
    for trade_date, group in scores.groupby("trade_date", sort=True):
        base_rank = "base1_rank"
        base10 = top_set(group, base_rank, 10)
        base20 = top_set(group, base_rank, 20)
        base50 = top_set(group, base_rank, 50)
        row: dict[str, float | str] = {"trade_date": trade_date}
        row["base1_top1_top5_gap"] = float(
            group.nlargest(min(1, len(group)), base_rank)[base_rank].mean()
            - group.nlargest(min(5, len(group)), base_rank)[base_rank].min()
        )
        for alias in SOURCES:
            score_col = f"{alias}_score"
            ...
            row[f"{alias}_rank_std"] = float(group[rank_col].std())
...
def evaluate_condition(
    *,
    ...
    for prefix, current, base in [("full", full, base_full), ("recent63", recent63, base63), ("recent20", recent20, base20)]:
        for metric in ["rank_ic", "top1", "top3", "top5", "top10", "top20", "top50"]:
            row[f"{prefix}_{metric}_delta_vs_base1"] = current[metric] - base[metric]
...
def scan(scores: pd.DataFrame, labels: pd.DataFrame, features: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    base_daily = source_metrics[source_metrics["source"] == "base1"].set_index("trade_date")
    feature_idx = features.set_index("trade_date")
    rows = []
    daily_rows = []
    alt_sources = ["cond5", "recent10", "rank3", "recent5", "recent3", "formal10", "formal5", "formal3", "formal1", "dyn1", "strict1", "cross1"]
    ...
    for alt in alt_sources:
        ...
        candidate_features = [
            ...
            if col.startswith(f"{alt}_")
            or col.endswith(f"_{alt}")
            or col.startswith("base1_")
        ...
        for feature in sorted(set(candidate_features)):
            ...
            for threshold in thresholds:
                for op in ["<=", ">="]:
                    ...
                    if len(active_dates) < 8 or len(active_dates) > int(0.50 * len(feature_idx)):
                        continue
                    ...
                    row["pass_basic"] = bool(
                        ...
                        and row["recent63_top1_delta_vs_base1"] >= 0.0
                        and row["recent63_top5_delta_vs_base1"] >= 0.0
                        and row["recent20_top5_delta_vs_base1"] >= -0.0002
                        and row["full_top5_delta_vs_base1"] >= -0.0002
                        and row["full_rank_ic_delta_vs_base1"] >= -0.0008
                        and row["min_period_top5_delta"] >= -0.0003
                        and row["positive_top5_periods"] >= 3
                    )
                    row["objective"] = (
                        1.4 * row["recent63_top1_delta_vs_base1"]
                        + 2.0 * row["recent63_top5_delta_vs_base1"]
                        + 0.8 * row["recent63_top10_delta_vs_base1"]
                        + row["recent20_top5_delta_vs_base1"]
                        + 0.8 * row["full_top5_delta_vs_base1"]
                        + 0.3 * row["full_rank_ic_delta_vs_base1"]
                        + row["min_period_top5_delta"]
                        + 0.008 * row["positive_top5_periods"]
...
def write_outputs(scan_result: pd.DataFrame, daily: pd.DataFrame, source_metrics: pd.DataFrame, stats: dict | None) -> None:
    ...
    scan_path = REPORT_DIR / "condition_gate_scan_results.csv"
    daily_path = REPORT_DIR / "condition_gate_daily_deltas.csv"
    ...
    manifest = {
        ...
        "research_hypothesis": "use current strong 5d/10d/3d research assets as conditional 1d auxiliary ranks",
        "best_scan": best,
        "asset_stats": stats,
    ...
    if stats:
        lines.extend(
            [
                "本轮找到一个相对当前 1D 最强研究基线通过基础门槛的 research-only 候选。",
                "",
                "候选规则：",
                "",
                "```text",
        ...
        lines.append("本轮未找到相对当前 1D 最强研究基线通过基础门槛的候选；不写入新的预测研究表。")
    lines.extend(
        [
            "",
            "## 证据路径",
