...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_dynamic_max_positions_v211_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v211_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def max_positions_schedule(arrays: dict, definition: dict) -> np.ndarray:
    momentum = v134.market_momentum(arrays, 10)
    result = np.full(len(momentum), int(definition["nonweak_max_positions"]), dtype=np.int16)
    weak = np.isfinite(momentum) & (momentum < 0.0)
    result[weak] = int(definition["weak_max_positions"])
    return result
...
def run_case(arrays, score, order, definition, protocol, end, start=None):
    return v162.run_case(
        ...
        max_positions_schedule_override=max_positions_schedule(arrays, definition),
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 动态并发持仓上限研究")
    ...
    rows, definitions = [], {}
    for weak_cap, nonweak_cap in product(
        protocol["grid"]["weak_max_positions"],
        protocol["grid"]["nonweak_max_positions"],
    ):
        definition = {
            **protocol["fixed_definition"],
            "weak_max_positions": int(weak_cap),
            "nonweak_max_positions": int(nonweak_cap),
        }
