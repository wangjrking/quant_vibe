...
REPORT_DIR = SOURCE_DIR / "execution_knobs_20260624"
...
ENV_VARIANTS = [
    {"tag": "base", "env": {}},
    {"tag": "force_sell_mkt", "env": {"GM_FORCE_SELL_MARKET_ORDER": "1"}},
    {"tag": "force_all_mkt", "env": {"GM_FORCE_MARKET_ORDER": "1", "GM_FORCE_BUY_MARKET_ORDER": "1", "GM_FORCE_SELL_MARKET_ORDER": "1"}},
    {"tag": "lightstop5", "env": {"GM_LIGHT_STOP_LOSS_PCT": "0.05", "GM_MIN_HOLDING_DAYS_BEFORE_LIGHT_STOP": "1"}},
    {"tag": "lightstop8", "env": {"GM_LIGHT_STOP_LOSS_PCT": "0.08", "GM_MIN_HOLDING_DAYS_BEFORE_LIGHT_STOP": "1"}},
    {"tag": "continue100", "env": {"GM_SCORE_CONTINUE_ENTRY_RATIO": "1.00"}},
    {"tag": "continue103", "env": {"GM_SCORE_CONTINUE_ENTRY_RATIO": "1.03"}},
]

BASE_CASES = [
    {"holding_days": 2, "target_pct": 0.75, "exit_ratio": 0.96, "min_hold": 1},
    {"holding_days": 3, "target_pct": 0.75, "exit_ratio": 0.96, "min_hold": 1},
]
...
def _case_name(case: dict, variant: dict) -> str:
    return (
        f"exec_{variant['tag']}_liq_amt10w_mv20w_w90_5d10"
...
def _build_signal(base: pd.DataFrame, market: dict[str, dict[str, dict]], next_date: dict[str, str], case: dict, variant: dict) -> tuple[Path, list[dict]]:
        ...
        rows.append(
            {
                ...
                "execution_variant": variant["tag"],
            }
        )
    path = REPORT_DIR / "signals" / f"{_case_name(case, variant)}.csv"
...
def _run_case(case: dict, variant: dict, signal_file: Path, start_name: str, start: str) -> dict:
    name = _case_name(case, variant)
    ...
    return {
        "name": name,
        "variant": variant["tag"],
        "extra_env": json.dumps(variant["env"], ensure_ascii=False, sort_keys=True),
...
def _write_report(summary: list[dict], audit: dict) -> None:
    lines = [
        ...
        "## 当前结论",
        "",
        "本轮不改变模型、股票池过滤、Top1 选股或主仓位规则，只测试执行层参数是否能缓解 `holding_days=2` 邻域失效。",
        "",
        "## 掘金汇总",
        "",
        "| 策略 | 执行变体 | 持有 | 年化 | 夏普 | 最大回撤 | 近期开仓最差年化 | 近期开仓中位年化 | 开仓 | 平均持仓率 |",
        "| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    for row in sorted(summary, key=lambda item: _f(item["score"]), reverse=True):
        lines.append(
            f"| {row['name']} | {row['variant']} | {row['holding_days']} | "
            f"{_f(row['full_annual']):.4f} | {_f(row['full_sharpe']):.4f} | {_f(row['full_max_drawdown']):.4f} | "
            f"{_f(row['late_min_annual']):.4f} | {_f(row['late_median_annual']):.4f} | "
            f"{row['open_count']} | {_f(row['full_avg_invested_pct']):.4f} |"
        )
    lines.extend([
        "",
        "## 硬过滤审计",
        ...
        f"- 硬过滤审计：`{REPORT_DIR / 'execution_knobs_hard_gate_audit.json'}`",
        f"- 掘金日志：`{REPORT_DIR / 'logs'}`",
        f"- 信号文件：`{REPORT_DIR / 'signals'}`",
    ])
    (REPORT_DIR / "execution_knobs_report.md").write_text("\n".join(lines), encoding="utf-8")
...
def main() -> int:
    ...
    total = len(BASE_CASES) * len(ENV_VARIANTS) * len(STARTS)
    done = 0
    for case in BASE_CASES:
        for variant in ENV_VARIANTS:
            signal_file, signal_rows = _build_signal(base, market, next_date, case, variant)
            ...
            for start_name, start in STARTS:
                ...
                if (name, start_name) in existing:
                    ...
                    continue
                row = _run_case(case, variant, signal_file, start_name, start)
                rows.append(row)
    ...
    (REPORT_DIR / "execution_knobs_hard_gate_audit.json").write_text(json.dumps(audit, ensure_ascii=False, indent=2), encoding="utf-8")
