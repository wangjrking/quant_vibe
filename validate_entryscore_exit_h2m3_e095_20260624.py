...
import tune_dynamic_h2_m3_entryscore_exit_20260624 as exp
import tune_strict_sync_liquidity_refill_20260624 as liq
import validate_force_sell_pos56_candidate_20260624 as validator


CASE_NAME = "entryscore_exit_h2m3c098_e095"
REPORT_DIR = exp.SOURCE_DIR / "formal_horizon_entry_confirmation_20260624" / "entryscore_exit_h2m3_e095_validation_20260624"
SIGNAL_FILE = exp.REPORT_DIR / "signals" / f"{CASE_NAME}.csv"
...
def _run(signal_file: Path, tag: str, start: str, end: str = liq.BACKTEST_END) -> dict:
    ...
    if indicator is None:
        ...
        env["GM_SCORE_EXIT_ENTRY_RATIO"] = "0.95"
        env["GM_MIN_HOLDING_DAYS_BEFORE_SCORE_EXIT"] = str(exp.MIN_HOLD)
        env["GM_SCORE_CONTINUE_ENTRY_RATIO"] = str(exp.CONTINUE_RATIO)
        command = [
            ...
            "1",
            "--holding-days",
            str(exp.HOLDING_DAYS),
            "--max-holding-days",
            str(exp.MAX_HOLDING_DAYS),
            "--target-position-pct",
            str(exp.TARGET_PCT),
            "--score-db",
            str(liq.SCORE_DB),
...
def _write_report(time_rows: list[dict], nearby_summary: list[dict], stress_summary: list[dict], proxy_stats: dict, audit: dict) -> None:
    lines = [
        "# entryscore exit h2_m3 e095 完整验证",
        "",
        "## 当前结论",
        "",
        f"验证对象：`{CASE_NAME}`。该版本使用混合入场分数作为退出 score table，退出比例 `0.95`。",
        "",
        "它明显改善 recent60，但全周期收益低于 200% 级别候选，是当前偏持续性候选，不是收益最高候选。",
        "",
        "## 时间切片",
        "",
...
def main() -> int:
    ...
    for name, start, end in validator.TIME_SLICES:
        ...
        print(f"time {name} annual={row['annual']} sharpe={row['sharpe']}", flush=True)
    ...
    try:
        ...
        validator.HOLDING_DAYS = exp.MAX_HOLDING_DAYS
        validator.TARGET_PCT = exp.TARGET_PCT
    ...
    (REPORT_DIR / "validation_summary.json").write_text(
        json.dumps({"time_slices": time_rows, "nearby_summary": nearby_summary, "stress_summary": stress_sum, "proxy": proxy_stats, "audit": audit}, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
