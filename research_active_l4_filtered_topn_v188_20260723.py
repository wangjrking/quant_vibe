...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_filtered_topn_v188_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v188_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def case_protocol(protocol: dict, definition: dict) -> dict:
    result = copy.deepcopy(protocol)
    result["fixed_strategy"] = {"top_n_per_rebalance": int(definition["top_n"]), "max_hold_days": int(definition["max_hold_days"]), "rebalance_every": 1}
    return result


def run_case(arrays, score, order, definition, protocol, end, start=None):
    gross, target = v153.schedules(arrays, definition)
    target = target / float(definition["top_n"])
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 filtered TopN study")
    ...
    rows, definitions = [], {}
    for top_n in protocol["grid"]["top_n"]:
        definition = {**protocol["fixed_definition"], "top_n": int(top_n)}
