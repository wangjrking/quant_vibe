...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_1d_new5d_fixed4y_lowstd_gate_refined_20260626"
...
TARGET_TABLE = "stock_predict_data_model_agent_1d_new5d_fixed4y_lowstd_gate_refined_20260626_executable_1d_open_return_research"
ALT_PATH = (
    DATA_DIR
    / "reports"
...
def add_gate(scores: pd.DataFrame) -> pd.DataFrame:
    ...
    for trade_date, group in overlap.groupby("trade_date", sort=True):
        gate_rows.append({"trade_date": trade_date, FEATURE: float(group["base_score"].std())})
...
def evaluate(scored: pd.DataFrame, labels: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, dict[str, object]]:
    ...
    candidate = daily_metrics(eval_frame, "pred_prob", "fixed4y_lowstd_gate_refined")
    formal = daily_metrics(eval_frame, "formal_rank", "formal")
    base = daily_metrics(eval_frame, "base_rank", "base_new5d_gate")
...
def write_outputs(
    ...
    metrics_path = REPORT_DIR / "lowstd_gate_refined_daily_metrics.csv"
    delta_formal_path = REPORT_DIR / "lowstd_gate_refined_delta_vs_formal.csv"
    delta_base_path = REPORT_DIR / "lowstd_gate_refined_delta_vs_base.csv"
    manifest_path = REPORT_DIR / "lowstd_gate_refined_research_manifest.json"
    report_path = REPORT_DIR / "lowstd_gate_refined_research_report.md"
    ...
    manifest = {
        ...
        "candidate_id": "model_agent_1d_new5d_fixed4y_lowstd_gate_refined_20260626",
        "score_formula": stats["formula"],
        "research_hypothesis": "use_fixed4y_fs40_candidate_on_refined_low_base_std_days_with_current_1d_mainline",
    ...
    lines = [
        "# 1D new5d 与近四年候选的 refined low-std 门控研究报告（20260626）",
        "",
        "## 当前结论",
        "",
        "这是一个 research-only 的 1D 全覆盖候选，不是 formal L4 或 L5 生产资产。",
        f"它只在与近四年候选重叠的 {stats['overlap_days']} 个交易日里，当 `base_std <= {THRESHOLD:.15f}` 时切到近四年候选，其余日期保持当前 1D 主线不变。",
        "",
        "## 评分公式",
        "",
        "```text",
        ...
        "## 关键评价摘要",
        "",
        f"- 相对当前 1D 主线：full objective delta `{summary['full_candidate_objective'] - summary['full_base_objective']:.12f}`，full RankIC delta `{summary['full_vs_base_rank_ic_delta']:.12f}`，full Top5 delta `{summary['full_vs_base_top5_delta']:.12f}`。",
        f"- 相对当前 1D 主线：recent63 Top1 delta `{summary['recent63_vs_base_top1_delta']:.12f}`，recent63 Top5 delta `{summary['recent63_vs_base_top5_delta']:.12f}`，recent20 Top5 delta `{summary['recent20_vs_base_top5_delta']:.12f}`。",
        ...
        "## 当前判断",
        "",
        "- 这条分支是在原有 1D repair 候选上做 full-window refine 后得到的更优版本。",
