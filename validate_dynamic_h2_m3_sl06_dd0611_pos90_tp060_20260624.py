...
import validate_dynamic_h2_m3_sl06_dd0611_pos86_tp060_20260624 as val86


CASE_NAME = "dynamic_h2_m3_c098_w78_5d12_3d10_pos90_e097_mh1_sl06_dd0611_tp060"
REPORT_DIR = (
    val86.val.SOURCE_DIR
    / "formal_horizon_entry_confirmation_20260624"
    / "dynamic_h2_m3_sl06_dd0611_pos90_tp060_validation_20260624"
)
SIGNAL_FILE = (
    val86.val.SOURCE_DIR
    / "formal_horizon_entry_confirmation_20260624"
    / "dynamic_h2_m3_dd0611_tp060_position_20260624"
    / "signals"
...
def _rewrite_report() -> None:
    ...
    lines = [
        ...
        "## 当前结论",
        "",
        f"验证对象：`{CASE_NAME}`。该版本固定 formal L4 多周期模型入场、Top1 单票、目标仓位 `90%`、动态持有 `h2_m3_c098`、分数退出 `0.97`、日内 `6%` 止损和通用 `6%` 止盈；账户回撤缩放为 `soft=6% / hard=11% / recover=3% / scale=0.70/0.50`。",
        "",
