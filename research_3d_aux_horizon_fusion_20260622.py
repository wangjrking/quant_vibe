...
OUT_DIR = DATA_DIR / "reports" / "model_agent_3d_aux_fusion_tune_20260622"
...
def materialize(best_name: str, merged: pd.DataFrame, score: pd.Series) -> dict:
    ...
    manifest = {
        ...
        "notes": "3D formal 与辅助周期预测分数rank融合研究资产；不作为L5正式入口。",
    }
...
def main() -> int:
    ...
    with sqlite3.connect(MODEL_DB) as conn:
        merged = read_prediction(conn, BASE_TABLE, "formal_3d_pred_prob", keep_payload=True)
    ...
    score_cols = ["formal_3d_pred_prob"] + [f"{alias}_pred_prob" for alias in AUX_TABLES]
    ...
    for col in aux_rank_cols:
        merged[col] = merged[col].fillna(merged["rank_formal_3d"])

    candidates: dict[str, pd.Series] = {
        "formal_3d_raw": merged["formal_3d_pred_prob"],
        "formal_3d_rank": merged["rank_formal_3d"],
    }
    for alias in AUX_TABLES:
        candidates[f"{alias}_rank"] = merged[f"rank_{alias}"]
        for weight in [0.90, 0.85, 0.75, 0.65, 0.50]:
            rest = 1.0 - weight
            candidates[f"rank_blend_3dw{str(weight).replace('.', 'p')}_{alias}w{str(rest).replace('.', 'p')}"] = (
                merged["rank_formal_3d"] * weight + merged[f"rank_{alias}"] * rest
            )

    candidates["rank_avg_3d_1daux_5d_10d"] = merged[
        ["rank_formal_3d", "rank_research_1d_aux", "rank_formal_5d", "rank_formal_10d"]
    ].mean(axis=1)
    candidates["rank_avg_1daux_5d_10d"] = merged[
        ["rank_research_1d_aux", "rank_formal_5d", "rank_formal_10d"]
    ].mean(axis=1)
    candidates["rank_weighted_3d_0p50_1daux_0p25_10daux_0p25"] = (
        merged["rank_formal_3d"] * 0.50 + merged["rank_research_1d_aux"] * 0.25 + merged["rank_research_10d_aux"] * 0.25
    )
    candidates["rank_weighted_3d_0p40_1daux_0p30_10daux_0p30"] = (
        merged["rank_formal_3d"] * 0.40 + merged["rank_research_1d_aux"] * 0.30 + merged["rank_research_10d_aux"] * 0.30
    )
    candidates["rank_weighted_3d_0p30_1daux_0p35_10daux_0p35"] = (
        merged["rank_formal_3d"] * 0.30 + merged["rank_research_1d_aux"] * 0.35 + merged["rank_research_10d_aux"] * 0.35
    )
    candidates["rank_consensus_min_3d_1daux_10daux"] = merged[
        ["rank_formal_3d", "rank_research_1d_aux", "rank_research_10d_aux"]
    ].min(axis=1)
    candidates["rank_consensus_max_3d_1daux_10daux"] = merged[
        ["rank_formal_3d", "rank_research_1d_aux", "rank_research_10d_aux"]
    ].max(axis=1)
    ...
    baseline = by_name["formal_3d_raw"]
    ...
    def strict_gate(row: dict) -> bool:
        ...
        return (
            row["name"] != "formal_3d_raw"
            and row_delta["front_selection_score"] > 0.001
            and row_delta["daily_rank_ic_mean"] >= -0.001
            and row_delta["top_k_mean_returns"]["1"] >= -0.001
            and row_delta["top_k_mean_returns"]["3"] >= -0.001
        )

    def front_rank_gate(row: dict) -> bool:
        row_delta = delta(row, baseline)
        return (
            row["name"] != "formal_3d_raw"
            and row_delta["front_selection_score"] > 0.001
            and row_delta["daily_rank_ic_mean"] > 0.02
            and row_delta["top_k_mean_returns"]["1"] > 0.005
            and row_delta["top_k_mean_returns"]["5"] >= -0.0005
            and row_delta["top_k_mean_returns"]["10"] >= 0.0
        )

    strict_candidates = [row for row in summaries if strict_gate(row)]
    front_rank_candidates = [row for row in summaries if front_rank_gate(row)]
    selected = None
    selected_gate = None
    if strict_candidates:
        selected = max(strict_candidates, key=lambda row: row["front_selection_score"])
        selected_gate = "strict_top3_guard"
    elif front_rank_candidates:
        selected = max(front_rank_candidates, key=lambda row: row["front_selection_score"])
        selected_gate = "front_rank_research_gate_top3_can_decline"
    selected_delta = delta(selected, baseline) if selected is not None else None
    materialize_asset = selected is not None
    ...
    payload = {
        ...
        "baseline": baseline,
        "best": best,
        "best_delta_vs_formal_3d": best_delta,
        "selected": selected,
        "selected_delta_vs_formal_3d": selected_delta,
        ...
        "selection_rule": (
            "先尝试严格门槛：front_selection_score 改善且 RankIC/Top1/Top3 不明显回撤；"
            "若严格门槛无候选，则允许 front-rank 研究门槛：Top1、Top5、Top10 与 RankIC 改善，"
            "但 Top3 可回撤并需在报告中标注。辅助分数缺失日回退到3D基线rank。"
        ),
    }
    ...
    if materialize_asset:
        lines.append(
            f"综合最高候选为 `{best['name']}`；最终落库候选为 `{selected['name']}`，"
            f"落库门槛为 `{selected_gate}`，已生成 research-only L4 资产。"
        )
    else:
    ...
    lines.extend(
        [
            ...
            "- 辅助分数缺失日回退到 3D 基线 rank，避免缩短最新预测覆盖。",
            "",
            "## 证据路径",
            "",
