...
import research_v260_fixed10_max_hold_recheck_20260822 as round7
import research_v260_lowrisk_score_tuning_20260821 as research_base
from research_v260_runtime import fixed10_safe_v109 as safe_v109


OUTPUT_ROOT = (
    REPO / "quant/data_file/reports/strategy_agent_v260_fixed10_min_hold_20260822"
)
MIN_HOLD_MODES = ("production_dynamic", 2, 3, 4)


def candidate_policy(min_hold_mode: str | int) -> dict:
    current = round7.candidate_policy(25)
    current["min_hold_mode"] = min_hold_mode
    return current
...
def main() -> None:
    ...
    if access["logical_max_date"] != round1.DEVELOPMENT_END:
        raise PermissionError("2026 data entered min-hold development")
    definition = harness.production_definition(protocol)
    score, order = harness.v95.score_pair(arrays, 0.0, 7, 0.1)

    results = {}
    run_cache = {}
    for mode in MIN_HOLD_MODES:
        candidate_id = f"min_hold_{mode}"
        current = candidate_policy(mode)
    ...
    if not all(deterministic.values()):
        raise RuntimeError("min-hold deterministic replay failed")
    ...
    result = {
        ...
        "candidate_budget": list(MIN_HOLD_MODES),
        "selection_rule": (
