...
def without_confirmed_weak_sell_priority(policy: dict) -> dict:
    result = copy.deepcopy(policy)
    if not result.get("confirmed_weak_sell_priority"):
        raise ValueError("confirmed-weak sell priority is already absent")
    result["confirmed_weak_sell_priority"] = None
    return result


def run_arm(context, policy: dict, cost: float, use_volatility_priority: bool):
    extra_age, volatility_priority = sizing.width_tools.build_overrides(context, policy)
    ...
    return sizing.age_guard.run_policy_at_cost(
        ...
        cost,
        simulator=simulator,
        score_sell_priority_override=(
            volatility_priority if use_volatility_priority else context.score
        ),
...
def main() -> None:
    ...
    if checkpoint.get("validation_2026_opened") is not False:
        raise PermissionError("2026 entered sell-priority simplification")
    current_policy = copy.deepcopy(checkpoint["selected_policy"])
    simplified_policy = without_confirmed_weak_sell_priority(current_policy)
    changed = sorted(
        key
        for key in set(current_policy) | set(simplified_policy)
        if current_policy.get(key) != simplified_policy.get(key)
    )
    if changed != ["confirmed_weak_sell_priority"]:
        raise RuntimeError("sell-priority simplification changed unexpected policy keys")
    ...
    arms = {}
    frames = {}
    for name, policy, use_priority in (
        ("current_rank_sizing", current_policy, True),
        ("remove_confirmed_weak_volatility_priority", simplified_policy, False),
    ):
        daily, actions = run_arm(context, policy, sizing.BASELINE_COST, use_priority)
        stress_daily, stress_actions = run_arm(
            context, policy, sizing.STRESS_COST, use_priority
        )
        arms[name] = {
    ...
    selected_policy = (
        current_policy
        if selected == "current_rank_sizing"
        else simplified_policy
    )
    repeat_daily, repeat_actions = run_arm(
        context,
        selected_policy,
        sizing.BASELINE_COST,
        selected == "current_rank_sizing",
    )
    deterministic = {
    ...
    if not all(deterministic.values()):
        raise RuntimeError("sell-priority simplification replay failed")

    challenger = arms["remove_confirmed_weak_volatility_priority"]
    result = {
        "status": "rank_sizing_sell_priority_simplification_complete_2026_not_opened",
        "only_change": (
            "remove confirmed-weak trailing-volatility sell ordering and use the "
            "original exit-score ordering; exit eligibility and count are unchanged"
        ),
