...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_long_hold_allocation_micro_v151_20260722"
...
def stable_id(definition: dict) -> str:
    ...
    return "v151_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="正式L4长持有策略仓位微邻域研究")
    ...
    rows, definitions = [], {}
    for target_cohorts, low_gross in product(protocol["grid"]["target_cohorts"], protocol["grid"]["low_gross"]):
        definition = {**protocol["fixed_definition"], "target_cohorts": target_cohorts, "low_gross": low_gross}
