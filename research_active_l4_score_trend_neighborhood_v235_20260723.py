...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_score_trend_neighborhood_v235_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v235_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 分数趋势参数邻域研究")
    ...
    if args.export_selected_actions:
        if args.lookback is None or args.sensitivity is None:
            raise SystemExit("--export-selected-actions 需要同时指定 --lookback 和 --sensitivity")
        frozen = json.loads(FROZEN.read_text(encoding="utf-8"))
        selected = next(
            item
            for item in frozen["candidates"]
            if int(item["definition"]["score_trend_lookback"]) == int(args.lookback)
            and abs(
                float(item["definition"]["score_trend_sensitivity"]) - float(args.sensitivity)
            )
            < 1e-12
        )
        cache_protocol = json.loads(v234.latest_export.BASE_PROTOCOL.read_text(encoding="utf-8"))
        ...
        v234.latest_export.populate_latest_signal_row(full)
        v234.latest_export.add_liquidity_arrays(full)
        score, order = v95.score_pair(full, 0.0, 7, 0.1)
        ...
        tag = "lookback{}_sensitivity{}".format(
            int(args.lookback), str(args.sensitivity).replace(".", "p")
        )
        daily_path = OUT / f"daily_local_{tag}_through_latest.csv"
        actions_path = OUT / f"replay_actions_{tag}_through_latest.csv"
