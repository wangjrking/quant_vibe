...
OUT_DIR = DATA_DIR / "reports" / "model_agent_5d_v3_local_reblend_1dbest_10dstd035_20260623"
...
AUX10_TABLE = "stock_predict_data_model_agent_10d_proxy5d_lite_daygate_std035_20260623_executable_10d_open_return_research"
RESEARCH_TABLE = "stock_predict_data_model_agent_5d_v3_local_reblend_1dbest_10dstd035_20260623_executable_5d_open_return_research"
...
STARTS = ["20260201", "20260115"]
ZONES = [0.002, 0.003, 0.004, 0.005, 0.006]
GRID = [
    (0.0, 0.002),
    (0.0, 0.005),
    (0.0, 0.01),
...
def materialize(frame: pd.DataFrame, score: pd.Series) -> dict:
    out = frame[["trade_date", "stock_code", LABEL_COL, "base_pred_prob", "rank_base", "rank1", "rank10"]].copy()
    out["pred_prob"] = score.astype(float)
    out = out[["trade_date", "stock_code", LABEL_COL, "pred_prob", "base_pred_prob", "rank_base", "rank1", "rank10"]]
...
def main() -> int:
    ...
    frame["rank10"] = frame["rank10"].fillna(frame["rank_base"])
    ...
    for start in STARTS:
        for zone in ZONES:
            for alpha1, alpha10 in GRID:
                active_mask = (frame["trade_date"] >= start) & (frame["rank_base"] >= 1.0 - zone)
                score = frame["rank_base"].copy()
                score.loc[active_mask] = (
                    frame.loc[active_mask, "rank_base"]
                    + alpha1 * frame.loc[active_mask, "rank1"]
                    + alpha10 * frame.loc[active_mask, "rank10"]
                )
    ...
    manifest = {
        ...
        "label": LABEL_COL,
        "candidate_id": "model_agent_5d_v3_local_reblend_1dbest_10dstd035_20260623",
        ...
        "selected_rule": {
            ...
            "zone": best_rule["zone"],
            "alpha1": best_rule["alpha1"],
            "alpha10": best_rule["alpha10"],
            "formula": "if trade_date >= start and base_rank >= 1-zone: score = base_rank + alpha1 * rank1d + alpha10 * rank10d; else score = base_rank",
        },
        ...
        "notes": "5D research 候选以当前 v3 最优候选为底座，只在局部头部区域叠加最新 1D best 与 10D std035 排序，用于检查 5D 是否还能继续小幅增强。不进入 formal L4/L5。",
    }
    ...
    lines = [
        "# 5D v3 局部叠加 1D best + 10D std035 研究候选",
        "",
        "## 当前结论",
        "",
        ...
        f"- alpha1：`{best_rule['alpha1']}`",
        f"- alpha10：`{best_rule['alpha10']}`",
