...
OUT_DIR = DATA_DIR / "reports" / "model_agent_5d_v5_topgate_on_v4_3drefined_20260623"

BASE_TABLE = "stock_predict_data_model_agent_5d_v4_daily_gate_d3best_d1best_20260623_executable_5d_open_return_research"
AUX_TABLE = "stock_predict_data_model_agent_3d_proxy10d5d_bestmix_refine_20260623_executable_3d_open_return_research"
RESEARCH_TABLE = "stock_predict_data_model_agent_5d_v5_topgate_on_v4_3drefined_20260623_executable_5d_open_return_research"
...
DATE_GATES = ["20260201", "20260301", "20260401"]
BASE_RANK_THRESHOLDS = [0.994, 0.995, 0.996, 0.997, 0.998, 0.999]
AUX_ALPHAS = [0.003, 0.005, 0.008, 0.01]
...
def main() -> int:
    ...
    for date_gate in DATE_GATES:
        for threshold in BASE_RANK_THRESHOLDS:
            for alpha in AUX_ALPHAS:
                ...
                pass_guard = (
                    eval_summary["recent80"]["top3"] >= base_recent80_top3 - 0.0002
                    and eval_summary["recent80"]["top5"] >= base_recent80_top5 - 0.0002
                )
                row = {
                    "date_gate": date_gate,
    ...
    payload = {
        ...
        "candidate_id": "model_agent_5d_v5_topgate_on_v4_3drefined_20260623",
        ...
        "guard_rule": {
            ...
            "recent80_top5_min": base_recent80_top5 - 0.0002,
        ...
        "governance_notes": [
            "research only",
            "no training",
            "5d v4 current best receives a small 3d refined rank add-on only inside the top bucket after a date gate",
    ...
    manifest = {
        ...
        "notes": "5D v5 研究候选：在 v4 daily-gate 当前最优底座上，仅对尾段日期的头部样本叠加 3D refined 局部重排，用于验证是否还能继续提升评价指标；不进入 formal L4/L5。",
    }
    ...
    report_lines = [
        "# 5D v5 头部门控研究候选",
        "",
        "- 候选 ID：`model_agent_5d_v5_topgate_on_v4_3drefined_20260623`",
        f"- 基座表：`{BASE_TABLE}`",
        f"- 辅助表：`{AUX_TABLE}`",
        f"- 最优规则：`date_gate={best_rule['date_gate']}`、`base_rank_threshold={best_rule['base_rank_threshold']}`、`aux_add_alpha={best_rule['aux_add_alpha']}`",
        f"- Focus：`{best_focus:.12f}`，相对基座变化：`{best_focus - base_focus:+.12f}`",
        f"- recent80 守卫：Top3 最低 `>= {base_recent80_top3 - 0.0002:.12f}`，Top5 最低 `>= {base_recent80_top5 - 0.0002:.12f}`，本次结果通过：`{best_passed}`",
        "",
        "## 窗口表现",
        "",
        f"- full Top1/Top3/Top5：`{best_summary['full']['top1']:.12f}` / `{best_summary['full']['top3']:.12f}` / `{best_summary['full']['top5']:.12f}`",
        f"- recent1y Top1/Top3/Top5：`{best_summary['recent1y']['top1']:.12f}` / `{best_summary['recent1y']['top3']:.12f}` / `{best_summary['recent1y']['top5']:.12f}`",
        f"- recent80 Top1/Top3/Top5：`{best_summary['recent80']['top1']:.12f}` / `{best_summary['recent80']['top3']:.12f}` / `{best_summary['recent80']['top5']:.12f}`",
        "",
        "## 边界",
        "",
        "- 仅研究用途。",
