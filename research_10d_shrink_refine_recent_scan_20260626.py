...
REPORT_DIR = DATA_DIR / "reports" / "model_agent_10d_shrink_refine_recent_scan_20260626"
...
TARGET_TABLE = "stock_predict_data_model_agent_10d_shrink_recent_refine_20260626_executable_10d_open_return_research"

SOURCES = {
    "base": BASE_TABLE,
    "raw_v5": "stock_predict_data_model_agent_10d_risk_balanced_v5_20260625_executable_10d_open_return_research",
    "recent_v4": "stock_predict_data_model_agent_10d_recent_enhance_v4_20260625_executable_10d_open_return_research",
    "cross_v3": "stock_predict_data_model_agent_10d_cross_horizon_guard_v3_20260625_executable_10d_open_return_research",
...
def load_scores() -> pd.DataFrame:
    with sqlite3.connect(MODEL_DB, timeout=300) as conn:
        base = read_source(conn, "base", SOURCES["base"])
        out = base
...
def objective(row: dict[str, float | int]) -> float:
    return (
        2.5 * row["recent63_top5_delta"]
        + 2.2 * row["recent20_top5_delta"]
        + 1.0 * row["full_top5_delta"]
        + 1.2 * row["recent63_top1_delta"]
        + 0.7 * row["recent20_top1_delta"]
        + 0.5 * row["full_top1_delta"]
        + 0.6 * row["recent63_rank_ic_delta"]
        + 0.3 * row["full_rank_ic_delta"]
        + 0.005 * row["positive_top5_periods"]
...
def scan(scores: pd.DataFrame, labels: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, pd.DataFrame], pd.DataFrame]:
    ...
    for alt in ["raw_v5", "recent_v4", "cross_v3", "formal10"]:
        feature_table = build_feature_table(scores, alt)
        feature_tables[alt] = feature_table
        alt_daily = source_daily[alt]
        for feature in FEATURES:
            vals = feature_table[feature].replace([np.inf, -np.inf], np.nan).dropna()
            if vals.nunique() < 3:
                continue
            ...
            for threshold in thresholds:
                for op in ["<=", ">="]:
                    ...
                    if len(active_dates) < 6 or len(active_dates) > int(0.25 * len(feature_table)):
                        continue
                    recent63_active = len(set(trade_dates[-63:]) & active_dates)
                    recent20_active = len(set(trade_dates[-20:]) & active_dates)
                    if recent63_active < 3 or recent20_active < 1:
                        continue
                    candidate = base_daily.copy()
                    overlap = [d for d in active_dates if d in alt_daily.index]
                    if overlap:
                    ...
                    row["pass_basic"] = bool(
                        ...
                        and row["recent63_rank_ic_delta"] >= -0.00050
                        and row["full_top5_delta"] >= 0.0
                        and row["recent63_top5_delta"] >= 0.0
                        and row["recent20_top5_delta"] >= 0.0
                        and row["recent63_top1_delta"] >= -0.002
    ...
    if not result.empty:
        result = result.sort_values(["pass_basic", "objective", "recent63_top5_delta"], ascending=[False, False, False]).reset_index(drop=True)
    return result, feature_tables, pd.concat([d.assign(source=k).reset_index() for k, d in source_daily.items()], ignore_index=True)
...
def main() -> int:
    ...
    result.to_csv(REPORT_DIR / "shrink_refine_recent_scan_results.csv", index=False, encoding="utf-8-sig")
    source_daily.to_csv(REPORT_DIR / "shrink_refine_recent_source_daily.csv", index=False, encoding="utf-8-sig")
    ...
    summary = {
        ...
        "scope": "research_only_10d_shrink_refine_recent_scan",
    ...
    (REPORT_DIR / "shrink_refine_recent_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    return 0
