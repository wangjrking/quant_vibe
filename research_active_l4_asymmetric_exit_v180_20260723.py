...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_asymmetric_exit_v180_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v180_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def exit_schedules(arrays: dict, definition: dict) -> tuple[np.ndarray, np.ndarray]:
    momentum = v134.market_momentum(arrays, 10)
    below = np.full(len(momentum), float(definition["sell_score_below"]), dtype=float)
    advantage = np.full(len(momentum), float(definition["replacement_advantage"]), dtype=float)
    weak = np.isfinite(momentum) & (momentum < 0.0)
    strong = np.isfinite(momentum) & (momentum >= 0.04)
    below[weak] = float(definition["weak_sell_score_below"])
    advantage[weak] = float(definition["weak_replacement_advantage"])
    below[strong] = float(definition["strong_sell_score_below"])
    advantage[strong] = float(definition["strong_replacement_advantage"])
    return below, advantage


def run_case(arrays, score, order, definition, protocol, end, start=None):
    below, advantage = exit_schedules(arrays, definition)
    return v162.run_case(
        ...
        sell_score_below_override=below,
        replacement_advantage_override=advantage,
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 asymmetric exit study")
