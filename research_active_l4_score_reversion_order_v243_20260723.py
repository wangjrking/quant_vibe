...
OUT = ROOT / "quant/data_file/reports/strategy_agent_active_l4_score_reversion_order_v243_20260723"
...
def stable_id(definition: dict) -> str:
    ...
    return "v243_" + hashlib.sha256(raw.encode("utf-8")).hexdigest()[:16]


def reversion_order(score: np.ndarray, lookback: int, tilt: float) -> np.ndarray:
    if abs(tilt) < 1e-12:
        return np.argsort(-np.nan_to_num(score, nan=-np.inf), axis=1, kind="stable")
    delta = np.zeros(score.shape, dtype=np.float32)
    valid = np.isfinite(score[lookback:]) & np.isfinite(score[:-lookback])
    later = delta[lookback:]
    later[valid] = score[lookback:][valid] - score[:-lookback][valid]
    ranking_score = score + float(tilt) * delta
    return np.argsort(-np.nan_to_num(ranking_score, nan=-np.inf), axis=1, kind="stable")


def run_case(arrays, score, definition, protocol, end, start=None):
    order = reversion_order(
        score,
        int(definition["score_reversion_lookback"]),
        float(definition["score_reversion_tilt"]),
    )
    return v195.run_case(arrays, score, order, definition, protocol, end, start)


def main() -> None:
    parser = argparse.ArgumentParser(description="Active L4 高分候选均值回归排序研究")
    ...
    score, _ = v95.score_pair(full, 0.0, 7, 0.1)
                ...
                rows.append(
                    {
                        ...
                        "score_reversion_lookback": definition["score_reversion_lookback"],
                        "score_reversion_tilt": definition["score_reversion_tilt"],
                        "buy_start": start,
    ...
    for item in protocol["grid"]:
        ...
        daily = run_case(observation, score_obs, definition, protocol, protocol["observation_end"])
